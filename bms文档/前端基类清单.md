...
- **设计令牌**：`src/styles/tokens.scss`（双端同款，`main.ts` 导入；基础令牌五组〔色 / 字 / 间距 / 圆角 / 阴影，`--bms-*`〕+ size / density 档位〔字号 / 间距引用基础令牌〕+ 语义变量 + 属性协议选择器 + 组件库映射〔PC=Element Plus 含色阶、移动端=Vant〕）——**基础令牌已交付**（`03_06`，2026-09-17）；暗色主题 / 租户品牌覆盖归品牌化任务；
