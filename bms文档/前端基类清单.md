...
> **为何是「角色」而非「分层」**（对照《[后端基类清单](后端基类清单.md)》「分层总览」）：后端为**单轨纵向继承树**（父层唯一、层可编号）；前端为**继承 + 组合双轨**——能力片段是组合式、**不在继承链上**（彼此无父子、可任意组合、不设层数上限），无法用「第几层」描述；故以「角色」（根系 / 组件根 / 机制基类 / 能力片段 / 域基类）表达职责定位。两套体系**同构**（根 → 组件根 / 模块基类 → 机制 / 横切 → 具体 + 旁路），关系链见 §8「继承链与代码位置」（口径见《[架构设计 · 前端组件体系](设计/架构设计/05_架构设计_前端组件体系.md)》「前端基类不做固定层号 / 数量枚举，按角色 + 开放扩展组织」）。
...
- **格式化层（`02_06_03` 已交付，2026-09-16）**：`src/utils/format.ts`（格式化纯函数 + 默认基准注入）/ `src/utils/useTimezone.ts` / `src/utils/validators.ts`（规则补强）/ `src/components/base/formatters.ts`（名义格式化器注册表，展示域分发接入）。
...
- **设计令牌**：`src/styles/tokens.scss`（双端同款，`main.ts` 导入；基础令牌五组〔色 / 字 / 间距 / 圆角 / 阴影，`--bms-*`〕+ size / density 档位〔字号 / 间距引用基础令牌〕+ 语义变量 + 属性协议选择器 + 组件库映射〔PC=Element Plus 含色阶、移动端=Vant〕）——**基础令牌已交付**（`03_06`，2026-09-16）；暗色主题 / 租户品牌覆盖归品牌化任务；
...
- **格式化层**：`src/utils/format.ts` / `src/utils/useTimezone.ts` / `src/utils/validators.ts` / `src/components/base/formatters.ts`（名义格式化器注册表；`useDisplayBase` 分发接入）——**已交付**（`02_06_03`，2026-09-16）；
- **依赖方向**：只准上层依赖下层（具体组件 → 域基类 / 片段 → 组件根 → 根系）；基类不得依赖具体组件；片段之间依赖须单向并登记。**层次由依赖关系决定，不设层数上限**（继承链与组合依赖链详见 §8）。
...
> 前端不做固定层号（见 §2 命名理由），继承关系以「**继承链 + 组合依赖链**」双轨表达：**身份经继承（类）、横切能力经组合（片段 / 组合式）**——与《[后端基类清单](后端基类清单.md)》「继承链与代码位置」节同构（后端为类 → 类单轨，前端为类 → 类 + 组件 → 片段双轨）。
...
- **组合依赖链（片段，组合轨）**：
  ...
  - 依赖方向：具体组件 → 域基类 / 片段 → 组件根 → 根系；片段之间**单向**（`depends` 登记，权威表见 §5 与 `src/components/base/fragments.ts`）、**无环**、不得依赖上层（护栏 `tests/guard-dependency.spec.ts`）。
- **具体组件挂接**（代码位置）：
...
- **目录**：机制层在 `src/base/`；能力片段与域基类在 `src/components/base/<能力域>/`；具体组件在 `src/components/<域>/`（明细见 §7「目录与依赖」）。
