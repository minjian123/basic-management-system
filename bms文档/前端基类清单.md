...
| PC 插件 | `frontend/packages/ui-ep/src/` | 具体组件（按族分目录）+ 组件编排与基类投影 `composables/`（`useBaseLayout` / `useBaseDisplay` / `useBaseTreeData` / `useBaseAccess` / `useBaseFormPage` / `useBasePersistedState` / `useBaseInput` / `useBaseField` / `useModalShell` / `useFeedback` / `useConfirm` / `useFormModal` / `useTabNav` / `useSideMenu`）+ 注入点（`checkPerm` / `confirm` / `menuSource` / `viewResolver`） |
