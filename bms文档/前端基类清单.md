...
| 93 | `BaseModuleTelemetry` | `BaseCapability` | 模块治理 | 类 | `capabilities/module-telemetry.ts` | 模块可观测能力域基类：遥测记录 / 聚合快照 / 重置契约（框架无关、不触 DOM；能力键 `module-telemetry`） | 已交付 |
| 94 | `BaseModuleReporter` | `BasePluggable` | 模块治理 | 类 | `capabilities/module-reporter.ts` | 模块上报实现插件基类：可替换投递通道（缺省内存实现 `InMemoryModuleReporter`；注册表 `ModuleReporterRegistry` 未登记即不投递，不阻断） | 已交付 |
...
flowchart TD
    ...
    PLG --> FAC
    CAP --> PRV
    MT["能力域基类 BaseModuleTelemetry（模块可观测）"]
    MTR["插件基类 BaseModuleReporter（可替换上报实现 + 注册表）"]
    CAP --> MT
    PLG --> MTR
...
| `BaseChartEngine` / `BaseRealtimeChannel` / `BaseAiStream` / `BaseSearchEngine` / `BaseOrgSource` / `BaseDictSource` / `BaseUploadTransport` / `BaseCaptchaSource` / `BaseModuleReporter` | `BasePluggable` | 可替换实现（纵向）插件基类：图表引擎 / 实时通道 / AI 流式 / 检索引擎 / 组织数据源 / 字典数据源（普通取数与高级查询同一数据源）/ 上传通路（哈希 / 秒传 / 整包 / 分片 / 会话 / 元数据 / 预签名）/ 验证码数据源（出题 / 短信 / 校验 / 策略）/ **模块上报实现**（缺省内存实现 `InMemoryModuleReporter`）；内建与定制实现继承之 | 已交付 |
| `ChartEngineRegistry` / `RealtimeChannelRegistry` / `AiStreamRegistry` / `SearchEngineRegistry` / `OrgSourceRegistry` / `DictSourceRegistry` / `UploadTransportRegistry` / `CaptchaSourceRegistry` / `ModuleReporterRegistry` | `BaseProviderRegistry` | 各域注册表（注册项 `ChartEngineProvider` / `RealtimeChannelProvider` / `AiStreamProvider` / `SearchEngineProvider` / `OrgSourceProvider` / `DictSourceProvider` / `UploadTransportProvider` / `CaptchaSourceProvider` / `ModuleReporterProvider` 继承 `BaseProvider`） | 已交付 |
...
│   │   │   ├── capabilities/            # 能力基类（值族 / 尺寸族 / 数据状态族 / 通知族 / 令牌族 …）+ 组件基类（输入 / 展示 / 表格 / 树 / 容器 / 布局 / 模态 …）+ 模块可观测（module-telemetry 能力域基类 / module-reporter 插件基类与注册表）+ manifest.ts 登记
│   │   │   ├── contracts/               # 契约类型 + BaseDataObject / BaseEntity / BasePageQuery + 服务寻址（service-endpoint：域→服务前缀单一来源）
│   │   │   ├── module/                  # 模块契约与模块加载器（defineModule / 清单解析（含加载形态与可见性）/ 契约版本常量 / 清单驱动加载器（超时 + 阶段计时 + 观测钩子）+ 入口解析器 / 遥测记录类型（telemetry.ts）/ 令牌机制 / 文案承载；非基类实现物）
│   │   │   ├── registries/              # 扩展点注册项与八类注册表 + createRegistries + 统一装配器
│   │   │   └── domain/                  # 框架无关领域纯函数与共享工厂（含路由守卫判定 route-guard）
│   │   ├── testing/                     # 契约用例工厂（@bms/core/testing）
│   │   └── tests/                       # 契约与护栏用例
│   ├── api-types/src/                   # 服务契约生成类型（openapi-typescript 从 deploy/contracts 快照生成；按服务命名空间导出；勿手改）
...
├── scripts/                          # 前端共享断言与护栏脚本（共享声明读取 / 产物级断言 / 白名单与体积护栏 / 模块隔离扫描 / 产物元数据 / 发布回滚工具（四关强校验含 sourcemap、体积留痕）/ 清单与 sourcemap 护栏 / 发布存储服务 / 模块体积计量单一实现 module-metrics）
├── apps/
...
| 宿主装配 | `frontend/apps/desktop/`、`frontend/apps/mobile/` | 路由（静态 + 动态菜单 + **模块路由注册 / 卸载**）/ 菜单运行时 / 页面 / `adapters/` / **模块宿主**（`src/module/`：清单获取、**远端入口解析器**（`federation.ts`，Module Federation 运行时登记与加载）、本地入口表解析器（`entries.ts`）、按加载形态分派、加载器装配与上下文注入、**单模块重试**与路由 → 模块解析）/ **模块边界**（`components/ModuleBoundary.vue`，加载失败 / 超时 / 渲染异常兜底，带模块名与版本）/ **可观测**（`src/observability/`：模块感知上报 sink（`configureBase`）、Web Vitals 采集（`webVitals.ts`）、活动模块作用域（`scope.ts`）、模块路由作用域与渲染耗时（`index.ts`）+ 开发态观测面板 `views/ModuleObservabilityView.vue`）；请求 · 权限 · 格式化横切底座在宿主侧 |
| 模块工程 | `frontend/modules/<模块名>/` | 独立构建与独立发布的**运行时模块**（MF remote）：自有依赖清单 / 锁文件 / 构建配置 / 类型与 Lint 配置 / 体积预算脚本（复用 `module-metrics` 单一计量）/ 独立预览壳；源码导出模块定义（默认导出，**版本构建期注入**、**自报契约版本**）与页面；产物目录 `dist/` 不并入主应用产物（首个工程：`frontend/modules/demo`；首个**真实形态样例**：`frontend/modules/sample`——八类注册声明 + 查询筛选 / 列表 / 详情 / 表单，2026-09-22 `04_01` 落地，第三方 `element-plus` 经 `output.codeSplitting.groups` 独立分包）。**sourcemap**：远端构建产出**外部 `.map`**（随发布按版本归档，线上可映射）。**两个构建目标**：缺省 `dist/` 为**远端产物**（只含容器与暴露模块与产物元数据 `module.meta.json`，不带 HTML 壳）、`build:standalone` → `dist-standalone/` 为**独立预览产物**（带 HTML 壳与本地框架副本）。**契约用例**：`tests/module-contract.spec.ts` 经契约工厂跑同一套断言（含共享 / 隔离事实）。**隔离护栏**：源码面随宿主护栏用例（`desktop-check`），产物面随 `module-build`（`guard:isolation`） |
...
| 模块发布治理 | `frontend/module-contract.json`、`frontend/scripts/`、`frontend/releases/` | **契约版本单一来源**（`module-contract.json`，与核心常量一致性由护栏用例断言）+ 产物元数据与版本注入（`module-meta.mjs`：构建期注入 `package.json` 版本、产出 `module.meta.json`）+ **模块体积计量单一实现**（`module-metrics.mjs`：入口闭包口径，预算 / 发布 / 护栏复用）+ **发布 / 回滚 / 停用工具**（`release-module.mjs`：**四关强校验（版本 / 隔离 / 共享 / sourcemap）** → 版本归档（含 sourcemap）→ 清单更新 → 发布记录（含产物体积））；回滚 = 清单版本回退 + **清单 / 版本发现 / sourcemap / 契约用例齐备护栏**（`check-module-manifest.mjs`，按模块汇总体积）+ **发布存储服务**（`serve-module-releases.mjs`，本地演练形态，托管 map）；发布记录 `releases/release-log.json`（真源）与 `release-log.md`（摘要）随仓库提交，版本目录不入库 |
| 宿主横切底座（非基类） | `frontend/apps/*/src/{api,stores,utils,directives,router}/` | 请求层（`adapter` / `error` / `request`（服务段组装）/ `token` / `http`）、能力源端点解析（`api/endpoints.ts`：按服务注入 endpoint + 令牌头）、`apiUrl` 服务段入口、路由守卫接线（`router/guard.ts`，默认关闭）、权限层（`usePermissionStore` / `v-perm` / `hasPerm` / `canAccess`）、格式化层（`format` / `useTimezone` / `validators`）、`createCrudStore` / `stableStringify` |
