...
- **定位**：前端基类**权威清单**——回答「有哪些基类 / 能力类 / 投影与插件实现、什么职责、代码在哪、是否已交付」。**强制用法口径与扩展规则**见《[前端开发规范](规范/前端开发规范.md)》「前端基类体系（开放式，强制）」节；体系机制见平台《[架构设计 · 前端组件体系](设计/架构设计/05_架构设计_前端组件体系.md)》；**通用组件的语义与契约边界**见《[组件设计 · 总览](设计/组件设计/01_组件设计_总览.md)》对应节点（组件设计不写实现侧命名 / 代码落点 / 接口细节，具体实现契约细节见项目详细设计）。
...
## 2. 分层总览（原「角色总览」） <a id="roles"></a>

> **前端基类按「链上分层」描述**（对照《[后端基类清单](后端基类清单.md)》「分层总览」，两套体系同构）：前端为**继承 + 组合双轨**——**主体内建走继承轨**（**能力即继承链上的层**：根系 → 组件根 → 机制基类 → 能力层 → 域基类 → 具体组件 / 件，层间依赖由继承关系确定，不得链外自由挂接），**项目二次开发 / 定制走组合轨**（不继承平台基类、经注册通道接入、不回改平台基类）；两轨汇入同一登记与契约测试体系。关系链见『继承链与代码位置』节（口径见《[前端开发规范](规范/前端开发规范.md)》§1「架构铁律」与 §3.4；体系机制见《[架构设计 · 前端组件体系](设计/架构设计/05_架构设计_前端组件体系.md)》「框架无关核心与插件架构」节）。
>
> **2026-09-17 二次返工修正（推翻重来级）**：本节原为「**为何是「角色」而非「分层」**」框架（理由为「能力类**不在继承链上**、彼此无父子、可任意组合、不设层数上限，无法用第几层描述」）——**该框架与其理由均已废**：能力已入继承链，层级由继承关系确定。该口径直接导致能力平铺、无层次、无统一管理、散落各处被各自创建使用。现行口径见《[前端开发规范](规范/前端开发规范.md)》§1「架构铁律」与 §3.4；本清单的「链层登记表」格式（层名 / 父层 / 契约 / 归属域 / 装配方 / 消费方）随重构设计落地。

```mermaid
flowchart TD
    CORE["核心 frontend/packages/core（纯 TS，框架无关）<br/>根系 BaseFrontend / 组件根 BaseComponent / 机制基类 ×6<br/>能力层 ×31（BaseXxx）/ domain 领域 / contracts 契约"]
...
| 机制基类 | `BaseRegistry<T>` + `RegistryItemLike`（唯一性拒重 / 未命中返回 `undefined` / 保序 / 只读快照） | 类（继承组件根） | 组件根 | `frontend/packages/core/src/mechanisms/registry.ts` | 注册表基座（**命名按项目口径** `BaseRegistry` / `RegistryItemLike`，对齐后端同职责基类 `BaseProviderRegistry` / `BaseProvider`）；供能力 / 扩展点（路由·菜单、组件、字段渲染器、图标、工作台卡片）注册复用（域 10 微前端骨架） | 已交付 |
...
| 能力层（开放集合） | 预置 31 个能力类：`BaseValue` / `BaseFieldShell` / `BaseFieldPerm` / `BaseField` / `BaseInteractive` / `BaseInputControl` / `BaseDisplayControl` / `BaseContainer` / `BaseFormContainer` / `BaseMedia` / `BaseLayout` / `BaseTabs` / `BasePersistedState` / `BaseOptionSource` / `BaseUploadEngine` / `BaseAsyncTask` / `BaseEditorKernel` / `BaseTreeData` / `BaseUserDisplay` / `BaseDynamicRoutes` / `BaseFormMeta` / `BasePresignedUrl` / `BaseDragDrop` / `BaseDesignToken` / `BaseFormPage` / `BaseLocale` / `BaseAccess` / `BaseColumnConfig` / `BaseQueryScheme` / `BaseWatermark` / `BaseModuleContext` | 类（+ 按需组合式投影） | 组件根（+ 能力机制） | `frontend/packages/core/src/capabilities/<能力域>.ts`（一域一文件；登记表 `capabilities/manifest.ts`）；投影按需落 `frontend/packages/vue/src/bindings/useXxx.ts`（`useXxx` 只存在于此） | 每能力一类横切能力；组件按需组合多个（经投影）；**新增能力直接加文件 + 登记 + 契约用例**（§6 与 `manifest.ts` 同步） | 已交付 |
| 契约 / 领域层 | `contracts/`（能力与注入契约 + 选项契约 `option.ts`）+ `domain/`（滚动位置 / 尺寸 / 虚拟区 / 输入域工厂 / 密码强度 / 数字域 / 选项域 / **布尔域** / 注入共享工厂） | 接口 / 类型 / 纯函数 | 核心内部 | `frontend/packages/core/src/contracts/`、`frontend/packages/core/src/domain/` | 渲染插件必须满足的稳定接口面（能力描述 / 绑定 / 确认 / 权限 / 菜单源 / 视图解析）与框架无关领域逻辑 | 已交付 |
| 契约用例工厂 | `@bms/core/testing`（`describeXxxContract` / 组件契约套件 / 输入控件契约套件） | 测试工具（纯 TS） | 核心 | `frontend/packages/core/testing/` | 各插件对同一契约跑同一套断言——「同接口多实现」的机器保障 | 已交付 |
| 域基类 | 输入域口径包装（核心 `createInputContext` + 投影 `useInput` + 插件 `BaseInput.vue`）；其余 3 域（展示 / 树 / 编辑器）随回补波重建 | 核心实现 + 投影 + 插件包装 | 核心能力（域基类位于能力层之上） | `frontend/packages/core/src/domain/input.ts`、`frontend/packages/vue/src/bindings/useInput.ts`、`frontend/packages/{ui-ep,ui-vant}/src/components/input/BaseInput.vue` | **继承链上的一层**（能力层之上、具体组件 / 件之下）：域语义归类与最终细化，主体内建经其派生（2026-09-17 双轨修正，原「可选派生、不强制固定长链」已废）；**新增域按需登记** | 部分（输入域已重建，2026-09-17） |
| 具体组件 | 布局 / 容器 / 菜单 / 标签 / 弹窗 / 反馈（PC 34 件）+ 容器 / 权限按钮 / 反馈件（移动端 13 件）+ **输入件（文本 / 文本域 / 密码 / 数字 / 下拉 / 单选 / 复选 / 开关，PC 与移动端各 8 件 + 包装件）** | Vue 组件（挂组件根） | 能力 / 组件根 | `frontend/packages/{ui-ep,ui-vant}/src/components/<域>/` | 两端同契约多实现；域 05+（基础控件 / 字段 / 展示 / 交互）随各域任务交付 | 部分（域 02 ~ 05 已交付；域 05 八基础控件齐，2026-09-17 收官） |
...
| 机制基类 | `BaseRegistry<T>` + `RegistryItemLike` | 类（继承组件根） | 组件根 | `frontend/packages/core/src/mechanisms/registry.ts` | 注册表基座（唯一性拒重 / 未命中返回 `undefined` / 保序 / 只读快照）；供能力与扩展点注册复用（域 10 微前端骨架） | 已交付 |
...
- **上下文机制（非能力）**：字段壳 ↔ 控件**不引入** `provide` / `inject` 通道，改由插件包装件默认插槽的**作用域参数**传递（受控值 / 写入口 / 事件桥接，见各插件 `src/components/input/BaseInput.vue`），随 `05_01` 落地（2026-09-17）；原 `useFieldContext` 组合式随旧层删除（历史经 git 追溯）。
...
## 7. 域基类 <a id="domains"></a>

| 域 | 形态（新体系） | 组合面 | 代码位置 | 说明 | 状态 |
| --- | --- | --- | --- | --- | --- |
| 输入域 | 核心能力 + 投影 + 插件包装（**已重建**） | `field` + `input-control`（字段上下文 provide / inject 随后续任务） | `frontend/packages/core/src/domain/input.ts`（域工厂 `createInputContext`）+ `frontend/packages/vue/src/bindings/useInput.ts`（投影）+ `frontend/packages/{ui-ep,ui-vant}/src/components/input/BaseInput.vue`（包装） | 输入域最终细化（受控绑定 / 归一 / 清空 / 只读判定 / composition），供基础控件类与字段类使用 | **已重建（2026-09-17，`05_01`；域 05 收官复核对：双端自检 13/13）** |
...
> 域基类**框架无关**（核心实现 + 可替换渲染）；原 `BaseInput.vue` / `BaseDisplay.vue` / `BaseTree.vue` / `BaseEditor.vue` 与 `useXxxBase` 组合式随旧层删除（历史经 git 追溯），域 05+ 回补波按「核心实现 + 投影（+ 插件层包装）」重建。新增域按需新增并登记。
>
> **2026-09-17 二次返工修正（推翻重来级）**：域基类是**继承链上的一层**（位于能力层之上、具体组件 / 件之下），主体内建一律经链上各层派生——原表述「域基类只做**语义归类**，组件组合能力即可工作（**不强制经域基类**）」已废。现行口径见《[前端开发规范](规范/前端开发规范.md)》§1「架构铁律」与 §3.4「继承与组合双轨」。
...
| 基础控件领域层 | 纯函数 | `src/domain/{input,password,number,option,boolean}.ts` | 输入域上下文工厂（受控绑定 / 归一 / 清空 / 只读判定 / composition）、密码强度、数字解析与格式化、选项归一（去重保序 / 上限 / 文本 / 分组）、布尔归一（`1/0` ↔ `boolean \| null`，`null` 与 `false` 区分）——双端同一套口径（`05_01` ~ `05_03` 交付） | 已交付 |
| 契约用例工厂 | 测试工具（仅测试消费） | `frontend/packages/core/testing/`（`@bms/core/testing`） | `describeConfirmContract` / `describePermissionContract` / 容器件 / 权限按钮 / 反馈件 / 输入控件契约套件（各插件同一套断言；kit 可选 `configureConfirm` 注入确认实现） | 已交付 |
...
> 继承关系沿继承链表达（层次由继承关系确定，见『分层总览』节）：**主体内建经继承链派生**（根系 → 组件根 → 机制基类 → 能力层 → 域基类 → 具体组件 / 件）；**平台之外的定制走组合轨**（注册通道接入，不回改平台基类）。与《[后端基类清单](后端基类清单.md)》「继承链与代码位置」节同构。（**2026-09-17 二次返工**：原「前端不做固定层号（见『角色总览』节命名理由）」表述已废——能力已入继承链、按链上分层描述。）
...
- **继承链（类，身份轨）**：
  ...
  - `BaseRegistry<T>` 继承组件根（机制基类层；能力注册表 `CapabilityRegistry`、提供者注册表 `ProviderRegistry` 基于它扩展）。
  - `BaseError → Error`（前端错误基座，挂接机制层，与后端 `BizError` 同口径）；宿主 `ApiError → BaseError`。
- **组合依赖链（能力 + 投影，组合轨）**：
  - 具体组件 → 组件根 + 链上各层（能力层 / 域基类）：`useComponentBase()`（`@bms/vue`）+ 链层投影 / 核心实例化——**主体内建必须经链上各层派生**（2026-09-17 双轨修正：原「不经域基类亦合法（组合轨默认；域基类只是可选语义归类）」已废）；平台之外的定制实现走组合轨（注册通道接入，不回改平台基类）。
...
- **同步范围**：新增 / 调整基类或能力须同步本清单（分层总览表与章节）+ 组件设计对应节点（契约与原型）+ 平台《架构设计 · 前端组件体系》登记。
