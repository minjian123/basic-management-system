...
│   │   │   ├── module/                  # 模块契约与模块加载器（defineModule / 清单解析 / 本地与清单驱动加载器 / 令牌机制 / 文案承载；非基类实现物）
...
└── apps/
    ├── desktop/src/                     # PC 宿主：路由 / 菜单运行时 / 页面 / adapters / api / stores / utils / module（清单获取与模块装配）/ public（模块清单）
    └── mobile/src/                      # 移动端宿主（同上；横切底座同契约）
```

| 层 | 路径 | 内容 / 状态 |
| --- | --- | --- |
| 核心 | `frontend/packages/core/src/` | 固定三段（总基类 / 能力域基类 / 插件基类）+ 注册表与工厂 + 组件侧（组件根 / 能力基类 / 组件基类）+ 资源占位错误 + 契约基类 + 领域纯函数 + **模块契约与模块加载器**（`module/`：`defineModule` / 模块清单解析（`parseModuleManifest`）/ `LocalModuleLoader`（本地定义表）/ `ManifestModuleLoader`（**清单驱动**，宿主运行期实现）/ 主题令牌汇聚与应用（核心不触 DOM，写回目标由宿主注入）/ 模块文案承载（并入 / 还原 / 取文案），均非基类实现物）+ **扩展点注册表**（`registries/`：`createRegistries` 与路由·菜单 / 页面区域 / 通用组件 / 字段渲染器 / 图标 / 工作台卡片 / 主题令牌 / i18n 文案包八注册表，均基于注册表基座 `BaseProviderRegistry` 与注册项契约 `BaseProvider`；`assemble.ts` 统一装配器承载平台自身注册与模块注册同一通道）；护栏 `guard-core-framework-agnostic` |
