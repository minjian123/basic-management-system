...
- **机制层**：`src/base/`——`BaseFrontend.ts` + `useFrontendBase.ts` + `BaseFrontend.vue`（根系，**已交付**）/ `BaseComponent.ts` + `useComponentBase.ts` + `BaseComponent.vue`（组件根，**已交付**）/ `placeholder.ts`（占位，**已交付**）/ `resource.ts`（异步资源，**已交付**）/ `error.ts`（错误，**已交付**）/ `subscription.ts`（订阅，**已交付**）/ `capability.ts`（片段机制，待交付）/ `provider.ts`（注册表 + 注册项，待交付）——各文件口径以对应《组件设计》节点 §2「位置」列为准；
