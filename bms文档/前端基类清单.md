...
flowchart TD
    ...
    CAP["能力片段（开放集合，组合轨）<br/>预置 19：形态 7 + 值/字段链 4 + 选项源 1 + 复用类 7"]
...
| 能力片段（开放集合） | 预置 19：`useInteractive` / `useInputControl` / `useDisplayControl` / `useContainer` / `useFormContainer` / `useMedia` / `useLayout` / `useValue` / `useFieldShell` / `useFieldPerm` / `useField` / `useOptionSource` / `useUploadEngine` / `useAsyncTask` / `useTabs` / `usePersistedState` / `useEditorKernel` / `useTreeData` / `useUserDisplay` | 组合式（正交可组合） | 组件根（+ 片段机制） | `src/components/base/<能力域>/index.ts`（规划） | 每片段一类横切能力；组件按需组合多个；**新增片段直接加文件 + 登记** | 待交付 |
...
## 5. 能力片段（开放集合，预置 19） <a id="capabilities"></a>
