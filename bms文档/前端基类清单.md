...
| 请求 / 权限 / 格式化层 | `src/api/`、`src/stores/`、`src/directives/`、`src/utils/`、`src/components/common/` | 见「契约与横切底座」节（`02_06` 已交付） |
...
- **双端同款基线**：apps/desktop 与 frontend-mobile 同步扩展，不得只在单端加基类能力。
...
- **具体组件挂接**（代码位置）：
  - 布局 / 容器 / 菜单 / 标签 / 弹窗 / 反馈类（42 件，`src/components/{layout,container,menu,tabs,modal,feedback,common}/`）：`useComponentBase`（组件根）——2026-09-16 基类与片段合规审计确认 **42/42**（护栏 `inheritance.component-root`）。
  - 字段 / 展示 / 交互类（回补波）：经 `useXxxBase`（域基类组合式）落地；域包装 `BaseXxx.vue` 供模板直接使用。
- **目录**：机制层在 `src/base/`；能力片段与域基类在 `src/components/base/<能力域>/`；具体组件在 `src/components/<域>/`（明细见「目录与依赖」节）。
