...
| 层 | 路径 | 内容 / 状态 |
| --- | --- | --- |
| 核心 | `frontend/packages/core/src/` | 固定三段（总基类 / 能力域基类 / 插件基类）+ 注册表与工厂 + 组件侧（组件根 / 能力基类 / 组件基类）+ 资源占位错误 + 契约基类 + 领域纯函数 + **模块契约与本地加载器**（`module/`：`defineModule` / `LocalModuleLoader`，非基类实现物）+ **扩展点注册表**（`registries/`：`createRegistries` 与路由·菜单 / 页面区域 / 通用组件 / 字段渲染器 / 图标 / 工作台卡片 / 主题令牌 / i18n 文案包八注册表，均基于注册表基座 `BaseProviderRegistry` 与注册项契约 `BaseProvider`；`assemble.ts` 统一装配器承载平台自身注册与模块注册同一通道）；护栏 `guard-core-framework-agnostic` |
