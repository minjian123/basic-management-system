...
- **机制层**：`src/base/`——`BaseFrontend.ts` + `useFrontendBase.ts` + `BaseFrontend.vue`（根系，**已交付**）/ `BaseComponent.ts` + `useComponentBase.ts`（组件根）/ `capability.ts`（片段机制）/ `placeholder.ts`（占位）/ `resource.ts`（异步资源）/ `provider.ts`（注册表 + 注册项）/ `error.ts`（错误）/ `subscription.ts`（订阅）——除根系外各文件口径以对应《组件设计》节点 §2「位置」列为准，随各自任务登记；
