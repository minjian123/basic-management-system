...
flowchart TD
    ...
    MECH["机制基类（体系公共机制，组件根统一挂接）<br/>BaseCapability / BasePlaceholder / BaseAsyncResource / BaseProviderRegistry + BaseProvider / BaseError / BaseSubscription"]
...
| 片段机制 | `BaseCapability` + `useCapabilityBase`（组合轨机制：片段声明 / 依赖登记 / 单向校验 / 开发告警） | 类 + 组合式 | 组件根 | `src/base/capability.ts`（规划） | 片段组合的公共机制（登记、依赖单向校验、开发告警），供全部能力片段复用（对齐后端 `BaseCapability`） | 待交付 |
| 机制基类 | `BasePlaceholder` + `usePlaceholder`（占位标记 / `describe` / 空态·禁用·降级语义） | 类 + 组合式 | 组件根 | `src/base/placeholder.ts`（规划） | **占位先行**承载：占位组件 / 片段统一标记与语义（对齐后端 `BasePlaceholder` / `BaseNullObject` / `BaseStub`） | 待交付 |
| 机制基类 | `BaseAsyncResource` + `useAsyncResource`（订阅 / 定时器 / AbortController 自动释放，`onScopeDispose`） | 类 + 组合式 | 组件根 | `src/base/resource.ts`（规划） | 异步资源生命周期（对齐后端 `BaseAsyncResource`）；组件卸载自动清理 | 待交付 |
| 机制基类 | `BaseProviderRegistry` + `BaseProvider`（注册项 `key` + `describe`；唯一性 / 未命中 / 查询，组合轨） | 类 + 组合式 | 组件根 | `src/base/provider.ts`（规划） | 提供者注册表基座（对齐后端 `BaseProviderRegistry` / `BaseProvider`）；供路由·菜单、组件、字段渲染器、图标、工作台卡片注册复用（09-2） | 待交付 |
| 机制基类 | `BaseError`（错误码 + 用户提示映射 + 上报钩子） | 类 | 组件根 | `src/base/error.ts`（规划） | 前端异常基座（对齐后端 `BizError` 体系）；统一错误边界与提示口径 | 待交付 |
| 机制基类 | `BaseSubscription` + `useSubscription`（事件订阅：注册 / 取消 / 占位实现） | 类 + 组合式 | 组件根 | `src/base/subscription.ts`（规划） | 事件订阅基座（对齐后端 `BaseEventWorker` / 事件域）；解耦具体来源（socket / store），便于占位与替换 | 待交付 |
| 能力片段（开放集合） | 预置 31：`useInteractive` / `useInputControl` / `useDisplayControl` / `useContainer` / `useFormContainer` / `useMedia` / `useLayout` / `useValue` / `useFieldShell` / `useFieldPerm` / `useField` / `useOptionSource` / `useUploadEngine` / `useAsyncTask` / `useTabs` / `usePersistedState` / `useEditorKernel` / `useTreeData` / `useUserDisplay` / `useDynamicRoutes` / `useFormMeta` / `usePresignedUrl` / `useDragDrop` / `useDesignToken` / `useFormPage` / `useLocale` / `useAccess` / `useColumnConfig` / `useQueryScheme` / `useWatermark` / `useFragmentContext` | 组合式（正交可组合） | 组件根（+ 片段机制） | `src/components/base/<能力域>/index.ts`（规划） | 每片段一类横切能力；组件按需组合多个；**新增片段直接加文件 + 登记** | 待交付 |
| 域基类（按需） | 预置 `BaseInput`（输入域）/ `BaseDisplay`（展示域）；新增 `BaseTree`（树域：树选择 / 组织树 / 权限树共用）/ `BaseChart`（图表域，随图表卡）/ `BaseEditor`（编辑器域：组合 `useEditorKernel`） | 组合式 + 组件 | 片段组合 | 同上（`<能力域>/`） | 域语义归类与最终细化（可选派生，不强制固定长链）；**新增域按需登记** | 待交付 |
...
| `useUploadEngine` | 上传引擎：分片 / 秒传 / 断点续传 / 进度 / 取消（文件上传、富文本、头像、导入共用；依赖后端能力占位降级） |
| `useAsyncTask` | 异步任务：提交 / 轮询进度 / 取消 / 结果下载（导入导出、报表与大屏导出共用；占位先行） |
| `useTabs` | 页签状态：打开 / 关闭 / 固定 / 缓存键 / keep-alive 清单（多标签导航、表单框架双层 Tab、内容页签共用） |
| `usePersistedState` | 偏好持久化：本地 + 远端偏好同步（`sys_user_preference` / `sys_query_scheme`；表格列配置、查询方案、偏好面板、主题与语言共用） |
| `useEditorKernel` | 编辑器内核：懒加载 / 销毁 / 内容协议（CodeMirror 与富文本 / Markdown 共用） |
| `useTreeData` | 树数据：加载 / 懒加载 / 选中 / 过滤（树选择、组织树、权限树、树形主从共用；归 `BaseTree` 域） |
| `useUserDisplay` | 用户 / 组织展示：姓名 / 头像 / 状态 / 部门路径（头像组件、组织选择回显、审批流、通知共用） |
| `useDynamicRoutes` | 动态路由注册：菜单树 → 路由 → 注册 / 卸载（前端不硬编码路由表；配合宿主薄壳与注册表基座） |
| `useFormMeta` | 表单元数据：加载 / 缓存 / 版本比对（菜单下发表单元数据；表单渲染器与设计器共用） |
| `usePresignedUrl` | 预签名：获取 / 失效重取 / 降级下载（文件预览、下载、导出共用） |
| `useDragDrop` | 拖拽基座：拖拽 / 排序 / 跨区（表单设计器、报表与大屏设计器、工作台卡片共用） |
| `useDesignToken` | 设计令牌消费：间距 / 色板 / 密度 / 断点（组件根令牌统一入口） |
| `useFormPage` | 表单页组合：三态（新增 / 编辑 / 详情）/ 提交 / 脏数据 / 返回（与 `useListPage` / `useFormModal` 对称） |
| `useLocale` | 语言上下文：locale / 时区 / 格式上下文（切换与缓存；收敛时区格式化） |
| `useAccess` | 权限上下文：权限码集合 / 路由·菜单过滤 / 按钮显隐（动作权限与动态菜单共用） |
| `useColumnConfig` | 列配置：显隐 / 顺序 / 宽度 / 持久化（通用表格与明细区共用） |
| `useQueryScheme` | 查询方案：条件模型 / 方案保存·应用（`sys_query_scheme`；查询筛选区） |
| `useWatermark` | 水印：用户 / 租户信息注入（覆盖内容与导出；与打印导出共用） |
| `useFragmentContext` | 片段上下文：宿主注入 router / pinia / i18n / 用户 / 租户（只读约束；片段侧统一入口） |
...
- `BaseTree`（树域）：组合 `useTreeData`，供树选择 / 组织树 / 权限树 / 树形主从共用；`BaseChart`（图表域）随图表卡、`BaseEditor`（编辑器域）按需新增。
- 新增域按需新增并登记；域基类只做**语义归类**，组件组合片段即可工作。

## 7. 目录与依赖 <a id="layout"></a>

- **机制层**：`src/base/`——`base.ts`（根系 `BaseFrontend`）/ `component.ts`（组件根 `BaseComponent`）/ `capability.ts`（片段机制 `BaseCapability`）/ `placeholder.ts`（占位 `BasePlaceholder`）/ `resource.ts`（异步资源 `BaseAsyncResource`）/ `provider.ts`（注册表 `BaseProviderRegistry` + 注册项 `BaseProvider`）/ `error.ts`（错误 `BaseError`）/ `subscription.ts`（订阅 `BaseSubscription`）；
...
- **护栏**：本清单与代码的一致性由 **CI 对账脚本**校验（清单 ↔ 代码双向，仿后端 `check-base.py` 模式）；执行护栏清单见《架构设计 · 前端组件体系》「执行护栏」节与阶段四任务完成标准（口径指引见《[前端开发规范](规范/前端开发规范.md)》§3.5）。
