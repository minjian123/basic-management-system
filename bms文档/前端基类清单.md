...
| 59 | `BaseDataObject` | `BaseObject` | 契约 | 类 | `contracts/data-object.ts` | 数据对象：不可变语义、字段声明、归一与稳定序列化 | 待交付 |
| 60 | `BaseEntity` | `BaseDataObject` | 契约 | 类 | `contracts/entity.ts` | 实体：标识 / 版本 / 软删除回显（现为纯接口） | 部分（须上提为类） |
| 61 | `BasePageQuery` | `BaseDataObject` | 契约 | 类 | `contracts/page-query.ts` | 分页查询：页码 / 页长 / 排序 / 筛选（现为纯接口） | 部分（须上提为类） |
...
| `BaseDataObject` | `BaseObject` | 数据对象：不可变语义 / 字段声明 / 归一 / 稳定序列化 | 待交付 |
| `BaseEntity` | `BaseDataObject` | 实体：标识 / 版本 / 软删除回显 | 部分（须上提为类） |
| `BasePageQuery` | `BaseDataObject` | 分页查询：页码 / 页长 / 排序 / 筛选 | 部分（须上提为类） |
