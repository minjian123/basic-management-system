...
| 域基类（按需） | 输入域口径包装（核心 `createInputContext` + 投影 `useInput` + 插件 `BaseInput.vue`）；其余 3 域（展示 / 树 / 编辑器）随回补波重建 | 核心域工厂 + 投影 + 插件包装 | 核心能力 | `frontend/packages/core/src/domain/input.ts`、`frontend/packages/vue/src/bindings/useInput.ts`、`frontend/packages/{ui-ep,ui-vant}/src/components/input/BaseInput.vue` | 域语义归类与最终细化（可选派生，不强制固定长链）；**新增域按需登记** | 部分（输入域已重建，2026-09-17） |
