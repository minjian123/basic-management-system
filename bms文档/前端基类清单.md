...
- **双端同款基线**：apps/desktop 与 frontend-mobile 同步扩展，不得只在单端加基类能力（新体系口径：同接口多实现 + 契约测试同一套断言，见《[前端开发规范](规范/前端开发规范.md)》。
...
- **具体组件挂接**（代码位置）：
  ...
  - 字段 / 展示 / 交互类（回补波）：经 `useXxxBase`（域基类组合式）落地；域包装 `BaseXxx.vue` 供模板直接使用。
- **目录**：机制层在 `src/base/`；能力片段与域基类在 `src/components/base/<能力域>/`；具体组件在 `packages/ui-ep/src/components/<域>/`（宿主 `apps/desktop` 经 `@bms/ui-ep` 消费，明细见「目录与依赖」节）。
