...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `BaseObject` | —（根） | 固定主干 ① | 类 | `base/BaseObject.ts` | **总基类**：命名空间 / 版本、日志、错误上报、配置读取、生命周期（`dispose`）；不含 UI 与业务语义 | 已交付（现名 `BaseFrontend`） |
| 2 | `BaseCapability` | `BaseObject` | 固定主干 ② | 类 | `mechanisms/capability.ts` | **能力域基类**（对齐后端同名基类）：能力键声明、依赖登记、单向校验、开发态告警 | 已交付（继承须上移） |
| 3 | `BasePluggable` | `BaseCapability` | 固定主干 ③ | 类 | `mechanisms/pluggable.ts` | **插件基类**（对齐后端同名基类）：能力键 / 实现名 / 契约版本 / 注册与解析 / 生命周期 / `describe` | 待交付 |
| 4 | `BaseProviderRegistry` | `BasePluggable` | 插件与注册 | 类 | `mechanisms/registry.ts` | 提供者注册表：唯一性拒重（严格域抛错 / 一般域告警保留首个）、未命中返回 `undefined`、保序、只读快照 | 已交付（现名 `BaseRegistry`） |
| 5 | `CapabilityRegistry` | `BaseProviderRegistry` | 插件与注册 | 类 | `capabilities/registry.ts` | 能力注册表：能力登记 / 解析 / 能力矩阵 | 已交付 |
| 6 | `BaseProvider` | `BaseCapability` | 插件与注册 | 类 | `mechanisms/provider.ts` | 注册项公共契约：抽象 `key` + 自描述 `describe()`（各扩展点注册项继承） | 待交付（现为 `RegistryItemLike` 接口） |
| 7 | `BaseFactory` | `BasePluggable` | 插件与注册 | 类 | `mechanisms/factory.ts` | **工厂基类**：一切工厂类之根（创建入口标识 / 参数校验 / 产出物释放登记） | 待交付 |
| 8 | `BaseComponent` | `BasePluggable` | 组件侧 · 组件根 | 类 | `base/BaseComponent.ts` | **组件根**：所有 UI 组件公共协议——标识与命名空间类名、尺寸 / 密度、`loading` / `disabled`、**令牌属性协议**（`data-size` / `data-density` / `aria-*` + 状态类）、attrs 透传（保留键过滤）、生命周期通知、机制装配点 | 已交付（继承须调整） |
...
| 11 | `BaseSized` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `base/sized.ts` | 尺寸档位：尺寸 / 密度档位与设计令牌消费统一入口 | 待交付 |
| 12 | `BaseDataState` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `base/data-state.ts` | 数据状态：`loading → ready / empty / error` 状态机与竞态处理 | 待交付 |
| 13 | `BaseOverlay` | `BaseSized` | 组件侧 · 能力基类 | 类 | `capabilities/overlay.ts` | 浮层：遮罩、挂载点、层级、焦点陷阱、关闭语义 | 待交付 |
| 14 | `BaseNotice` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `base/notice.ts` | 提示通知：类型 / 时长 / 关闭语义 / 队列 | 待交付 |
| 15 | `BaseLabeled` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `base/labeled.ts` | 标签语义：label 文案 / 位置 / 宽度、必填标记、帮助与错误位 | 待交付 |
| 16 | `BaseValidatable` | `BaseLabeled` | 组件侧 · 能力基类 | 类 | `base/validatable.ts` | 校验：规则汇总 / 触发时机 / 结果回传 / 错误定位 | 待交付 |
| 17 | `BasePersistedState` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/persisted-state.ts` | 偏好持久化：本地即时 + 远端偏好同步（`sys_user_preference` / `sys_query_scheme`） | 已交付（继承须调整） |
| 18 | `BaseMounted` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `mechanisms/mounted.ts` | 可挂载：挂载 / 卸载生命周期与释放登记统一入口 | 待交付 |
| 19 | `BaseDesignToken` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/design-token.ts` | 设计令牌消费：间距 / 色板 / 密度 / 断点 / 主题订阅 | 已交付（继承须调整） |
| 20 | `BaseClickable` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/interactive.ts` | 可点击：loading / disabled / 防抖 / 危险确认 / 键盘触发 / 埋点（现名 `BaseInteractive`） | 已交付（须改名） |
| 21 | `BaseDragDrop` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/drag-drop.ts` | 拖拽基座：拖拽 / 排序 / 跨区（表单设计器、报表与大屏设计器、工作台卡片共用） | 已交付（继承须调整） |
| 22 | `BaseTabs` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/tabs.ts` | 页签状态：打开 / 关闭 / 固定 / 缓存键清单 | 已交付（继承须调整） |
| 23 | `BaseLocale` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/locale.ts` | 语言上下文：locale / 时区 / 格式上下文（切换与缓存） | 已交付（继承须调整） |
| 24 | `BaseAccess` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/access.ts` | 权限上下文：权限码集合与判定（`has` / `hasAny` / `hasAll`） | 已交付（继承须调整） |
| 25 | `BaseModuleContext` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/module-context.ts` | 上下文基类：宿主注入 router / store / i18n / 用户 / 租户（只读约束） | 已交付（继承须调整） |
| 26 | `BaseAsyncTask` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/async-task.ts` | 异步任务：提交 / 轮询进度（退避）/ 取消 / 结果下载 | 已交付（继承须调整） |
| 27 | `BaseUploadEngine` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/upload-engine.ts` | 上传引擎：分片 / 秒传 / 断点续传 / 进度 / 取消 | 已交付（继承须调整） |
| 28 | `BaseEditorKernel` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/editor-kernel.ts` | 编辑器内核：懒加载 / 创建销毁 / 内容协议（CodeMirror 与富文本 / Markdown 共用） | 已交付（继承须调整） |
| 29 | `BaseOptionSource` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/option-source.ts` | 选项源：加载 / 缓存与版本比对 / 搜索 / 回显（字典、组织、枚举、树、级联共用） | 已交付（继承须调整） |
| 30 | `BasePresignedUrl` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/presigned-url.ts` | 预签名：获取 / 失效重取 / 降级下载（文件预览、下载、导出共用） | 已交付（继承须调整） |
| 31 | `BaseWatermark` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/watermark.ts` | 水印：用户 / 租户信息注入（覆盖内容与导出） | 已交付（继承须调整） |
| 32 | `BaseUserDisplay` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/user-display.ts` | 用户 / 组织展示：姓名 / 头像 / 状态 / 部门路径 | 已交付（继承须调整） |
| 33 | `BaseDynamicRoutes` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/dynamic-routes.ts` | 动态路由：菜单树 → 路由构建 / 注册 / 卸载 | 已交付（继承须调整） |
| 34 | `BaseFormMeta` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/form-meta.ts` | 表单元数据：加载 / 缓存 / 版本比对 | 已交付（继承须调整） |
| 35 | `BaseFormPage` | `BaseComponent` | 组件侧 · 能力基类 | 类 | `capabilities/form-page.ts` | 表单页组合：三态（新增 / 编辑 / 详情）/ 提交 / 脏数据 / 返回 | 已交付（继承须调整） |
| 36 | `BaseInput` | `BaseField` | 组件侧 · 组件基类 | 类 | `capabilities/input.ts` | **输入组件基类**（输入族）：受控绑定、归一、清空、只读判定、composition、焦点（原 `BaseInputControl` + 输入域工厂） | 部分 |
| 37 | `BaseDisplay` | `BaseValue` | 组件侧 · 组件基类 | 类 | `capabilities/display.ts` | **展示组件基类**（展示族）：只读呈现、密度、空值占位、省略 / tooltip、点击取值（现名 `BaseDisplayControl`） | 已交付（须改名） |
| 38 | `BaseTable` | `BaseDataState` | 组件侧 · 组件基类 | 类 | `capabilities/table.ts` | **表格组件基类**：表格数据 / 排序 / 选中 / 分页状态 | 部分（现为宿主 `useListPage`） |
| 39 | `BaseList` | `BaseDataState` | 组件侧 · 组件基类 | 类 | `capabilities/list.ts` | **列表组件基类**：列表数据 / 滚动加载 / 空态与错误态 / 刷新 | 部分（宿主级） |
| 40 | `BaseTreeData` | `BaseDataState` | 组件侧 · 组件基类 | 类 | `capabilities/tree-data.ts` | **树组件基类**（树族）：加载 / 懒加载 / 归一 / 选中 / 勾选 / 过滤 / 缓存 | 已交付（继承须调整） |
| 41 | `BaseModalShell` | `BaseOverlay` | 组件侧 · 组件基类 | 类 | `capabilities/modal-shell.ts` | **模态组件基类**：模态语义 / 关闭拦截 / 底部操作区 | 部分（现为插件层组合式） |
| 42 | `BaseFormContainer` | `BaseValidatable` | 组件侧 · 组件基类 | 类 | `capabilities/form-container.ts` | **表单容器基类**：label 位置与宽度 / 栅格 / 规则汇总 / 校验 / 错误定位 / 只读态 | 已交付（继承须调整） |
| 43 | `BaseContainer` | `BaseSized` | 组件侧 · 组件基类 | 类 | `capabilities/container.ts` | **容器组件基类**：插槽 / 折叠 / 分栏 / 区域令牌 | 已交付（继承须调整） |
| 44 | `BaseLayout` | `BaseSized` | 组件侧 · 组件基类 | 类 | `capabilities/layout.ts` | **布局组件基类**：栅格 / 断点、间距与对齐令牌、根元素与透传、显隐 | 已交付（继承须调整） |
| 45 | `BaseMediaContent` | `BaseSized` | 组件侧 · 组件基类 | 类 | `capabilities/media.ts` | **媒体组件基类**：加载状态机 / 宽高比 / 懒加载 / 过期重取 / 回退（现名 `BaseMedia`） | 已交付（须改名） |
| 46 | `BaseFeedback` | `BaseNotice` | 组件侧 · 组件基类 | 类 | `capabilities/feedback.ts` | **反馈组件基类**：`loading → ready / empty / error` 反馈状态机与降级呈现 | 部分（现为宿主 `useFeedback`） |
| 47 | `BaseNotification` | `BaseNotice` | 组件侧 · 组件基类 | 类 | `capabilities/notification.ts` | **通知组件基类**：站内信 / 公告的展示与已读状态 | 待交付 |
| 48 | `BaseColumnConfig` | `BasePersistedState` | 组件侧 · 组件基类 | 类 | `capabilities/column-config.ts` | **列配置基类**：显隐 / 顺序 / 宽度 / 冻结 / 持久化 | 已交付（继承须调整） |
| 49 | `BaseQueryScheme` | `BasePersistedState` | 组件侧 · 组件基类 | 类 | `capabilities/query-scheme.ts` | **查询方案基类**：条件模型 / 方案保存·应用 / 参数化 | 已交付（继承须调整） |
| 50 | `BaseFieldShell` | `BaseLabeled` | 组件侧 · 组件基类 | 类 | `capabilities/field-shell.ts` | **字段壳基类**：label / 必填 / 帮助 / 错误 / 栅格跨度 / 只读回显 | 已交付（继承须调整） |
| 51 | `BaseFieldPerm` | `BaseFieldShell` | 组件侧 · 组件基类 | 类 | `capabilities/field-perm.ts` | **字段权限基类**：`permVisible` / `editable` / `required` 三态 + 脱敏 | 已交付（继承须调整） |
| 52 | `BaseAsyncResource` | `BaseObject` | 资源与占位 | 类 | `mechanisms/resource.ts` | 异步资源：登记 / 逆序释放 / 幂等 / 释放后拒绝登记（对齐后端同名基类） | 已交付（继承须调整） |
| 53 | `BaseSubscription` | `BaseAsyncResource` | 资源与占位 | 类 | `mechanisms/subscription.ts` | 订阅：`topic` 点分小写（与后端事件表同源）、来源解耦、取消登记 | 已交付 |
| 54 | `BasePlaceholder` | `BaseObject` | 资源与占位 | 类 | `mechanisms/placeholder.ts` | 占位标记：三类原因（`pending` / `null` / `stub`）× 三类降级；占位不请求（对齐后端同名基类） | 已交付（继承须调整） |
| 55 | `BaseNullObject` | `BasePlaceholder` | 资源与占位 | 类 | `mechanisms/placeholder.ts` | 空实现（Null Object）：无副作用、恒定返回（对齐后端同名基类） | 待交付 |
| 56 | `BaseStub` | `BasePlaceholder` | 资源与占位 | 类 | `mechanisms/placeholder.ts` | 未实现桩：`_not_implemented()` 统一抛错（对齐后端同名基类） | 待交付 |
| 57 | `BaseError` | `BaseObject` + `Error` | 资源与占位 | 类（多重继承） | `mechanisms/error.ts` | 错误基座（对齐后端 `BizError`）：`code` / `userMessage`、提示映射（i18n `error.{code}`）、上报委托总基类；保留原生 `instanceof Error` 与堆栈 | 已交付（继承须调整） |
| 58 | `BaseApi` | `BaseObject` | 契约 | 类 | 宿主包 `src/api/base.ts` | 模块 API：路径前缀 / 请求方法 / 自动幂等键 | 已交付（继承 `BaseFrontend`） |
| 59 | `BaseValueObject` | `BaseObject` | 契约 | 类 | `contracts/value-object.ts` | 值对象：不可变语义、字段声明、归一与稳定序列化 | 待交付 |
| 60 | `BaseEntity` | `BaseValueObject` | 契约 | 类 | `contracts/entity.ts` | 实体：标识 / 版本 / 软删除回显（现为纯接口） | 部分（须上提为类） |
| 61 | `BasePageQuery` | `BaseValueObject` | 契约 | 类 | `contracts/page-query.ts` | 分页查询：页码 / 页长 / 排序 / 筛选（现为纯接口） | 部分（须上提为类） |
...
- **现状**：已交付，现名 `BaseFrontend`（`frontend/packages/core/src/base/BaseFrontend.ts`），按本清单改名为 `BaseObject`。
...
- **现状**：已交付（`mechanisms/capability.ts`），按本清单把父基类由组件根上移至总基类。
...
- **现状**：待交付（`mechanisms/pluggable.ts`）。
...
| 基类 | 父基类 | 职责 | 状态 |
| --- | --- | --- | --- |
| `BaseApi` | `BaseObject` | 模块 API：路径前缀 / 请求方法 / 自动幂等键 | 已交付（继承须调整） |
| `BaseValueObject` | `BaseObject` | 值对象：不可变语义 / 字段声明 / 归一 / 稳定序列化 | 待交付 |
| `BaseEntity` | `BaseValueObject` | 实体：标识 / 版本 / 软删除回显 | 部分（须上提为类） |
| `BasePageQuery` | `BaseValueObject` | 分页查询：页码 / 页长 / 排序 / 筛选 | 部分（须上提为类） |
...
- **资源 / 占位 / 错误 / 契约**：`BaseObject → BaseAsyncResource → BaseSubscription`；`BaseObject → BasePlaceholder → BaseNullObject / BaseStub`；`BaseError → (BaseObject, Error)`；`BaseObject → BaseApi`；`BaseObject → BaseValueObject → BaseEntity / BasePageQuery`。
- **目录**：
...
- **命名（强制）**：通用基类与后端同名；前端特有基类 `BaseXxx`；投影 `useXxx`（仅 `frontend/packages/vue/src/bindings/`）。统一遵循《[命名规范](规范/命名规范.md)》「前端命名」节。
...
| `BaseEntity` / `BasePageQuery`（宿主纯接口） | `BaseEntity` / `BasePageQuery` → `BaseValueObject` → `BaseObject` | 上提为类 |
