...
| `BaseChartEngine` / `BaseRealtimeChannel` / `BaseAiStream` / `BaseSearchEngine` / `BaseOrgSource` | `BasePluggable` | 可替换实现（纵向）插件基类：图表引擎 / 实时通道 / AI 流式 / 检索引擎 / 组织数据源；内建与定制实现继承之 | 已交付 |
| `ChartEngineRegistry` / `RealtimeChannelRegistry` / `AiStreamRegistry` / `SearchEngineRegistry` / `OrgSourceRegistry` | `BaseProviderRegistry` | 各域注册表（注册项 `ChartEngineProvider` / `RealtimeChannelProvider` / `AiStreamProvider` / `SearchEngineProvider` / `OrgSourceProvider` 继承 `BaseProvider`） | 已交付 |
