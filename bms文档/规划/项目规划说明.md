...
| 插件化（能力可替换，纵向） | 平台基础能力（**机制**）：能力基类（端口）+ 插件注册表 + 配置 `provider` + 提供者 `get_xxx`；平台内建实现继承（中间层 `BasePluggable`）、项目二次开发组合注册（显式注册 / 虚拟子类 / `Protocol`）；换实现只改配置、业务代码不改；三阶段（配置切换 → 运行时热插拔 → entry points 第三方）；覆盖 ID 生成 / 对象存储 / 通知 / 权限判定 / 检索 / LLM 等，支持外置集团统一权限平台等商业组件 | 机制（跨阶段回补） |
