...
- **展示类 01-05（已建）＋ 06-09（规划中）**：通用表格（`ProTable`）、查询筛选区（`QueryFilter`）、状态标签与描述列表（`StatusTag`/`DescList`）、图表卡（`EChart`/`ChartCard`）、异常与空状态（`ErrorPage`/`EmptyState`/`SkeletonBlock`）；规划通知与消息、全局搜索、文件预览、审计差异查看；
