...
| 后端插件化（能力可替换，纵向）   | 平台基础能力（**机制**）：能力基类（端口）+ 插件注册表 + 配置 `provider` + 提供者 `get_xxx`；平台内建实现继承（中间层 `BasePluggable`）、项目二次开发组合注册（显式注册 / 虚拟子类 / `Protocol`）；换实现只改配置、业务代码不改；三阶段（配置切换 → 运行时热插拔 → entry points 第三方）；覆盖 ID 生成 / 对象存储 / 通知 / 权限判定 / 检索 / LLM 等，支持外置集团统一权限平台等商业组件                                                                                                                                                                                                    | 机制（跨阶段回补）            |
| 前端插件化（模块与扩展点，横向） | 平台基础能力（**机制**）：前端注册表（路由/菜单、组件、字段渲染器、图标、工作台卡片、主题）+ 动态导入分包；产品（biz/cw）前端模块产物**构建期合并**进平台前端应用，动态路由生效、前端免发版；MVP 不引入运行时远程加载 / 微前端                                                                                                                                                                                                                                                                                                                                                                 | 机制（随前端组件库/页面回补） |

> 末两行为**平台基础能力（机制）**，非面向终端用户的功能模块，不计入上方用户功能模块数量；与「平台扩展接入流程」节（横向产品接入）配套，机制细节见《[架构设计 · 扩展点与插件化](../设计/架构设计/10_架构设计_子系统_扩展点与插件化.md)》与知识档案《[插件化架构](../资料/知识档案/插件化架构/01_插件化架构_总览.md)》。
...
| 二 后端基座                     | **后端基座（地基波）**——core 基座补齐（异常体系 `BizError` + 统一响应、配置 / 安全基座）、模块基类落地与异步化（`BaseRepository` 数据库实现、`BaseService`、`BaseSchema`、`BaseModel` 雪花 ID/软删除/审计/乐观锁）、数据访问底座（引擎 / 会话工厂 / 三库方言 / 读写分离占位）、多租户数据拓扑（三库拓扑 / 平台库基础表 / 租户解析 / 引擎注册表 / 批量迁移）、模块注册表（sys_module 表与种子 / 唯一性校验 / 查询接口）；跨阶段基座登记（缓存 Region 分域 / 数据范围注入 / 分片路由 / 事件 / 任务 / 审计捕获）、插件化中间层 `BasePluggable` 与插件注册表基座（能力实现注册/选择/配置装配，纵向）。架构见《[架构设计 · 后端基础类体系](../设计/架构设计/04_架构设计_后端基础类体系.md)》 | `BizError` 与统一响应生效、`BaseModel` 落库（雪花 ID/软删除/审计/乐观锁）、三库拓扑可路由（MySQL/PostgreSQL/达梦 DM8）、租户隔离与批量迁移正确、模块注册校验生效（表前缀/错误码段/事件域冲突被拒）、基类异步切换完成；跨阶段基座登记齐备                                                            |
...
| 扩展点与插件化 | 配置切换 provider 生效且业务代码不改；非法 provider / 重名 / 契约版本不符启动被拒；双轨注册（继承 / 组合）可用；全部注册实现契约测试通过；前端扩展点注册生效、产品前端模块构建期合并后动态路由可见                   | 二 / 三     |
