...
> **组件实现阶段（承接组件设计）**：地基波——基础组件类 L0-L4（14）/ 布局组件类（11）/ 容器组件类（9）/ 基础控件类（8）/ 基础类（3），约 45 个节点——在**阶段三 前端组件库**交付，是前端页面开发的前置基础（不完成则页面开发无法推进），架构见《[架构设计 · 前端组件体系](../设计/架构设计/32_架构设计_前端组件体系.md)》；依赖后端能力的字段类（12）/ 展示类（14）/ 交互类（19）约 45 个节点随对应阶段回补（字典/文件→阶段六 通用能力、审批→阶段七 工作流、报表与大屏→阶段十四 报表 BI、全局搜索→阶段十六 全文检索、AI 助手→阶段十七 AI 能力）。
>
...
> 前端工程结构、菜单机制细粒度（动态路由注册、v-perm 指令、字段权限渲染）、双工程技术要点与性能预算见《[架构设计 · 前端架构](../设计/架构设计/28_架构设计_前端架构.md)》；组件分层、八类边界与分包/测试约束见《[架构设计 · 前端组件体系](../设计/架构设计/32_架构设计_前端组件体系.md)》。
...
| 二 后端基座 | **后端基座（地基波）**——core 基座补齐（异常体系 `BizError` + 统一响应、配置 / 安全基座）、模块基类落地与异步化（`BaseRepository` 数据库实现、`BaseService`、`BaseSchema`、`BaseModel` 雪花 ID/软删除/审计/乐观锁）、数据访问底座（引擎 / 会话工厂 / 三库方言 / 读写分离占位）、多租户数据拓扑（三库拓扑 / 平台库基础表 / 租户解析 / 引擎注册表 / 批量迁移）、模块注册表（sys_module 表与种子 / 唯一性校验 / 查询接口）；跨阶段基座登记（缓存 Region 分域 / 数据范围注入 / 分片路由 / 事件 / 任务 / 审计捕获）。架构见《[架构设计 · 后端基础类体系](../设计/架构设计/33_架构设计_后端基础类体系.md)》 | `BizError` 与统一响应生效、`BaseModel` 落库（雪花 ID/软删除/审计/乐观锁）、三库拓扑可路由（MySQL/PostgreSQL/达梦 DM8）、租户隔离与批量迁移正确、模块注册校验生效（表前缀/错误码段/事件域冲突被拒）、基类异步切换完成；跨阶段基座登记齐备 |
