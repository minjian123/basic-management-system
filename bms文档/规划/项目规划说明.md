...
**通用组件（组件设计体系）**：管理端反复出现的界面元素沉淀为可复用组件，位于布局设计之下、页面实现之上；契约（职责、Props 与事件、取数与缓存、交互与边界）见《[组件设计 · 总览](../设计/组件设计/01_组件设计_总览.md)》，按六类分文件夹（基础组件类 / 字段类 / 展示类 / 交互类 / 基础类 / 基础控件类，类内编号），组件清单与路线图以总览为权威：
...
- **基础控件类 01-08（规划中）**：文本框、文本域、密码框、数字框、下拉框（静态选项）、单选、复选、开关——均继承 00-01 基类 + 00-02 字段壳 + 00-03 字段权限。
