...
- **容器组件类（已建 1 / 规划 8）**：已建 弹窗/抽屉表单（`FormDialog`/`FormDrawer`/`ConfirmDialog`）；规划 滚动容器（`ScrollContainer`）、懒加载容器（`LazyContainer`）、全屏容器（`FullscreenContainer`）、自适应高度容器（`AutoHeight`）、虚拟列表容器（`VirtualList`）、加载遮罩容器（`LoadingContainer`）、分区容器（`SectionContainer`）、宽高比容器（`AspectRatio`）——继承 02 基础组件类 L2-04/L2-05；
...
| 组件设计 | 通用组件契约（职责、Props 与事件、取数与缓存、交互与边界），总纲 + 六类节点（基础组件 L0-L4 共 14 / 字段 01-12 / 展示 01-14 / 交互 01-22 / 基础 01-03；基础控件 01-08 规划中） | 阶段十一前（已交付）；00/05 于 2026-09 补充 |
