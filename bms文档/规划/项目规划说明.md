...
> 关键选型的完整决策记录（ADR，含逐项取舍评估）见《[架构设计·决策与风险](../设计/架构设计/33_架构设计_决策与风险.md)》，本节为概览性说明；两处口径以架构节点为准。
...
**插件化相关结构**：后端平台能力实现经统一插件注册与装配入口（目标 `app/core/plugin.py`：能力基类 `BasePluggable` + 插件注册表 + `resolve_plugin`）注册，平台内建实现集中 `backend/app/plugins/`；前端以注册表挂载扩展点，产品（biz/cw）前端模块经**产物构建期合并**进入 `frontend/src/`（`views/{模块}/`、组件与字段渲染器按注册表挂接）。两轴机制见《[架构设计 · 扩展点与插件化](../设计/架构设计/34_架构设计_子系统_扩展点与插件化.md)》。
...
> 末两行为**平台基础能力（机制）**，非面向终端用户的功能模块，不计入上方用户功能模块数量；与「平台扩展接入流程」节（横向产品接入）配套，机制细节见《[架构设计 · 扩展点与插件化](../设计/架构设计/34_架构设计_子系统_扩展点与插件化.md)》与知识档案《[插件化架构](../资料/知识档案/插件化架构/01_插件化架构_总览.md)》。
...
> 机制细节（主体链收敛、权限分层推导、字段/数据权限、两级校验点、三权分立）见《[架构设计 · 子系统 · 权限计算引擎](../设计/架构设计/14_架构设计_子系统_权限计算引擎.md)》。

### 7.1 业务与动作清单（权限码总表） <a id="perm-list"></a>

权限码 = `业务:动作`。业务码存 sys_business、动作码存 sys_action，均【平台库】统一维护，租户侧可见、可分配、不可增删；作为种子数据与菜单/按钮挂接依据。

> MVP 全量清单（业务码逐码说明、动作码含义与典型权限码）见《[架构设计 · 子系统 · 权限计算引擎](../设计/架构设计/14_架构设计_子系统_权限计算引擎.md)》「业务与动作清单」节与《[英文简称规范](../规范/英文简称规范.md)》「动作码简称」节，此处不再重复维护。
...
> 路由实现（租户 engine 懒加载/池控制、读写分离、分片机制）见《[架构设计 · 子系统 · 多租户路由](../设计/架构设计/11_架构设计_子系统_多租户路由.md)》与《[架构设计 · 子系统 · 数据访问与分片](../设计/架构设计/12_架构设计_子系统_数据访问与分片.md)》。
...
> 机制细节（双 token 存储/轮换、限流验证码、密码与账号策略、会话黑名单与踢出、密钥轮换、上传与基础防护、AI 开关约束）见《[架构设计 · 子系统 · 认证与会话](../设计/架构设计/13_架构设计_子系统_认证与会话.md)》与《[架构设计 · 性能与安全](../设计/架构设计/30_架构设计_性能与安全.md)》「安全架构」节。
...
> 机制细节（捕获、链构造、跨月衔接、校验巡检、留存销毁）见《[架构设计 · 子系统 · 审计哈希链](../设计/架构设计/18_架构设计_子系统_审计哈希链.md)》。
...
> 上述机制的完整设计（连接规划、key 空间、三防与一致性协议、分页/索引、降级矩阵、超时资源）见《[架构设计 · 性能与安全](../设计/架构设计/30_架构设计_性能与安全.md)》「性能架构」节与《[架构设计 · 数据架构](../设计/架构设计/07_架构设计_数据架构.md)》「缓存架构」节（缓存）。
...
> 完整测试策略（分层定义、新功能覆盖全量清单、兼容性矩阵）见《[架构设计 · 测试与质量](../设计/架构设计/31_架构设计_测试与质量.md)》。
...
> 部署细节（Compose 编排、版本维护）见《[架构设计 · 部署与运维](../设计/架构设计/32_架构设计_部署与运维.md)》。
...
> 种子结构、顺序与幂等要求见《[架构设计 · 部署与运维](../设计/架构设计/32_架构设计_部署与运维.md)》「初始化数据」节。
...
> 完整检查单与上线步骤见《[架构设计 · 部署与运维](../设计/架构设计/32_架构设计_部署与运维.md)》「GA 正式上线计划」节。
...
> 值班口径、问题分级时限表、容量与成本监控、License 健康、机密与权限复核见《[架构设计 · 部署与运维](../设计/架构设计/32_架构设计_部署与运维.md)》「运营支持模型」节。
...
平台运行依赖若干外部服务与第三方能力，统一登记降级口径（降级矩阵细化见《[架构设计 · 性能与安全](../设计/架构设计/30_架构设计_性能与安全.md)》）：
