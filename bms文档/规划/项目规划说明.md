...
**可替换作为风险对冲**：上述组件均为开源，上游无持续维护承诺，且非商业版功能深度有限。因此平台把「可替换」（插件化）作为选型配套策略：关键能力以能力基类（端口）+ 注册表 + 配置选择 + 提供者组织，**上游停维护、许可变更或客户指定商业组件时，切换实现而不改业务代码**（机制见「平台能力插件化」节，策略与迁移见知识档案《[开源与商业组件可替换](../资料/知识档案/插件化架构/02_插件化架构_开源与商业组件可替换.md)》）。
...
> 关键选型的完整决策记录（ADR，含逐项取舍评估）见《[架构设计·决策与风险](../设计/架构设计/34_架构设计_决策与风险.md)》，本节为概览性说明；两处口径以架构节点为准。
...
- **ElasticSearch 8.x**：全文检索，覆盖三类场景——主数据全局搜索（用户/部门/角色/字典/业务单据/文件元数据/帮助文章）、审计日志检索（操作/登录/开放接口/数据变更，按月索引与分片表对齐）、文件内容检索（PDF/Word/Excel/TXT，经 Celery 任务解析文本后入库索引；扫描件 OCR 由阶段十九 AI 支持）。数据同步走 RocketMQ 事件驱动异步消费（独立 event-worker 进程，复用现有事件模型；索引重建时重放事件）；ES 只承担检索匹配，返回业务 ID + 高亮，详情回查数据库取最新数据，避免双写一致性难题。索引按用途分（主数据/日志/文件），均含 tenant_id 字段，查询强制租户隔离与数据权限过滤；生产开启安全认证，MVP 单节点部署。
- **AI 能力（阶段十九）**：统一 LLM 适配层——默认接入外部大模型 API（DeepSeek/Qwen 等），敏感场景可配置私有化端点（Ollama/vLLM），API key 走 Secret 管理，模型可切换不侵入业务代码。八项能力（智能报表助手、智能审批辅助、AI 内容生成、语义搜索与文件问答、OCR 与自动分类、审计异常检测、知识库问答机器人、帮助文档智能维护）与模块的结合见"功能模块"节；向量检索用 Milvus（独立部署，依赖 etcd 与 MinIO，MinIO 已复用），embedding 管线经 RocketMQ 事件驱动（复用 event-worker）；OCR 默认本地 PaddleOCR，可切换多模态模型 API。帮助文档智能维护：输入源为设计文档与表单元数据（sys_form/sys_field/业务权限码清单），支持手动触发（维护人点击生成/更新草稿）、变更事件自动触发（菜单/表单/字段变更经 form.updated 事件驱动，自动生成更新草稿并通知维护人）、定时质量评估扫描（评估陈旧度/完整性/准确性并生成改进建议）；生成结果一律为草稿（draft），人工审核后发布，不自动覆盖已发布文章，全量落 ai_chat_log 审计。设计约束：AI 交互全量落 ai_chat_log 审计、提示词与数据不出租户边界（向量库按租户隔离、prompt 注入防护）、AI 结果默认仅辅助展示（意见、草稿、检索、告警建议）、AI 接口独立限流。AI 自动执行写操作与 AI 自动审批为两个独立可选功能，各自由 sys_config 开关控制且默认关闭，AI 自动审批依赖 AI 自动执行写操作开启（见"认证与安全"节）。
...
- **插件化（能力可替换）**：平台能力统一以「能力基类（端口）+ 插件注册表 + 配置选择 + 提供者 `get_xxx`」组织，业务代码只依赖基类与提供者、不直连具体实现；换实现只改 `config.toml` 的 `provider`，重启生效。机制搭配——平台内建实现继承统一基类（中间层 `BasePluggable`），项目二次开发可用组合注册（显式注册 / 虚拟子类 / `Protocol`）；契约版本与唯一性在启动与 CI 校验。用于对冲开源供应链风险与客户指定商业组件（如权限外置集团统一权限平台、存储换客户既有 S3）；完整机制、三阶段演进与开发 SOP 见知识档案《[插件化架构](../资料/知识档案/插件化架构/01_插件化架构_总览.md)》。
...
- **前端插件化（扩展点与构建期合并）**：前端以**扩展点 + 注册表**组织可扩展面——路由/菜单、页面区域、通用组件、字段渲染器、图标、工作台卡片、主题令牌各设注册表（如 `iconRegistry`、字段类型注册表），插件以注册方式挂接、业务页面不直接互相依赖；插件页面一律**动态导入、独立分包**，不进首屏主包。产品（biz/cw）前端模块自阶段五起以**运行时片段**（Module Federation）接入平台前端应用（构建期合并保留为过渡形态）：菜单/表单/按钮/字段走平台注册、动态路由生效，片段独立构建与发布，类型由 OpenAPI 生成保持前后端契约一致；阶段四先行交付宿主薄壳与片段契约骨架；前端基类体系（开放式基类体系）与注册表约定见平台《前端基类清单》与《[前端开发规范](../规范/前端开发规范.md)》，完整机制见知识档案《[前端插件化](../资料/知识档案/插件化架构/14_插件化架构_前端插件化.md)》。
...
│   │   ├── views/        # 页面（按模块）
│   │   ├── layouts/      # 主布局
│   │   ├── components/   # 通用组件（按域分目录：chart / query / table / display / modal / import-export / editor / icon / approval 等，见组件设计（六类节点））
│   │   ├── i18n/         # 中英文文案
...
├── scripts/          # 开发期工具链（WOL / bg / defect / backup / gitlab / reorder-design / base-check）
...
**插件化相关结构**：后端平台能力实现经统一插件注册与装配入口（目标 `app/core/plugin.py`：能力基类 `BasePluggable` + 插件注册表 + `resolve_plugin`）注册，平台内建实现集中 `backend/app/plugins/`；前端以注册表挂载扩展点，产品（biz/cw）前端模块经**产物构建期合并**进入 `frontend/src/`（`views/{模块}/`、组件与字段渲染器按注册表挂接）。两轴机制见《[架构设计 · 扩展点与插件化](../设计/架构设计/10_架构设计_子系统_扩展点与插件化.md)》。
...
| AI 能力                          | 智能报表助手（N片段SQL 问数）、智能审批辅助（参考意见/风险提示）、AI 内容生成（邮件/通知模板、审批意见、申请理由草稿）、语义搜索与文件问答（Milvus）、OCR 与文件自动分类、审计异常检测、知识库问答机器人；LLM 适配层（外部 API/私有化可切）、AI 交互审计（ai_chat_log）；AI 自动执行写操作（独立开关，默认关闭）与 AI 自动审批（独立开关，默认关闭，依赖写操作开关开启）                                                                                                                                                                                                                                     | 阶段十九                      |
...
> 表级清单（表名 + 归属 + 说明）以架构与概要设计为准：全量表名见《[架构设计 · 数据架构](../设计/架构设计/07_架构设计_数据架构.md)》，逐表关键字段与字段语义见各《[概要设计](../设计/概要设计/01_概要设计_总览.md)》模块节点与《[数据库设计](../设计/数据库设计/01_数据库设计_总览.md)》，不再在本节维护逐字段时间。

> **平台能力实现（插件）配置不入库**：能力的实现选择（`provider`）是**环境配置**，存 `config.toml`（`[<domain>] provider/options`），不建表；插件注册为**进程内注册表**（启动构建、运行只读）。第三方插件治理表 `sys_plugin`（来源 / 版本 / 契约版本 / 状态）属**阶段四评估项，MVP 不落地**（机制见「平台能力插件化」节）。
...
采用 RBAC + 主体链 + 权限分层（菜单→表单→业务、按钮→动作、字段可见可编辑）+ 动作级数据权限。要点：主体角色绑定用通用表 sys_role_assign（任意主体链，最终收敛为用户→角色）；业务/动作权限码命名 `业务:动作`，平台统一维护；前端按权限码显隐、后端 `require_permission` 双重强制校验；平台超管 + 租户侧三权分立（系统/安全/审计三类管理员权限互斥）。插件清单查看归平台超管（sys 业务）。

> 机制细节（主体链收敛、权限分层推导、字段/数据权限、两级校验点、三权分立）见《[架构设计 · 子系统 · 权限计算引擎](../设计/架构设计/15_架构设计_子系统_权限计算引擎.md)》。

### 7.1 业务与动作清单（权限码总表） <a id="perm-list"></a>

权限码 = `业务:动作`。业务码存 sys_business、动作码存 sys_action，均【平台库】统一维护，租户侧可见、可分配、不可增删；作为种子数据与菜单/按钮挂接依据。

> MVP 全量清单（业务码逐码说明、动作码含义与典型权限码）见《[架构设计 · 子系统 · 权限计算引擎](../设计/架构设计/15_架构设计_子系统_权限计算引擎.md)》「业务与动作清单」节与《[英文简称规范](../规范/英文简称规范.md)》「动作码简称」节，此处不再重复维护。
...
- 错误码：平台段 1xxxx~9xxxx 为 5 位数字（万位为模块段、后 4 位段内序号），产品扩展段 10xxxx 起升为 6 位（分配见"平台扩展接入流程"节）；一经发布稳定不变，文案由前端按 i18n 映射。分段明细见[《架构设计 · 接口与集成》](../设计/架构设计/09_架构设计_接口与集成.md#api-error)。
...
- **插件清单（平台超管）**：`GET /api/v1/plugins`、`GET /api/v1/plugins/{key}` 只读返回各能力当前 provider 与已注册实现（名称 / 版本 / 状态，不含密钥）；**无运行时切换写接口**（配置切换重启生效属阶段三 后端插件化；运行时热插拔属演进路线，另设计开关与审计）。

> 完整约定（分页格式、RESTful 命名、错误码结构、防重放、契约快照）见《[架构设计 · 接口与集成](../设计/架构设计/09_架构设计_接口与集成.md)》。
...
事件命名 `{域}.{动作}`，Webhook 按事件名订阅；默认事件全量清单见架构 09。

### 9.4 开放接口管理模块 <a id="integ-open"></a>

第三方应用注册（client_id/secret）、scope 分配与回收、IP 白名单、调用日志与统计。

> 对外 API / 出站集成 / 事件模型全量清单（防重放细节、分页、订阅开关矩阵）见《[架构设计 · 接口与集成](../设计/架构设计/09_架构设计_接口与集成.md)》。
...
> 路由实现（租户 engine 懒加载/池控制、读写分离、分片机制）见《[架构设计 · 子系统 · 多租户路由](../设计/架构设计/12_架构设计_子系统_多租户路由.md)》与《[架构设计 · 子系统 · 数据访问与分片](../设计/架构设计/13_架构设计_子系统_数据访问与分片.md)》。
...
主键（雪花 ID）、软删除（复合唯一索引 + 回收站）、审计字段、乐观锁、UTC 时间、多语言 i18n 附表、字符集、索引与 SQL 规范等约定见《[架构设计 · 数据架构](../设计/架构设计/07_架构设计_数据架构.md)》「数据规范」节。

## 12. 认证与安全 <a id="security"></a>

认证与安全要点：双 token（access 30 分钟 + refresh 14 天滚动轮换，refresh 存 httpOnly + Secure + SameSite cookie）；SSO 单点登录（OIDC 为主 + CAS 兼容，外部 IdP 回跳 JIT 自动建号，本地账号密码并存作超管应急）；移动端企业微信/钉钉内嵌 H5 免登；密码 PBKDF2 + 失败限流；找回密码双通道、token 短时效单次有效；数据脱敏（掩码 + 导出脱敏 + data:plain 明文权限）；多端会话（上限 5、强制踢出即时生效）；上传安全（白名单 + 分片 SHA256 + MinIO 私有桶 + 预签名 URL）；JWT secret 轮换、密钥走 Secret 管理；AI 安全（交互全量审计、ai.auto_execute / ai.auto_approve 独立开关默认关闭且 approve 依赖 execute 开启）；可替换能力 / 插件的实现密钥一律走 Secret 管理，**第三方插件默认视为不可信**（受控来源 + 必要时进程/容器隔离，见「平台能力插件化」节）。

> 机制细节（双 token 存储/轮换、限流验证码、密码与账号策略、会话黑名单与踢出、密钥轮换、上传与基础防护、AI 开关约束）见《[架构设计 · 子系统 · 认证与会话](../设计/架构设计/14_架构设计_子系统_认证与会话.md)》与《[架构设计 · 性能与安全](../设计/架构设计/31_架构设计_性能与安全.md)》「安全架构」节。
...
| 依赖与镜像 | Renovate 自动升级；依赖漏洞与 Trivy 镜像扫描 CRITICAL / HIGH 阻断发版；SBOM + NOTICE 随交付维护（见「开源与许可协议」节）                                                        |
...
审计合规要点：关键表字段级变更审计（ORM 事件自动记录，按月分表）；审计类日志哈希链防篡改（prev_hash + record_hash，RocketMQ 分区有序消费保证链序，链校验工具定期巡检）；三权分立（系统/安全/审计管理员权限互斥）；合规留存（90 天热表 + 归档冷存储，归档前链完整性校验，哈希链随归档导出，合规销毁留日志）。能力 provider 切换与插件启停留审计（谁在何时把某能力切到哪个实现）。

> 机制细节（捕获、链构造、跨月衔接、校验巡检、留存销毁）见《[架构设计 · 子系统 · 审计哈希链](../设计/架构设计/19_架构设计_子系统_审计哈希链.md)》。
...
- **前端性能基线**：**数值不在此重复，以《[架构设计 · 前端架构](../设计/架构设计/08_架构设计_前端架构.md)》的性能预算为准**（包体积、Core Web Vitals、移动端档位、Lighthouse 抽测与超预算跟踪均在该节点定义）。规划层只约束三条工程要求：路由级按需分包、ECharts 与大屏组件懒加载、构建产物超预算即 CI 告警并列入优化清单。
...
- **能力替换与降级**：可替换能力应有备选 / 兜底实现（外部实现不可用切 fallback provider，如存储降级本地、权限判定本地引擎兜底）；权限判定、ID 生成等**热路径**走本地 / 缓存，避免每次外呼（见「平台能力插件化」节）。

> 上述机制的完整设计（连接规划、key 空间、三防与一致性协议、分页/索引、降级矩阵、超时资源）见《[架构设计 · 性能与安全](../设计/架构设计/31_架构设计_性能与安全.md)》「性能架构」节与《[架构设计 · 数据架构](../设计/架构设计/07_架构设计_数据架构.md)》「缓存架构」节（缓存）。

## 15. 前端页面清单与菜单机制 <a id="pages"></a>

**页面（PC 管理端）**：登录页、主布局、首页工作台、工作台卡片管理、工作台模板管理、表单布局管理、用户管理、账号锁定、岗位管理、角色管理、菜单管理（含表单/按钮/字段维护与字段权限配置）、业务权限管理、动作权限管理、部门管理、字典管理、系统参数、操作日志、登录日志、文件管理、会话管理、公告管理、帮助中心、帮助管理、流程建模、流程实例、我的待办/已办、通知中心、开放接口管理、Webhook 管理、系统监控、任务调度、邮件模板、邮件日志、短信记录、租户管理（含配额与模块开通/开关）、IdP 配置、数据集管理、报表设计器、报表查看、大屏设计器、大屏播放、数据变更审计、语言管理、语言包维护、归档策略、归档执行记录、备份管理、回收站、归档数据查询、全局搜索、AI 助手、模型配置、插件清单、个人中心（改密/语言/时区/喜好偏好）。

> 业务页面（biz 的采购/收付款/供应商/销售/仓储，cw 的创作模块）由产品仓库前端模块提供，经平台动态路由合并展示，不在平台页面清单。

**菜单机制**：菜单树由后端按用户权限动态返回（菜单→表单→业务权限过滤，按 locale 关联 i18n 附表翻译），表单内按钮按动作权限显隐（默认无）、字段按字段权限控制；前端不硬编码路由表，新增菜单/表单/按钮/字段无需发版。前端以**扩展点 + 注册表**挂接（路由/菜单、通用组件、字段渲染器、图标、工作台卡片、主题），插件页面动态导入、独立分包、不进首屏；产品前端模块以运行时片段加载（阶段五起；构建期合并为过渡，见「平台能力插件化」节）。

**通用组件（组件设计体系）**：管理端反复出现的界面元素沉淀为可复用组件，位于布局设计之下、页面实现之上；契约（职责、Props 与事件、取数与缓存、交互与边界）见《[组件设计 · 总览](../设计/组件设计/01_组件设计_总览.md)》，按八类分文件夹（基础组件类 / 布局组件类 / 容器组件类 / 基础控件类 / 字段类 / 展示类 / 交互类 / 基础类，类内编号），组件清单与路线图以总览为权威：

- **基础组件类（基类节点 15（机制基类 2 + 能力片段 11 + 域基类 2） 篇，已建）**：根系基类（`BaseFrontend`/`useFrontendBase`，所有基类之根）、组件根基类（`BaseComponent`，透传/尺寸密度主题/i18n/可访问性/loading…）、**能力片段（开放集合，预置 31，组合式 `useXxx`：形态 7（`useInteractive`/`useInputControl`/`useDisplayControl`/`useContainer`/`useFormContainer`/`useMedia`/`useLayout`）/ 值·字段链 4（`useValue`/`useFieldShell`/`useFieldPerm`/`useField`）/ 选项源 1（`useOptionSource`）/ 复用类 19（上传引擎 `useUploadEngine`、异步任务 `useAsyncTask`、页签 `useTabs`、偏好持久化 `usePersistedState`、编辑器内核 `useEditorKernel`、树数据 `useTreeData`、用户展示 `useUserDisplay`、动态路由 `useDynamicRoutes`、表单元数据 `useFormMeta`、预签名 `usePresignedUrl`、拖拽 `useDragDrop`、设计令牌 `useDesignToken`、表单页 `useFormPage`、语言上下文 `useLocale`、权限上下文 `useAccess`、列配置 `useColumnConfig`、查询方案 `useQueryScheme`、水印 `useWatermark`、片段上下文 `useFragmentContext`））**、机制基类（占位 `BasePlaceholder`/异步资源 `useAsyncResource`/注册表 `BaseRegistry`/异常 `BaseError`/事件订阅 `useSubscription`）、域基类（`BaseInput` 输入域/`BaseDisplay` 展示域，`BaseTree` 树域随实施新增）；体系权威见《前端开发规范》「前端基类体系（开放式，强制）」节与《[前端基类清单](../前端基类清单.md)》「能力片段」节；
- **布局组件类 01-11（已建）**：主框架布局壳（`AppLayout`）、表单框架壳（`FormFrame`）、页面容器（`PageContainer`）、多标签导航（`TabsNav`/`FormFrameTabs`）、树形主从布局（`MasterDetail`/`MasterTree`）、栅格（`Grid`）、间距/分割线（`Space`/`Divider`）、分栏（`Split`）、内容页签（`Tabs`）、卡片（`Card`）、折叠面板（`Collapse`）——均组合 02 基础组件类 布局片段；
- **容器组件类 01-09（已建）**：弹窗/抽屉表单（`FormDialog`/`FormDrawer`/`ConfirmDialog`）、滚动容器（`ScrollContainer`）、懒加载容器（`LazyContainer`）、全屏容器（`FullscreenContainer`）、自适应高度容器（`AutoHeight`）、虚拟列表容器（`VirtualList`）、加载遮罩容器（`LoadingContainer`）、分区容器（`SectionContainer`）、宽高比容器（`AspectRatio`）——均继承 02 基础组件类 容器片段/表单容器片段；
- **字段类 01-12（已建）**：字典字段（`DictSelect`）、日期时间字段（`DateField`/`DateRangeField`）、数值与金额字段（`NumberField`/`AmountField`）、树选择字段（`DeptTreeSelect`）、组织选择字段（`UserSelectField`）、文件上传字段（`FileUploadField`）、富文本字段（`RichTextField`/`MarkdownField`）、开关与枚举字段（`SwitchField`/`EnumField`）；穿梭框（`TransferField`）、验证码（`Captcha`）、标签输入（`TagInput`）、级联选择（`CascaderField`）；
- **展示类 01-14（已建）**：通用表格（`ProTable`）、查询筛选区（`QueryFilter`）、状态标签与描述列表（`StatusTag`/`DescList`）、图表卡（`EChart`/`ChartCard`）、异常与空状态（`ErrorPage`/`EmptyState`/`SkeletonBlock`）、通知与消息（`NotificationBell`/`NotificationList`）、全局搜索（`GlobalSearch`/`SearchResults`）、文件预览（`FilePreview`/`ImageViewer`/`PdfViewer`）、审计差异查看（`AuditDiffViewer`/`DiffTable`/`FieldDiff`）、指标卡（`StatCard`）、快捷入口（`QuickEntry`）、头像与用户信息（`UserAvatar`/`UserPill`）、二维码（`QrCode`）、水印（`Watermark`）；
- **交互类 01-19（已建）**：导入导出（`ImportDialog`/`ExportButton`）、审批流展示（`FlowProgress`/`ApprovalTimeline`/`ApprovalActionPanel`）、图标选择器（`IconPicker`/`IconDisplay`）、代码/表达式编辑器（`SqlEditor`/`ExpressionEditor`/`CronEditor`，CodeMirror 6）、表单设计器（`FormDesigner`/`DesignerCanvas`，vuedraggable）、流程建模器（`ProcessDesigner`/`BpmnCanvas`，bpmn-js）、权限配置（`PermissionConfig`）、报表设计器（`ReportDesigner`/`ReportCanvas`，ECharts + gridstack）、大屏设计器与播放（`ScreenDesigner`/`ScreenPlayer`，vue-flow）、AI 助手（`AiAssistant`/`ChatPanel`，SSE 流式）、国际化文案编辑器（`I18nEditor`/`MessageGrid`）、表单渲染器（`FormRenderer`）、侧边菜单（`SideMenu`）、向导/步骤（`Wizard`）、偏好设置面板（`PreferencePanel`）、批量操作栏（`BatchActionBar`）、主题与品牌化（`ThemeSwitcher`）、租户切换（`TenantSwitcher`）；打印/导出 PDF（`PrintTemplate`）；
- **基础类 01-03（已建）**：请求封装与错误处理（`useRequest`/`useListPage`）、权限指令（`v-perm`/`hasPerm`/`canAccess`；字段权限已迁至 02 基础组件类 字段权限片段）、格式化工具与组合式函数（`formatDateTime`/`formatAmount`/`validators`）；
- **基础控件类 01-08（已建）**：文本框（`TextField`）、文本域（`TextareaField`）、密码框（`PasswordField`）、数字框（`NumberInput`）、下拉框（`SelectInput`，静态选项）、单选（`RadioField`）、复选（`CheckboxField`）、开关（`SwitchInput`）——均组合 02 基础组件类 输入域基类（经 片段/片段/组件根 至 根系）；字典/枚举/金额等带数据源或格式语义的字段由 04 字段类继承它们扩展。

> **组件实现阶段（承接组件设计）**：**地基波**——基础组件类（基类节点 15）/ 布局组件类（11）/ 容器组件类（9）/ 基础控件类（8）/ 基础类（3），加上随地基波先行的**展示类「异常与空状态」**与**交互类「侧边菜单」**（各 1），合计 48 个节点——在**阶段四 前端组件库**交付，是前端页面开发的前置基础（不完成则页面开发无法推进），架构见《[架构设计 · 前端组件体系](../设计/架构设计/05_架构设计_前端组件体系.md)》；依赖后端能力的字段类（12）/ 展示类（13）/ 交互类（18）约 43 个节点随对应阶段回补（字典/文件→阶段八 通用能力、审批→阶段九 工作流、报表与大屏→阶段十六 报表 BI、全局搜索→阶段十八 全文检索、AI 助手→阶段十九 AI 能力）。
>
> **波次口径（阶段四，2026-09-15 任务重排）**：地基波组件全部为本期实施范围，其中「异常与空状态」（原属展示类回补项）与「侧边菜单」（原属交互类回补项）因被容器 / 布局 / 宿主外壳复用而**前置到地基波先行**；字段 / 展示 / 交互类的后端依赖项本期交付**占位版**（空态 / 禁用态 / 降级语义，对齐后端 Null 占位模式），真实实现随对应阶段回补。阶段四任务顺序、工时（地基波 202h + 占位 49h + 回补 66h）与排期见《前端组件库计划》。
>
> 组件按域分目录置于 `frontend/src/components/`（能力片段与域基类按能力域置于 `components/base/<能力域>/`，机制层在 `src/base/`，契约/数据根在 `src/api/`、`src/stores/`、`src/utils/`），工具与组合式函数置于 `frontend/src/utils/`；组件契约变更须先改组件设计节点再改代码。移动端 `frontend-mobile/` 不复用 PC 组件（Vant 生态，部分节点单独说明移动端口径）。
...
> 前端工程结构、菜单机制细粒度（动态路由注册、v-perm 指令、字段权限渲染）、双工程技术要点与性能预算见《[架构设计 · 前端架构](../设计/架构设计/08_架构设计_前端架构.md)》；基类体系与目录、八类边界与分包/测试约束见《[架构设计 · 前端组件体系](../设计/架构设计/05_架构设计_前端组件体系.md)》。

## 16. 测试策略与测试流程 <a id="test"></a>

分层测试与工具选型：单元/接口测试 pytest + httpx（SQLite，CI 每次 PR 执行）；集成测试由 GitLab CI 起 MySQL/PostgreSQL 容器（达梦复用 mjbk 常驻库）覆盖多数据源、读写分离、真实分片与三库方言迁移；前端 Vitest 组件测试 + Playwright E2E；压测 locust（登录/列表/审批场景）；安全专项（越权/注入/XSS/上传绕过/限流爆破/脱敏泄露/会话劫持）+ 依赖漏洞扫描 + Trivy 容器镜像扫描（CRITICAL / HIGH 阻断发版）；兼容性 PC 四浏览器自动化、移动端缩减矩阵；测试数据独立种子脚本。新功能覆盖清单逐项随模块交付纳入测试，全量条目见架构 32。换实现保障：平台能力端口以**契约测试覆盖全部注册实现**（新增实现自动纳入、换实现不破），见「平台能力插件化」节。

> 完整测试策略（分层定义、新功能覆盖全量清单、兼容性矩阵）见《[架构设计 · 测试与质量](../设计/架构设计/32_架构设计_测试与质量.md)》。
...
- 开发环境依赖（Redis/RocketMQ/ElasticSearch/MinIO）由 Docker Compose 统一提供；Milvus（含 etcd）随阶段十九 AI 加入 Compose。
...
> 部署细节（Compose 编排、版本维护）见《[架构设计 · 部署与运维](../设计/架构设计/33_架构设计_部署与运维.md)》。
...
> 种子结构、顺序与幂等要求见《[架构设计 · 部署与运维](../设计/架构设计/33_架构设计_部署与运维.md)》「初始化数据」节。

## 19. 部署与运维 <a id="deploy"></a>

### 19.1 部署拓扑 <a id="deploy-topo"></a>

入口 nginx（TLS 终止、/api 反代、前端静态托管）→ backend 多副本（无状态可横向扩展）；数据层 MySQL/PostgreSQL 主从（1 平台库 + N 租户库 + 1 归档库）/达梦 DM8（DM 数据守护）+ Redis + RocketMQ + ElasticSearch + Milvus（阶段十九）+ MinIO（按租户 bucket 隔离）；监控 Prometheus + Loki + Grafana + Alertmanager + Jaeger（otel-collector 采集）。
...
- **版本号**：语义化版本（主.次.修订），Git tag 与 CI 镜像一一对应；与里程碑分层映射——Alpha `0.x.y`（阶段一~十，0.8 收敛为 Alpha 候选）、Beta `0.9.x ~ 0.13.x`（阶段十一~十五功能冻结，0.13 为 Beta 候选）、GA 后首个正式发布 `1.0.0`。
...
首年基线：50 租户 × 200 用户、单租户库 ≤ 10GB、审计月表 ≤ 500 万行；ElasticSearch 日志索引按月滚动、JVM 堆 ≤ 31GB、磁盘 70% 告警；RocketMQ commitlog 保留 72 小时、消费滞后告警；Redis maxmemory + volatile-lru、内存 70% 告警；指标保留 15 天、日志热保留 30 天；阶段十三、阶段十四各修订一次基线，此后随运维复盘年度修订。
...
> 完整检查单与上线步骤见《[架构设计 · 部署与运维](../设计/架构设计/33_架构设计_部署与运维.md)》「GA 正式上线计划」节。

### 19.8 运营支持模型 <a id="deploy-support"></a>

> **本节为运营期口径，GA 后启用**——MVP 开发期（阶段一~十九）不执行值班与月度复核，不做相应留痕。
...
> 值班口径、问题分级时限表、容量与成本监控、License 健康、机密与权限复核见《[架构设计 · 部署与运维](../设计/架构设计/33_架构设计_部署与运维.md)》「运营支持模型」节。
...
17 个阶段体量接近完整产品而非传统意义的"最小可行产品"，为控制交付风险、避免范围失控，引入行业通行的三级发布分层对阶段分组（仅标注交付节奏，不改变各阶段内容与完成标准）：
...
| 阶段                            | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 完成标准                                                                                                                                                                                                                                                                                                |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 一 项目骨架                     | monorepo（含 frontend-mobile）、配置管理、日志体系、健康检查、CI 流水线激活；前端契约/数据根与后端 core 基座首批（`BaseObject` 根基类 / 有序集合 / 并发集合 / Redis 封装 / `BaseRepository`·`BaseService`·`BaseSchema`）；**后端基座体系（骨架 + 接口占位）**——根系 根基类 / 集合体系 / 模块基类 / core 横切（`BizError` + 统一响应、配置 / 安全 / 日志结构）/ 跨阶段基座登记 / 机制底座接口占位（数据访问 / 多租户拓扑 / 模块注册），真实实现随阶段二                                                                                    | 本地起服务、/healthz /readyz 正常、CI 流水线激活通过；治理脚本`collect_metrics.py` / `review_stage.py` 可用（度量采集与阶段末复盘清单执行，见 25.3）；基座体系占位就位（依赖注入可解析、应用可启动、占位可断言），异常体系生效 |
| 二 后端基座                     | **后端基座（真实实现）**——数据访问底座落地（引擎 / 会话工厂 / 四库方言 / 读写分离 / 连接池、`DbUnitOfWork` 事务、`BaseRepository` 异步化与 DB 实现）、Alembic 在线迁移与租户库初始化（`ops` 批量迁移幂等）、多租户数据拓扑落库与实测（平台库表 / 租户解析全局中间件 / 引擎注册表 LRU 与连接预算 / 租户隔离）、模块注册表落库（`sys_module` 表与种子 / 启动与 CI 接库唯一性校验）、`BaseModel` 落库（雪花 ID / 软删除 / 审计 / 乐观锁 / 四库类型映射）、排序契约 DB 侧（ORDER BY 白名单 / 四库 NULLS / 分页限深）、三库真库集成与达梦方言实测、CI 三库 job（`verify/db` 档）与阶段验收；工程约束：事件发布占位（`sys_outbox` 表与投递器接口，可靠性口径见知识档案《[微服务 · 05 通信与分布式一致性](../资料/知识档案/微服务/05_微服务_通信与分布式一致性.md)》）、分层依赖硬校验（自研 `ops/check_layers.py` 挂 CI）。架构见《[架构设计 · 后端基础类体系](../设计/架构设计/04_架构设计_后端基础类体系.md)》 | 数据访问真实落库：三库拓扑可路由（MySQL/PostgreSQL/达梦 DM8）、租户隔离与批量迁移正确、模块注册接库校验生效（表前缀/错误码段/事件域冲突被拒）、`BaseModel` 落库与排序 DB 侧生效、基类异步切换完成、三库真库集成与达梦方言实测通过、阶段测试报告与 M2 门禁齐备 |
...
| 四 前端组件库                   | **前端基础组件库（地基波）**——基础类体系（根系基类 / 组件根基类 / 能力片段（开放集合，预置 31，组合式 `useXxx`） / 域基类，契约/数据根已在阶段一落地不重复）、基础类 01-03（请求封装/权限指令/格式化工具）、基础控件类 01-08、容器组件类 01-09、布局组件类 01-11（含侧边菜单）、展示类「异常与空状态」（随地基波先行）、前端扩展点与注册表基座（路由/菜单、组件、字段渲染器、图标、工作台卡片注册）与微前端骨架起步（宿主薄壳与片段契约；运行时加载随阶段五，构建期合并为过渡）；契约以《[组件设计](../设计/组件设计/01_组件设计_总览.md)》各节点与可交互原型为准，含组件单测；字段类/展示类/交互类等依赖后端能力的组件按对应阶段回补                                                                                                                                                              ；设计令牌与组件契约保持可被独立片段复用的形态、**微前端骨架起步**（宿主外壳薄壳与片段契约）。微前端机制见《[架构设计 · 前端架构](../设计/架构设计/08_架构设计_前端架构.md)》 | 基础类体系与地基波组件全部可用、组件单测通过、与可交互原型一致（原型即验收基准）；宿主外壳与片段契约就位（可加载演示片段验证）；组件不完成则前端页面开发无法推进                                                                                                                                                                                      |
| 五 前端插件化 | **前端插件化（扩展点与运行时组合）**：前端注册表（路由/菜单、通用组件、字段渲染器、图标、工作台卡片、主题）+ **Module Federation 运行时加载**（宿主/片段契约、片段清单与版本字段、shared 依赖 singleton 基础配置）+ 动态导入分包；**新业务页面按片段形态独立构建与发布**（首个业务页面即首个片段样例），平台自身页面在主应用内；构建期合并保留为过渡/存量形态。机制见《[架构设计 · 扩展点与插件化](../设计/架构设计/10_架构设计_子系统_扩展点与插件化.md)》与《[架构设计 · 前端架构](../设计/架构设计/08_架构设计_前端架构.md)》 | 宿主可运行时加载片段（演示片段验证：动态路由生效、产物独立构建、shared 依赖单实例）；注册表与片段清单运行时可用；首个业务页面按片段形态开发通过；插件页面动态导入、独立分包、不进首屏 |
| 六 认证与安全                   | 双 token、登录/刷新/登出、限流、图形验证码、会话管理、账号锁定/解锁管理、SSO（OIDC 客户端 + CAS 兼容、JIT 建号、BMS 兼作 IdP、企业微信/钉钉免登）、密码策略合规、数据脱敏。**建表衔接**：本阶段先行落地 `sys_user` 最小模型（账号状态/锁定/密码哈希等字段，锁定/解锁读写其中），供登录链路与 SSO JIT 建号使用；用户管理完整 CRUD 与组织归属归阶段七                                                                                                                                                                                                                                                                                                                                                 | 登录链路（本地 + SSO，均基于 sys_user 最小模型）E2E 通过、JIT 建号落库正确、策略强制生效                                                                                                                                                                                                                |
...
| 十 系统集成与消息               | 开放接口管理（OAuth2）、Webhook、事件总线事件模型；**独立服务接入最小骨架清单**（`sys_client` 注册要素 / 最小 scope 集合 / 必供项与兼容承诺，见《[平台可扩展性规划](平台可扩展性规划.md)》5.7）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 外部调用鉴权通过、事件推送成功；「独立服务接入最小骨架清单」随本阶段交付                                                                                                                                                                                                                                                                          |
...
| 十二 业务外置（供应商/销售/仓储） | 供应商/销售/仓储等业务随 biz 仓库排期接入（平台扩展方式，见「平台扩展接入流程」节），不在本平台阶段计划                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 见 biz《biz 企业运营管理规划》                                                                                                                                                                                                                                                                          |
| 十三 分布式与监控               | 真实分片规则落地、租户独立库路由与批量迁移演练、系统监控、缓存管理、备份管理、链路追踪、任务调度                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 分片查询正确、租户隔离正确、监控指标与追踪链路可见、缓存清理生效、备份任务与记录可查、任务页面可管理；治理脚本`check_release.py` / `drill_restore.py` / `check_inventory.py` 可用（发版检查、恢复演练、容量与许可检查，见 25.3）                                                                  |
...
| 十七 移动端 H5                  | frontend-mobile 工程、审批中心（附件预览）、通知、公告查看、个人中心、数据查询、企业微信/钉钉免登                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 移动端审批、附件预览与免登 E2E 通过                                                                                                                                                                                                                                                                     |
...
| 十九 AI 能力                    | LLM 适配层；智能报表助手（N片段SQL）；智能审批辅助；AI 内容生成；语义搜索与文件问答（Milvus）；OCR 与文件自动分类；审计异常检测；知识库问答机器人；帮助文档智能维护；AI 自动执行写操作与 AI 自动审批（开关默认关闭）；**组件回补**：AI 助手                                                                                                                                                                                                                                                                                                                                                                                                                                                             | N片段SQL 安全执行验证通过；默认仅辅助、两开关关闭时无自动写操作验证通过；开关开启场景白名单/二次确认/审计/撤销验证通过；审批开关依赖校验验证通过；检索/问答租户隔离验证通过；AI 交互全量落审计日志；异常检测告警可达；模型配置页面可管理；帮助文档 AI 生成草稿与人工审核发布链路验证通过；AI 助手组件可用 |
...
| 域             | 验收点                                                                                                                                                                                                               | 关联阶段    |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| 后端基座       | core 基座与模块基类可用（`BizError` 统一异常与统一响应生效、`BaseModel` 落库、基类异步切换完成）；三库拓扑可路由、租户隔离与批量迁移正确、模块注册校验生效；跨阶段基座登记齐备                                   | 二          |
...
| 前端插件化 | 前端扩展点注册生效；宿主运行时加载片段（独立构建产物、动态路由、shared 依赖单实例）；新业务页面按片段形态独立发布；插件页面动态导入、独立分包、不进首屏 | 五 |
...
> 初版 MVP 验收清单遗漏了阶段十八（检索）与阶段十九（AI），已补齐；并新增**后端基座**域（阶段一交付骨架与接口占位、阶段二落地真实实现，后端全部模块的前置基础）与**前端组件库**域（阶段四地基波，前端页面开发的前置基础）。GA 定版前按上表全量走查（见 19.7 GA 上线检查单）。
...
| 文档           | 说明                                                                                                                                                                                                               | 产出阶段                                                 |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- |
| 后端开发规范   | 后端基座体系（分层与继承约定）、分层职责、异常与错误码用法、SQL 规范、测试写法                                                                                                                                     | 阶段一、阶段二（基座体系章节随骨架交付与真实实现同步更新）               |
| 前端开发规范   | 目录约定、组件与状态管理约定、提交规范                                                                                                                                                                             | 阶段一                                                   |
| 组件设计       | 通用组件契约（职责、Props 与事件、取数与缓存、交互与边界），总纲 + 八类节点（基础组件类 基类节点 15（机制基类 2 + 能力片段 11 + 域基类 2） / 布局 01-11 / 容器 01-09 / 基础控件 01-08 / 字段 01-12 / 展示 01-14 / 交互 01-19 / 基础 01-03，含可交互原型） | 阶段四前（设计与原型已交付，是阶段四组件实施的前置依据） |
| 开发部署规划   | 开发环境两台机器分工、服务部署、磁盘/端口规划、CI 基础设施                                                                                                                                                         | 阶段一前（已交付）                                       |
| 接口契约快照   | CI 导出的 swagger.json 存档目录与版本对应表，外部系统集成依据                                                                                                                                                      | 阶段六起随版本维护                                       |
| 部署手册       | Compose 编排、环境变量清单、备份恢复演练步骤、安全响应头说明                                                                                                                                                       | 阶段十三                                                 |
| 运维手册       | 监控告警配置、日志检索、常见故障排查                                                                                                                                                                               | 阶段十三                                                 |
| 压测报告       | 压测场景、结果与优化记录                                                                                                                                                                                           | 阶段十四                                                 |
...
| 测试报告       | 每阶段末测试汇总（用例执行统计、缺陷统计、覆盖率、风险与遗留项）；MVP 验收前总测试报告（落点 `项目/{阶段目录}/NN_测试报告_{阶段名}.md`，如 `项目/01_项目骨架/01_测试报告_项目骨架.md`）                                                                                                                             | 阶段一起每阶段末                                         |
...
- Kubernetes 化部署。
- 能力插件化演进：配置切换、重启生效（阶段三 后端插件化）→ 运行时热插拔（演进，灰度/回滚）→ 第三方 entry points 自动发现（演进，受控来源 + 隔离）；前端扩展点注册表与运行时片段（Module Federation，阶段五起；构建期合并为过渡，见「平台能力插件化」节）。
...
| 方式             | 说明                                                                                                                                                                                                                                                                                                 | 适用场景                                                          |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| 平台扩展（主推） | 产品独立 git 仓库按既有分层（api/models/schemas/services/repositories）开发业务模块，前端模块以运行时片段接入平台前端应用（阶段五起；构建期合并为过渡）、后端以扩展包并入 BMS 进程；经种子数据注册业务/动作权限码、菜单/表单/按钮/字段（含 i18n 附表）；数据落租户库，复用工作流、报表、导入导出、全文检索、审计、回收站等全部平台能力 | biz（采购/收付款/供应商/销售/仓储）、CW（创作系统）等独立产品业务 |
...
0. **工作区与文档基座接入**：在工作区根并置 bms 克隆与产品仓库（工作区模型），产品文档根按项目命名（`<产品>文档/`，如 `biz文档/`）且仓库根按《平台可扩展性规划》「新产品文档初始化流程」节 建 `bms文档` 软链（→ `../bms/bms文档`）并排引用基座。
1. **模块注册**：确定模块简称、表前缀、业务码、错误码段、事件域，在 sys_module 平台注册（阶段一已落注册表结构与离线校验，阶段二 后端基座落库含表 / 种子 / 接库唯一性校验），并登记业务/动作码（见 23.3/23.4/23.5），避免冲突。
...
新增模块在平台库 sys_module 统一登记以下要素（一处登记、全局生效；该机制阶段一已落注册表结构与离线 CI 校验，阶段二 后端基座落库并启用接库唯一性校验，表前缀/错误码段/事件域冲突即拒），简称全小写、与英文名一致：
...
- 模块前缀、错误码段、事件域等注册于 sys_module（阶段一落结构与离线校验、阶段二 后端基座落库并接库校验，平台库种子维护），随产品模块交付同步更新种子；《[命名规范](../规范/命名规范.md)》表前缀规则与本文档保持一致。
...
### 23.10 平台能力插件化（纵向可替换） <a id="addon-plugin"></a>

本章前 9 节讲**横向**扩展（产品业务模块挂进平台）；本节讲**纵向**可替换（平台自身能力的实现可换）。两者最终统一到同一套「注册 → 契约校验 → 装配 → 生命周期」机制上，差异只在**注册要素**（能力名 vs 表前缀/错误码段/事件域）与**权限边界**。

```mermaid
flowchart TB
    subgraph 统一机制
        REG["统一注册与装配<br/>契约 · 唯一性 · 生命周期"]:::k
    end
    P1["产品 biz（横向）<br/>业务模块挂接"]:::h --> REG
    P2["产品 cw（横向）<br/>业务模块挂接"]:::h --> REG
    CHG["上游停维护 / 许可变更 /<br/>客户指定商业组件（纵向）"]:::c --> REG
    REG --> CAP["平台能力实现可替换<br/>ID / 存储 / 通知 / 权限 / 检索 / LLM"]:::v
    classDef k fill:#e8eef4,stroke:#38a,color:#111
    classDef h fill:#f4ece8,stroke:#a73,color:#111
    classDef c fill:#f4f0e8,stroke:#a83,color:#111
...
| 要素                    | 说明                                                                                                                                                                               |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 能力基类（端口）        | 每个平台能力一个抽象基类（如对象存储、通知、权限校验、LLM 适配），定义方法契约与数据契约；业务代码只依赖它                                                                         |
| 中间层`BasePluggable` | 承载「实现可替换」的装配语义：插件键、实现名、注册、按配置解析、契约版本、生命周期（推荐演进，见知识档案《[端口适配器](../资料/知识档案/插件化架构/03_插件化架构_端口适配器.md)》） |
| 插件注册表              | 名称 → 实现；继承自动登记与显式注册（组合）双入口，重名在启动与 CI 拒绝                                                                                                           |
| 配置选择                | `config.toml` 的 provider + options 选定实现；未指定用默认（Null/占位）实现；密钥走 Secret                                                                                       |
| 提供者                  | `get_xxx` 薄封装，业务只调它，不直连具体实现                                                                                                                                     |
| 契约测试                | 全部注册实现跑同一组契约测试，保证换实现不破                                                                                                                                       |
| 三阶段演进              | 一：配置切换、重启生效；二：运行时热插拔（灰度/回滚）；三：第三方安装即发现（受控来源 + 隔离）                                                                                     |

**边界与口径**：

- **不引入微服务**：阶段三（后端插件化）/阶段五（前端插件化）均为进程内扩展包；第三方受控来源 + 必要时进程/容器隔离属演进项，不因扩展新增常驻服务进程（与微服务的边界见知识档案《[微服务 · 01 总览](../资料/知识档案/微服务/01_微服务_总览.md)》§7）。
- **按风险定优先级**：客户容易指定、上游风险高的能力（存储/权限/检索/AI 等）优先抽象；不需替换的低风险依赖不为抽象而抽象。
- **契约先行**：端口定义、契约测试、异常隔离是三阶段共同地基；注册要素唯一性在启动与 CI 校验，冲突即拒。
- **契约承诺同 23.9**：能力 provider 名、能力接口与事件命名视为对外契约，破坏性变更走公告与过渡期。
- **典型场景**：ID 生成算法替换、对象存储换客户既有 S3、权限外置集团统一权限平台（策略与迁移见知识档案《[开源与商业组件可替换](../资料/知识档案/插件化架构/02_插件化架构_开源与商业组件可替换.md)》）。

**前端插件化（配套）**：纵向能力替换之外，前端同样以**扩展点 + 注册表**组织可扩展面——路由/菜单、页面区域、通用组件、字段渲染器、图标、工作台卡片、主题令牌各设注册表，插件以注册方式挂接；插件页面**动态导入、独立分包**、不进首屏。**引入微前端（Module Federation，分步落地）**：阶段四交付宿主外壳（薄壳）与片段契约骨架，阶段五完成运行时加载与片段清单；**新业务页面一律按片段形态开发**（独立构建产物、运行时加载、独立发布，首个业务页面即首个片段样例），平台自身页面仍在主应用内。治理按**最小闭环**（宿主薄壳 + 运行时加载 + 片段契约 + 清单；隔离与依赖共享先用基础配置：Scoped/令牌、shared 依赖 singleton），灰度与版本发现后补；`iframe/qiankun` 沙箱式微前端不采用（异构栈/强隔离时再评估）。构建期合并保留为过渡与存量形态；类型由 OpenAPI 生成对齐后端契约。取舍与治理见知识档案《[微前端](../资料/知识档案/微前端/01_微前端_总览.md)》专题；机制与选型对比见知识档案《[前端插件化](../资料/知识档案/插件化架构/14_插件化架构_前端插件化.md)》。

> 完整机制、发现/注册/配置/生命周期/第三方生态与开发 SOP 见知识档案《[插件化架构](../资料/知识档案/插件化架构/01_插件化架构_总览.md)》；扩展点方向与契约承诺见《[平台可扩展性规划](平台可扩展性规划.md)》「需要 BMS 后续补齐的扩展点」节。
...
| **片段 事件触发**         | 只在特定条件出现时才由人动笔                                                 | 人             | 5 分钟/次，低频 |
| **片段 砍除 / GA 后启用** | 一次性仪式直接砍掉；运营期动作标记为 GA 后启用                               | —             | 0               |
...
**治理脚本清单**（登记于 `scripts/tools/governance/`，与 `defect/`、`base-check/` 同构；**未实现前由 AI 照清单手动执行等价命令**，实现后替换为调脚本）。脚本落地已写入阶段完成标准——阶段一（`collect_metrics` / `review_stage`）、阶段十三（`check_release` / `drill_restore` / `check_inventory`，见「开发计划与验收标准」节），阶段末复盘时按此核对，避免遗漏：
...
| 需求 / Schema / 核心依赖变更留痕            | 片段   | 三类触发时        | 人写一行决策           |
| 兼容性公告与过渡期                          | 片段   | 破坏性变更时      | 有产品接入后才需执行   |
| 外部依赖台账更新                            | 片段   | 新增依赖或故障时  | 人补一行               |
| 运营值班、月度复核、问题分级时限            | 片段   | GA 后启用         | 见 19.8                |
| 项目复盘会、上线移交签收等一次性仪式        | 片段   | **砍除**    | 见下                   |
...
平台运行依赖若干外部服务与第三方能力，统一登记降级口径（降级矩阵细化见《[架构设计 · 性能与安全](../设计/架构设计/31_架构设计_性能与安全.md)》）：

| 依赖                      | 用途                    | 不可用影响                       | 降级与兜底                                                                          |
| ------------------------- | ----------------------- | -------------------------------- | ----------------------------------------------------------------------------------- |
| 外部 LLM API              | AI 八项能力（阶段十九） | AI 功能不可用                    | AI 开关默认关闭即为成本与可用性闸门；可切私有化端点；主链路不受影响                 |
...
| Alpha / Beta / GA        | 三层里程碑分组：内部可用（阶段一~十）/ 功能完整并冻结范围（阶段十一~十五，七/八为业务外置注记）/ 正式发布（阶段十六~十九）          | 开发计划与验收标准   |
...
| 治理分级（根系~片段）        | 单人兼职下的机制落地分级：根系 自动门禁 / L1 脚本 + AI 执行与交叉审核 / 片段 事件触发 / 片段 砍除或 GA 后启用                            | 治理机制的落地方式   |
