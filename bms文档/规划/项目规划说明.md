...
- **基础组件类 00-01～00-04（已建）**：基础输入组件基类（`BaseInput`/`useFieldBase`，被 05/01 字段控件继承）、字段壳（`FieldWrapper`，label/必填/帮助/错误/栅格/只读回显）、字段权限（`useFieldPerm`/`v-plain`，字段 `visible`/`editable`/`required`/脱敏）、基础展示组件基类（`DisplayBase`/`useDisplayBase`，只读值格式化/空值占位/密度/令牌）；
...
| 组件设计 | 通用组件契约（职责、Props 与事件、取数与缓存、交互与边界），总纲 + 六类节点（基础组件 00-04 / 字段 01-12 / 展示 01-14 / 交互 01-22 / 基础 01-03；基础控件 01-08 规划中） | 阶段十一前（已交付）；00/05 于 2026-09 补充 |
