...
- **基础组件类（基类节点 15（机制基类 2 + 能力片段 11 + 域基类 2） 篇，已建）**：根系基类（`BaseFrontend`/`useFrontendBase`，所有基类之根）、组件根基类（`BaseComponent`，透传/尺寸密度主题/i18n/可访问性/loading…）、预置能力片段（七枚）（交互 `BaseInteractive`/输入 `BaseInputControl`/展示 `BaseDisplayControl`/容器 `BaseContainer`/表单容器 `BaseFormContainer`/媒体 `BaseMedia`/布局 `BaseLayout`）、能力片段（值 / 字段链）（`BaseValue` → `BaseField`，组合 `BaseFieldShell`/`BaseFieldPerm`）、域基类（`BaseInput` 输入域/`BaseDisplay` 展示域）；体系权威见《前端开发规范》「前端基类体系」节；
...
> **组件实现阶段（承接组件设计）**：地基波——基础组件类（基类节点 15）/ 布局组件类（11）/ 容器组件类（9）/ 基础控件类（8）/ 基础类（3），约 45 个节点——在**阶段四 前端组件库**交付，是前端页面开发的前置基础（不完成则页面开发无法推进），架构见《[架构设计 · 前端组件体系](../设计/架构设计/05_架构设计_前端组件体系.md)》；依赖后端能力的字段类（12）/ 展示类（14）/ 交互类（19）约 45 个节点随对应阶段回补（字典/文件→阶段八 通用能力、审批→阶段九 工作流、报表与大屏→阶段十六 报表 BI、全局搜索→阶段十八 全文检索、AI 助手→阶段十九 AI 能力）。
>
...
| 前端组件库     | 前端基类体系（开放式）与地基波组件（布局 / 容器 / 基础控件 / 基础类）可用；组件契约与《组件设计》节点及可交互原型一致；组件单测通过、覆盖率达标                                                                         | 三          |
