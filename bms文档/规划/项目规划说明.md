...
| 四 前端组件库                   | **前端基础组件库（地基波）**——基础类体系（L0 总基类 / L1b 最基础组件基类 / L2 七形态基类 / L3 受控值与字段基类 / L4 域基类，L1a 数据/契约基类已在阶段一落地不重复）、布局组件类 01-11、容器组件类 01-09、基础控件类 01-08、基础类 01-03（请求封装/权限指令/格式化工具）、前端扩展点与注册表基座（路由/菜单、组件、字段渲染器、图标、工作台卡片注册）与产品前端模块构建期合并底座；契约以《[组件设计](../设计/组件设计/01_组件设计_总览.md)》各节点与可交互原型为准，含组件单测；字段类/展示类/交互类等依赖后端能力的组件按对应阶段回补                                                                                                                                                              ；设计令牌与组件契约保持可被独立片段复用的形态（为运行时组合保留一致性基础） | 基础类体系与地基波组件全部可用、组件单测通过、与可交互原型一致（原型即验收基准）；组件不完成则前端页面开发无法推进                                                                                                                                                                                      |
| 五 前端插件化 | **前端插件化（扩展点与构建期合并）**：前端注册表（路由/菜单、通用组件、字段渲染器、图标、工作台卡片、主题）+ 动态导入分包；产品（biz/cw）前端模块产物**构建期合并**进平台前端应用、动态路由生效、前端免发版。机制见《[架构设计 · 扩展点与插件化](../设计/架构设计/10_架构设计_子系统_扩展点与插件化.md)》；**运行时组合架子**：注册入口与插件清单按运行时契约设计（片段可被宿主运行时加载）、宿主外壳保持薄壳、片段契约（宿主注入上下文 / 片段注册声明）先行 | 注册表与清单可运行时枚举（不做运行时加载实现，保留 Module Federation 演进路径）；前端扩展点注册生效；产品前端模块构建期合并后动态路由可见；插件页面动态导入、独立分包、不进首屏 |
...
| 前端插件化 | 前端扩展点注册生效；产品前端模块构建期合并后动态路由可见；插件页面动态导入、独立分包、不进首屏 | 五 |
...
- Kubernetes 化部署。
- 能力插件化演进：配置切换、重启生效（阶段三 后端插件化）→ 运行时热插拔（演进，灰度/回滚）→ 第三方 entry points 自动发现（演进，受控来源 + 隔离）；前端扩展点注册表与产品前端模块构建期合并（见「平台能力插件化」节）。
...
**前端插件化（配套）**：纵向能力替换之外，前端同样以**扩展点 + 注册表**组织可扩展面——路由/菜单、页面区域、通用组件、字段渲染器、图标、工作台卡片、主题令牌各设注册表，插件以注册方式挂接；插件页面**动态导入、独立分包**、不进首屏。产品（biz/cw）前端模块经**产物构建期合并**进平台前端应用，菜单/表单/按钮/字段走平台注册、动态路由生效、前端免发版（类型由 OpenAPI 生成对齐后端契约）。**MVP 不引入运行时远程加载 / 前端微前端**（取舍与触发条件见知识档案《[微前端](../资料/知识档案/微前端/01_微前端_总览.md)》专题），但按「运行时组合可演进」搭建架子：注册入口与插件清单保持运行时可用、宿主外壳保持薄壳、片段契约先行（阶段四/五交付）。机制与选型对比见知识档案《[前端插件化](../资料/知识档案/插件化架构/14_插件化架构_前端插件化.md)》。
