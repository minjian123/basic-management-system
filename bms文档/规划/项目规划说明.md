...
- **变更控制**：Beta 起功能范围冻结（见「开发计划与验收标准」节），新增需求进「演进路线」节或走变更登记（见「变更管理与项目度量」节）；平台对产品的兼容性承诺见 23.9；需求 / 任务 / 计划的变更口径见《[项目管理规范](../规范/项目管理规范.md)》。

## 2. 技术栈 <a id="stack"></a>

> 技术栈选型全景（后端核心 / 前端 / 工程化与质量 / 部署与运维四张清单）见[《架构设计 · 总体架构》](../设计/架构设计/03_架构设计_总体架构.md)第 6 节，本规划不再复制表格与细节；选型理由见第 3 节，许可与预算见 2.1 / 2.2 节。
>
> **编号说明**：技术栈四张清单（原 2.1~2.4：后端核心 / 前端 / 工程化与质量 / 部署与运维）已迁至《架构设计 · 总体架构》第 6 节，本节只保留**许可协议（2.1）**与**项目预算（2.2）**，原 2.5 / 2.6 顺延前移；`bms文档/项目/` 下任务与需求文档对旧 2.x 的引用已同步更正为「第 2 节」或对应选型小节（3.1~3.4）。
...
- **前端性能基线**：PC 首屏可交互 ≤ 3s（局域网访问）、路由切换 ≤ 500ms；移动端 H5 首屏 ≤ 5s（4G 模拟）；主包 gzip ≤ 500KB、路由级按需分包；ECharts 与大屏组件懒加载；构建产物超预算 CI 告警（细化口径见《[架构设计 · 前端架构](../设计/架构设计/28_架构设计_前端架构.md)》）。
...
**恢复演练**：每季度至少一次（GA 上线前必须完成一次），范围轮换覆盖平台库 / 租户库 / MinIO 对象 / 归档库；演练结论（时间、恢复对象、耗时、是否达标、遗留项）登记 `sys_backup_log` 的恢复演练字段，未达标项进缺陷流程跟踪闭环。
...
> 承诺的原则口径见《[平台可扩展性规划](平台可扩展性规划.md)》5.5 节「平台对产品的契约与兼容性承诺」；本节为操作细节，两处口径一致、互为引用。
...
| 必须人工审核后生效 | 规划与设计文档、需求与任务文档、数据库迁移脚本、权限码与模块注册、CI 流水线变更、涉及安全与权限的代码 | 逐项人工审核后生效；审核历史由 git 承载，文档内不留修订痕迹（依《[文档生成规范](../规范/文档生成规范.md)》9.1） |
...
变更分四类，登记后方可执行（个人开发期由本人确认即生效，记录随阶段复盘走查）：

| 变更类型 | 触发 | 处理口径 |
| --- | --- | --- |
| 需求变更 | 功能新增或调整 | Beta 起功能范围冻结（见「开发计划与验收标准」节）：范围内调整须同步更新阶段完成标准与验收清单；范围外需求进「演进路线」节，不挤占当前阶段 |
| Schema 变更 | 表结构变化 | 只加列不删不改、新列可空或带默认值；删列/改类型延后两个版本分步执行；三库方言迁移同步；不兼容变更随主版本停机窗口（见「版本与发布管理」节） |
| 依赖变更 | 组件升级 | Renovate 自动提 MR，升级须 CI 通过 + 许可变化复核（见「开源与许可协议」节）；核心依赖（Python、数据库驱动、工作流引擎）升级前评估兼容性风险 |
| 文档与基线变更 | 规划/设计口径调整 | 结论变更先改上游（规划 → 架构 → 概要）再传导下游；节号调整须复核引用该节号的任务与需求文档（见 1.1） |

### 25.2 项目度量与报告 <a id="metrics"></a>

开发期每阶段末复盘采集以下度量，随测试报告输出（见「文档计划」节）：

| 度量 | 口径 | 用途 |
| --- | --- | --- |
| 阶段工期偏差 | 实际 vs《[总体项目规划](总体项目规划.md)》基线工期 | 排期与估算校准 |
| 用例通过率与趋势 | Kiwi TCMS 执行结果（功能 + 安全专项） | 准出判定（≥ 98%，见 16.4） |
| 缺陷分布与收敛 | 按 P0~P3 与模块统计，看收敛趋势 | 质量风险识别 |
| 覆盖率 | 核心模块行覆盖 ≥ 80%、整体 ≥ 70%（CI 门禁） | 测试充分性 |
| CI 流水线健康 | 成功率、平均时长、flaky 用例数 | 工程效率与稳定性 |
| 容量与成本水位 | 磁盘/内存水位、外部 API 用量（LLM/短信） | 扩容与成本闸门（见「容量规划」节） |

**报告节奏**：每阶段末测试汇总报告（用例执行统计、缺陷统计、覆盖率、风险与遗留项）；MVP 验收前输出总测试报告；运营期按月复核容量、成本与缺陷渠道（见「运营支持模型」节）。
