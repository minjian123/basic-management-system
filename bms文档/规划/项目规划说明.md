...
**通用组件（组件设计体系）**：管理端反复出现的界面元素沉淀为可复用组件，位于布局设计之下、页面实现之上；契约（职责、Props 与事件、取数与缓存、交互与边界）见《[组件设计 · 总览](../设计/组件设计/01_组件设计_总览.md)》，按六类分文件夹（基础组件类 / 基础控件类 / 字段类 / 展示类 / 交互类 / 基础类，类内编号），组件清单与路线图以总览为权威：

- **基础组件类（L0-L4 共 14 篇，已建）**：L0 总基类（`BaseFrontend`/`useFrontendBase`，所有基类之根）、L1b 最基础组件基类（`BaseComponent`，透传/尺寸密度主题/i18n/可访问性/loading…）、L2 六形态基类（交互 `BaseInteractive`/输入 `BaseInputControl`/展示 `BaseDisplayControl`/容器 `BaseContainer`/表单容器 `BaseFormContainer`/媒体 `BaseMedia`）、L3 受控值与字段基类（`BaseValue` → `BaseField`，组合 `BaseFieldShell`/`BaseFieldPerm`）、L4 域基类（`BaseInput` 输入域/`DisplayBase` 展示域）；分层权威见《前端开发规范》「前端基础类体系」节；
