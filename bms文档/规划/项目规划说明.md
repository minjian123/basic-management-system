...
- **基础组件类（L0-L4 共 14 篇，已建）**：L0 总基类（`BaseFrontend`/`useFrontendBase`，所有基类之根）、L1b 最基础组件基类（`BaseComponent`，透传/尺寸密度主题/i18n/可访问性/loading…）、L2 七形态基类（交互 `BaseInteractive`/输入 `BaseInputControl`/展示 `BaseDisplayControl`/容器 `BaseContainer`/表单容器 `BaseFormContainer`/媒体 `BaseMedia`/布局 `BaseLayout`）、L3 受控值与字段基类（`BaseValue` → `BaseField`，组合 `BaseFieldShell`/`BaseFieldPerm`）、L4 域基类（`BaseInput` 输入域/`BaseDisplay` 展示域）；分层权威见《前端开发规范》「前端基础类体系」节；
- **布局组件类 01-11（已建）**：主框架布局壳（`AppLayout`）、表单框架壳（`FormFrame`）、页面容器（`PageContainer`）、多标签导航（`TabsNav`/`FormFrameTabs`）、树形主从布局（`MasterDetail`/`MasterTree`）、栅格（`Grid`）、间距/分割线（`Space`/`Divider`）、分栏（`Split`）、内容页签（`Tabs`）、卡片（`Card`）、折叠面板（`Collapse`）——均继承 02 基础组件类 L2-07 布局组件基类；
- **容器组件类 01-09（已建）**：弹窗/抽屉表单（`FormDialog`/`FormDrawer`/`ConfirmDialog`）、滚动容器（`ScrollContainer`）、懒加载容器（`LazyContainer`）、全屏容器（`FullscreenContainer`）、自适应高度容器（`AutoHeight`）、虚拟列表容器（`VirtualList`）、加载遮罩容器（`LoadingContainer`）、分区容器（`SectionContainer`）、宽高比容器（`AspectRatio`）——均继承 02 基础组件类 L2-04/L2-05；
...
- **基础类 01-03（已建）**：请求封装与错误处理（`useRequest`/`useListPage`）、权限指令（`v-perm`/`hasPerm`/`canAccess`；字段权限已迁至 02 基础组件类 L3-04）、格式化工具与组合式函数（`formatDateTime`/`formatAmount`/`validators`）；
- **基础控件类 01-08（已建）**：文本框（`TextField`）、文本域（`TextareaField`）、密码框（`PasswordField`）、数字框（`NumberInput`）、下拉框（`SelectInput`，静态选项）、单选（`RadioField`）、复选（`CheckboxField`）、开关（`SwitchInput`）——均继承 02 基础组件类 L4-01 输入域基类（经 L3/L2/L1b 至 L0）；字典/枚举/金额等带数据源或格式语义的字段由 04 字段类继承它们扩展。

> **组件实现阶段（承接组件设计）**：地基波——基础组件类 L0-L4（14）/ 布局组件类（11）/ 容器组件类（9）/ 基础控件类（8）/ 基础类（3），约 45 个节点——在**阶段四 前端组件库**交付，是前端页面开发的前置基础（不完成则页面开发无法推进），架构见《[架构设计 · 前端组件体系](../设计/架构设计/05_架构设计_前端组件体系.md)》；依赖后端能力的字段类（12）/ 展示类（14）/ 交互类（19）约 45 个节点随对应阶段回补（字典/文件→阶段八 通用能力、审批→阶段九 工作流、报表与大屏→阶段十六 报表 BI、全局搜索→阶段十八 全文检索、AI 助手→阶段十九 AI 能力）。
>
> 组件按域分目录置于 `frontend/src/components/`（基础类按层置于 `components/base/<layer>/`，L1a 数据/契约基类在 `src/api/`、`src/stores/`、`src/utils/`），工具与组合式函数置于 `frontend/src/utils/`；组件契约变更须先改组件设计节点再改代码。移动端 `frontend-mobile/` 不复用 PC 组件（Vant 生态，部分节点单独说明移动端口径）。
...
> 前端工程结构、菜单机制细粒度（动态路由注册、v-perm 指令、字段权限渲染）、双工程技术要点与性能预算见《[架构设计 · 前端架构](../设计/架构设计/08_架构设计_前端架构.md)》；组件分层、八类边界与分包/测试约束见《[架构设计 · 前端组件体系](../设计/架构设计/05_架构设计_前端组件体系.md)》。
...
| 前端组件库     | 基础类体系（L0-L4）与地基波组件（布局 / 容器 / 基础控件 / 基础类）可用；组件契约与《组件设计》节点及可交互原型一致；组件单测通过、覆盖率达标                                                                         | 三          |
...
| 组件设计       | 通用组件契约（职责、Props 与事件、取数与缓存、交互与边界），总纲 + 八类节点（基础组件类 L0-L4 共 14 / 布局 01-11 / 容器 01-09 / 基础控件 01-08 / 字段 01-12 / 展示 01-14 / 交互 01-19 / 基础 01-03，含可交互原型） | 阶段四前（设计与原型已交付，是阶段四组件实施的前置依据） |
