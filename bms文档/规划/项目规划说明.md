...
| 五 前端插件化 | **前端插件化（扩展点与构建期合并）**：前端注册表（路由/菜单、通用组件、字段渲染器、图标、工作台卡片、主题）+ 动态导入分包；产品（biz/cw）前端模块产物**构建期合并**进平台前端应用、动态路由生效、前端免发版。机制见《[架构设计 · 扩展点与插件化](../设计/架构设计/10_架构设计_子系统_扩展点与插件化.md)》 | 前端扩展点注册生效；产品前端模块构建期合并后动态路由可见；插件页面动态导入、独立分包、不进首屏 |
