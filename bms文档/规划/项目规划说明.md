...
| 层级 | 覆盖阶段 | 达成标志 |
| --- | --- | --- |
| Alpha（内部可用） | 阶段一~八 | 平台底座与前后端地基完整：**后端基座（地基波）可用**（core 基座 / 四层基类 / 异常体系 / 数据访问底座 / 多租户拓扑 / 模块注册）、**前端基础组件库（地基波）可用**，认证 / RBAC / 租户 / 通用能力 / 工作流 / 系统集成可运行，团队内部可登录操作 |
...
| 二 后端基座 | **后端基座（地基波）**——core 基座补齐（异常体系 `BizError` + 统一响应、配置 / 安全基座）、四层基类落地与异步化（`BaseRepository` 数据库实现、`BaseService`、`BaseSchema`、`BaseModel` 雪花 ID/软删除/审计/乐观锁）、数据访问底座（引擎 / 会话工厂 / 三库方言 / 读写分离占位）、多租户数据拓扑（三库拓扑 / 平台库基础表 / 租户解析 / 引擎注册表 / 批量迁移）、模块注册表（sys_module 表与种子 / 唯一性校验 / 查询接口）；跨阶段基座登记（缓存 Region 分域 / 数据范围注入 / 分片路由 / 事件 / 任务 / 审计捕获）。架构见《[架构设计 · 后端基础类体系](../设计/架构设计/33_架构设计_后端基础类体系.md)》 | `BizError` 与统一响应生效、`BaseModel` 落库（雪花 ID/软删除/审计/乐观锁）、三库拓扑可路由（MySQL/PostgreSQL/达梦 DM8）、租户隔离与批量迁移正确、模块注册校验生效（表前缀/错误码段/事件域冲突被拒）、基类异步切换完成；跨阶段基座登记齐备 |
...
| 域 | 验收点 | 关联阶段 |
| --- | --- | --- |
| 后端基座 | core 基座与四层基类可用（`BizError` 统一异常与统一响应生效、`BaseModel` 落库、基类异步切换完成）；三库拓扑可路由、租户隔离与批量迁移正确、模块注册校验生效；跨阶段基座登记齐备 | 二 |
