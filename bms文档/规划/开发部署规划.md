...
本文档规划 BMS 项目开发环境的整体部署方案：在《[本地资源](../用户文档/本地资源.md)》记载的两台机器上（开发服务器 **mjbk** 已装 Ubuntu 24.04.4 LTS 桌面版，开发机 **mjpc** 已装 Ubuntu 26.04.1 LTS（GNOME，Wayland）），搭建可支撑《[项目规划说明](项目规划说明.md)》全部 17 个开发阶段、直至 MVP 验收（含 500 并发压测）的完整开发环境，包括代码托管与 CI/CD、开发依赖服务、常驻数据库、监控与备份。

依据与约束：

- 《项目规划说明》「环境与配置」「部署与运维」「开发计划与验收标准」「外部依赖与降级」「变更管理与项目度量」各节的技术选型、阶段划分与变更/降级口径
- 《[架构设计-部署与运维](../设计/架构设计/32_架构设计_部署与运维.md)》的部署拓扑与集群设计要点
- 《[命名规范](../规范/命名规范.md)》对基础设施命名（Compose 服务名、环境变量、数据库、容器镜像）的约定
- 《[文档生成规范](../规范/文档生成规范.md)》对文档格式与布局的要求

本文档为规划性质，确定方案、目录、端口与落地节奏；命令级细节（Compose 文件内容、环境变量清单、备份恢复演练步骤）由阶段十一交付的《部署手册》细化。
...
| 监控与追踪（阶段十一） | Prometheus、Loki、Grafana、Alertmanager、Jaeger、otel-collector | 浏览器查看 Grafana / Jaeger |
| AI 基础设施（阶段十七） | Milvus + etcd（MinIO 复用），PaddleOCR 可选 | — |
| 日常开发 | — | 编辑代码、uv 后端开发服务器、Vite 前端开发服务器、本地 SQLite、压测客户端（阶段十二） |
...
graph TD
    ...
    B --> B2["monitor.yml prometheus、loki、grafana、alertmanager、jaeger、otel-collector（阶段十一）"]
    B --> B3["ai.yml milvus、etcd（阶段十七，MinIO 复用）"]
    B --> B4["gitlab.yml gitlab、gitlab-runner、renovate"]
    A --> C[".env.example 环境变量模板（BMS_ 前缀）"]
    A --> D["nginx/ 生产形态配置（阶段十一部署手册细化）"]
```
...
| rocketmq | apache/rocketmq:5.x（nameserver + broker 两个容器） | 9876 / 10911 | 命名卷（NVMe SSD） | 1 GB | 阶段六 |
| elasticsearch | docker.elastic.co/elasticsearch/elasticsearch:8.x（single-node） | 9200 / 9300 | 命名卷（NVMe SSD） | 2 GB（堆 1–2 GB） | 阶段十六（可提前） |
| prometheus | prom/prometheus | 9090 | 命名卷 | 2.5 GB（合计） | 阶段十一 |
| loki / grafana / alertmanager | grafana/loki、grafana/grafana、prom/alertmanager | 3100 / 3000 / 9093 | 命名卷 | 2.5 GB（合计） | 阶段十一 |
| jaeger / otel-collector | jaegertracing/all-in-one、otel/opentelemetry-collector | 16686 / 4317、4318 | — | 1 GB | 阶段十一 |
| milvus / etcd | milvusdb/milvus、quay.io/coreos/etcd | 19530 / 2379（内部） | 命名卷（NVMe SSD） | 4 GB | 阶段十七 |

> ES 按《项目规划说明》「环境与配置」节属开发环境依赖，阶段一即可随 base 组拉起（单节点、`discovery.type=single-node`、生产安全认证配置不启用），也可推迟至阶段十六按需引入；上表按内存预算给出两种节奏，见第 10 节。

内存合计：阶段一约 8 GB，阶段十一后约 15 GB，阶段十七后约 19 GB；叠加 GitLab（4 GB）与 Ubuntu 桌面 + Docker 开销（约 5 GB），峰值约 28 GB，32 GB 预算内留有余量，部署后监控实际占用并按第 11 节调优。
...
3. **Renovate**：阶段四起以容器方式随 gitlab.yml 编排，自动提依赖升级 MR；需 mjbk 可访问外网。
...
| 9876 / 10911 | RocketMQ nameserver / broker | 阶段六起，仅内网 |
| 9200 / 9300 | ElasticSearch HTTP / 节点通信 | 阶段十六起（可提前），仅内网 |
| 9090 / 3100 / 3000 / 9093 | Prometheus / Loki / Grafana / Alertmanager | 阶段十一起，仅内网 |
| 16686 / 4317 / 4318 | Jaeger UI / otel-collector OTLP | 阶段十一起，仅内网 |
| 19530 | Milvus | 阶段十七起，仅内网 |
...
| 第二批 | mjbk：rocketmq（nameserver + broker） | 阶段六（消息与任务基础设施） |
| 第三批 | mjbk：monitor 组（prometheus / loki / grafana / alertmanager / jaeger / otel-collector）+ 开发服务器备份脚本 | 阶段十一（分布式与监控） |
| 第四批 | mjbk：elasticsearch（单节点）；若已提前引入则无需动作 | 阶段十六（全文检索） |
| 第五批 | mjbk：milvus + etcd（MinIO 复用）；PaddleOCR 按需本地部署（可选） | 阶段十七（AI 能力） |

每批环境就位后，对应阶段验收标准中的「环境相关项」即可验证（如阶段一 /healthz /readyz、阶段十一监控链路与追踪可见、阶段十二压测在 mjpc 发起、目标 500 并发 / P99 ≤ 1s）。
...
| 内存预算约 28 GB（三库常驻 + 桌面 + Docker） | ES 堆限 1–2 GB；GitLab 4 GB 封顶；达梦仅联调与 CI 时段可停（docker compose stop dameng）；按需关闭 Grafana/Loki（阶段十一前不启动）；部署后监控 `docker stats` 与实际占用 |
...
- 本文档确定方案与节奏；**《部署手册》**（阶段十一交付，规划「文档计划」节）细化 Compose 文件、环境变量清单、备份恢复演练步骤、安全响应头说明；开发期工具的容器编排与备份细节一并纳入。
- **《运维手册》**（阶段十一）覆盖监控告警配置、日志检索、常见故障排查。
- Compose 编排与环境变量模板随代码仓库 `deploy/` 目录版本管理，与《[命名规范](../规范/命名规范.md)》保持一致。
- 文档变更后在工作区根运行 `graphify update .` 保持工作区知识图谱最新；graphify 工具使用见《[graphify 部署使用说明](../资料/AI/graphify部署使用说明.md)》。
