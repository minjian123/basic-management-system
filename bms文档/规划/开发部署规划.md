...
本文档规划 BMS 项目开发环境的整体部署方案：在《[本地资源](../用户文档/本地资源.md)》记载的两台机器上（开发服务器 **mjbk** 已装 Ubuntu 24.04.4 LTS 桌面版，开发机 **mjpc** 已装 Ubuntu 26.04.1 LTS（GNOME，Wayland）），搭建可支撑《[项目规划说明](项目规划说明.md)》全部 15 个开发阶段、直至 MVP 验收（含 500 并发压测）的完整开发环境，包括代码托管与 CI/CD、开发依赖服务、常驻数据库、监控与备份。
...
服务清单（服务名遵循《命名规范》「基础设施命名」节「Compose 服务名小写单名、与组件同名」）：
