...
开发机按**工作区模型**组织：工作区根为一个配置仓库（AGENTS/opencode/忽略规则），其内平级克隆 **bms**（平台/基座权威源）与**产品仓库**；AI 工作区与知识图谱在工作区根，扩展组合只需新增产品仓库克隆（详见 bms《平台可扩展性规划》「工作区模型与基座引用」节）。
...
- 知识图谱按需更新（仅用户要求时在工作区根运行 `graphify update .`）；graphify 工具使用见《[graphify 部署使用说明](../资料/AI/graphify部署使用说明.md)》。
