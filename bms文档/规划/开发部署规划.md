...
| 监控与追踪（阶段十三） | Prometheus、Loki、Grafana、Alertmanager、Jaeger、otel-collector | 浏览器查看 Grafana / Jaeger |
| AI 基础设施（阶段十九） | Milvus + etcd（MinIO 复用），PaddleOCR 可选 | — |
| 日常开发 | — | 编辑代码、uv 后端开发服务器、Vite 前端开发服务器、本地 SQLite、压测客户端（阶段十四） |
...
graph TD
    ...
    B --> B2["monitor.yml prometheus、loki、grafana、alertmanager、jaeger、otel-collector（阶段十三）"]
...
2. **分层流水线**（gitlab-runner 在 mjbk 执行；按改动内容与分支分层，避免文档类小改动空耗 CI）：
   - **免跑层**：纯文档/资料推送不建任何流水线（workflow rules 以 changes 排除法判定——paths 仅列代码与基础设施路径，任一命中即建，宁可多跑不可漏跑）；**基座入口《文档首页》虽在《基座文档清单》内，但不在触发路径**（2026-09-14 评估定案）——它是任务收尾的高频改动点（每次回写进度表都会动），纳入会导致每次纯文档推送都拉起整条冒烟层，与免跑层目的相悖；其链接自洽由**本地** `python3 scripts/tools/base-check/check-links.py`（须在 bms 仓库根运行）兜底，同批推送若另有基座路径改动则随 `base-integrity`（`check-base.py` 四项）一并校验。将来若确需给「仅《文档首页》改动」加 CI 校验，须另建只含 `gate-smoke` + `base-integrity` 的轻量流水线（job 级 `changes` 规则与「不得用 changes 区分文档/代码场景给 job 放行」的既有教训冲突，须先评估）；
...
与《总体项目规划》「阶段内容与完成标准」的阶段对齐，环境按需引入、避免一次性拉满：
...
| 第二批 | mjbk：rocketmq（nameserver + broker） | 阶段八（消息与任务基础设施） |
| 第三批 | mjbk：monitor 组（prometheus / loki / grafana / alertmanager / jaeger / otel-collector）+ 开发服务器备份脚本 | 阶段十三（分布式与监控） |
| 第四批 | mjbk：elasticsearch（单节点）；若已提前引入则无需动作 | 阶段十八（全文检索） |
| 第五批 | mjbk：milvus + etcd（MinIO 复用）；PaddleOCR 按需本地部署（可选） | 阶段十九（AI 能力） |

每批环境就位后，对应阶段验收标准中的「环境相关项」即可验证（如阶段一 /healthz /readyz、阶段十三监控链路与追踪可见、阶段十四压测在 mjpc 发起、目标 500 并发 / P99 ≤ 1s）。
...
- **《运维手册》**（阶段十三）覆盖监控告警配置、日志检索、常见故障排查。
