...
```bash
export BMS_DB_URL="mysql+aiomysql://bms_dev:<password>@<mjbk-IP>:3306/bms_dev"
export BMS_DB_URL="postgresql+psycopg://bms_dev:<password>@<mjbk-IP>:5432/bms_dev"
export BMS_DB_URL="dm+dmPython://bms_dev:<password>@<mjbk-IP>:5236/bms_dev"
```
...
> 环境变量名以仓库 config 实现为准，上表为规划口径（均带 `BMS_` 前缀，符合《命名规范》「基础设施命名」节）；完整清单随《部署手册》交付。
