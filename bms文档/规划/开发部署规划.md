...
2. **分层流水线**（gitlab-runner 在 mjbk 执行；按改动内容与分支分层，避免文档类小改动空耗 CI）：
   ...
   - **冒烟层**（含代码改动的 main 直推自动触发）：gate-smoke 门禁冒烟 + 后端 ruff + pyright 类型检查 + pytest（SQLite，pytest-cov 覆盖率门禁：核心 ≥ 80%、整体 ≥ 70%）+ 前端 ESLint + Vitest（frontend / frontend-mobile 双端）+ 双端构建 + 模块注册清单校验（`backend/ops/check_modules.py`，随 04-1 接入）——各 job 以 exists 守卫随代码目录落地自动激活；
