...
- AI 助手（opencode）执行预计超过 10 秒的命令，按工作区根《AGENTS.md》纪律后台化；
...
| bg.js | opencode 插件：向 AI 助手注册上述四组工具（`bg_run` / `bg_status` / `gl_watch_pipeline` / `bg_stop`） | 位于工作区根 `.opencode/plugins/`，经 `opencode.json` 登记加载 |

Python 脚本全部位于工作区内的基座仓 `<工作区>/bms/scripts/tools/bg/`（watch_pipeline.py 位于同级的 `gitlab/`）；插件文件 `bg.js` 位于工作区根 `.opencode/plugins/`，内置脚本路径按工作区根相对定位（见 3.2 节）。
...
在工作区根 `.opencode/opencode.json` 的 `plugin` 数组登记插件文件（相对工作区根的路径），重启 opencode 生效：

```json
{
  "plugin": [
    ".opencode/plugins/graphify.js",
    ".opencode/plugins/bg.js"
  ]
}
```

> 注意：插件文件在工作区根 `.opencode/plugins/` 下，**不是**在 `bms/` 仓库内；登记路径相对工作区根书写（本工作区实测值如上）。
...
本项目对命令做分级处理，禁止长时间无反馈等待（细则见工作区根《AGENTS.md》「后台任务执行」节）：
...
| 插件工具不可用 | 检查工作区根 `.opencode/opencode.json` 的 plugin 数组是否登记 `.opencode/plugins/bg.js`（另有 `.opencode/plugins/graphify.js`），登记后重启 opencode。 |
| 命令跑到了 `$HOME` 而不是项目根 | **背景**：opencode 插件进程的 cwd 为 `$HOME`，`bg_run` 不传 `workdir` 时，`bg-run.py` 默认用 `os.getcwd()`（即 `$HOME`），项目内相对路径命令（如 `graphify update .`）会跑偏、误扫整个 home。**处置**：对依赖项目根相对路径的命令，`bg_run` 必须显式传 `workdir=/home/minjian/develop/bizs/bms`（当前工作区根；换工作区时为对应 `<工作区>/bms`）（或直接用绝对路径命令）。误在 `$HOME` 生成的 `graphify-out/` 需手动删除（见《远程控制部署使用说明》排障记录同源问题）。
