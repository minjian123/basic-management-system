...
微前端的实现差异集中在三个问题：**片段的边界是什么**（页面 / 区域 / 组件）、
...
| 服务端组合 | 服务器 / CDN 边缘 | ✓（片段可独立部署） | 需要服务端编排、响应拼装与缓存策略 |
...
片段以 HTML/SSR 输出，在**服务端或 CDN 边缘**拼装成完整页面。

| 手段 | 说明 | 案例/备注 |
| --- | --- | --- |
| SSI（Server Side Includes） | Web 服务器模板包含（`#include`），简单稳定 | micro-frontends.org 示例工程 |
| ESI（Edge Side Includes） | CDN 边缘拼装片段，可缓存、可降级 | **IKEA**（页面 = ESI 片段，自包含片段含各自 CSS/JS）；DAZN 评述其缓存与可用性优势 |
| 服务端渲染组合（Fragments/SSR） | 应用层按片段组装响应（Node/边缘函数） | **Zalando** Interface Framework（Renderers 服务端渲染） |
| 边缘计算（Edge SSR） | 在边缘节点执行 SSR 与拼装 | IKEA 后续演进方向 |

- **优点**：首屏友好（HTML 直出）、SEO 好、片段可独立部署；缓存策略可下沉到 CDN。
- **缺点**：最慢片段决定整体响应时间；需要骨架屏/占位避免布局跳动（CLS）；开发与调试链路更长（微前端站点有多篇专述）。
...
> 但任何框架都只是「编排」，**不能替代**片段内部的模块化与契约（见《[04 隔离与治理](04_微前端_隔离与治理.md)》）。
...
- **机制**：片段以自定义元素暴露（标签 + 属性 + 事件即契约），框架无关；Shadow DOM 提供样式封装。
- **优点**：浏览器原生、跨框架、长期稳定；适合**叶子组件/设计系统**级复用。
- **限制**：Shadow DOM 对弹窗/下拉挂 body 的组件不友好；SSR 支持弱（无 JS 即无 Custom Elements）；事件与属性通信偏手动。
- **定位**：micro-frontends.org 推荐它做主集成原语；Thoughtworks 观察「Web Components 在微前端领域一直难以落地」——两者并不矛盾：**做片段外壳可行，做全站路由级方案不成熟**。

### 4.4 iframe

- **机制**：每个片段独立文档上下文，浏览器级隔离（JS/样式/路由全隔离）。
...
- **机制**：浏览器用 import map 将裸模块名映射到 URL，片段按约定暴露 ESM 入口。
...
```mermaid
flowchart TD
    Q1{需要片段独立发版吗？} -->|不需要| BT[构建期合并<br/>先做好模块化]
    Q1 -->|需要| Q2{首屏/SEO 敏感？}
...
> 本项目现状：单后端 + 单一 API 形态（OpenAPI 契约 + 统一响应），**无需 BFF**；
> 若未来出现「多前端形态（PC/H5/大屏）差异大」或微前端化，再按此模式评估。
