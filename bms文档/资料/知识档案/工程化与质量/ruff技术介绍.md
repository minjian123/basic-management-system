...
- 作为平台《架构设计 · 总体架构》「技术栈全景 · 工程化与质量」节 节指定的代码规范工具，替换 flake8 / black / isort（选型依据见平台《项目规划说明》「工程化与质量」节）。
- 配置收进 `pyproject.toml` 的 `[tool.ruff]` 段，随《[uv](uv技术介绍.md)》统一管理，不额外引入配置文件。
- CI 前端门禁：MR 流水线第一道关卡就是后端 ruff 检查，不通过直接失败（见平台《项目规划说明》「部署与运维」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 工程化与质量」节 节 | 技术栈：代码规范 = ruff |
| 平台《项目规划说明》「工程化与质量」节 | 选型说明：lint + format 一体化、性能极高 |
| 平台《项目规划说明》「部署与运维」节 | CI 流水线：ruff 为后端第一道门禁 |
