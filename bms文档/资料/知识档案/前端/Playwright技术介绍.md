...
[文档首页](../../../文档首页.md) › [知识档案](../技术栈知识档案总览.md) › [前端](../技术栈知识档案总览.md#apps/desktop) › Playwright 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **定位**：本项目  E2E 测试框架，main 流水线独立 job 运行（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节）。
...
- **E2E 覆盖范围**：登录、主体链 RBAC 权限（含按钮显隐与字段权限）、核心 CRUD 主流程（见平台《项目规划说明》「测试策略与测试流程」节）。
- **main 流水线独立 job**：Playwright E2E 在 main 流水线单独成 job 运行，不阻塞 MR 流水线（见平台《项目规划说明》「部署与运维」节 GitLab CI）。
- **E2E 环境**：服务容器起 backend（SQLite）与前端产物，job 内自包含（见平台《开发部署规划》「开发工作流与 CI」节）。
- **测试报告与归档**：Playwright 生成自动化测试报告（CI 产物归档），结果经官方插件导入 Kiwi TCMS（见平台《项目规划说明》「测试报告」节）。
- **移动端兼容基线**：设备模拟覆盖移动端 H5（审批/通知/查询链路），支撑「浏览器与移动端真机兼容基线」验收（见平台《总体项目规划》「阶段内容与完成标准」节）。
- **替代 Selenium**：自动等待 + 录制器 + 跨浏览器，替代 Selenium 的手工等待与冗长样板（见平台《项目规划说明》「前端」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 | 前端技术栈（Playwright 条目） |
| 平台《项目规划说明》「前端」节 | 选型理由：自动等待、录制器、跨浏览器，替代 Selenium |
| 平台《项目规划说明》「测试策略与测试流程」节 | 测试策略：E2E 覆盖范围与报告归档 |
| 平台《项目规划说明》「部署与运维」节 | GitLab CI：main 流水线 Playwright 独立 job |
...
| 平台《开发部署规划》「开发工作流与 CI」节 | E2E 环境：服务容器起 backend 与前端产物 |
