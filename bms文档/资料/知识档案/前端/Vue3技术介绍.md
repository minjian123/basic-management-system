...
- PC 管理端（apps/desktop）与移动端 H5（frontend-mobile）**双工程同用 Vue 3**，分别配合 Element Plus 与 Vant 组件库，复用同一套后端 API 与会话体系（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节）。
...
| 平台《项目规划说明》「项目目录结构」节 | apps/desktop / frontend-mobile 目录结构 |
