...
- **定位**：本项目  frontend-mobile 独立工程的 UI 组件库，承载移动端 H5（审批、通知、数据查询）全部界面（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 节）。
...
- **frontend-mobile 独立工程**：Vue 3 + Vant 的移动端 H5 工程，与 PC 端 frontend 各自独立（各自 package-lock.json、ESLint/Prettier/TS 配置，互不共享），复用后端 API 与会话体系（见平台《项目规划说明》「前端」节、平台《项目规划说明》「项目目录结构」节）。
...
- **企微/钉钉免登**：企业微信/钉钉工作台内嵌免登，手机浏览器可直接访问；审批链路在移动端闭环（见平台《项目规划说明》「开发计划与验收标准」节验收标准）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 节 | 前端技术栈（Vue 3 + Vant 条目） |
