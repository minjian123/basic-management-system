...
- **定位**：本项目  frontend-mobile 独立工程的 UI 组件库，承载移动端 H5（审批、通知、数据查询）全部界面（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节）。
...
- **frontend-mobile 独立工程**：Vue 3 + Vant 的移动端 H5 工程，与 PC 端 apps/desktop 各自独立（各自 package-lock.json、ESLint/Prettier/TS 配置，互不共享），复用后端 API 与会话体系（见平台《项目规划说明》「前端」节、平台《项目规划说明》「项目目录结构」节）。
