...
> 移动端 Vue 组件库 · 本项目  frontend/apps/mobile 界面基础

[文档首页](../../../文档首页.md) › [知识档案](../技术栈知识档案总览.md) › [前端](../技术栈知识档案总览.md#apps/desktop) › Vant 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **定位**：本项目  frontend/apps/mobile 独立工程的 UI 组件库，承载移动端 H5（审批、通知、数据查询）全部界面（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节）。
...
- **frontend/apps/mobile 工程**：Vue 3 + Vant 的移动端 H5 工程，依赖随根 pnpm workspace 统一锁定（根 pnpm-lock.yaml），ESLint/Prettier/TS 配置独立，复用后端 API 与会话体系（见平台《项目规划说明》「前端」节、平台《项目规划说明》「项目目录结构」节）。
- **移动端 H5 页面**：登录（账号密码/SSO/企业微信/钉钉免登）、首页工作台（待办/通知/公告/快捷入口卡片）、我的待办/已办、审批操作（同意/驳回/评论，附件在线预览）、通知列表（批量已读/全部已读）、公告查看、我发起的、数据查询（报表查看）、帮助中心、个人中心（改密/语言/喜好偏好/退出登录）（见平台《项目规划说明》「前端页面清单与菜单机制」节）。
- **企微/钉钉免登**：企业微信/钉钉工作台内嵌免登，手机浏览器可直接访问；审批链路在移动端闭环（见平台《总体项目规划》「阶段内容与完成标准」节验收标准）。
...
- **升级走评审**：大版本变更升级走人工评审 + 回归，重点验证审批链路。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 | 前端技术栈（Vue 3 + Vant 条目） |
| 平台《项目规划说明》「前端」节 | 选型理由：移动端独立工程，复用后端 API |
| 平台《项目规划说明》「前端页面清单与菜单机制」节 | 移动端 H5 页面清单（审批/通知/查询/免登） |
