...
[文档首页](../../../文档首页.md) › [知识档案](../技术栈知识档案总览.md) › [前端](../技术栈知识档案总览.md#apps/desktop) › Vue Router 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
TypeScript 编写。本项目的 PC 管理端（frontend/apps/desktop）与移动端 H5（frontend/apps/mobile）
均为 SPA，路由是页面组织的骨架。
...
| 动态路由（addRoute） | 运行期追加/删除路由记录：本项目菜单树由后端按用户权限返回，前端据此动态注册路由（见《项目规划说明》「前端页面清单与菜单机制」节菜单机制） |
| 路由守卫 | 导航过程中的拦截钩子：全局（beforeEach）、路由级（beforeEnter）、组件内；本项目用它做登录校验与权限拦截 |
| 路由懒加载 | 组件写成 `() => import('@/views/...')`，页面按需打包与加载，配合静态资源压缩（见《项目规划说明》「前端页面清单与菜单机制」节前端性能） |
...
- **前端路由（2.2 节）**：frontend/apps/desktop 与 frontend/apps/mobile 双工程各自使用 Vue Router 4 管理页面导航（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节）。
- **动态路由（15 节菜单机制）**：菜单树由后端按用户权限动态返回，前端根据菜单树动态注册路由与生成侧边栏；前端不硬编码路由表，新增菜单/表单/按钮/字段无需发版（见平台《项目规划说明》「前端页面清单与菜单机制」节）。
- **路由守卫配合业务权限做菜单级控制（3.2 节）**：全局守卫在每次导航前校验登录态与路由 meta 中的权限码，无权页面重定向到 403/首页（见平台《项目规划说明》「前端」节与 7 节权限模型的双重控制）。
- **静默续期（12 节认证与安全）**：access token 仅存前端内存、刷新页面后静默续期——路由守卫恰是"刷新后先续期再放行"的落点（见平台《项目规划说明》「认证与安全」节）。
- **路由懒加载**：按 15 节前端性能要求，页面组件全部懒加载，控制首屏体积。
- **目录约定**：动态路由生成逻辑集中在 `src/router/`（见平台《项目规划说明》「项目目录结构」节目录结构）。
- **移动端复用**：frontend/apps/mobile 同样基于 Vue Router 4，页面栈与参数传递约定与 PC 端保持一致。
...
| Nuxt 文件路由 | 约定式路由、开箱 SSR，但引入整套 Nuxt 框架 | 《项目规划说明》「AI 辅助开发与交付治理」节已明确前端不采用 SSR/Nuxt |
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 | 前端技术栈（Vue Router 4 条目） |
| 平台《项目规划说明》「前端页面清单与菜单机制」节 | 菜单机制：动态路由生成与侧边栏、前端不硬编码路由表 |
| 平台《项目规划说明》「前端」节 | 选型理由：路由守卫配合业务权限做菜单级控制 |
