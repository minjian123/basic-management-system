...
- **定位**：本项目前端实时推送客户端——待办/通知实时到达、待办角标实时刷新、会话失效即时生效（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 节）。
...
- **与后端配合**：后端用 python-socketio（Redis 适配器跨实例广播），前端用 socket.io-client，协议两端对齐（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节）。
- **重连与降级**：断线自动重连、WebSocket 被拦自动降级轮询；重连成功后重新拉取最新待办/通知，避免断线期间漏推（见平台《项目规划说明》「后端核心」节）。
- **验收口径**：MVP 验收要求「待办提醒经 Socket.IO 实时触达」（见平台《项目规划说明》「开发计划与验收标准」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 节 | 前端技术栈（socket.io-client 条目） |
