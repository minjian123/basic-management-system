...
- **定位**：本项目表单设计器专用——字段排序、分组与跨分区拖拽、查询区/明细列顺序调整（见平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 节）。
...
- **表单设计器（表单布局管理）**：选表单 → 布局编辑——分区/分组/列数/跨列/顺序/标签宽度、查询表单区、明细区列配置、字段属性、租户自建字段，列表排序与跨分区拖拽统一用 vuedraggable（见平台《项目规划说明》「前端页面清单与菜单机制」节、平台《项目规划说明》「开发计划与验收标准」节验收标准）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 前端」节 节 | 前端技术栈（vuedraggable 条目） |
