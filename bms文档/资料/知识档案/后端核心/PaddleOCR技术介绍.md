...
本项目用它做扫描件的文字识别（阶段十七），让扫描件里的内容也能被全文检索和语义检索命中。

- **定位**：本项目阶段十七「OCR 与文件自动分类」能力的默认本地识别引擎，可切换多模态模型 API。
...
- **扫描件文字识别**（阶段十七）：PDF 扫描件/图片 → OCR 提取文本 → 文本落库并进 ElasticSearch 索引（文件内容检索）、向量化进 Milvus（语义检索），扫描件从此「搜得到、问得了」。
- **与文件检索结合**：OCR 是文件内容检索链路对扫描件的支持环节——可解析格式（PDF/Word/Excel/TXT）直接解析文本，扫描件走 OCR（见《[ElasticSearch 技术介绍](ElasticSearch技术介绍.md)》「在本项目中的用途」节）。
...
| 平台《项目规划说明》「后端核心」节 | OCR 默认本地 PaddleOCR、可切换多模态 API 的选型说明 |
| 平台《项目规划说明》「开发计划与验收标准」节 | 阶段十七「AI 能力」验收标准（含 OCR 与文件自动分类） |
