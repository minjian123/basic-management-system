...
- **找回密码**：自助找回密码时发送含短时效 token 的重置链接（见平台《项目规划说明》「认证与安全」节）。
- **模板在线编辑**：邮件模板入库 `sys_mail_template`，页面在线编辑，保存前做语法校验与沙箱渲染测试；多语言主题/内容存 `sys_mail_template_i18n` 附表（见平台《项目规划说明》「核心数据表清单」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《项目规划说明》「功能模块」节 | 功能模块：邮件通知（模板入库、SMTP 发送、发送记录） |
| 平台《项目规划说明》「核心数据表清单」节 | 核心数据表：`sys_mail_template`、`sys_mail_log` |
| 平台《项目规划说明》「认证与安全」节 | 认证与安全：找回密码邮件通道 |
