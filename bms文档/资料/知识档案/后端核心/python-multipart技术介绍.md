...
- 配合文件类型白名单：接口层通过 `UploadFile.content_type` 与扩展名校验，只放行允许的类型，见平台《项目规划说明》「功能模块」节「文件管理」模块。
- 配合《[MinIO 技术介绍](MinIO技术介绍.md)》：≤20MB 整包上传，超限走分片上传；接口收到文件后由 services 层异步写入对象存储。
- 普通表单字段（元数据、用途分类等）一并走 `Form()` 解析，随文件一起提交。
- 接口层只做解析与校验，文件落盘逻辑下沉 services 层，遵循分层架构约束（平台《项目规划说明》「项目目录结构」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 后端技术栈：文件上传条目 |
| 平台《项目规划说明》「功能模块」节 | 文件管理模块：类型白名单与上传规则 |
