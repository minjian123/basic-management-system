...
这正是阶段十七「语义搜索与文件问答」需要的能力。

- **定位**：本项目阶段十七 AI 能力的向量存储底座，承载文件内容分段向量与知识库向量。
...
- **部署**：Docker Compose 独立容器（milvus + etcd），端口 19530（gRPC），仅内网，阶段十七随 AI 能力引入。
...
- **语义搜索与文件问答**（阶段十七，八项 AI 能力之一）：文件内容分段后，分段文本落库（`ai_file_segment` 表：`file_id`、`chunk_no`、`content`、`vector_id`），向量存 Milvus；检索时先取 top-k 相似分段，再交给 LLM 生成答案（RAG，见《[LLM 适配层技术介绍](LLM适配层技术介绍.md)》）。
- **租户隔离**：collection 按租户命名（`bms-{tenant}-doc` 等，见《[命名规范](../../../规范/命名规范.md)》「基础设施命名」节），查询强制携带租户过滤，向量库按租户边界隔离（规划 3.1 节设计约束）。
- **embedding 管线事件驱动**：文件新增/变更事件经 RocketMQ 投递，event-worker 消费后做分段、向量化、入库（复用现有事件模型，见《[RocketMQ 技术介绍](RocketMQ技术介绍.md)》）。
- **独立部署**：milvus + etcd 容器组，MinIO 复用（平台《开发部署规划》第五批，阶段十七引入）；端口 19530 仅内网。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 | 后端技术栈：AI（阶段十七）条目 |
| 平台《项目规划说明》「后端核心」节 | 向量检索选型与租户隔离约束 |
| 平台《项目规划说明》「核心数据表清单」节 | `ai_file_segment` 表：分段文本落库、向量存 Milvus |
| 平台《项目规划说明》「环境与配置」节 | Milvus（含 etcd）随阶段十七加入 Compose |
| 《[命名规范](../../../规范/命名规范.md)》「基础设施命名」节 | Milvus collection 命名 `bms-{租户}-{用途}` |
