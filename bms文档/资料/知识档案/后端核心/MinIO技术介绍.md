...
- **许可**：AGPL-3.0。仅作独立服务进程部署、不修改源码时无传染性；官方另提供商业订阅（见平台《项目规划说明》「开源与许可协议」节许可分析）。
...
- **统一文件存储**：附件、导入模板、导出结果、图片/PDF 等全部对象存储，backend 多实例共享同一端点，无状态横向扩展（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节）。
- **预签名 URL 限时下载**：下载/预览接口生成短时效预签名 URL（含权限校验），浏览器直连 MinIO 拉取，避免大文件经过后端转发。
- **大文件分片上传**：≤20MB 整包上传；超限分片 + 断点续传，分片用 `.part` 标识的临时对象存 MinIO，合并后清理，见平台《项目规划说明》「功能模块 · 文件管理」节模块。
...
- **AI 能力复用**：阶段十七 Milvus 向量库依赖的对象存储直接复用 MinIO。
...
- **AGPL 边界**：MinIO 以独立服务进程运行、不改源码即不传染；禁止为优化而修改 MinIO 源码内嵌项目（平台《项目规划说明》「开源与许可协议」节合规要求）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 | 后端技术栈：对象存储条目 |
| 平台《项目规划说明》「功能模块」节 | 文件管理模块：分片上传、断点续传、预签名下载规则 |
