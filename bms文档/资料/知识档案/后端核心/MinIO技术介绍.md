...
- **统一文件存储**：附件、导入模板、导出结果、图片/PDF 等全部对象存储，backend 多实例共享同一端点，无状态横向扩展（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节）。
- **预签名 URL 限时下载**：下载/预览接口生成短时效预签名 URL（含权限校验），浏览器直连 MinIO 拉取，避免大文件经过后端转发。
- **大文件分片上传**：≤20MB 整包上传；超限分片 + 断点续传，分片用 `.part` 标识的临时对象存 MinIO，合并后清理，见平台《项目规划说明》「功能模块」节「文件管理」模块。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 后端技术栈：对象存储条目 |
