...
- 生产数据库之一，与 PostgreSQL、达梦 DM8 三库并行支持；开发/测试默认 SQLite（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节）。
- **aiomysql 异步驱动**（dialect `mysql+aiomysql`）；同步 fallback 可换 PyMySQL；备选 asyncmy 性能更好但维护活跃度一般，故不采用（见平台《项目规划说明》「后端核心」节）。
- 数据层主从：MySQL 主从读写分离 + 故障切换，1 平台库 + N 租户库 + 1 归档库（见平台《项目规划说明》「部署拓扑」节）。
- 字符集 utf8mb4；连接池按 worker 数规划（≤ 70% max_connections）；慢查询日志开发联调用（见平台《项目规划说明》「数据规范」节与平台《开发部署规划》，部署口径见《[开发服务器部署使用说明总览](../../开发服务器/linux/开发服务器部署使用说明总览.md)》）。
- 备份与恢复：每日全量备份 + Binlog 时间点恢复（PITR），保留 30 天异机存储（见平台《项目规划说明》「备份与恢复」节）。
...
- **GPL 传染误读**：社区版 GPL-2.0 只约束 MySQL 自身及衍生修改；本项目作为独立应用通过协议连接、不修改不分发其源码即不传染。介意或需官方支持时采购商业版（见平台《项目规划说明》「开源与许可协议」节）。
...
- **保留字**：如 sys_config 的 config_key 字段即因 key 为保留字改名，新表设计时避开保留字（见平台《项目规划说明》「核心数据表清单」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 技术栈：MySQL 8.x 与 aiomysql 条目 |
| 平台《项目规划说明》「开源与许可协议」节 | GPL-2.0 使用边界与商用注意事项 |
| 平台《项目规划说明》「数据库兼容」节 | 数据库兼容：跨方言类型与字符集约定 |
