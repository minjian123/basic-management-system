...
- **许可**：**LGPL-3.0**（GNU 宽通用公共许可证，弱 copyleft）。注意：任务清单与《项目规划说明》「开源与许可协议」节曾标注为 MIT，经核实 GitHub 仓库与 PyPI 实际为 LGPL-3.0，两者法律含义不同，**建议复核并同步修正规划文档**。
...
- **解析执行 BPMN XML**：支持用户任务（审批节点）、网关（分支/并行）、结束事件等标准元素（见平台《项目规划说明》「后端核心」节）。
- **同步执行 + 线程池**：SpiffWorkflow 是**同步**库，在 FastAPI 异步栈中放入线程池运行（`run_in_executor`），避免阻塞事件循环（见《[FastAPI 技术介绍](FastAPI技术介绍.md)》）。
- **状态由本项目自建表维护**：引擎本身不持久化，流程实例、任务、审批记录由本项目自建表（如 `wf_process`、流程实例表、审批记录表）存储，经序列化/反序列化跨请求恢复（见平台《项目规划说明》「核心数据表清单」节）。
- **流程建模闭环**：管理端用 bpmn-js 拖拽绘制 → 导出 BPMN XML → SpiffWorkflow 执行；流程定义发布新版本、实例启动、节点通过、驳回、结束等经事件总线发布（见平台《项目规划说明》「事件总线事件模型」节 `wf.*` 事件）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《项目规划说明》「后端核心」节 | 选型说明：SpiffWorkflow 条目与 Python 3.14 兼容口径 |
| 平台《项目规划说明》「功能模块」节 | 功能模块：审批（流程建模、流程实例、我的待办/已办） |
| 平台《项目规划说明》「事件总线事件模型」节 | 事件总线：`wf.*` 流程事件 |
