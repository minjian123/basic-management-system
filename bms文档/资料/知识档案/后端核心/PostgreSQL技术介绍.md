...
- 生产数据库之一，与 MySQL、达梦 DM8 三库并行支持；开发/测试默认 SQLite（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节）。
- **psycopg 3 驱动**：PostgreSQL 官方维护、原生异步（dialect `postgresql+psycopg`），替代已冻结的 psycopg2（见平台《项目规划说明》「后端核心」节）。
- 数据层主从：PostgreSQL 主从（读写分离 + 故障切换），1 平台库 + N 租户库 + 1 归档库（见平台《项目规划说明》「部署拓扑」节）。
- 软删除优化：PostgreSQL 可另加部分唯一索引优化（基础方案四库统一的 (唯一字段, deleted_at) 复合唯一索引）（见平台《项目规划说明》「数据规范」节）。
- 备份与恢复：每日全量备份 + WAL 时间点恢复（PITR），保留 30 天异机存储（见平台《项目规划说明》「备份与恢复」节）。
...
- **方言差异**：部分索引、JSONB、ILIKE 等是 PG 特性，ORM 层统一用跨方言写法，方言优化只在三库验证通过的迁移/查询中使用（见平台《项目规划说明》「数据库兼容」节）。
- **连接池规划**：与 MySQL 同口径——worker × (pool_size + max_overflow) ≤ max_connections 的 70%（见平台《项目规划说明》「性能与高并发设计」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 技术栈：PostgreSQL 16+ 与 psycopg 3 条目 |
| 平台《项目规划说明》「后端核心」节 | 选型说明：psycopg 3 替代 psycopg2 |
| 平台《项目规划说明》「数据规范」节 | 数据规范：软删除与部分唯一索引优化 |
