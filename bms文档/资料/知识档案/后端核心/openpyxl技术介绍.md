...
- 支撑**用户批量导入**：解析上传的 xlsx，逐行校验后落库（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节、功能模块「导入导出」）。
...
- 导入模板、导出文件经 [python-multipart](python-multipart技术介绍.md) 上传、产物可存 [MinIO](MinIO技术介绍.md)（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节文件管理）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 后端技术栈：Excel 处理（openpyxl）条目 |
