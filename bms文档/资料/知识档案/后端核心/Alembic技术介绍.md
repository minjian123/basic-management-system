...
- 作为唯一 schema 版本管理工具：目录 `alembic/` + `alembic.ini`，URL 从 config.toml 读取，不写死连接串（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 | 技术栈：数据库迁移条目（URL 来自 config.toml） |
