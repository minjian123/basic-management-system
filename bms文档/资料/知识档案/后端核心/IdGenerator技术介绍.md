...
- 分布式全局唯一 ID 生成：**所有业务表统一用雪花 ID 作主键**，关联表用联合主键（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节、11.1 节「数据规范」）。
- 分布式环境下无需中心协调即可生成全局唯一 ID，契合本项目多实例无状态部署（见平台《项目规划说明》「集群设计要点」节）。
- **时钟回拨处理配置**：部署时需关注 NTP 时间同步与回拨容忍配置，避免极端情况下 ID 冲突（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节yitter 条目）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 后端技术栈：ID 生成（yitter/IdGenerator）条目 |
