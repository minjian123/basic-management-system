...
- **定时任务**：celery beat + 自研数据库调度器——任务定义入库 `sys_task`（定义/cron/启停），beat 启动时加载、任务变更事件触发重载，页面可动态增删改与启停（见平台《项目规划说明》「功能模块」节「任务调度」）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《项目规划说明》「功能模块」节 | 功能模块：任务调度（sys_task/sys_task_log、内置周期任务） |
| 平台《项目规划说明》「多数据源策略」节 | 多数据源策略：分片表预创建（Celery 定时任务） |
| 平台《项目规划说明》「数据规范」节 | 数据规范：过期软删除数据由 Celery 定时物理清理 |
