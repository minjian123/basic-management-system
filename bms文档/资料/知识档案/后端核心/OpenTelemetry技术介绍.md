...
由 **Grafana Tempo** 存储与展示，并随 Grafana 统一可视化。

- **定位**：本项目链路追踪（可观测性三件套之一），MVP 即接入。
- **版本**：Python SDK 1.x（Traces API/SDK 已稳定），部分实验性组件为 0.x。
- **许可**：OpenTelemetry / otel-collector 为 Apache-2.0；Grafana Tempo 为 AGPLv3（自托管内部使用）。
...
| Grafana Tempo | trace 的存储与展示后端，原生 OTLP 接收，HTTP 查询端口 3200（经 Grafana 统一可视化） |
...
- 链路追踪：应用经 **otel-collector** 上报 trace 至 **Tempo** 存储展示（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节）。
- **trace_id 与 structlog 的 request_id 关联**：日志里带上 trace_id，实现「日志 ↔ 链路」互跳定位（见平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节structlog 条目）。
- 随 **Grafana** 统一可视化：Grafana 配置 Tempo 数据源，与 Prometheus 指标、Loki 日志同屏排查（见平台《项目规划说明》「部署与运维」节监控）。
- **MVP 即接入**：链路追踪不放到后期，登录、审批等核心链路从早期就可观测。
- 部署端口：Tempo HTTP 3200、otel-collector OTLP 4317/4318，仅内网（见平台《开发部署规划》「端口与网络规划」节端口规划）。
- 验收口径：「链路追踪（Tempo 链路可见）」纳入功能测试与 MVP 验收（见平台《项目规划说明》「测试策略与测试流程」节）。

## 4. 选型对比 <a id="compare"></a>

| 候选技术 | 优缺点 | 结论 |
| --- | --- | --- |
| **OpenTelemetry + Tempo（选中）** | CNCF 标准、厂商中立、自动埋点成熟、生态最大 | 行业标准组合，避免厂商锁定 |
...
- **service.name 必设**：Resource 里不设置 `service.name`，trace 在 Tempo 里可能查不到或归不到服务。
...
- **开销与采样**：高并发下全量 trace 存储压力大，需配置采样策略；生产经 collector 统一处理，不建议应用直连 Tempo。
...
| Grafana Tempo 官网 | https://grafana.com/docs/tempo/latest/ | trace 后端：部署、查询、OTLP 接收 |
| Tempo GitHub | https://github.com/grafana/tempo | Tempo 源码与发布 |

## 7. 项目内关联文档 <a id="related"></a>

| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 | 后端技术栈：链路追踪（OpenTelemetry + Tempo）条目 |
| 平台《项目规划说明》「部署与运维」节 | 部署与运维：监控体系（Prometheus + Loki + Grafana + Tempo） |
| 平台《开发部署规划》「端口与网络规划」节 | 端口规划：Tempo HTTP 3200 / otel-collector 4317、4318 |
