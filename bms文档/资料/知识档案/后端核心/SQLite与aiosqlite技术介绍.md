...
| 与四库兼容口径 | 规划要求 SQLite/MySQL/PostgreSQL/达梦四库行为一致：软删除复合唯一索引等 SQLite 也需验证（见平台《项目规划说明》「数据规范」节） |

## 3. 在本项目中的用途 <a id="usage"></a>

- 开发环境默认数据库：`BMS_ENV=dev` 时连接本地 SQLite 文件，零部署即可起后端（见平台《开发部署规划》「前端开发运行」节 节）。
- 多库拓扑模拟：用多个 SQLite 文件分别模拟平台库、租户库与归档库，与生产多数据源策略一一对应（见平台《项目规划说明》「多数据源策略」节）。
- **测试统一用 SQLite 保证可移植**：pytest 单测/接口测试全部跑 SQLite，本地与 CI 同源，任何人克隆即测（见平台《项目规划说明》「测试策略与测试流程」节）。
- 异步驱动 aiosqlite 经 SQLAlchemy `sqlite+aiosqlite` dialect 接入，与整体异步栈一致（见《[SQLAlchemy 技术介绍](SQLAlchemy技术介绍.md)》）。
- SQLite 开发库自动建表（不跑 Alembic 迁移），schema 一致性由三库 CI 方言测试兜底（见平台《项目规划说明》「数据库兼容」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 后端核心」节 节 | 技术栈：SQLite（开发/测试，aiosqlite）条目 |
| 平台《项目规划说明》「数据库兼容」节 | 数据库兼容：SQLite 纳入四库行为一致口径 |
