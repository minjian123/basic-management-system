...
而不需要前端轮询。前端对应使用 `socket.io-client` 库（见《[知识档案总览](../技术栈知识档案总览.md#apps/desktop)》前端索引）。

- **待办/通知实时到达**：审批待办产生、通知发布时，实时推送给对应用户，待办角标即时更新（见平台《项目规划说明》「功能模块 · 通知中心」节）。
- **会话失效广播**：管理员强制踢出某会话时，经 Socket.IO 广播「会话失效」，关闭该会话全部实时连接，即时生效（见平台《项目规划说明》「认证与安全 · 多端会话」节）。
...
- **连接数监控**：Socket.IO 连接数纳入系统监控（见平台《项目规划说明》「功能模块 · 系统监控」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《项目规划说明》「功能模块」节 | 功能模块：通知中心（待办角标实时推送）、系统监控（连接数） |
| 平台《项目规划说明》「认证与安全」节 | 认证与安全：多端会话、会话失效广播 |
