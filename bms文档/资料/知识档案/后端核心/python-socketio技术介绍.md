...
- **待办/通知实时到达**：审批待办产生、通知发布时，实时推送给对应用户，待办角标即时更新（见平台《项目规划说明》「功能模块」节「通知中心」）。
- **会话失效广播**：管理员强制踢出某会话时，经 Socket.IO 广播「会话失效」，关闭该会话全部实时连接，即时生效（见平台《项目规划说明》「认证与安全」节「多端会话」）。
...
- **连接数监控**：Socket.IO 连接数纳入系统监控（见平台《项目规划说明》「功能模块」节「系统监控」）。
