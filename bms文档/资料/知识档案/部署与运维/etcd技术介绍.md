...
- **定位**：本项目阶段十七 AI 能力的配套组件——Milvus 向量库的元数据与集群协调依赖（平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节）。
- **版本**：3.5 / 3.6 系列（3.5 为长期维护线，3.6 为新版）。
- **许可**：Apache-2.0，免费开源（平台《项目规划说明》「开源与许可协议」节）。
- **落地形态**：随 Milvus 于阶段十七加入 Compose 编排，MinIO 复用（平台《开发部署规划》「基础设施服务」节）。
...
- **阶段十七随 AI 落地**：etcd 与 Milvus 一并加入 Compose 编排，MinIO 对象存储复用现有实例（平台《开发部署规划》「基础设施服务」节）。
