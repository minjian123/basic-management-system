...
- **定位**：本项目生产入口层，所有浏览器流量先到 nginx 再分发（平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节 节）。
...
- **反向代理**：`/api` 反代到后端多副本；生产同源部署，不开放跨域（平台《项目规划说明》「环境与配置」节）。
- **负载均衡 + TLS 终止**：后端无状态可横向扩展，nginx 负责分发与 HTTPS 解密（平台《项目规划说明》「部署拓扑」节）。
- **子域名租户路由**：多租户按子域名区分入口，配泛域名证书（平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节 节）。
- **WebSocket 支持**：`/socket.io` 路径放行 Upgrade/Connection 头，负载均衡用 ip_hash / Cookie 亲和，保证 Socket.IO 连接升级后固定在同一后端实例（平台《项目规划说明》「集群设计要点」节）。
- **安全响应头**：统一配置 CSP、HSTS（dev 关闭）、X-Frame-Options（SAMEORIGIN，兼容同源 Grafana 嵌入）、X-Content-Type-Options、Referrer-Policy、Permissions-Policy（平台《项目规划说明》「集群设计要点」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节 节 | 部署与运维技术栈（nginx 条目） |
