...
- **定位**：本项目依赖自动升级，自动提 MR 升级依赖（平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节）。
- **版本**：3x.x 系列（持续快速迭代）。
- **许可**：AGPL-3.0。本项目自托管运行、不修改源码，无传染影响（平台《项目规划说明》「开源与许可协议」节）。
- **落地形态**：阶段二起以容器方式随 gitlab.yml 编排在 mjbk 运行，需 mjbk 可访问外网（平台《开发部署规划》「GitLab 与 CI 基础设施」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节 | 部署与运维技术栈（依赖升级条目） |
| 平台《项目规划说明》「部署与运维」节 | 选型说明：原生支持自托管 GitLab、自动提 MR |
| 平台《开发部署规划》「GitLab 与 CI 基础设施」节 | mjbk 运行形态：随 gitlab.yml 编排、外网依赖 |
