...
- **定位**：本项目依赖自动升级，自动提 MR 升级依赖（平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节 节）。
- **版本**：3x.x 系列（持续快速迭代）。
- **许可**：AGPL-3.0。本项目自托管运行、不修改源码，无传染影响（平台《项目规划说明》「开源与许可协议」节）。
- **落地形态**：阶段二起以容器方式随 gitlab.yml 编排在 mjbk 运行，需 mjbk 可访问外网（平台《开发部署规划》「GitLab 与 CI 基础设施」节 节）。
...
- **运行方式**：阶段二起以容器方式随 gitlab.yml 编排，常驻 mjbk；需 mjbk 可访问外网（拉取版本发布信息），不可达时镜像同步与依赖升级暂缓（平台《开发部署规划》「GitLab 与 CI 基础设施」节 节、11 节）。
...
- **AGPL 边界**：作为独立服务运行、不改源码、不深度定制，遵守平台《项目规划说明》「开源与许可协议」节约束。
- **外网依赖**：Renovate 要访问版本发布源与 GitLab API，mjbk 外网不可达时升级暂停，属预期行为（平台《开发部署规划》「风险与注意事项」节）。
...
| 文档 | 说明 |
| --- | --- |
| 平台《架构设计 · 总体架构》「技术栈全景 · 部署与运维」节 节 | 部署与运维技术栈（依赖升级条目） |
| 平台《项目规划说明》「部署与运维」节 | 选型说明：原生支持自托管 GitLab、自动提 MR |
| 平台《开发部署规划》「GitLab 与 CI 基础设施」节 节 | mjbk 运行形态：随 gitlab.yml 编排、外网依赖 |
