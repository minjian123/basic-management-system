...
- **ID 生成实现的可插拔候选**：为 `bms.plugins.id_generator` 组注册 `snowflake` / `segment` / `uuid7` 等实现，主程序按 `config.toml` 选用（见 [10 配置驱动实现选择](10_插件化架构_配置驱动实现选择.md)）。
...
- **与注册表的关系**：entry points 负责「发现」，注册表负责「按名查找与按配置选用」，二者分工明确（见 [09 自建注册表](09_插件化架构_自建注册表.md)）。
...
选择建议：阶段一/二用标准库 + 自建注册表即可；需要复杂驱动管理与启动性能优化时再评估 Stevedore（见 [08 Stevedore](08_插件化架构_Stevedore.md)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[05 插件发现机制](05_插件化架构_插件发现机制.md)》 | 三种发现方式总述与对比 |
| 《[08 Stevedore](08_插件化架构_Stevedore.md)》 | 基于入口点的驱动管理器 |
| 《[09 自建注册表](09_插件化架构_自建注册表.md)》 | 发现结果如何注册与选用 |
| 《[13 第三方插件生态](13_插件化架构_第三方插件生态.md)》 | 自动发现后的隔离与兼容 |
