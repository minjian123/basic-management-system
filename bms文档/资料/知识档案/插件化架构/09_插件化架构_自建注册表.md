...
- **ID 生成实现的替换**：以 `IdGenerator` 端口为例注册 `snowflake` / `segment` / `uuid7`，`config.toml` 选一，业务代码只调抽象（见 [10 配置驱动实现选择](10_插件化架构_配置驱动实现选择.md)）。
...
- **实例生命周期**：无状态实现可复用单例；有状态/需连接的实现走 [12 生命周期与热插拔](12_插件化架构_生命周期与热插拔.md) 管理。
...
| 文档 | 说明 |
| --- | --- |
| 《[03 端口适配器](03_插件化架构_端口适配器.md)》 | 端口与实现分离的理论 |
| 《[04 接口契约设计](04_插件化架构_接口契约设计.md)》 | 注册对象的契约定义 |
| 《[06 EntryPoints](06_插件化架构_EntryPoints.md)》 | 阶段三叠加的自动发现 |
| 《[10 配置驱动实现选择](10_插件化架构_配置驱动实现选择.md)》 | 装配入口如何按配置选用 |
| 《[12 生命周期与热插拔](12_插件化架构_生命周期与热插拔.md)》 | 有状态实现的启停管理 |
