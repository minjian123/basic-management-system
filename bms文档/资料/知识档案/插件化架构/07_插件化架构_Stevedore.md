...
- **当前取舍**：阶段一/二平台内建实现用自建注册表已足够；引入 Stevedore 属「第三方依赖换开发量」，按项目「薄封装、少依赖」取向需谨慎评估（见 [08 自建注册表](08_插件化架构_自建注册表.md)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[05 EntryPoints](05_插件化架构_EntryPoints.md)》 | Stevedore 的底层发现机制 |
| 《[06 Pluggy](06_插件化架构_Pluggy.md)》 | 钩子式方案对比 |
| 《[08 自建注册表](08_插件化架构_自建注册表.md)》 | 零依赖替代方案 |
