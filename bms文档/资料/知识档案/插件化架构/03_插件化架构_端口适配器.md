...
平台《后端基类清单》已定义 根系 + `BaseCapability` + 能力域基类的分层体系，插件化概念与之逐项对应：
...
### 5.4 分层边界与理由：为何不下沉到 根系 <a id="carrier-boundary"></a>

- `BaseObject` 是**所有**可继承类之根（含集合、ORM `BaseModel`、`BaseSchema`），把切换机制放上去会变**上帝基类**，把无关类一并拖入；
- 平台《后端基类清单》已立口径：「公共字段/方法只在对应层基类维护」「任一方的特有逻辑不得上提 根系」「加层优先于在既有层堆逻辑」；
...
- **一职责一层**：配置读取在装配入口、业务方法在能力基类、公共方法在 根系，都不上提到 `BasePluggable`。
