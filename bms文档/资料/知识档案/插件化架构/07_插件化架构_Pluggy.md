...
- **与自建注册表并用**：阶段一/二可用自建注册表承载单选能力；出现「多插件叠加」需求时，再评估引入 Pluggy 作为钩子层（见 [09 自建注册表](09_插件化架构_自建注册表.md)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[05 插件发现机制](05_插件化架构_插件发现机制.md)》 | 插件如何被发现 |
| 《[08 Stevedore](08_插件化架构_Stevedore.md)》 | 驱动管理式方案对比 |
| 《[09 自建注册表](09_插件化架构_自建注册表.md)》 | 单选能力的零依赖实现 |
