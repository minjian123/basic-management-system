...
302 主入口走回环，ufw 启用前后行为一致；全接口监听 14870/31131 被默认拒绝挡住，属预期。若日后开启 302 的 DNS 重定向模式（iptables 模式），与 ufw 共管 iptables 规则集的注意事项见《[防火墙部署使用说明](防火墙部署使用说明.md)》「容器回访宿主端口的网桥放行」节。
