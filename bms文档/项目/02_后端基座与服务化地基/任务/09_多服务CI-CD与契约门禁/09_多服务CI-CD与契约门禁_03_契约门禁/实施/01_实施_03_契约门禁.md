...
2. **工具环境**：`deploy/ci/Dockerfile.backend` 加装 docker CLI（静态二进制 `docker-29.7.2.tgz`，阿里云 docker-ce static 镜像下载，仅 `/usr/local/bin/docker`），使 `ci-backend` 同一 job 内可执行 ops 编排（Python/uv）并 `docker run` 调工具镜像与起服务镜像；`scripts/tools/docker/镜像清单.txt` 增 `tufin/oasdiff:v1.32.1`、`schemathesis/schemathesis:4.28.0`。
...
5. **冒烟 CLI（重设计）**：`backend/ops/contract_smoke.py`——**用已构建服务镜像**起容器（`docker run -d -e BMS_ENV=dev`，就绪轮询 `docker inspect` `healthy`）→ Schemathesis 容器 `--network container:<服务容器>` 打真实服务（只读 GET/HEAD + 每操作 1 例 + 仅 `not_a_server_error` + 排除探针端点），**按服务、仅本次变更的服务**运行（CI 9 个 job，`rules:changes` 各守本服务路径），非阻断。
6. **CI**：新增档位 `deploy/ci/verify/contract-gate`；`.gitlab-ci.yml` verify 阶段增 `contract-gate`（阻断，oasdiff + 破坏数 after_script 推送）/ 9 个 `contract-smoke-<服务>`（仅变更服务、用已构建镜像、非阻断）；`deploy/ci/verify/contract` 说明更新。
...
8. **门禁**：全量 `pytest`（1332 passed / 37 skipped）+ `ruff check` / `ruff format --check` / `pyright`（0 error）+ 基座 / 边界 / 状态 / 契约校验 + `check-preflight` 全绿。
...
| # | 问题 | 现象 | 处置 |
| --- | --- | --- | --- |
| 1 | 工具走 docker 镜像、ops 编排需 Python，二者不在同一 job 环境 | `ci-backend` 无 docker CLI、`docker:27-cli` 无 Python | `ci-backend` 加装 docker CLI 静态二进制（决策 10） |
...
| 10 | **首推流水线 `contract-smoke` 全服务失败** | ① `--network container:$HOSTNAME` 报 `No such container: runner-…-concurrent-0`（Docker executor 的 `$HOSTNAME` 非 daemon 容器引用）；② `-v` 挂 job 路径的 schema 宿主不可见，报 `The specified file does not exist` | 改 `--network host` + **job 容器 IP**（`socket.gethostbyname(hostname)`，可经 `BMS_SMOKE_BIND_HOST` 覆盖），schema 经 `docker create + docker cp + docker start -a` 复制进容器 `/tmp`；mjbk 以 host 网络 + 容器 IP 实测通过（510 用例）；已回写设计 §4.2 / §5 / 决策 14、开放项与部署说明 |
| 11 | **`contract-smoke` 运行后 9 服务仍失败（Schemathesis 返回码 1）** | 机制已通，失败源为 dev 环境：探针端点 `/readyz` 无 Redis 返回 503（每服务都有）、platform `GET /api/v1/outbox/dead-letters` 在 dev/SQLite 未迁移表下 500 | 排除基础设施端点 `/healthz` `/readyz` `/metrics`；`contract-smoke` 设 `allow_failure: true`（**非阻断冒烟**，阻断门禁为 oasdiff `contract-gate`）；dev 环境 5xx 记计划 §7 第 32 项归口；`run` 失败时打印 Schemathesis 输出尾部便于定位；已回写设计 §4.2 / §4.4 / 决策 15、开关文件与部署说明 |
| 12 | **冒烟口径重设计（用户拍板）** | 源码直跑 9 服务逐一起进程，慢且不可固定重现；用户要求「用打好容器、只跑变更的那点」 | 弃源码直跑；改**已构建服务镜像起容器**（固定 tag ⇒ 可重现）+ **按服务、仅变更服务**运行（9 个 job，`rules:changes` 各守本服务路径，`needs: trigger-<服务>`）；Schemathesis `--network container:<服务容器>`（命名容器引用有效，替代此前的 host+IP）；`--max-examples 1`；已回写设计 §4.2 / §4.4 / 决策 5、16 与部署说明 |

## 5. 验证结果 <a id="verify"></a>

| 验证项 | 命令 / 操作 | 结果 |
| --- | --- | --- |
| 全量用例 | `uv run pytest -q` | **1332 passed / 37 skipped**（新增 19 例含内） |
...
| mjbk Schemathesis（重设计） | 用已构建镜像 `docker run -d -e BMS_ENV=dev` 起容器 + Schemathesis `--network container:<服务容器>` 打 | 通过，exit 0（固定 tag 可重现；CI 按服务仅变更服务运行） |
...
| 落点 | 内容 |
| --- | --- |
| 《部署发布规范》「契约门禁」节（新 §12） | 基线 / 快照分离；oasdiff「只加不删」；基线评审合入；Schemathesis **复用已构建固定 tag 镜像、按服务仅跑变更服务**打真实服务零 Broker；门禁位置与档位；工具固定 tag；检查清单 |
| 《测试规范》「被测物与执行范围」节（新 §2.1，通用强制） | 一切针对构建产物的测试：被测物＝已构建固定版本产物（不重建、不另起源码 / 临时构建物）；按变更范围只跑受影响部分；例外必须先向用户上报确认、不得以「情况不同」自行绕过；可重现（固定 tag / digest） |
| 《AI开发规范》红线与检查清单 | 新增「测试产物复用不开口子」红线：不得以「这次情况不一样」自行绕过，例外先上报确认 |
...
| 7 | ~~dev 环境 5xx（platform `outbox/dead-letters` 等 7 处）——冒烟已排除探针类端点、`contract-smoke` 设非阻断~~ **已闭环（2026-09-24，任务 06_04）**：根因更正为「租户解析把 IP 字面量主机名当子域名 + 兜底把请求值当租户编码 → 派生非法库名 `bms_platform_127.0.0.1` → `ConfigError` 500」（**非**「未迁移表 / 无 Redis 依赖」）；修复后 `contract-smoke-<服务>` 已由非阻断改为**阻断**（本任务 §6 开关文件与《契约门禁与契约测试使用说明》同步更新） | 计划 §7 第 32 项 + GitLab Issue #47（06_04 交付闭环） |
| 8 | 真实流水线契约门禁验收（两 job 全绿 + `bms_contract_breaking_total` 可查） | 见[测试记录](../测试/01_测试_03_契约门禁.md) §4 |
