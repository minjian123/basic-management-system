...
8. **门禁**：全量 `pytest`（1335 passed / 37 skipped）+ `ruff check` / `ruff format --check` / `pyright`（0 error）+ 基座 / 边界 / 状态 / 契约校验 + `check-preflight` 全绿。
...
| 验证项 | 命令 / 操作 | 结果 |
| --- | --- | --- |
| 全量用例 | `uv run pytest -q` | **1335 passed / 37 skipped**（新增 22 例含内） |
