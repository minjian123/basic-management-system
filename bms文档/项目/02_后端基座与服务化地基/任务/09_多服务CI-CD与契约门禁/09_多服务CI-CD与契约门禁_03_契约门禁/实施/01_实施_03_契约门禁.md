...
| 7 | dev 环境依赖缺口 5xx（platform `outbox/dead-letters`、`/api/v1/modules/*` 等；无 Redis 的依赖端点）——冒烟已排除探针类端点、`contract-smoke` 设非阻断 | 另立任务：计划 §7 第 32 项 + GitLab Issue #47（修复转绿后再把冒烟改阻断） |
