...
| 命令 | `oasdiff breaking <基线> <当前> --fail-on ERR`（默认即 ERR 级；返回非零即破坏性变更） |
...
- **oasdiff 调用**：默认 `docker run --rm -v <root>:/repo tufin/oasdiff:v1.32.1 breaking /repo/deploy/contracts/baseline/<svc>.json /repo/<tmp 当前契约> --fail-on ERR --format json`；`--oasdiff` / 环境变量可覆盖命令（单测以桩替换，不真联 docker）。
