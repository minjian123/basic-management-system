...
- **范围（本任务）**：
  ...
  3. **Schemathesis 契约冒烟**：新增 `backend/ops/contract_smoke.py`——用**已构建的服务镜像**（`bms-{service}:$CI_COMMIT_SHORT_SHA`）起真实容器 + Schemathesis 容器 `--network container:<服务容器>` 打真实服务（零 Broker），只读方法（GET/HEAD）+ 每操作 1 例的冒烟口径；**按服务、仅本次变更的服务运行**（父流水线按服务 `rules:changes` 调度）。
  4. **门禁挂载**：新增档位 `deploy/ci/verify/contract-gate`；父流水线 verify 阶段新增 `contract-gate`（阻断）与 9 个按服务 `contract-smoke-<服务>` job（仅变更服务、非阻断）。
  ...
  6. **工具执行环境**：`deploy/ci/Dockerfile.backend` 加装 docker CLI（静态二进制），使同一 job 既能以 python/uv 执行 ops 编排、又能 docker 调 oasdiff / Schemathesis 与起服务镜像；两镜像登记 `scripts/tools/docker/镜像清单.txt`。
...
| 真实服务来源 | 09_02 服务可容器化运行（Compose）；宿主直跑可 | 以已构建服务镜像起容器 + Schemathesis 容器共享其网络；按服务仅变更服务运行 |
...
```mermaid
flowchart TD
    A["按服务（仅本次变更的服务）"] --> B["docker run -d 已构建镜像<br/>bms-{service}:$CI_COMMIT_SHORT_SHA<br/>-e BMS_ENV=dev"]
    B --> C["等待镜像 HEALTHCHECK /healthz = healthy"]
    C --> D["Schemathesis 容器<br/>schemathesis/schemathesis:4.28.0<br/>--network container:&lt;服务容器&gt;，--url http://127.0.0.1:8000"]
    D --> E{"无 5xx / schema 违反？"}
    E -->|通过| F["停服务容器"]
    E -->|失败| G["打印输出尾部 → 停服务容器"]
    F --> H["该服务 job 结论（非阻断）"]
...
| 被测服务来源 | **已构建服务镜像**——子流水线 `service-build` 产物 `bms-{service}:$CI_COMMIT_SHORT_SHA`（固定 tag ⇒ 可固定重现），`docker run -d -e BMS_ENV=dev` 起容器；**不由 CLI 源码起进程、不重复构建** |
| 调度范围 | **按服务、仅本次变更的服务**——9 个 `contract-smoke-<服务>` job 各以 `rules:changes: backend/services/<服务>/**/*` 放行；未变更服务不跑（变更检测与 trigger job 同口径） |
| 网络 | Schemathesis 容器以 `--network container:<服务容器>` 与服务容器**共享网络命名空间**，经 `127.0.0.1:8000` 打真实服务（服务容器为命名容器，`container:` 引用有效） |
| 就绪判定 | 轮询 `docker inspect -f '{{.State.Health.Status}}'`（镜像 `HEALTHCHECK` 探测 `/healthz`）直至 `healthy`（缺省最多 60s） |
| 契约输入 | 以 `deploy/contracts/<svc>.json` 为 schema（与门禁同源，不依赖服务在线文档端点）；经 `docker cp` 复制进容器 `/tmp/<svc>.json`（**不用 `-v` 挂 job 路径**：宿主不可见，实测报 schema 文件不存在） |
| 覆盖范围 | **只读方法**（GET / HEAD），不写数据、不依赖登录链路（当前认证为占位） |
| 范围排除 | 排除基础设施端点 `/healthz` `/readyz` `/metrics`（探针 / 指标非业务契约，且无依赖时 503 属预期） |
| 深度 | 每操作 1 例（`--max-examples 1`，冒烟），按该服务自身 schema 的各操作各生成 1 例 + **仅 `not_a_server_error` 检查**（无 5xx 即通过）；**非 2xx（4xx / 401 / 403 / 404）视为可达通过** |
| 零 Broker | 无需 Broker / 平台，工具与用例自足 |
| 失败语义 | 该服务命中 5xx / schema 违反即该服务 job 失败并打印输出尾部；**job 设 `allow_failure: true`（非阻断）**——dev 环境依赖缺口会产生 5xx 噪声，阻断门禁为 oasdiff `contract-gate` |
| 边界收敛 | dev 环境依赖缺口导致的 5xx（如未迁移表）按缺陷归口修复；不在冒烟内整体豁免（仅排除探针类基础设施端点） |
...
| 子命令 | 行为 |
| --- | --- |
| `run --service X --image <服务镜像> [--max-examples N] [--root]` | 单服务：`docker run -d -e BMS_ENV=dev <服务镜像>` 起容器 → `docker inspect` 轮询 `healthy` → Schemathesis 容器 `--network container:<服务容器>` 打 → `docker rm -f` 清理；返回码非 0 即该服务失败 |

- **纯函数**：服务容器名、`schemathesis_args`、`service_up` / `service_down`、`docker_schemathesis`（create→cp→start→rm）、`run` 编排——均以可注入的 `run` 子进程执行器表达，单测不真起容器。
...
flowchart LR
    subgraph P["父流水线 verify 阶段"]
        ...
        CG --> CS["contract-smoke-&lt;服务&gt; ×9<br/>已构建镜像打真实服务（仅变更服务）"]
    end
...
| `contract-smoke-<服务>`（9 个） | 同上 | `trigger-<服务>` optional | main + MR 预留；`changes: [backend/services/<服务>/**/*]`（**仅变更服务**）；`exists: [deploy/ci/verify/contract-gate]` | `uv run python -m ops.contract_smoke run --service <服务> --image "$REGISTRY_IMAGE_PREFIX/bms-<服务>:$CI_COMMIT_SHORT_SHA"`；**`allow_failure: true`（非阻断）** |
...
**问题**：oasdiff / Schemathesis 走固定 tag Docker 镜像，需 `docker` CLI；而 `ops` 两个 CLI（含 `docker` 编排逻辑）需 Python / uv。现有 `ci-backend:py314` 无 docker CLI、`docker:27-cli` 无 Python，二者不能同 job。
...
| CI 配置 | `.gitlab-ci.yml` 含 `contract-gate`（verify 阶段、`ci-backend:py314`、规则含 `backend/**` / `deploy/**`、档位守卫、after_script 含破坏数推送）+ 9 个 `contract-smoke-<服务>`（各自 `changes: backend/services/<服务>/**/*`、`needs: trigger-<服务>`、`allow_failure: true`、用 `bms-$SERVICE:$CI_COMMIT_SHORT_SHA`）；档位文件存在 |
| 基础镜像 | `deploy/ci/Dockerfile.backend` 含 docker CLI 安装 |

**新增用例** `backend/libs/bms_core/tests/ops/test_contract_smoke.py`（Kiwi 用例）：容器名 / `schemathesis_args`（schema 路径、`127.0.0.1:8000`、只读方法、排除探针）；`service_up`（起容器 + `docker inspect` 轮询 healthy / 启动失败 / 超时）、`service_down`；`docker_schemathesis` 走 create→cp→start→rm（`--network container:<服务容器>`）；`run`（未就绪失败清理 / 通过 / 失败）——均桩替换 `run`，不真起容器。
...
| 服务镜像缺失 / 起容器失败 / 就绪超时 | 该服务 job 失败并清理容器（非阻断，`allow_failure`）；镜像由子流水线按提交构建，缺镜像多为该服务未变更 / 未构建 |
| 被测服务返回 5xx | Schemathesis `not_a_server_error` 失败即该服务冒烟失败（打印输出尾部） |
| 被测服务返回 4xx（占位认证） | 视为可达通过（冒烟口径） |
| Schemathesis 容器地址解析失败 | 报错非零退出（不静默跳过）；可经 `BMS_SMOKE_BIND_HOST` 显式指定被测服务地址 |
...
| `libs/bms_core/tests/ops/test_contract_smoke.py` | 单元 | 容器名 / `schemathesis_args` / `service_up`·`service_down` / `docker_schemathesis` 序列 / `run` 编排（桩替换 `run`） |
...
| Schemathesis 冒烟可运行 | mjbk 真机冒烟（已构建镜像起容器 + Schemathesis 容器打真实服务通过）+ 命令构造与编排桩用例 |
...
- **开放项（冒烟镜像可用性）**：冒烟依赖子流水线按提交构建的服务镜像（`bms-{service}:$CI_COMMIT_SHORT_SHA`）；共享 / 工作区变更不触发服务子流水线时该服务不跑冒烟（与 09_01 口径一致）。
- **开放项（冒烟路径收敛）**：某服务 dev GET 因缺表 5xx 时按缺陷归口修复，不整体豁免（仅排除探针类基础设施端点）。
...
5. 冒烟 CLI：`backend/ops/contract_smoke.py`（已构建镜像起容器 + Schemathesis 容器打，按服务）。
...
9. **mjbk 真机冒烟**（远程操作，先读部署说明）：预拉 `tufin/oasdiff:v1.32.1` / `schemathesis/schemathesis:4.28.0` → `contract_gate check`（通过）→ 构造破坏性变更验证拦截 + 中文清单 → 还原 → `contract_smoke run --service platform --image <已构建镜像>` 对真实服务通过。
...
uv run python -m ops.contract_smoke run --service platform --image bms-platform:test  # Schemathesis 打真实服务
...
| 5 | 被测真实服务来源 | 已构建服务镜像（`bms-{service}:$CI_COMMIT_SHORT_SHA`）起容器；按服务仅变更服务运行（见决策 16） |
...
| 14 | Schemathesis 调用方式（实施发现） | job 容器 `$HOSTNAME` 非 daemon 容器引用、job 路径宿主不可见 → 不用 `--network container:` 与 `-v`；改 `--network host` + job 容器 IP（`BMS_SMOKE_BIND_HOST` 可覆盖），schema 经 `docker create + docker cp + docker start -a` 复制进容器 `/tmp` |
| 15 | 冒烟范围与阻断性（实施发现） | 排除基础设施端点 `/healthz` `/readyz` `/metrics`（无依赖时 503 属预期）；`contract-smoke` 设 `allow_failure: true`（非阻断冒烟，阻断门禁为 oasdiff `contract-gate`）；dev 环境依赖缺口 5xx 按缺陷归口 |
| 16 | 冒烟重设计（用户拍板，2026-09-24） | 弃源码直跑全服务；改**用已构建服务镜像起容器**（固定 tag ⇒ 可固定重现）+ **按服务、仅本次变更的服务**运行（9 个 job，`rules:changes` 各守本服务路径）；Schemathesis 容器 `--network container:<服务容器>`；`--max-examples 1` |
