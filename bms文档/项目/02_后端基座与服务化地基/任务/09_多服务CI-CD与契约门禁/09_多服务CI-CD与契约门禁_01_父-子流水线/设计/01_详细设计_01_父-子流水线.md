...
- **移除**：`backend-image`（单体镜像，由子流水线按服务构建取代）、`container-scanning`（扫 `bms-backend`，由 release job 按服务扫描取代）；`frontend-image` 与 `.build-push-base` 锚点保留（前端镜像档不变）。
...
service-build:
  ...
  script:
    ...
    - docker build --build-arg SERVICE="$SERVICE" -t "$IMAGE" -f backend/Dockerfile backend/
...
service-release:
  ...
  script:
    ...
    - docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image --exit-code 1 --severity CRITICAL,HIGH --ignore-unfixed "$IMAGE"
  after_script:
...
# 构建：docker build --build-arg SERVICE=platform -f backend/Dockerfile backend/
...
- **跨项目（产品仓库 / 其他工作区仓库）**：`include` 走项目引用并**固定完整 SHA**（禁止分支名 / 短 SHA / tag 漂移）：
  ```yaml
  include:
    - project: bms/bms
  ...
  ```
  理由：模板是构建 / 发布口径的公共事实源，分支引用会让下游在无感知下换口径；固定 SHA 使升级成为显式提交（Renovate 可自动提升级 MR）。
...
| mjbk 真实构建 | 集成（人工留痕） | `docker build --build-arg SERVICE=platform` 真实构建通过、容器可启动 `/healthz`（远程 docker，实测记录） |
...
- **开放项（Trivy 镜像与 DB 缓存）**：trivy 镜像 tag 现为 `latest`（沿用既有 job 口径），随运维阶段固定 tag + 镜像预拉；DB 每次下载（无缓存），如需加速随 09_02 评估。
...
7. **mjbk 真实构建冒烟**（远程操作，先读部署说明）：同步 `backend/` → `docker build --build-arg SERVICE=platform -f backend/Dockerfile backend/` → `docker run` 起容器验证 `/healthz`（环境变量注入 SQLite 或跳过 DB 依赖）→ 清理留痕；预拉 `aquasec/trivy:latest` 并验证扫描口径。
...
docker build --build-arg SERVICE=platform -f backend/Dockerfile backend/ -t bms-platform:smoke
