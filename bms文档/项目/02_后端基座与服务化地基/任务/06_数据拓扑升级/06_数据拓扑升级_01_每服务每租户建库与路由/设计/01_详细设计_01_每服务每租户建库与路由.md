...
- **Kiwi TCMS**：一任务一条用例（**Kiwi 2177**，2026-09-23 先登记后编码）；代码以 `@pytest.mark.kiwi_id(2177)` 标注（见 `tests/db/test_keys.py`、`tests/db/test_tenant_routing.py`、`tests/alembic/test_auto_create_tables.py`、`tests/ops/test_provision_tenant.py`、`tests/health/test_catalog_check.py`）。
...
- **开放项**：
  ...
  5. 开发环境「各服务库含该链全量表」的收敛时点——随 06_03 分链落地。**已闭环（06_02，2026-09-23）**：分链机制并入 06_02 实施，`db/bootstrap.py` 改按**本服务分链表集**（`chain_tables(service, datasource)` ∩ 已有模型，与服务迁移同一派生点）建表，且只处理「有迁移脚本的链」，只补不删；开发库与真库表集口径一致。
