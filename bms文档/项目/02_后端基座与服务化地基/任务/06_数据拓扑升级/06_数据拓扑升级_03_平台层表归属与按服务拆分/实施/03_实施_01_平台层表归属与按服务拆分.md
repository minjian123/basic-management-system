...
- **偏差（已登记）**：
  ...
  4. 租户注册版本号递增按缓存基座既有能力实现：内存域经 `MemoryCacheRegion.bump_version()`（进程内计数）、Redis 域写回域内版本键（`get_global_version` 读该键）；`CacheRegion` 契约无原子自增，**Redis 原生 `INCR` 待缓存基座扩展**（字典域已有专用扩展 `DictCacheRegion`，租户域按其模式扩展随缓存回补）。
- **遗留（本轮未实施，按详细设计 §9 续做）**：
