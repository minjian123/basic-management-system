...
> **范围承接与编号变更（06_01 回写，2026-09-23）**：
>
> 1. 本设计**遗留 2**（服务目录快照只读契约 + 启动接库校验改经契约 + `CatalogHealthCheck` 非必需项降级）**由 06_01 承接交付**（决策：`platform` 即目录权威时本地直读本服务平台库、其余服务经 `service_client` 契约），本任务收口时只留遗留 3（过渡豁免移除）与遗留 4、5（分链 + `SysModule` 迁出）；
> 2. 本设计第 4.1 节规划的 `sys_table_ownership` 迁移编号由 **`0005` 顺延为 `0006`**（06_01 已用平台链 `0005_sys_tenant_db_key_nullable` 承载 `sys_tenant.db_key` 废弃改可空）。
