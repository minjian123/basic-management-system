...
| 结论 | 进行中：分链机制 / 模型迁出 / 归属登记落库与对账 / 批量编排与开通即迁移 / 按服务连接预算 / 库数量可观测 / 字典跨服务读出口 已交付；余量见 §5（三库真库演练、Kiwi 用例编号） |
...
- **三库真库建库 → 迁移 → 校验 → 清理演练未执行**：本机 MySQL（3306）/ PostgreSQL（5432）/ 达梦（5236）端口均不可达、亦无测试连接串环境变量（`BMS_TEST_*`），已改用 **SQLite 多服务多库全量演练**（见测试记录 §3）验证同一编排路径；真库演练待环境可用后补做（登记为遗留）。
...
| `ops/check_tables.py` | 通过（清单 39 项 + 模型表对账 + 脚本表集；接库对账通过） |
