...
- **`render_global_rules()`**：新增全局规则 `edge-sanitize`——`proxy-rewrite.headers.remove` 统一剥除客户端伪造身份头（`STRIPPED_HEADERS`，含网关标记头）。
- **`render_routes()`**：路由级 `proxy-rewrite.headers.set` 置**网关专属标记** `X-Gateway-Identity: bms-edge`，并合并注入钩子 `ROUTE_HEADERS_SET: dict[str, dict[str, str]] = {}`（按 `service_key` 追加身份头，默认空）：07_03 填入 `{"X-User-Id": "$jwt_claim_sub", ...}` 即完成「注入网关验证过的身份」，不改生成结构。
- **`render_apisix_config()`** 输出 `{upstreams, routes, global_rules}`；确定性、零漂移校验不变（`ops/gateway_config.py render|check`）。

> **07_03 回写（2026-09-23）**：APISIX **无 `$jwt_claim_*` 变量**、`openid-connect` 仅产 `X-Userinfo` / `X-ID-Token`，本节预留的「`ROUTE_HEADERS_SET` 填 `$jwt_claim_sub`」**写法作废**；07_03 改为每条服务路由与登录路由挂 `forward-auth` 转调认证服务内部端点（用户 JWT 全校验后认证服务返回契约身份头 + 换发网关服务 JWT，网关 `upstream_headers` 注入并覆盖上游 `Authorization`）。`ROUTE_HEADERS_SET` 保留为空声明式钩子；`proxy-rewrite` 仍只置网关标记。身份头契约新增 `X-User-Subject`（见 07_03 设计与《后端基类清单》）。
...
- **真机实测结论（mjbk，2026-09-23，已回写）**：`global_rules` 的 `proxy-rewrite.headers.remove` 生效；但 `global_rules` 的 `headers.set` 在路由级 `proxy-rewrite` 执行后**被丢弃**（同一插件全局 / 路由两处配置不叠加标记置入）。故**标记置入与身份注入统一落路由级** `proxy-rewrite.headers.set`，剥头保留在 global 规则（集中一次生效）——即本节实现与原设计的偏差（标记从 global 移到 route）；实测证实「global 剥头 → route 置标记 / 注入」的合流顺序正确（伪造头剥除、标记与身份头可达上游）。
...
| 网关 `global_rules` 的 `headers.set` 被路由级 `proxy-rewrite` 丢弃（mjbk 实测） | 标记置入与身份注入统一落路由级 `proxy-rewrite.headers.set`，剥头保留 global 规则（已按此实现，见 §4.4） |
