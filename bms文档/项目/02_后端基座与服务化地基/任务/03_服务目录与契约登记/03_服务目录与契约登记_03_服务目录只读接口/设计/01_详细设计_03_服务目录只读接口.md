...
- **范围（本任务）**：
  ...
  6. **基座缺口修复（实施中发现，已确认）**：`page_query` / `sort_query` / `cursor_query` 依赖工厂内契约校验（如页码限深、游标 `limit`）抛裸 `ValidationError` → 500；统一捕获转 `ParamError`（10001），与请求级校验失败同一口径，登记《后端基类清单》接口层基座行。
...
│   ├── api/base.py                              # 改：三个依赖工厂契约校验失败统一转 ParamError（10001）
...
│     # 2) 排序经 _resolve_sort 白名单（sortable_fields；NULL 恒末位 + id 兜底），LIMIT/OFFSET
│     # 3) 同条件 COUNT（作用域 + 软删过滤 + 筛选），total 为筛选后总数
└── get_by_service_key(service_key) -> SysModule | None                               # 新：服务维度精确匹配（未软删）
```

- **排序白名单口径（实施细化）**：基座要求「可排序字段须有索引或为主键」（`assert_sortable_fields_indexed` 口径：主键 + 表级索引列 + 单列唯一约束；复合唯一不视为单列可排序索引）。`sys_module` 仅有复合唯一约束（无单列索引），故 `sortable_fields = frozenset({"id"})`——当前仅开放按主键排序，其余排序请求项按白名单口径忽略、回落 `id` 升序；将来按查询需要加单列索引再扩白名单。
...
| `order_by` / `order` | 同上（`BaseSortQuery`） | 多值逗号分隔 + 方向数组；白名单逐项校验、非法忽略（当前白名单仅 `id`），缺省 `id` 升序 |
...
| Kiwi | 用例 | 类型 | 断言要点 |
| --- | --- | --- | --- |
| TBD | 服务目录只读接口 | 接口（SQLite 真库） | 清单分页结构（total=16 / size 分页 / page 越界）；`status` / `group` 筛选与组合；排序白名单（`id` 降序命中、非法字段忽略回落 `id` 升序）；字段集严格等于 `ModuleResponse`；明细 `service_key` 命中 / `module_key` 回退 / 未登记 404 + 10002 + 统一响应体；POST/PUT/DELETE 405 |
...
| TBD | 依赖工厂参数口径 | 单元 | `page_query` 页码限深 / `cursor_query` limit 越界 → `ParamError`（10001）；HTTP 层 `page=101` 返回 200 + 10001 |
...
| 《后端基类清单》 | 数据访问横切行与会话依赖行补 `get_platform_read_db`（强制平台库 + 只读）；接口层基座行补「依赖工厂契约校验失败统一转参数错误（10001）」；模块注册制行补只读接口消费与仓储分页 / 按服务键查询 |
...
| 12 | 依赖工厂校验口径（实施中发现，追加确认） | `page_query` / `sort_query` / `cursor_query` 内契约校验失败统一转 `ParamError`（10001），与请求级校验同口径；本任务顺带修复并登记《后端基类清单》 |
| 13 | 排序白名单口径（实施细化） | 基座要求可排序字段须有索引（主键 / 表级索引 / 单列唯一）；`sys_module` 仅复合唯一，故 `sortable_fields={"id"}`，其余排序请求忽略回落 `id` 升序 |
