...
flowchart LR
    ...
    A --> C["module-build：pnpm run guard:isolation<br/>产物面扫描（CSS + 自有 JS 块）"]
...
7. **CI 接线**（`.gitlab-ci.yml` + 模块 `package.json`）：`module-build` 构建与预算后追加 `pnpm run guard:isolation`（`node ../../scripts/check-module-isolation.mjs --product`）；源码面随既有 `desktop-check`（不新增 job）。
...
| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 核心三包门禁 | 根 `pnpm run check` | 通过（core 93 文件 / 1158 用例、vue 2 / 5、ui-ep 61 / 901） |
...
| **产物面隔离扫描** | `cd frontend/modules/demo && pnpm run guard:isolation` | 通过：CSS 2 个 / 模块自有 JS 块 3 个（暴露块 + 页面块），零违规 |
...
- **遗留**：
  ...
  3. **接入指南**：业务页面按模块形态接入的自检清单（含隔离检查项）随 `04_01` 沉淀；本任务已在模块契约 §7 给检查项。（**已闭环 2026-09-22**：接入指南已沉淀为《微前端 · 10 业务模块接入指南》，§9 自检清单含隔离项；）
