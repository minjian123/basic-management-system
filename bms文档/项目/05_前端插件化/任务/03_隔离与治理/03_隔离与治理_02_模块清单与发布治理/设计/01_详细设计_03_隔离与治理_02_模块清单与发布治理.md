...
| CI 基础镜像 | `deploy/ci/Dockerfile.frontend` 的模块预装由硬编码 `demo` 改为遍历 `frontend/modules/*`（每模块 `pnpm install --frozen-lockfile`） |
...
```bash
cd frontend/modules/demo && pnpm run build                 # 1. 模块独立构建（产出 dist/ 与 module.meta.json）
...
cd frontend/apps/desktop && pnpm run dev                   # 4. 起宿主（终端 B）
```

- 模块迭代：改代码 → `pnpm run build` → `publish --module demo --force`（同版本本地重发，记录动作 `republish`）；版本升级则先改 `package.json` 版本再发布（**版本目录不可变，回滚依赖旧产物**）；
...
| 护栏全绿 | `cd frontend/apps/desktop && pnpm exec vue-tsc -b && pnpm run lint && pnpm run test:cov && pnpm run build && pnpm run budget`；`cd frontend/modules/demo && pnpm run lint && pnpm run typecheck && pnpm run test && pnpm run build && pnpm run budget && pnpm run guard:isolation`；根 `pnpm run check`；三条平台脚本实跑 |
