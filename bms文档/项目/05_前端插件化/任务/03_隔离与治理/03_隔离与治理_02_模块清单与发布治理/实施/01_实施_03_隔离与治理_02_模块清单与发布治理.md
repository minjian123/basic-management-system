...
| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 核心三包门禁 | 根 `pnpm run check` | 通过（core 95 文件 / 1172 用例、vue 2 / 5、ui-ep 61 / 901） |
...
- **遗留**：
  ...
  3. **sourcemap 与按模块可观测**：归 `03_03`——**已交付**（2026-09-22）：加载超时与降级单模块重试 / 错误上报带模块名与版本并按模块聚合 / 加载渲染耗时与产物体积归因（发布记录 `size` 字段）/ Web Vitals 按活动模块 / 模块产物 sourcemap 随发布按版本归档并发布前强校验（四关）；
  4. **回滚后源码处理**：为流程要求（回退源码版本或准备修复版本发布），护栏提示不一致；不自动回退源码；
  5. **`04_01` 接收项**：样例模块按本任务发布 / 回滚流程接入并沉淀接入指南；契约用例经同一工厂；接入指南引用《部署发布规范》「模块发布与回滚」节与模块契约 §7 检查项。（**已闭环 2026-09-22**。）
