...
| 6 | 仓库级无 prettier 配置，改动文件格式口径不一 | 既有约定：用宿主 `.prettierrc.json` 手工跑 | 仅对本次改动文件执行 `pnpm exec prettier --config frontend/apps/desktop/.prettierrc.json --write`（未全库格式化，避免无关 diff） |

## 5. 验证结果 <a id="verify"></a>

| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 类型 / Lint / 单测（核心三包） | 根 `pnpm run check` | 通过（core 93 文件 / 1155 用例；vue 2 / 5；ui-ep 61 / 901） |
...
- **遗留**：
  ...
  2. **清单治理**：清单唯一来源、版本发现、灰度 / 回滚归 `03_02`（**已完成**：清单唯一来源（含可见性 `enabled`）/ 版本发现（清单 = 产物元数据 = 源码）/ 发布与回滚（清单指向切换）/ 契约版本双端校验就位；生产部署侧清单归部署阶段）；
