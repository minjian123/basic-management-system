...
| 实施日期 | 2026-09-21 |
| 实施人 | minjian |
| 实施环境 | 本地开发机（Node v24.20.0 / pnpm 11.x，npmmirror）；宿主 Vite 8.2.2（Rolldown）；`@module-federation/vite` 1.22.1 |
...
| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 核心三包门禁 | 根 `pnpm run check` | 通过（core 93 文件、vue 2、ui-ep 61 全绿） |
...
| 独立预览目标 | `pnpm run build:standalone` | 通过；产物 `dist-standalone/` 含 HTML 壳 |
| **构建期产物级断言** | `pnpm run budget:shared` | 通过：两侧声明一致、宿主为提供方、模块 `import:false`、**依赖实例数 = 1** |
| **白名单与体积护栏** | `pnpm run guard:shared` | 通过：白名单 6 项、依赖与实装版本合规、模块产物在阈值内（实测 454.8 KB / 阈值 546 KB） |
...
- **遗留**：
  1. **模块工程类型声明生成报错（`#TYPE-001`）**：既有问题（旧配置同样复现），不影响构建产物与门禁；宜随模块工程类型声明治理或 `04_01` 统一处置。（**已闭环 2026-09-22，`04_01`**：根因为 MF DTS 以 plain `tsc` 编译暴露入口、`src/env.d.ts` 未入其编译图——缺 `__BMS_MODULE_VERSION__` 环境声明与 `.vue` shim；经入口 `/// <reference path="./env.d.ts" />` + `env.d.ts` 补 `declare module '*.vue'` 后日志转为 `Federated types created correctly`、`dist/@mf-types/**` 正常产出；demo 与 sample 双模块验证。）
  ...
  6. **临时进程待清理**：端到端验证启动的两个 `vite preview`（5002 / 4173）在收尾时因审批未显示未能停止，需用户或后续会话清理。（**已闭环 2026-09-22**：核对 5002 / 4173 端口无监听、无残留 `vite preview` 进程。）
