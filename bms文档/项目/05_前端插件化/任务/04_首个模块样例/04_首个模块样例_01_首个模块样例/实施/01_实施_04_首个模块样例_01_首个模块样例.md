...
| 实施日期 | 2026-09-22 |
| 实施人 | minjian |
| 实施环境 | 本地开发机（Node v24.20.0 / pnpm 11.x，npmmirror）；Vite 8.3.0（Rolldown）；`@module-federation/vite` 1.22.1 |
...
| 8 | 模块构建报 `[Module Federation DTS] #TYPE-001`（类型声明生成失败，`02_02` 遗留项，宜随 `04_01` 处置） | MF DTS 以 plain `tsc` 编译暴露入口，`src/env.d.ts` 未入其编译图——缺 `__BMS_MODULE_VERSION__` 环境声明与 `.vue` shim | 入口 `src/index.ts` 增 `/// <reference path="./env.d.ts" />`（同文件 `eslint-disable` 说明），`env.d.ts` 补 `declare module '*.vue'`；构建日志转为 `Federated types created correctly`、`dist/@mf-types/**` 正常产出；**demo 与 sample 双模块**同步修复并重发归档 |
| 9 | CI `module-build` 失败：`[module-isolation] 不通过：模块产物目录不存在：frontend/modules/sample` | 流水线 #310（`b3ec03a1`）。`module-build` 逐模块循环构建，而模块 `guard:isolation` 默认**全量**扫描 `frontend/modules/*/dist`——处理第一个模块（demo）时 sample 尚未构建，触发 `runProductScan` 的「缺产物目录即抛错」 | 模块 `package.json` 的 `guard:isolation` 改传 **`--module .`**（只扫自身产物，符合 CLI `--module` 口径；全量扫描仍由发布前强校验 `scanModuleProducts` 与本地 CLI 承载）；本地复现「sample dist 缺失时 demo guard 通过」验证修复 |
...
| MF 类型声明（`#TYPE-001` 闭环） | `pnpm run build`（demo / sample） | 通过：日志 `Federated types created correctly`；`dist/@mf-types/**` 产出；重发归档（`--force`，动作 `republish`） |
| CI 流水线 | 推送后流水线核对 | 流水线 #310（首推）`module-build` 因逐模块 `guard:isolation` 全量扫描失败（见 §4 问题 9）；修复后本地复跑全部门禁通过，待重推验证 |
...
- **偏差**：
  ...
  4. **模块 `guard:isolation` 改 `--module .`**：CI 逐模块循环下全量扫描会误报未构建模块，改为只扫自身（见 §4 问题 9）。
- **遗留**：
...
| `02_01` / `02_02` | 模块工程类型声明生成 `#TYPE-001`（宜随 `04_01` 统一处置） | **已闭环**：入口引用 `env.d.ts` + `.vue` shim，日志转 `Federated types created correctly`（demo / sample 双模块） |
| `02_02` | 临时 `vite preview` 进程（5002 / 4173）待清理 | **已闭环**：核对端口无监听、无残留进程 |
