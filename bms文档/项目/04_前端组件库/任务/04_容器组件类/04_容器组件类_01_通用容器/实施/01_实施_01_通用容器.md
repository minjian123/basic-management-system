...
| 单测（核心） | `core` `npm test` | 26 文件 / 143 用例全通过（新增 6：可视区纯函数） |
| 单测（ui-ep） | `npm run ui-ep:test` | 16 文件 / 102 用例全通过（新增 15：基础 12 / 能力 8 / 四态 7 − 重叠计 15） |
...
- **遗留**：① 触底加载的数据接入（分页 / 游标）由宿主页面负责，本件只发信号；② 表格 / 树虚拟化复用 `computeVirtualRange` 随展示域 / 树域任务；③ 滚动锚定（锚点补偿）随性能优化任务；④ 滚动位置远端同步随阶段七（偏好）。
