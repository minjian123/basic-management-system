...
| 实施日期 | 2026-09-16 |
| 实施人 | minjian |
| 实施环境 | Linux 开发机（mjpc）；`frontend/`（Vue 3.5 + Vue Router + Pinia + Vitest 5 + Element Plus 2.14.5） |
| 工时 | 10h（重估口径） |
...
2. 决策确认（用户）：三件壳 + 宿主编排全量落地；菜单 → 路由注册统一经动态路由片段；未注册组件回退占位视图；keep-alive 精确缓存在实现层校验（`:include` 未命中则退 `:max`）。
...
| # | 问题 / 现象 | 原因 | 处置与落点 |
| --- | --- | --- | --- |
| 1 | keep-alive `:include`（组件名）未命中，切回标签页面重挂载 | router-view + 插槽场景下组件名匹配未按预期（根因未定位） | 改 `:max`（= maxOpen 12）LRU + 内容 key（标签键 + refresh 版本）；DOM 复用与 refresh 重挂载经用例验证；设计已回写（对齐记录 6）；关闭标签即时释放登记 §3.1-40 |
...
见[测试记录](../测试/01_测试_05_框架与容器.md) §3；全量 47 文件 / **301 用例全绿**（本任务新增 11 条）；覆盖率语句 87.49% / 分支 79.33%；构建与体积门禁通过（**164.2 / 200 KB**、49.0 / 100 KB）。

## 6. 偏差与遗留 <a id="deviations"></a>

| # | 项 | 归口 / 去向 |
| --- | --- | --- |
| 1 | keep-alive 落地口径（`:max` LRU + 内容 key）；菜单引导动态导入；刷新时序 | **已回写**设计（§4.4 / 对齐记录 6~8） |
| 2 | keep-alive 精确缓存（`:include` 按签）与关闭标签即时释放 | 机会项（计划 §3.1-40，页面任务后评估） |
