...
8. 验证：根 `pnpm run check`、`apps/desktop` lint / 单测 / 构建 / 体积预算。
...
| 单测（ui-ep） | `pnpm run ui-ep:test` | 4 文件 / 20 用例全通过 |
| 宿主单测 | `apps/desktop` `pnpm run test:cov` | 6 文件 / 19 用例；覆盖率 97.95% |
| 宿主构建 | `pnpm run build` | 通过 |
| 体积预算 | `pnpm run budget` | 合计 61.5 KB（预算 260 KB） |

## 6. 偏差与遗留 <a id="deviations"></a>

- **偏差**：无（按详细设计实施）。
- **遗留**：① 四态容器 `LoadingContainer` 随 `04_01`；② 骨架屏按真实页面结构的细化随 `04_01` / 域 05；③ 全局运行时错误处理器随宿主装配阶段。
...
+ **本任务整改点**：`EmptyState` / `ErrorPage` 经 `useBaseDisplay`（`BaseDisplay`，主文案值）；`SkeletonBlock` / `LoadingMask` 经 `useFeedback`（`BaseFeedback`，加载态状态机）。
