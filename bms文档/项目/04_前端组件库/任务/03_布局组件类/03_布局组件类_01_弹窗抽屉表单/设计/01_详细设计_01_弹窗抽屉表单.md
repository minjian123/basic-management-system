...
- **前置契约（已交付）**：域 02 基类体系（2026-09-16）——组件根 `useComponentBase`、容器片段 `useContainer`、`useRequest` / `stableStringify` / `hasPerm` / `v-perm` / `useAccess`；基础令牌（`03_06`，2026-09-17）——`--bms-*` 与 EP 映射（颜色 / 圆角 / 字体自动统一）。
...
| 2 | 片段组合 | 组件根 `useComponentBase` + 容器片段 `useContainer`；不组合 `useFormContainer`（字段元数据未就绪，避免空转） | 用户确认（2026-09-17） |
