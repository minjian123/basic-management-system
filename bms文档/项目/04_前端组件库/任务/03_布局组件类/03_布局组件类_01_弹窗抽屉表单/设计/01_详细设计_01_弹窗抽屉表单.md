...
- **目标**：落地 PC 管理端弹窗 / 抽屉表单容器与确认能力（仅 `apps/desktop/`）：表单抽屉 `FormDrawer`、表单对话框 `FormDialog`、确认对话框 `ConfirmDialog`、弹窗表单组合式 `useFormModal`、确认组合式 `useConfirm`——三态（新增 / 编辑 / 详情）、尺寸阶梯、生命周期、脏数据拦截与提交前确认统一口径。
...
```text
packages/ui-ep/src/components/modal/
...
| # | 事项 | 结论 | 来源 |
| --- | --- | --- | --- |
| 1 | 端范围 | 仅 `apps/desktop/`（移动端组件需求范围外） | 需求总览「范围外」 |
