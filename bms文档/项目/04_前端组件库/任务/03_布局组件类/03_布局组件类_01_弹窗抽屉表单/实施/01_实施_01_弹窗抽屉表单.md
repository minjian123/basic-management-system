...
6. 验证：根 `pnpm run check`（core + vue + ui-ep）全绿。
...
| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 类型检查 / Lint | 根 `pnpm run check`（含 ui-ep typecheck + lint） | 通过 |
| 单测（ui-ep） | `pnpm run ui-ep:test` | 3 文件 / 11 用例全通过 |
...
+ **本任务整改点**：`ConfirmDialog` 改经 `useModalShell`（`BaseModalShell`）；`useFormModal` 改经 `useBaseFormPage`（`BaseFormPage`，三态 / 脏数据 / 提交）+ `useModalShell`（显隐）。
