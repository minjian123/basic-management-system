...
- **整改**：组件统一接入 `ui-ep/src/composables/` 基类投影；新增护栏 `ui-ep/tests/guard-component-inheritance.spec.ts`；《前端基类清单》§9 回写投影落点口径；提交 `df3f0e44`（代码）与本次文档回写。
+ **本任务整改点**：布局族各件（栅格 / 分割线 / 分栏 / 卡片 / 折叠 / 页签导航 / 双层页签 / 右键菜单 / 侧边菜单）统一经 `useBaseLayout`（`BaseLayout`，含显隐）；`TreeMasterDetail` 另经 `useBaseTreeData`（`BaseTreeData`，节点 / 数据状态 / 过滤）。
