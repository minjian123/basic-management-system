...
| 单测（ui-ep） | `pnpm run ui-ep:test` | 7 文件 / 49 用例全通过 |
| 宿主构建 / 体积 | `apps/desktop` `build` + `budget` | 通过；合计 61.5 KB（预算 260 KB） |

## 6. 偏差与遗留 <a id="deviations"></a>

- **偏差**：无（按三份详细设计实施）。
- **遗留**：① 页签远端偏好同步（`sys_user_preference`）随偏好持久化能力就绪；② 页签拖拽排序与下拉折叠交互优化随后续任务；③ 树节点懒加载 / 右键菜单随域 05 树类；④ 宿主路由接入 `useTabNav`（页签栏装配）随 `03_05 框架与容器`。
