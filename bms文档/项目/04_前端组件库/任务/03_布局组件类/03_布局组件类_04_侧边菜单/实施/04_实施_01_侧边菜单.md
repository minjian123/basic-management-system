...
| 单测（核心） | `core` `npm test` | 23 文件 / 126 用例全通过（新增 10） |
| 单测（ui-ep） | `npm run ui-ep:test` | 8 文件 / 55 用例全通过（新增 6） |
...
+ **本任务整改点**：`SideMenu` 经 `useBaseLayout`（`BaseLayout`）；`useSideMenu` 权限判定改经 `useBaseAccess`（`BaseAccess`，支持 `codes` / `setCodes`，缺省放行语义保留）。
