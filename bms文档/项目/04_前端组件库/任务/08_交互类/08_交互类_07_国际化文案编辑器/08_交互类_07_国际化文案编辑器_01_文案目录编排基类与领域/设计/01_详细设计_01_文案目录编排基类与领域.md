...
- **前端轻校验**：`msg_key` 命名（`模块.页面.字段`，≥2 段且段非空，44003）与单值长度上限由前端快速拦截，**后端强制**；模板列核对与行级校验归后端。
...
| 文案值超长（> 2000） | `editCell` 拒绝写值并写文案；长值展示省略 + 悬浮提示（件层） |
...
| `tsc`、ESLint、Vitest 全通过 | 根 `npm run check` |
...
| 13 | 常量边界 | 值长 2000 / 网格页长 50 / 筛选页长 200 / 虚拟滚动阈值 200；错误行展示上限复用 `08_05` 既有常量 1000 |
