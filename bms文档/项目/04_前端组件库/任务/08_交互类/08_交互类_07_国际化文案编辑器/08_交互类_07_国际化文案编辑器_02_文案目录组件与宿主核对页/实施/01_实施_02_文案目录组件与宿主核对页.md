...
| 命令 | 结果 |
| --- | --- |
| 根 `pnpm run check`（core + vue + ui-ep） | 50 + 2 + 42 文件 / 535 + 5 + 471 用例全绿（ui-ep 本次新增 32） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（全库语句覆盖 86.79%、行覆盖 87.61%、35 用例） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（合计 122.0 KB / 预算 260 KB；最大单文件 103.1 KB / 预算 150 KB；新代码未引入分包退化告警） |
