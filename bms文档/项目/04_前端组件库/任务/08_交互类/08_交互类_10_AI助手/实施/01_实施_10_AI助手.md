...
| 命令 | 结果 |
| --- | --- |
| 根 `pnpm run check`（core + vue + ui-ep） | 74 + 2 + 53 文件 / 914 + 5 + 730 用例全绿（core 新增 2 文件 / 36 用例，ui-ep 新增 1 文件 / 31 用例） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（10 文件 / 35 用例） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（首屏合计 119.8 KB / 预算 260 KB；首屏最大单文件 104.2 KB / 预算 150 KB；`marked` 落 `vendor-marked` 异步块，不进首屏） |
