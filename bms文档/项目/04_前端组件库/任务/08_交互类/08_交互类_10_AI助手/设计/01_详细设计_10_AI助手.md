...
/** 渲染 Markdown 为安全 HTML（marked + DOMPurify 白名单清洗；代码块交给调用方只读高亮）。 */
...
| `data-test` | `session-list` / `session-search` / `session-filter` / `session-new` / `session-item-{id}`（`data-active`）/ `session-remove-{id}` / `session-empty` |
...
| `data-test` | `ai-composer` / `composer-input` / `composer-send` / `composer-stop` / `composer-attach` / `composer-dataset` / `composer-count` / `composer-shortcuts` / `composer-error` / `composer-retry` |
