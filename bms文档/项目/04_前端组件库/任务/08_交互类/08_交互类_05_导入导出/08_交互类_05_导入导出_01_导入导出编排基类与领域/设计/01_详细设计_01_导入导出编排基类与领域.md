...
| 9 | 共享散列与文件名时间戳工具（FNV-1a 提取，`permission-config` 改引用、输出不变；`formatCompactTimestamp` 与既有格式化纯函数同族） | `core/src/domain/serialize.ts`、`core/src/domain/format.ts` |
...
| 文件名 | `templateFileName` = `{bizName}-导入模板.xlsx`；`errorReportFileName` = `{bizName}-导入错误明细-{yyyyMMddHHmmss}.xlsx`（时间戳复用 `domain/format` 的 `formatCompactTimestamp`，两域同名引用、无跨域依赖） |
...
| 时间戳 | `formatCompactTimestamp` 本地时区 `yyyyMMddHHmmss`（文件名用），落 `domain/format.ts` 与既有格式化纯函数同族、不依赖宿主格式化层 |
