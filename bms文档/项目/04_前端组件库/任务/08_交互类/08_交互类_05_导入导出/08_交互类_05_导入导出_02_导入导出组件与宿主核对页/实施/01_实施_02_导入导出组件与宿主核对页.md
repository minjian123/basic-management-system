...
| 命令 | 结果 |
| --- | --- |
| `pnpm run check`（core + vue + ui-ep） | 44 + 2 + 39 文件 / 362 + 5 + 402 用例全绿（ui-ep 本次新增 22） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（全库语句覆盖 86.79%、行覆盖 87.61%、35 用例） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（合计 121.1 KB / 预算 260 KB；最大单文件 102.3 KB / 预算 150 KB；无分包退化告警） |
