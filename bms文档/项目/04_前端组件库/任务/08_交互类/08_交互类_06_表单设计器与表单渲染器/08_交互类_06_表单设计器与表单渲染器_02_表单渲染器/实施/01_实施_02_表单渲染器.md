...
| 命令 | 结果 |
| --- | --- |
| `pnpm run check`（core + vue + ui-ep） | 48 + 2 + 41 文件 / 481 + 5 + 439 用例全绿（core 新增 56、ui-ep 新增 20） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（全库语句覆盖 86.79%、行覆盖 87.61%、35 用例） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（合计 121.9 KB / 预算 260 KB；最大单文件 103.0 KB / 预算 150 KB；**字段控件无分包退化告警**，剩余告警为 `element-plus` 既有项） |
