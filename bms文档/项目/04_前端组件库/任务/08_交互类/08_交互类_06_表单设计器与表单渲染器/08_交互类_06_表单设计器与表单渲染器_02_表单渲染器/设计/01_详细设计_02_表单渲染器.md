...
| 渲染计划 | `buildRenderPlan`：遍历 `effective.layout.main.sections`，逐分区产出字段项（`visible=false` **保留在计划中并打标**，由件层据 `visible` 决定是否渲染，便于统计与断言）；分区 `groups` **非空时**按分组产出 `groups`（组内字段），同时**迁移**到 `fields` 便于统一遍历；`groups` 为空时 `groups: []`、`fields` 平铺（既有行为不变） |
...
| 停用字段 | `disabled` 或 `status !== 'active'` → 记入 `disabledFields` 并保留渲染（只读回显，避免历史数据不可见） |
...
```ts
/** 控件语义键 → 组件（含懒加载器；`undefined` 表示未映射，回退纯文本）。 */
export const FIELD_WIDGET_COMPONENTS: Readonly<Record<FormWidget, unknown>>
getFieldWidget(widget: FormWidget): unknown
resolveFieldComponent(input: { widget: FormWidget; fieldType: string; registry?: FieldRendererRegistry | undefined }): unknown
resetLazyWidgetCache(): void
```

| 行为 | 口径 |
| --- | --- |
| 内建映射 | 语义键 → 组件：`text→TextInput`、`textarea→TextareaInput`、`number→NumberInput`、`datetime→DateTimeField`、`select→SelectInput`、`multi-select→SelectInput`（`multiple`）、`radio→RadioInput`、`checkbox→CheckboxInput`、`switch→SwitchInput`、`file→FileUploadField`、`image→FileUploadField`（图片限定）、`tree-select→TreeSelectField`、`org-select→OrgSelectField`、`transfer→TransferField`、`cascader→CascadeField`、`tags→TagInputField`、`captcha→CaptchaField`、`plain→null`（纯文本回退，由件层渲染 `<span>`） |
| 重控件懒加载 | `richtext→RichTextField` 经 `defineAsyncComponent(() => import('../components/field/RichTextField.vue'))`；`file` / `image` / `org-select` / `tree-select` / `transfer` / `cascader` 同理（**第三方内核与重件不进首屏**） |
...
| 可测 | 纯函数（除懒加载器构造外无副作用）；jsdom 用例断言语义键 → 组件解析与未映射回退，`vi.dynamicImportSettled()` 等待懒加载 |
