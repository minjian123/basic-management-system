...
| `FormField` 扩展（`08-6-2` 消费方按需新增，向后兼容） | 新增可选 `FieldRenderAttrs`（`required` / `placeholder` / `defaultValue` / `rules` / `options` / `readonly` / `hidden`）与声明式 `FieldRule`（`required` / `length` / `range` / `pattern` / `options`）；**既有 6 个字段名称 / 类型 / 语义一字不改**，省略即与扩展前逐字等价 |
| `LayoutEffective` 扩展（同上） | 新增可选 `permissions`（`Record<字段键, FieldPermission>`，字段权限标记随 `layout-effective` 一次下发）；`resolveEffectiveLayout` 与 `toRenderMetadata` **透传**（缺省不生成空对象，不破坏「设计产出 → 渲染输入同形」断言） |
| 扩展影响面 | `normalizeLayout` / `layoutEqual` / `isLayoutDirty` / `validateLayout` **不改**（`permissions` 挂 `LayoutEffective` 而非 `FormLayout`，不入 `stableStringify` 比对）；`08-6-1` 既有冻结用例**不改动即通过** |
...
| 18 | 前置契约扩展（**`08-6-2` 实施期回写**） | 消费方 `08-6-2` 表单渲染器按需**向后兼容新增可选字段**：`FormField` 加渲染属性（`required` / `placeholder` / `defaultValue` / `rules` / `options` / `readonly` / `hidden`）与 `FieldRule`，`LayoutEffective` 加 `permissions`（权限标记随元数据一次下发）；既有字段与函数语义不变，本任务冻结用例未改动即通过（不改契约形状、不重定义元数据） |
