...
| 命令 | 结果 |
| --- | --- |
| `pnpm run check`（core + vue + ui-ep） | 62 + 2 + 47 文件 / 739 + 5 + 604 用例全绿（core 新增 4 文件 / 50、ui-ep 新增 1 文件 / 38） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test` | 通过（10 文件 / 35 用例） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（首屏入口块合计 119.5 KB / 预算 260 KB；首屏最大单文件 104.1 KB / 预算 150 KB） |
