...
| 事件 | `update`（`{ id, patch }`）/ `canvas-update`（`Partial<ScreenCanvasConfig>`） |
| `data-test` | `property-panel-detail` / `prop-x` / `prop-y` / `prop-w` / `prop-h` / `prop-z` / `prop-text` / `prop-dataset` / `prop-chart-type` / `prop-background` / `prop-theme` / `prop-resolution` |
...
| `vue-tsc`、ESLint、Vitest 全通过 | 根 `npm run check` + 宿主 `vue-tsc -b` / `lint` / `test` / `build` / `budget` |
