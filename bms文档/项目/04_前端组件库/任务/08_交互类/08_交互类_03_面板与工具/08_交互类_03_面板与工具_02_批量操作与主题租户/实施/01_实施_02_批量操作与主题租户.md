...
| `pnpm run core:test` | 35 文件 / 222 用例全绿（本次新增 39） |
| `pnpm run ui-ep:typecheck` / `ui-ep:lint` | 通过 |
| `pnpm run check`（core + vue + ui-ep） | 36 文件 / 315 用例全绿（ui-ep 本次新增 27） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（全库行覆盖 86.79%；`utils/initialTheme.ts` 94.73%） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（合计 121.0 KB / 预算 260 KB；最大单文件 102.3 KB / 预算 150 KB） |
