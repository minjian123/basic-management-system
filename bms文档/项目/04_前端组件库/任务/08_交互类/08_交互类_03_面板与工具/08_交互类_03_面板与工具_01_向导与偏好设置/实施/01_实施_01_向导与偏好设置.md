...
| 验证点 | 方法 | 结果 |
| --- | --- | --- |
| 类型检查 / Lint / 单测（核心与插件） | 根 `npm run check`（core / vue / ui-ep） | 通过 |
| 单测（core） | `core:check` | 31 文件 / 183 用例（新增 21） |
| 单测（vue） | `vue:check` | 2 文件 / 5 用例 |
| 单测（ui-ep） | `ui-ep:check` | 35 文件 / 288 用例（新增 24，含两套契约套件驱动） |
| 宿主（PC）门禁 | `npx vue-tsc -b` / `npm run lint` / `npm run test:cov` | 通过；30 用例；覆盖率 86.91%（阈值 70） |
| 构建与体积预算 | `npm run build` / `npm run budget` | 通过；合计 120.7 KB / 260 KB，最大单文件 102.2 KB / 150 KB；`dist` 只有 `index.html`（核对页不入产物） |
