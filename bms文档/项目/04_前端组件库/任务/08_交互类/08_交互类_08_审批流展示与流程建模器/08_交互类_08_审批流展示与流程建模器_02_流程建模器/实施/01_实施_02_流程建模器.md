...
| Kiwi 用例 | 975（分类「平台骨架」/ P2 / CONFIRMED / 标签「自动化」） |
...
```mermaid
flowchart LR
    A["Kiwi 用例登记（975）"] --> B["核心 domain/process-modeler（零依赖结构提取与往返等价） + 能力基类 BaseProcessModeler"]
...
1. **Kiwi 先行**：经容器内 Django shell 登记用例，取得编号 **975**；用例文件首行标注 `// kiwi_id: 975`。
...
| 命令 | 结果 |
| --- | --- |
| 根 `pnpm run check`（core + vue + ui-ep） | 54 + 2 + 44 文件 / 614 + 5 + 543 用例全绿（ui-ep 本次新增 34） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（全库语句覆盖 86.79%、行覆盖 87.61%、35 用例） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（合计 123.1 KB / 预算 260 KB；最大单文件 104.1 KB / 预算 150 KB） |
