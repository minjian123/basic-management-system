...
| 单测（契约 + 组件） | `pnpm run ui-ep:test` | 19 文件 / 128 用例全通过（新增 8） |
...
- **偏差**：无（按详细设计实施）。
- **遗留**：① 真实数据通路随 `06_04` ~ `06_07`（不得改对外契约）——**组织件部分已闭环**（`06_05` 2026-09-21 真实实现落地，四件对外契约未改）；② 字段壳组装随字段壳族；③ 验证码滑块轨迹与限流细节随后续认证与安全能力。
