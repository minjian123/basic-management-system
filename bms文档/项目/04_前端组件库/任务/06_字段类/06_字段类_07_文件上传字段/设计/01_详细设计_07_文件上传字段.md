...
| 27 | 新增依赖（剪裁库） | `frontend/packages/ui-ep/package.json`（`cropperjs@^2.2.0`）+ 根 `package-lock.json` |
...
- **依赖与镜像**：`cropperjs@^2.2.0` 经 npmmirror 安装（`bms/frontend/.npmrc`）；CI 基础镜像随 `package-lock.json` 变更由 `ci-base-build` 自动重建（`deploy/ci/Dockerfile.frontend` 已按锁文件 `npm ci`）。
