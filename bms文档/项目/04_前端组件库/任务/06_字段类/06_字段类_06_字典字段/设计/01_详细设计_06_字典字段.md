...
| # | 设计口径 | 实施落定 | 原因 |
| --- | --- | --- | --- |
| 1 | `BaseDictStore` / `BaseDictQuery` 父基类 `BaseComponent` | 父基类改 **`BasePlaceholderState`**（依赖登记 `dict-store: ['placeholder-state']` / `dict-query: ['placeholder-state', 'dict-store']`） | 就绪 / 降级 / 占位零请求计数为占位语义基类职责（同 `BaseGlobalSearch` 先例），避免链外重复实现 |
| 2 | 核心类私有字段用 `#` | 字典缓存类私有字段改**下划线命名**（`_entries` 等） | 缓存实例经件 props 传递时会被 Vue 代理，`#private` 写入报错（`Cannot write private member`）；下划线命名运行时为普通属性，兼容代理 |
| 3 | 投影 `isLarge` 为 `ComputedRef` | 改 **`Ref`**（`sync()` 时从核心实例读取） | 核心实例非响应式，`computed` 不重算 |
| 4 | 投影 `storage` 缺省内建 localStorage 通道 | 显式传入 `storage`（含 `undefined`）以调用方为准；未传才用内建通道 | 契约 / 测试需「禁用本地持久化」，`?? ` 语义无法区分「未传」与「显式 undefined」 |
| 5 | 条件组构建件挂链方式未定 | 经 `useBaseInput` 投影挂链（条件组作为受控输入值） | 组件护栏要求每个 `.vue` 实质挂链；条件组编辑器本质为输入控件（受控 `modelValue`） |
| 6 | 大字典（`parentId` 非空）读取路径 | 级联父值非空走**数据源直取**（带 `parent_id`，不写缓存）；父值为空走缓存路径并本地**顶层过滤** | 带过滤的请求不应污染整类型缓存；「清空父级回到顶层条目」需本地过滤（缓存含子级） |
| 7 | 核心方法清单未列 `setLimitExceeded` | 补 `BaseDictSelect.setLimitExceeded`（投影同名面） | 件层受控控件超限截断需同步标记（与 `BaseOrgSelect` 口径一致） |
| 8 | 领域函数清单未列 `findDictItem` / `mergeDictItems` | 补两函数（对齐组织领域 `findOrgItem` / `mergeOrgItems`） | 选择族候选合并与查找复用（单一落点） |
| 9 | Kiwi 用例编号待登记 | **964**（分类「平台骨架」/ P2 / CONFIRMED / 标签「自动化」），已回填三份用例文件 | 测试步平台登记后回填 |
| 10 | 核对页「禁用项」自检 | 桩模式断言存在 `status=disabled` 条目；真实模式断言状态字段可读（种子条目均为启用状态） | 真实种子无停用状态条目（业务值「停用」状态为启用），避免假失败 |
