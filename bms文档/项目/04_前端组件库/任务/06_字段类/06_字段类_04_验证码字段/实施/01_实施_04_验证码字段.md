...
| 实施日期 | 2026-09-21 |
| 实施人 | minjian |
| 实施环境 | Ubuntu 开发机 · Node（pnpm workspace）· Vue 3.5 / TypeScript 严格模式 · Element Plus 2.14 · Vitest 5（node + jsdom）· chromium（CDP 轮询导出 DOM）· 后端占位服务（`uvicorn app.asgi:app --port 8000`，HTTP 模式核对用，用毕停止） |
...
| 命令 / 方式 | 结果 |
| --- | --- |
| 根 `pnpm run core:check`（typecheck + lint + test） | 86 文件 / **1109 用例**通过（新增 2 文件 / 38 用例 + 映射断言 1 项） |
| 根 `pnpm run ui-ep:check` | 60 文件 / **897 用例**通过（新增 `field-captcha.spec.ts` 32 用例 + 表单协同 1 用例） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run test:cov` | 通过（覆盖率门禁） |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（首屏合计 127.5 KB / 260 KB；首屏最大单文件 111.2 KB / 150 KB；无新增分包） |
