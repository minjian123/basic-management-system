...
normalizeCaptchaChallenge(raw, fallback?): CaptchaChallenge | undefined // 兼容 snake_case；captcha_id 缺失剔除；图片转 data URL；缺省字段回落请求侧
normalizeCaptchaPolicy(raw, scene?): CaptchaPolicy           // 兼容 fail_threshold；缺省回落平台默认
defaultCaptchaPolicy(scene?): CaptchaPolicy                  // required 按场景缺省（登录不强制 / 其余强制）；threshold 3 / ttl 300 / cooldown 60
...
| 事件 | `update:modelValue` / `change` / `refresh` / `invalid` / `pass` / `fail` / `loaded` |
...
- **状态**：待拖动 → 拖动中 → 校验中 → 通过（绿色勾选，`data-pass`）/ 失败（自动复位 + 「验证失败，请重试」可重试；失败提示在刷新挑战后仍保留，由件层最近失败态承载）。
...
- **发送与倒计时**：点「获取验证码」→ 族基类 `sendSms()`（成功启动倒计时，按钮置灰 `重新发送({n}s)`）；命中限流 20103 → 提示「发送过于频繁，请稍后再试」且**不启动 / 不重置倒计时**；`countdown` prop 为**冷却配置**（空闲按钮文案含配置秒数 `获取验证码（{n}s）`，兼容 `06_01` 冻结显示断言）。
...
| # | 设计口径 | 实施落定 | 原因 |
| --- | --- | --- | --- |
| 1 | `normalizeCaptchaChallenge(raw)` 无回落参数 | 增可选 `fallback`（请求侧场景 / 形态），响应缺省字段回落请求侧 | 后端响应缺 `scene` / `kind` 时保持请求口径（占位后端与真实实现兼容） |
| 2 | §3.1 注释 `defaultCaptchaPolicy` 恒 `required=false` | 改为**按场景缺省**（登录不强制、其余场景强制），与后端 `CAPTCHA_SCENE_POLICIES` 同源 | 避免前端策略口径与后端默认表漂移 |
| 3 | 领域常量未列轨迹上限 | 补 `CAPTCHA_TRACE_MAX_POINTS = 240`（超出丢弃） | 长拖拽采样内存无界；上限不影响提交口径 |
| 4 | 投影派生面（`needsChallenge` / `maskedTarget` / `inputMaxLength` / `inputHint` / `errorText` / `error` / `empty` / `hasImage`）设计为 `ComputedRef` | 实现为 `Ref`（随生命周期与值变更同步） | 核心实例非响应式，`computed` 不重算（同 `06_06` 字典对齐记录 3） |
| 5 | 投影返回面未列 `payload` / `error` / `policy` | 补齐三面（**只增**） | 件层渲染与核对页消费需要 |
| 6 | 图形件事件未列 `pass` | 补 `pass` 事件（三形态成功态一致） | `CaptchaField` 统一上抛 `pass` 需要 |
| 7 | 滑块失败提示口径未写 | 件层以**最近失败态**承载提示（刷新挑战后仍显示重试入口） | 失败后前端自动重取挑战会清错误态，需保留重试入口 |
| 8 | 核对页未列 HTTP 探测细节 | `?source=http` 模式对四端点做端到端探测（策略 / 出题 / 校验 / 短信） | 占位后端联通核对与阶段六联调前置 |
| 9 | 短信发送失败（非 20103）口径未写 | 不启动倒计时（失败不消耗冷却窗口） | 与后端冷却口径一致 |
| 10 | Kiwi 用例编号待登记 | **966**（分类「平台骨架」/ P2 / `CONFIRMED` / 标签「自动化」），已回填四份用例文件 | 测试步平台登记后回填 |
