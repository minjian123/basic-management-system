...
| 实施日期 | 2026-09-18 |
| 实施人 | minjian |
| 实施环境 | 本地开发机（Node v24.20.0 / npm 11.19.0，依赖走 npmmirror） |
...
flowchart LR
    ...
    F --> G["npm run core:check 全绿"]
```

- 复用未随前端清空删除的根 workspace 配置（`package.json` / `package-lock.json` / `tsconfig.base.json` / `.npmrc`），本期未新增依赖、**未改 `package-lock.json`**（不触发 `ci-base-build`）。
...
1. **复用根配置核对**：确认根 `package.json`（workspaces `frontend/packages/*`）、`package-lock.json`（含 `frontend/packages/core` 条目，无依赖）、`tsconfig.base.json`、`.npmrc` 均未删除。
...
7. **本地验证**：`npm run core:check`（typecheck + lint + test）全绿，5 用例通过。
...
| 2 | `deploy/ci/build-base.sh` / `Dockerfile.frontend` 仍引用已清空的 `frontend/apps/{desktop,mobile}/package-lock.json` | 前端整体重建中 | 不改 CI 脚本、不改根 `package-lock.json`（避免触发 `ci-base-build`）；登记为阶段级遗留（见 §6） |
...
| 单测 | `npm run core:test`（Vitest） | 2 文件 / 5 用例全通过 |
...
- **CI 锁文件遗留闭环**：`frontend/apps/desktop/package-lock.json` 存在且被 `deploy/ci/{Dockerfile.frontend,build-base.sh}` 有效引用。
- **提交**：`a1b4ddb9`。
