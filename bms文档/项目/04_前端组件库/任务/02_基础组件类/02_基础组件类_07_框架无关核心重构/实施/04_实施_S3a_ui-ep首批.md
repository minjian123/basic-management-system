...
| 结论 | S3a 完成：核心 `BaseComponent` 协议补全、`@bms/vue` 组件根投影落地、`frontend/packages/ui-ep` 建立并迁移首批 5 件，`npm run check` 全绿 |

## 2. 交付物 <a id="deliverables"></a>

| 块 | 内容 |
| --- | --- |
| 核心协议补全 | `frontend/packages/core/src/base/BaseComponent.ts`：`identifier` / `nsClass` / `rootAttrs`（档位属性 + 状态类 `is-loading`·`is-disabled`·`is-hidden` + `aria-*`）/ `passthroughAttrs`（保留键过滤）/ `setProps`（运行期可变字段）/ `notifyLifecycle` / `mechanisms`（装配点骨架）——**与旧实现行为同构（纯 TS）** |
| Vue 组件根投影 | `@bms/vue`：`useComponentBase`（实例化核心 + props 优先 sync + scope 释放；接口面与旧组合式同构——**迁移组件只改 import 源**） |
| EP 插件包（新） | `frontend/packages/ui-ep`：deps `element-plus` / `@bms/core` / `@bms/vue`；peer `vue`；测试栈 plugin-vue + jsdom + EP inline + vue-tsc；`env.d.ts`（EP 按需样式模块声明） |
| 首批迁移（5 件） | `GridRow` / `GridCol` / `Space` / `Divider`（**显式导入 EP 组件与按需样式**，替代旧自动导入插件依赖）+ `SkeletonBlock`（原生）——统一改 import 源为 `@bms/vue` |
| 用例（新增 3 条） | `frontend/packages/ui-ep/tests/components.spec.ts`：组件根协议（`nsClass` / `data-size` / 透传 `data-foo`）、栅格渲染、骨架行数与 `loading=false` 渲染内容 |
...
| 3 | 旧工程（`frontend/apps/desktop/`）仍承载宿主（路由 / 菜单 / 页面） | S5 宿主切流（`frontend/apps/desktop`）时归档旧工程 |
