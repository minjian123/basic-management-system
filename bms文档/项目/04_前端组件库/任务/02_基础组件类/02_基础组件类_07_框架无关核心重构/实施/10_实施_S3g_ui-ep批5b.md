...
| 块 | 内容 |
| --- | --- |
| 视图解析注入点（新） | `ui-ep/src/components/layout/viewResolver.ts`：`configureViewResolver(resolver)` / `resolveView(name)`（**未注入 = 未注册**，返回 `null` 回退占位视图）+ `nameComponent`（keep-alive 固定 name，纯 Vue 实现随迁、去宿主 glob） |
