...
7. 验证：根 `pnpm run check`（core + vue）与 `apps/desktop` 的 `lint` / `test:cov` / `build` / `budget` 全绿。
...
| 单测（核心） | `pnpm run core:test` | 22 文件 / 116 用例全通过 |
| 单测（vue 绑定） | `pnpm run vue:test` | 1 文件 / 2 用例全通过 |
| 单测（宿主） | `apps/desktop` `pnpm run test:cov` | 6 文件 / 19 用例全通过；覆盖率 97.95%（≥70） |
| 宿主 Lint / 构建 / 预算 | `apps/desktop` `pnpm run lint` / `build` / `budget` | 全绿（gzip 合计 50.7 KB） |
| 门禁串联 | 根 `pnpm run check` | 全绿 |
