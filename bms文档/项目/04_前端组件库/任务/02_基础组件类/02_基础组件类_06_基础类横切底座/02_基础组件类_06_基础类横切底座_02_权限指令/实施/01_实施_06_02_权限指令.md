...
8. **双端与验证**：`diff` 比对源码与用例一致；双端门禁全通过（apps/desktop 94.5 / 71.7 KB、mobile 77.4 / 77.0 KB，均在预算内）。
...
双端逐条命令与结果见[测试记录](../测试/01_测试_06_02_权限指令.md) §3；结论：`prettier` / `eslint` / `vue-tsc` / `vitest run --coverage` / `build` / `budget` 双端全通过（各 26 个用例文件 / 197 条用例；`src/directives` 覆盖 95.65% 语句；构建体积 apps/desktop 94.5 KB（预算 100）、frontend-mobile 77.4 KB（预算 83））。
...
| 6 | 基类与片段合规审计修复（2026-09-16）：`PermButton` 补挂组件根（`useComponentBase`：nsClass / 令牌属性协议 / 透传；「框架无关最小版」升为组件根版） | **已闭环**（[审计报告](../../../../../审计/01_审计_前端基类与片段合规.md)） |
