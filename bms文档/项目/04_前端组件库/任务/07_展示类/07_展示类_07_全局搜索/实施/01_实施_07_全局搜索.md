...
| 命令 | 结果 |
| --- | --- |
| 根 `pnpm run check`（core + vue + ui-ep） | 76 + 2 + 56 文件 / 942 + 5 + 763 用例全绿（core 新增 2 文件，ui-ep 新增 1 文件 / 25 用例） |
| 宿主 `pnpm exec vue-tsc -b` / `pnpm run lint` | 通过 |
| 宿主 `pnpm run build` / `pnpm run budget` | 通过（首屏合计 121.9 KB / 预算 260 KB；首屏最大单文件 106.0 KB / 预算 150 KB；15 个异步块不计入首屏） |
