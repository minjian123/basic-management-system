...
normalizeMessage(value: unknown): NotificationMessage | undefined
normalizeMessages(value: unknown): NotificationMessage[]
normalizePage(value: unknown): NotificationPage
...
filterMessages(items: readonly NotificationMessage[], query: NotificationQuery): NotificationMessage[]
mergeMessages(
...
buildQueryParams(query: NotificationQuery): { type?: string; is_read?: number; page?: number; size?: number }
...
| 行为 | 口径 |
| --- | --- |
| 消息归一 | `normalizeMessage`：`id` / `title` 缺失视为脏项剔除；`type` 非白名单回落 `notice`；`read` 归一布尔（缺省 `false`）；`bizId` 与 `bizType` 同时存在才构成跳转 |
| 页归一 | `normalizePage`：非对象 / 脏项回落 `{ items: [], total: 0 }`；`total` 缺省取 `items.length`；`unreadCount` 缺省不产出（不覆盖本地角标） |
...
| 筛选 | `filterMessages`：`type` 为 `all` / 缺省不过滤；`read` 为 `all` 不过滤、`unread` / `read` 按 `isUnread` |
...
| 查询参数 | `buildQueryParams`：`type` 非 `all` 才传；`read` → `is_read`（`unread` → `0`、`read` → `1`、`all` 不传）；`page` / `pageSize` 缺省不传（与后端 `/notifications` 查询契约同源） |
...
- **多标签同步**：给定 `channelName` 时经 `utils/notificationChannel.ts` 的 `BroadcastChannel` 通道同步 `{ unreadCount, latestId, action }`；收到远端事件即校准本地角标并按 `action` 更新列表（去重）；浏览器无 `BroadcastChannel` 时静默降级（不抛错）。
