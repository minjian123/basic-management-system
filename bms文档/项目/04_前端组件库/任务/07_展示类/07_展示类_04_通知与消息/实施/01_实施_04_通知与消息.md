...
| 命令 | 结果 |
| --- | --- |
| 根 `npm run check`（core + vue + ui-ep） | 72 + 2 + 52 文件 / 881 + 5 + 698 用例全绿（core 新增 2 文件 / 50 用例，ui-ep 新增 1 文件 / 32 用例） |
| 宿主 `npx vue-tsc -b` / `npm run lint` | 通过 |
| 宿主 `npm run test:cov` | 通过 |
| 宿主 `npm run build` / `npm run budget` | 通过（首屏合计 119.7 KB / 预算 260 KB；首屏最大单文件 104.2 KB / 预算 150 KB；14 个异步块不计入首屏） |
