...
[文档首页](../../../../../文档首页.md) › [02 仓库与CI](../02_仓库与CI.md) › 04 main 流水线　|　[← 父任务](../02_仓库与CI.md)
...
4. 容器镜像安全扫描：GitLab Container Scanning（Trivy 引擎）扫描 Registry 镜像，CRITICAL / HIGH 阻断发版
5. `swagger.json` 契约快照导出归档
6. 测试报告：Allure 报告归档（CI artifact），测试结果经官方插件导入 Kiwi TCMS 用例库

## 3. 完成标准 <a id="accept"></a>

main 流水线端到端通过；契约快照与 Allure 报告已归档；高危漏洞（CRITICAL / HIGH）时流水线阻断。

> 口径说明（2026-08-22）：镜像扫描 job 为规划新增项（《项目规划说明》「测试策略与测试流程」节安全专项，GitLab 内置模板增量小、不加工时）；模板镜像需外网拉取——mjbk 外网不可达时预缓存或与 06 一并暂缓，不阻塞其余 job。

## 4. 参考文档 <a id="ref"></a>

- 《开发部署规划》「开发工作流与 CI」节、《测试规范》
