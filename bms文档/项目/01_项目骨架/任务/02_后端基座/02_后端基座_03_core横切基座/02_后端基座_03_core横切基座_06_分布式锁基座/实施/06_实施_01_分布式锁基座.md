...
- **遗留归口（已登记，随对应阶段销项）**：
  - 真实实现（Redis `SET NX PX`、Lua 令牌比对释放 / 续租、`wait` 轮询与订阅、看门狗续租）→ **八 通用能力**；多副本互斥实测随**十三 分布式与监控阶段**收口——已登记计划「后续阶段待办」表「分布式锁真实实现」行，实现期要点见需求 02-30 `> 注：` 块。
  ...
  - 真实 Redis 集成用例（SETNX 竞争、令牌误放防护、TTL 到期）→ 已并入计划「后续阶段待办」表「Redis 集成用例」行（出处标注 02-3-6），随八 通用能力阶段补充并纳入重验证层 Redis job（需 `BMS_TEST_REDIS_URL`）。
