...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1「限流 / 幂等 / 防重放真实实现」行已在实施阶段登记；需求 02-25 `> 注：` 块（性能与安全 · 开放接口 · 首个落库阶段回补）已就位。
  - 消费方标注：[02-4-6 出站集成基座](../../../02_后端基座_04_跨阶段基座/02_后端基座_04_跨阶段基座_06_出站集成基座/02_后端基座_04_跨阶段基座_06_出站集成基座.md) 已补「前置契约（已交付）」（出站 Webhook 签名复用 `SignatureCodec`、串料口径差异）；[02-3-13 开放接口 OAuth2 服务端与 scope 基座](../../02_后端基座_03_core横切基座_13_开放接口OAuth2服务端与scope基座/02_后端基座_03_core横切基座_13_开放接口OAuth2服务端与scope基座.md) 补「前置契约（已交付）· 开放接口防重放与限流」（`/api/open` ingress 取 `BaseReplayGuard` / `SignatureCodec` 与 `BaseRateLimiter`）。
  - 无任务文档的消费方（性能与安全 / 租户管理 / 首个落库阶段 / 开放接口阶段）已在计划 §3.1 与需求 02-25 注块登记去向，待其阶段建任务后回引。
  - 未标注项：登录侧联动已在任务文档「登录侧联动（口径）」条登记（登录阶段无独立任务文档）。
