...
- **遗留归口（已闭环登记，随对应阶段销项）**：
  - 真实掩码规则（手机号 / 身份证 / 邮箱 / 银行卡 / 姓名）、`reveal` 真实解密、嵌套 `BaseSchema` 掩码 → **四 认证与安全**（需求 02-29 口径，同一任务文档不重复立项）；已登记计划「后续阶段待办」表「数据脱敏真实实现」行，实现期要点见需求 02-29 `> 注：` 块。
  - `data:plain` 真实权限查询与 RBAC 主体链 → **五 RBAC** 替换 `NullPermissionChecker`；已登记计划「后续阶段待办」表「`data:plain` 真实权限查询」行，并在 [02-3-9 权限校验基座](../../02_后端基座_03_core横切基座_09_权限校验基座/02_后端基座_03_core横切基座_09_权限校验基座.md) 任务文档标注「契约由 02-3-5 提前交付、实施时复用」。
  - 导出侧脱敏 → **导入导出阶段**（已登记计划「后续阶段待办」表）；日志脱敏 → **03_02 日志体系**（本阶段已排期）；AI 交互落库前掩码 → **十七 AI 能力**——三者统一经 `BaseMasker` 接入，均已登记计划「后续阶段待办」表。
  - 需求 02-24 状态回写 → 已在 02-3-9 任务文档标注契约归属与复用口径；**02-3-9 于 2026-09-14 核销时已回写需求状态与完成日期**（[02-3-9 实施记录](../../02_后端基座_03_core横切基座_09_权限校验基座/实施/09_实施_01_权限校验基座.md)），本条遗留已闭环。
