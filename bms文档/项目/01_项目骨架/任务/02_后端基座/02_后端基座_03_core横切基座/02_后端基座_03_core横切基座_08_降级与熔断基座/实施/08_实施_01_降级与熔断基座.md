...
- **遗留归口（已登记，随对应阶段销项）**：
  ...
  - 告警联动（降级 / 熔断状态接入告警）→ 随 **02-3-11 可观测性基座**与运维阶段协同（后续在 02-3-11 任务文档标注取用口径）。
  - 限流降级（Redis → memory 后端）→ **02-3-10 限流幂等防重放中间层**内部策略（实现时可按需问 `resolve("redis")` 取动作）。
  - 主从切换 / 只读模式（`database`）→ 落库阶段；缓存三防 → 阶段六 通用能力。
