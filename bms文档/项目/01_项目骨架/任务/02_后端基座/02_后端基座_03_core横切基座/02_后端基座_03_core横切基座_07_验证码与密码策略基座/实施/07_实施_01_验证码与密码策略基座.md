...
- **遗留归口（已登记，随对应阶段销项）**：
  - 真实实现（Pillow 出图经线程池、Redis 读写 `bms:global:captcha:{uuid}`、`verify` 单次失效、sys_config 策略规则、`pwd_history` 比对、90 天有效期）→ **四 认证与安全**；已登记计划「后续阶段待办」表「验证码与密码策略真实实现」行，实现期要点见需求 02-31 `> 注：` 块。
  - 验证码强制策略（连续失败 3 次）与账号锁定（`sys_account_lock`、180 天未登录扫描）→ 登录侧 / 任务调度（计划行已注明），不在本基座。
  - 登录失败限流（slowapi Redis 后端）→ 02-3-10 限流幂等防重放中间层与认证阶段。
  - 验证码图片传输形态（base64 / 二进制流）→ 认证阶段接口层定稿；本基座只返回字节。
  - 真实 Pillow / Redis 集成用例 → 四 认证与安全阶段补充，纳入重验证层 Redis job（需 `BMS_TEST_REDIS_URL`）。
