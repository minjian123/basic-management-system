...
6. **测试**：新增 `tests/core/test_logging.py`（Kiwi 34）。
...
| 完成标准 | 验证方法 | 实测结果 |
| --- | --- | --- |
| 日志基座接入点声明就位、可被上层引用 | Kiwi 34 用例 + 代码核对 | 通过 |
...
- **后续回写（2026-09-14，03-02 日志体系完成）**：`configure_logging` 已换芯 structlog 真实实现（dev console / test·prod JSON、上下文字段、脱敏、stdlib / uvicorn 统一渲染）；`BaseLogger` / `StdoutLogger` 门面契约与 Kiwi 34 结构回归保持，本任务遗留销项。详见[03-2 实施记录](../../../../03_后端基础能力/03_后端基础能力_02_日志体系/实施/01_实施_01_日志体系.md)。
