...
4. **测试**：新增 `tests/core/test_security.py`（Kiwi 33）。
...
| 完成标准 | 验证方法 | 实测结果 |
| --- | --- | --- |
| 安全基座接口声明就位、可被上层引用 | Kiwi 33 用例 + 代码核对 | 通过 |
...
- **无偏差**：按详细设计执行。
- 真实实现（密码哈希 bcrypt/argon2、JWT 双 token、会话黑名单）随认证阶段；依赖注入提供者随认证阶段接入（已登记：阶段一计划「后续阶段待办」表）。
