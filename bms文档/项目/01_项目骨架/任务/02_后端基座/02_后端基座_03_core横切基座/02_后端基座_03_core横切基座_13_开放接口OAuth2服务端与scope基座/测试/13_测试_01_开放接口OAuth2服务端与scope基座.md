...
| Kiwi 用例 | 46（平台登记：分类「平台骨架」、P2、CONFIRMED、标签「自动化」；待回填） |
| 结论 | 通过 |
...
| Kiwi 46 平台登记尚未完成 | 本地无 mjbk Kiwi TCMS 管理凭据 | 代码已以 `@pytest.mark.kiwi_id(46)` 关联；待用户在平台登记后回填本行 |
...
- Kiwi 平台登记：用例 46 待登记（见第 4 节）。
- **处理偏差与遗留（2026-09-14 闭环）**：真实签发 / scope 校验 / 撤销用例去向已在计划 §3.1「开放接口 OAuth2 服务端 / scope 真实实现」行与需求 02-41 `> 注：` 块登记；消费方 / 边界标注见[实施记录](../实施/13_实施_01_开放接口OAuth2服务端与scope基座.md)「处理偏差与遗留」；唯一未闭环项为 Kiwi 46 平台登记。
