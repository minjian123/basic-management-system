...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「通知渠道真实实现」行（出处 02-4-4）；需求 02-20 `> 注：` 块补齐实现期要点（站内信 / 邮件 / 短信 / 模板 / 偏好过滤 / 降级）。
  - 消费方标注：[02-4-7 工作流引擎适配基座](../../02_后端基座_04_跨阶段基座_07_工作流引擎适配基座/02_后端基座_04_跨阶段基座_07_工作流引擎适配基座.md) 任务文档补「前置契约（已交付）」（审批待办提醒 / 邮件取 02-4-4 `BaseNotifier`，不另接 SMTP / 短信 SDK）。
  - 无任务文档的消费方（通知公告阶段、认证与会话找回密码、告警通知、事件总线通知组）已在计划 §3.1 与需求 02-20 注块登记去向，待其阶段建任务后回引。
