...
- **处理偏差与遗留（2026-09-14 闭环）**：计划 §3.1 新增「审计哈希链真实实现」行（出处 02-4-11）；需求 02-34 `> 注：` 块补齐实现期要点（SHA256 链 / 跨月衔接 / 有序消费 / 巡检 / 归档前校验 / 留存）；消费方（审计哈希链阶段、事件总线、归档、索引取 02-4-3）已在计划与需求注块登记去向。
