...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「出站集成真实实现」行（出处 02-4-6）；需求 02-22 `> 注：` 块补齐实现期要点（httpx / 超时重试熔断 / Webhook 签名与退避 / 失败记录 / 异步推送）。
  - 消费方标注：[02-4-8 IdP 与会话存储扩展基座](../../02_后端基座_04_跨阶段基座_08_IdP与会话存储扩展基座/02_后端基座_04_跨阶段基座_08_IdP与会话存储扩展基座.md) 任务文档补「前置契约（已交付）」（对接外部 IdP 的 HTTP 调用取 02-4-6 `BaseHttpClient`，不另建 httpx 调用）。
  - 无任务文档的消费方（企业微信 / 钉钉、告警 webhook、事件总线 Webhook 推送）已在计划 §3.1 与需求 02-22 注块登记去向，待其阶段建任务后回引。
