...
| `MultipartInit` | `BaseSchema`（参数对象） | `key: str` / `size: int`（`gt=0`）/ `sha256: str` / `mime: str \| None = None` / `part_size: int \| None = None`（缺省取 `DEFAULT_PART_SIZE`） |
...
| `abort(upload_id) -> None` | 抽象方法（async） | 取消会话并清理临时分片（幂等） |
| `check(sha256, size) -> ExistingObjectRef \| None` | 抽象方法（async） | 秒传判定：命中返回既有对象引用、未命中 `None` |
| `NullMultipartUpload(BaseMultipartUpload, BaseNullObject)` | 占位实现 | 内存会话表（不落盘）+ 固定返回：`initiate` 登记会话并返回 `NULL_UPLOAD_ID` 前缀递增标识、`upload_part` 回显、`complete` 回显会话 key / size、`abort` 清会话、`check` 恒未命中 |
...
| 占位语义 | 占位带**内存会话表**（只记 key / size / mime / part_size；不落盘、不连 MinIO）；`check` 恒未命中（占位不判定秒传，避免上层误跳过上传） |
...
| DELETE | `/api/v1/files/uploads/{upload_id}` | — | `null` | 委托 `abort`（幂等；占位清会话条目） |
...
| 需求 [02-45](../../../../../需求/02_需求_后端基座.md#r02-45) | 参考文档后补 `> 注：实现期要点` 块（含「`initiate` 入参含目标对象 key」的需求原文偏差登记）；实施完成后回写状态 / 完成日期 |
| 需求总览《00_需求_项目骨架》 | 02-45 行状态 / 完成日期随实施回写 |
...
| 待登记 | 占位分片 / 合并 / 取消 | `upload_part` 返回 `MultipartPart`（`part_no` 回显 / `etag == NULL_ETAG` / `size == len(data)`）；`complete` 回显会话 `key` / `size`（`etag == NULL_ETAG`）；`abort` 幂等、会话失效后再 `complete` 抛 `NotFoundError` | 同上 |
