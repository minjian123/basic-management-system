...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「文件存储真实实现」行（出处 02-4-1）；需求 02-17 `> 注：` 块补齐实现期要点（MinIO / 私有桶 / 预签名 / 类型大小校验 / 配额 / 孤儿回收）。
  - 消费方标注：[02-4-10 导入导出基座](../../02_后端基座_04_跨阶段基座_10_导入导出基座/02_后端基座_04_跨阶段基座_10_导入导出基座.md) 与 [02-4-12 归档策略基座](../../02_后端基座_04_跨阶段基座_12_归档策略基座/02_后端基座_04_跨阶段基座_12_归档策略基座.md) 任务文档各补「前置契约（已交付）」（模板 / 导出 / 归档文件存取取 02-4-1 `BaseObjectStorage`，不另接 MinIO / 文件系统）。
  - 无任务文档的消费方（文件管理阶段、多租户路由配额、部署与运维备份归档）已在计划 §3.1 与需求 02-17 注块登记去向，待其阶段建任务后回引。
