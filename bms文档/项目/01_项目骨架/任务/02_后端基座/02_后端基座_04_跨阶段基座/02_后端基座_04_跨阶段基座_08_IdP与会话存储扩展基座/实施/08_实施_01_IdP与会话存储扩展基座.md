...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「IdP / 会话存储真实实现」行（出处 02-4-8）；需求 02-27 `> 注：` 块补齐实现期要点（authlib / 企业微信钉钉 / JIT 建号 / `sys_session` + Redis 踢出 / 双 token）。
  - 消费方：认证阶段（无任务文档）与 02-3-13（服务端侧，已标注边界协同契约）均已在计划 §3.1 与需求 02-27 注块登记去向，待认证阶段建任务后回引；外部 IdP HTTP 调用前置契约已标注（02-4-6）。
