...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「工作流引擎真实实现」行（出处 02-4-7）；需求 02-23 `> 注：` 块补齐实现期要点（SpiffWorkflow / `wf_*` 落库 / 会签 / 驳回 / 版本 / 待办推送 / 3.14 兼容）。
  - 消费方：本任务为审批待办**生产方**，其站内信 / 邮件取 02-4-4 `BaseNotifier`、实时提醒取 02-4-5 `BaseRealtimePublisher`（任务文档已有前置契约条，无需回标注）；业务单据联动归产品（biz）阶段，已在计划 §3.1 与需求 02-23 注块登记去向。
