...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「实时推送真实实现」行（出处 02-4-5）；需求 02-21 `> 注：` 块补齐实现期要点（socketio / 握手鉴权 / 连接注册 / 跨实例广播 / 事件路由 / 断线恢复 / 连接数）。
  - 消费方标注：[02-4-7 工作流引擎适配基座](../../02_后端基座_04_跨阶段基座_07_工作流引擎适配基座/02_后端基座_04_跨阶段基座_07_工作流引擎适配基座.md) 任务文档补「前置契约（已交付）」（审批待办实时提醒 `approval.todo` 取 02-4-5 `BaseRealtimePublisher`，不直接操作 Socket.IO）。
  - 无任务文档的消费方（通知公告阶段 `notification.new`、认证阶段 `session.revoked`、可观测性连接数）已在计划 §3.1 与需求 02-21 注块登记去向，待其阶段建任务后回引。
