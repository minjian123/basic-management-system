...
- **处理偏差与遗留（2026-09-14 闭环）**：
  - 计划 §3.1 新增「全文检索真实实现」行（出处 02-4-3）；需求 02-19 `> 注：` 块补齐实现期要点（ES 索引 / DSL / 高亮 / 别名 / 同步管线 / 重建 / 降级 / 租户权限）。
  - 消费方标注：[02-4-11 审计哈希链基座](../../02_后端基座_04_跨阶段基座_11_审计哈希链基座/02_后端基座_04_跨阶段基座_11_审计哈希链基座.md) 任务文档补「前置契约（已交付）」（审计日志按期索引与检索取 02-4-3 `BaseSearchIndex`，不另接 ES）。
  - 无任务文档的消费方（主数据全局搜索、文件内容检索、语义检索）已在计划 §3.1 与需求 02-19 注块登记去向，待其阶段建任务后回引。
