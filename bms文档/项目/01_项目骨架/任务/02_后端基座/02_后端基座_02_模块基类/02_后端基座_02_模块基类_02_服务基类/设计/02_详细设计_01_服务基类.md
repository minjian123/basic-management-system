...
- **依据**：需求 [02-6](../../../../../需求/02_需求_后端基座.md#r02-6)、《[架构设计 · 后端基础类体系](../../../../../../../设计/架构设计/04_架构设计_后端基础类体系.md)》「基座分层」节「基类清单与继承链」节「各层基类与模块约定」节「扩展规则」节「分层演进示例」节、《[后端开发规范](../../../../../../../规范/后端开发规范.md)》「后端基座体系（强制）」节、《[架构设计 · 数据访问与分片](../../../../../../../设计/架构设计/13_架构设计_子系统_数据访问与分片.md)》「事务边界规则」节、01-3-1《基础类详细设计》。
...
bms文档/
├── 设计/架构设计/04_架构设计_后端基础类体系.md   # 修改：§3/§4/§7 登记 BaseTransactionalService 与 UnitOfWork
...
| 层 | 类 | 文件 | 职责 |
| --- | --- | --- | --- |
| 根系 服务 | `BaseService[T]` | `services/base_service.py` | 通用 CRUD 委派仓储 + 统一不存在语义（**不含事务**） |
...
- 登记落点：《架构设计 · 后端基础类体系》§3 基类体系表 / §4 基类清单与继承链 / §7 模块约定；《后端开发规范》§3.1 / §3.4 / §3.6。
...
- **同步口径**：本任务为同步基线；**后续变更（02-5-1）**：`UnitOfWork` 事务边界与 `BaseService` / `BaseTransactionalService` CRUD 已统一切异步（`AbstractAsyncContextManager` / `async with`），`NullUnitOfWork` 继承 `BaseNullObject`，`DbUnitOfWork` 已接入；同步口径与本节如有出入，以 02-5-1 详细设计为准。
