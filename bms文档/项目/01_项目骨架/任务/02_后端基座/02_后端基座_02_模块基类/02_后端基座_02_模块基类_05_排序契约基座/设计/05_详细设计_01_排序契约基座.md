...
- **Kiwi 29 用例登记（实施回写）**：实施阶段因 Kiwi TCMS 管理员口令失效（本地资源文档与服务器 `~/deploy/.env` 两处取值在 Web 与 JSON-RPC 两条路径均登录失败）未能完成平台登记，代码侧已按 `@pytest.mark.kiwi_id(29)` 标注；口令重置后按第 9 节清单补登记（详见[实施记录](../实施/05_实施_01_排序契约基座.md)与[测试记录](../测试/05_测试_01_排序契约基座.md)遗留项）。
