...
1. **根系 调整**：`app/core/base.py` 的 `BaseObject` 去 `ABC`（规范 §3.2 称「普通类」），使 SQLAlchemy 声明式基类可混合继承（元类兼容）。
...
6. **测试**：新增 `tests/models/test_base_model.py`（Kiwi 30，SQLite 临时库）。
...
| 完成标准 | 验证方法 | 实测结果 |
| --- | --- | --- |
| `BaseModel` 字段与四库类型映射落地、表规范写入注释 | 代码核对 + Kiwi 30 | 通过 |
...
- **根系 偏差（已确认）**：`BaseObject` 去 `ABC`（原 02-01 交付为 ABC），规范 §3.2 本就称「普通类」；已回写 02-01 记录与架构33 §4。
- **雪花位宽调整（已确认）**：由设计初版示例 6+6 调整为 **41+10+12**（分布式 + 高并发）；WorkerId 配置注入，Redis 自动注册作为后续回补。
- **demo 模型改 ORM（2026-09-13 调整）**：`models/demo.py` 的 `Demo` 由内存 dataclass 改为继承 `BaseModel` 的 ORM 模型（表 `demo`）；示例链路仍走内存仓储（落库阶段替换为 DB 仓储）；01-3-1 交付记录已回写。
...
- **跨任务回写（2026-09-14，03-1 配置管理闭环）**：按用户拍板移除 `BMS_WORKER_ID` 旧名兜底（删除 `initial_worker_id()`），WorkerId 统一经配置基座 `BMS_APP__WORKER_ID` 注入（启动期 `configure_id_generator` 生效，导入期默认 0）；Kiwi 30 用例由「环境变量解析 / 回退」改为「导入期默认 0」；设计 §5 / §13 / §14 已同步回写。
