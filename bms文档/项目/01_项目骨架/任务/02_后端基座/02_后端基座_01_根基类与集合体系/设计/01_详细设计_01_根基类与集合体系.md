...
- **目标**：对阶段一已交付的 **根系 根基类**与**集合体系**（有序集合 / 进程内并发集合 / 跨副本 Redis 封装）做**职责 / 继承关系 / 代码位置**的**登记核对**——确保《[架构设计 · 后端基础类体系](../../../../../../设计/架构设计/04_架构设计_后端基础类体系.md)》「基类清单与继承链」节与《[后端开发规范](../../../../../../规范/后端开发规范.md)》「后端基座体系（强制）」节的登记与 `backend/app/core/` 代码一致、可核对；补登缺失项，补继承断言用例留证。
...
- **依据**：《架构设计 · 后端基础类体系》「基类清单与继承链」节「有序与并发集合」节、《后端开发规范》「后端基座体系（强制）」节、《命名规范》「后端命名」节、01-3-1《基础类详细设计》、01-3-2《并发集合与并发安全详细设计》、2026-09-13 需求并入确认。
...
| 项 | 代码事实 | 登记现状 | 差距 / 动作 |
| --- | --- | --- | --- |
| 根系 `BaseObject` | `core/base.py`，方法 `to_dict` / `to_json` / `__str__` / `__repr__` / `__eq__` / `__hash__` | 架构33 §4、规范 §3.2 已登记 | 一致，核对 |
| 根系 辅助类 `ValueHolder` | `core/base.py`，继承 `BaseObject`，供 `get_locked` 值替换 | 未登记 | **补登**架构33 §4 |
...
```text
bms文档/
├── 设计/架构设计/04_架构设计_后端基础类体系.md        # 修改：§4 补登 ValueHolder / LockStrategy / Redis 三类的职责·继承·位置
