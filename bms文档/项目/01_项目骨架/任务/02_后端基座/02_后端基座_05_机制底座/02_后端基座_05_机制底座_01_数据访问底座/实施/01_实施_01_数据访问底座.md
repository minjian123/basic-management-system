...
4. **测试**：既有 repo / service / api / uow 用例异步化；新增 `tests/db/test_data_access.py`（Kiwi 36）。
...
### 3.1 中间层抽取（补强） <a id="middle-layers"></a>

数据访问底座落地后复核横切 / 跨阶段基座的共性，抽取四类中间层基类（`app/core/capability.py`）：

| 中间层 | 作用 | 归属改造 |
| --- | --- | --- |
| `BasePlaceholder` | 占位实现公共父（`placeholder` 标记 + `describe()`） | `BaseNullObject` / `BaseStub` 继承 |
| `BaseNullObject` | 空实现（Null Object，无副作用） | `NullUnitOfWork` / `NullDataScope` / `NullShardingRouter` 继承 |
| `BaseStub` | 未实现占位：`_not_implemented()` 统一抛错 | `BaseDbRepository`、`BaseSecurity` 及原语继承 |
| `BaseCapability` / `BaseEventWorker` | 能力域契约（`key`）/ 事件工作单元（`event_type`） | `CacheRegion` / `DataScope` / `ShardingRouter` / `AuditCapturer` / `BaseTask` / `EventPublisher` / `EventConsumer` 继承 |
| `BaseAsyncResource` | 异步资源生命周期（`aclose()` + `async with`） | `EngineFactory` 继承；`create_app` 增 lifespan 关闭统一释放 |

- `EventConsumer.subscribed_type` 并入 `BaseEventWorker.event_type`；`EngineFactory.dispose` 更名 `aclose`。
...
| 完成标准 | 验证方法 | 实测结果 |
| --- | --- | --- |
| 接口 / 抽象就位、依赖注入可解析 | 引擎 / 会话 / `get_db` + Kiwi 36 | 通过 |
