...
- **目标**：在 backend 分层骨架（任务 03）之上，建立**根基类 BaseObject**（所有可继承类的公共方法落点）+ repositories / services / schemas 三套公共基类，把「公共方法、统一 CRUD 契约、公共配置」收敛到基类，后续模块只写差异部分，避免「每个类都改一遍」。
- **范围**：仅后端——新增 `core/base.py` 根基类；三套基类与 demo 数据模型/四件套改为继承；《后端开发规范》继承条款。ORM `BaseModel`（雪花 ID/审计/软删除/乐观锁）仍归 03-2，本任务只落继承约定与 `models/base.py` 继承点，**不重复实现**。
- **依据**：需求 [01-3](../../../../需求/01_需求_工程骨架.md#r01-3) 第 4/5 条、《[后端开发规范](../../../../../../规范/后端开发规范.md)》第 2/4 节、《[命名规范](../../../../../../规范/命名规范.md)》第 3/6 节、2026-09-10 设计调整确认（根基类）。
...
| `to_json() -> str` | JSON 字符串：`ensure_ascii=False`，不可序列化值降级 `str` |
...
class BaseObject(ABC):
    ...
    def to_json(self) -> str:
        """JSON 字符串（ensure_ascii=False，不可序列化值降级 str）。"""
```

**MRO 优先规则（重要）**：子类自带实现优先——dataclass 生成的 `__eq__/__repr__`、Pydantic 的 `__eq__/__repr__` 会覆盖根基类同名方法；`to_dict/to_json` 不与 Pydantic `model_dump` 冲突，由根基类统一提供。需要统一展示语义时显式调用或调整 MRO。

## 5. BaseRepository 设计 <a id="repository"></a>
...
## 6. BaseService 设计 <a id="service"></a>
...
新增用例通过本地测试替身（`Item(BaseObject)`、`ItemRepository(BaseRepository[Item])`）验证基类通用行为，同时断言三套基类与 demo 各类对 `BaseObject` 的继承关系。

## 11. 实施步骤 <a id="steps"></a>

1. Kiwi 登记用例 14 与 11–13（更新）并取 ID。
2. 新增 `core/base.py`（BaseObject）、`schemas/base.py`、`repositories/base_repository.py`、`services/base_service.py`、`core/exceptions.py` 的 `NotFoundError`。
3. 改造 demo 数据模型与三件套、`api/demo.py`；`main.py` 注册 404 临时处理器。
4. 《后端开发规范》§4 增补继承条款（含 BaseObject 默认继承）。
5. 新增四组基类用例（标注 `kiwi_id`），跑通全量测试。
...
- **同步状态**：本轮为 2026-09-10 设计调整（新增 BaseObject），当前仓库实现对应上一稿；代码/测试/记录待本设计确认后同步调整。
...
| 8 | 序列化口径 | `to_dict` 通用反射递归；`to_json` 降级 str；dataclass/Pydantic 自身同名方法优先（MRO） |
| 9 | 本轮范围 | 仅调整本设计文档（2026-09-10 二次讨论），实现/记录随后同步 |
