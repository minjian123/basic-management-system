...
| Alembic 在线迁移 | `alembic/env.py` 在线分支 `raise NotImplementedError`（占位），`versions/` 为空 | 占位期不跑迁移；`test_db.py migrate` 仅输出计划，真实迁移归阶段二 后端基座 | 登记 |
...
| 真实建库 / Alembic 在线迁移 / 真库用例与方言实测 | 阶段二 后端基座（计划 §3.1） |
