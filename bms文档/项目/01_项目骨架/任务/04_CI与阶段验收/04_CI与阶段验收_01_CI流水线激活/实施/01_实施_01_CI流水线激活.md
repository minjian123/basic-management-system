...
flowchart LR
    ...
    B --> C[.gitlab-ci.yml<br/>新增 backend-modules / mobile-check]
...
结果小结：冒烟层由 8 个 job 扩到 10 个（新增 `backend-modules`、`mobile-check`），前端双端接入 Vitest v8 coverage 并以 70% 阈值阻断，`needs` 编排后流水线总时长由分钟级降到 **49 秒**且全绿；`pytest` / `ruff` / `pyright` / `base-check` / `check_modules` 本地全绿。
...
2. **前端覆盖率配置**：`apps/desktop/vitest.config.ts` 与 `frontend-mobile/vitest.config.ts` 各增 `coverage` 段（`provider: v8`、`reporter: text/lcov/json-summary`、`reportsDirectory: coverage`、阈值 `statements/branches/functions/lines = 70`）；采集口径保持 vitest 默认（被用例导入的文件），全量 `src/**` 随后续扩面。
3. **CI 新增两 job**：`backend-modules`（lint 阶段，`cd backend && uv run python -m ops.check_modules`；**必须 `-m`**，直接跑脚本因 `sys.path` 不含 backend 根而 `ModuleNotFoundError`，本地已实测）；`mobile-check`（test 阶段，ESLint + `pnpm run test:cov`，与 `desktop-check` 完全对称）。
4. **前端 job 改造**：`desktop-check` / `mobile-check` 由 `pnpm run test` 改为 `pnpm run test:cov`，新增 `coverage` 正则（`/^All files\s*\|\s*(\d+(?:\.\d+)?)/`）与 `coverage/` 报告 artifacts（`when: always`、`expire_in: 30 days`）。
5. **needs 并行编排**：`desktop-check` / `mobile-check` → `ci-base-build`（optional）；`desktop-build` → `desktop-check`；`mobile-build` → `mobile-check`；`backend-image` → `backend-test`（均 `optional: true` 兼容 MR 流水线无 `ci-base-build` 的场景）；失败阻断语义不变。
...
| 4 | 提交时误把本地 `pnpm run test:cov` 生成的 `coverage/`（44 个 HTML）带入提交 | `.gitignore` 只忽略 `htmlcov/`、`.coverage*`，**缺 `coverage/` 规则**（该目录此前已被跟踪） | `git rm -r --cached` 双端 coverage（60 文件）+ `.gitignore` 补 `coverage/` + `.graphifyignore` 补产物排除项（提交 `b1dc02e`、工作区 `ca1434a`） |
...
| 前端双端 | `pnpm run lint && pnpm run test:cov` | 各 4 spec / 18 用例通过，`All files` **100%** ≥ 70% |
| 基座文档自检 | `python3 scripts/tools/base-check/check-base.py`（bms 根） | 五项通过（断链 0 / 跨层 0 / 措辞残留 0 / 清单 146=146 / 编号引用 0） |
| 流水线冒烟层 | 流水线 **170**（`4c6553dc`） | **success**，11 job 全绿，总时长 **49 秒**（`backend-modules` 2.6s、`mobile-check` 8.3s、`backend-typecheck` 24.6s） |
