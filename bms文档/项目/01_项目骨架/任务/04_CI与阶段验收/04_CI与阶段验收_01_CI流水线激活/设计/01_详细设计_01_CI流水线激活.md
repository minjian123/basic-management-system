...
| --- | --- | --- | --- |
| 冒烟层 job 集 | gate-smoke / base-integrity / backend-lint / **backend-typecheck**（2026-09-14 补）/ backend-test（`--cov-fail-under=70`）/ frontend-check（lint + test）/ frontend-build / frontend-mobile-build | 补「模块注册清单校验」与「移动端 lint+test」两个 job；前端覆盖率接入并设阈值 | 修改 + 新增 |
| 前端覆盖率 | `frontend` / `frontend-mobile` 各有 `vitest.config.ts`，**无 coverage 配置**；CI 跑 `npm run test`（`test:cov` 脚本闲置）；本地实测双端 4 spec / 18 用例全过、汇总 100% | 双端接入 v8 coverage + 阈值 70% 阻断；CI 改跑 `test:cov`，报告归档 | 修改（2 配置 + 2 job） |
...
| 后端核心模块 ≥ 80% | 仅整体 ≥ 70%；CI 注释为「随阶段一骨架确定模块路径后补入」 | 阶段一不落子阈值；核心模块路径（auth / rbac / wf / audit / 收付款）随认证 / RBAC 阶段接入 | 注释与需求口径对齐 |
| 重验证层 | 全部 job 以 `deploy/ci/verify-enabled` 守卫，**开关未创建**；`backend/tests/e2e` 目录不存在、`backend/tests/integration` 存在（2 条 Redis 集成用例，未配 `BMS_TEST_REDIS_URL` 时跳过） | 本任务**不创建**开关，逐项登记激活前置（[§8](#verify)） | 登记 |
...
```text
bms/
├── .gitlab-ci.yml                              # 修改：新增 backend-modules / frontend-mobile-check；前端 job 改跑 test:cov + 报告归档；needs 并行编排
├── frontend/vitest.config.ts                   # 修改：coverage（v8 + 阈值 70% + 报告目录）
...
| 冒烟层（test） | test | 同上 | `backend-test`、`frontend-check`、**`frontend-mobile-check`** | pytest + 后端覆盖率；双端 ESLint + Vitest + 前端覆盖率 |
| 冒烟层（build） | build | 同上 | `frontend-build`、`frontend-mobile-build` | 双端产物可构建 |
| 重验证层 | verify / .post | 仅 main + `deploy/ci/verify-enabled` | `e2e-playwright`、`integration-mysql` / `integration-postgres` / `integration-dm8`、`backend-image` / `frontend-image`、`container-scanning`、`swagger-snapshot`、`allure-report`、`defect-capture` | 见 [§8](#verify)（本任务不激活） |

粗体为本次新增。

### 4.2 needs 编排（并行化） <a id="needs"></a>

现状冒烟层 6 个 job 在 runner 并发上限 2 下按阶段串行（`frontend-check` 必须等全部 lint job 结束）。本任务用 `needs` 解除无关依赖的阶段栅栏，前后端两条链并行：
...
flowchart LR
    ...
    BASE --> FC["frontend-check"]
    BASE --> FMC["frontend-mobile-check"]
    FC --> FB["frontend-build"]
    FMC --> FMB["frontend-mobile-build"]
...
| job | needs |
| --- | --- |
| `frontend-check` / `frontend-mobile-check` | `ci-base-build`（optional） |
| `frontend-build` | `frontend-check`（optional） |
| `frontend-mobile-build` | `frontend-mobile-check`（optional） |
| `backend-image` | `backend-test`（optional，现状已等价） |
| `frontend-image` | `frontend-build`（optional，现状已有） |
...
- main 直推**仅当 changes 命中**代码与基础设施路径时建流水线：`backend/**`、`frontend/**`、`frontend-mobile/**`、`deploy/**`、`.gitlab-ci.yml` + 基座路径（`bms文档/规范/**`、`bms文档/资料/**`、`bms文档/资源/**`、`bms文档/基座文档清单.md`）。
...
| `frontend-check` | test | `ci-frontend:node22` | `frontend/package.json` | `cp -a /opt/ci/frontend/node_modules frontend/node_modules` → `cd frontend && npm run lint && npm run test:cov` | ESLint 有告警（`--max-warnings 0`）或用例失败或覆盖率 < 70% 即红 |
| **`frontend-mobile-check`** | test | `ci-frontend:node22` | `frontend-mobile/package.json` | 同构（复制 `/opt/ci/frontend-mobile/node_modules`） | 同上（移动端） |
| `frontend-build` | build | `ci-frontend:node22` | `frontend/package.json` | `npm run build`（`vue-tsc -b && vite build`） | 构建失败即红 |
| `frontend-mobile-build` | build | `ci-frontend:node22` | `frontend-mobile/package.json` | `npm run build` | 同上（移动端） |

口径说明：

- **exists 守卫**：`backend/pyproject.toml`、`frontend/package.json`、`frontend-mobile/package.json` 各自守卫对应 job（`exists` 是「任一命中」语义，故一个 job 只用一条守卫路径，避免误激活）。
- **MR 与 main 双分支规则**：每个带守卫的 job 同时写 `merge_request_event` 与默认分支两条 `rules`（沿用现状），保证单人期与未来多人期口径一致。
- **零安装**：前端 job 一律从 CI 基础镜像内预装目录 `cp -a` `node_modules`（不做 `npm ci`）；后端 job 一律 `uv run`（镜像内 `UV_PROJECT_ENVIRONMENT=/opt/bms-venv`、`UV_NO_SYNC=1`）。
...
`frontend/vitest.config.ts` 与 `frontend-mobile/vitest.config.ts` 增加 coverage 段（其余字段不变）：
...
- **采集口径（骨架期）**：沿用 vitest 默认行为——只统计**被用例导入**的文件；**不**显式 `include: ['src/**']` 全量拉入（否则页面 / 路由 / i18n 语言包等未覆盖文件会把汇总拉低，与本阶段「组件库尚未建设」的事实不符）。全量 `src/**` 采集与阈值随阶段四（前端组件库）扩面。
...
开关：仓库 `deploy/ci/verify-enabled`。本任务**不创建**该文件；各 job 的激活前置逐项登记如下，阶段一以「**就绪项全绿**」口径闭项（未就绪项不删 job、不放行）：

| job | 激活前置（未满足前的状态） | 判定 |
| --- | --- | --- |
| `e2e-playwright` | `backend/tests/e2e` 用例就位（**当前目录不存在**，激活必红） | 前置未满足 |
| `integration-mysql` / `integration-postgres` / `integration-dm8` | 首个落库阶段（Alembic 迁移 + 真库集成用例；当前仅 Redis 集成用例，未配 `BMS_TEST_REDIS_URL` 时跳过） | 随落库阶段（04-2 占位口径） |
...
- **标签口径**：`TAG = sha256(backend/uv.lock + frontend/package-lock.json + frontend-mobile/package-lock.json + deploy/ci/Dockerfile.backend + deploy/ci/Dockerfile.frontend)[:12]`，写入镜像 label `bms.lock-hash`；本地已存在同哈希镜像则跳过（构建与消费共用宿主 docker daemon，**本流水线即用新镜像**）。
...
- **前端镜像**（`ci-frontend:node22`）：`node:22-slim` → 复制 `frontend/` 与 `frontend-mobile/` 的 `package.json`/`package-lock.json` → `npm ci`（`npm_config_registry` 指向 npmmirror）。
...
| 重验证层 job 未就绪 | `deploy/ci/verify-enabled` 开关守卫（不建即不跑），前置逐项登记（[§8](#verify)） |
...
| 拟登记用例 | 断言要点 |
| --- | --- |
| CI 流水线激活与门禁 | ① 含代码改动的 main 推送自动建流水线且冒烟层全绿（job 清单含 `backend-typecheck`、`backend-modules`、`frontend-check`、`frontend-mobile-check`、双端构建）；② 覆盖率门禁生效：后端 < 70%（本地以 `--cov-fail-under` 复现）/ 前端 < 70%（本地改阈值复现）时失败；③ 模块注册清单校验：`uv run python -m ops.check_modules` 通过（退出码 0），构造冲突时退出码 1；④ 免跑层：纯业务文档推送不建流水线、基座路径改动触发 `base-integrity`；⑤ 重验证层开关守卫：无 `deploy/ci/verify-enabled` 时重验证 job 不出现 |
...
# 前端双端
cd ../frontend        && npm run lint && npm run test:cov
cd ../frontend-mobile && npm run lint && npm run test:cov
# 基座文档自检（须在 bms 仓库根）
...
2. **前端覆盖率配置**：双端 `vitest.config.ts` 增 coverage 段；本地 `npm run test:cov` 双端达标（≥ 70%）。
3. **CI job**：`.gitlab-ci.yml` 新增 `backend-modules`（lint）与 `frontend-mobile-check`（test）；`frontend-check` / `frontend-mobile-check` 改跑 `npm run test:cov` + artifacts；`backend-test` 注释口径对齐（核心 80% 前置）。
...
10. **提交**：CI 与前端配置（`feat(ci)` / `chore(frontend)`）与文档（`docs(项目)`）分开提交；推送走两级确认。
...
| # | 事项 | 定稿口径 | 落点 |
| --- | --- | --- | --- |
| 1 | 前端覆盖率采集口径 | 骨架期＝被测试导入文件；全量 `src/**` 随阶段四扩面 | [§6.2](#cov-frontend) |
| 2 | 前端覆盖率阈值 | 双端 70%（statements / branches / functions / lines），由 `vitest.config.ts` 自判阻断 | [§6.2](#cov-frontend) |
| 3 | 移动端口禁 | 新增 `frontend-mobile-check`（ESLint + Vitest），与 `frontend-check` 对称 | [§5](#smoke) |
...
| 9 | 重验证层开关 | 本任务**不创建** `deploy/ci/verify-enabled`；逐项登记激活前置 | [§8](#verify) |
...
| 16 | 实施结果（2026-09-14） | 无设计偏差：10 个冒烟层 job 全绿（流水线 170，总时长 **49 秒**），新增 `backend-modules` / `frontend-mobile-check` 生效，前端双端覆盖率阈值 70% 与报告 artifacts 生效；Kiwi 66 登记；实施中另修两处镜像问题（`libatomic1`、apt 清华源）并同步《开发部署规划》《测试规范》口径；Allure→Kiwi 口径已探明（`kiwitcms-pytest-plugin` + `--kiwitcms` + 凭据变量，执行时直写平台），接线随开关登记遗留 | [§12](#steps) / [§15](#risk) |
