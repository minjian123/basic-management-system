...
| `integration-mysql` / `integration-postgres` / `integration-dm8` | 阶段二 后端基座（Alembic 迁移 + 真库集成用例；当前仅 Redis 集成用例，未配 `BMS_TEST_REDIS_URL` 时跳过） | 随阶段二（04-2 占位口径） |
