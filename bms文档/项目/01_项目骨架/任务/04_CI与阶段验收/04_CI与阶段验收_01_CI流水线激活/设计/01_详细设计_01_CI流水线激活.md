...
| 前端覆盖率 | `apps/desktop` / `frontend-mobile` 各有 `vitest.config.ts`，**无 coverage 配置**；CI 跑 `npm run test`（`test:cov` 脚本闲置）；本地实测双端 4 spec / 18 用例全过、汇总 100% | 双端接入 v8 coverage + 阈值 70% 阻断；CI 改跑 `test:cov`，报告归档 | 修改（2 配置 + 2 job） |
...
| `frontend-check` | test | `ci-frontend:node22` | `apps/desktop/package.json` | `cp -a /opt/ci/apps/desktop/node_modules apps/desktop/node_modules` → `cd apps/desktop && npm run lint && npm run test:cov` | ESLint 有告警（`--max-warnings 0`）或用例失败或覆盖率 < 70% 即红 |
...
| `frontend-build` | build | `ci-frontend:node22` | `apps/desktop/package.json` | `npm run build`（`vue-tsc -b && vite build`） | 构建失败即红 |
...
- **标签口径**：`TAG = sha256(backend/uv.lock + apps/desktop/package-lock.json + frontend-mobile/package-lock.json + deploy/ci/Dockerfile.backend + deploy/ci/Dockerfile.apps/desktop)[:12]`，写入镜像 label `bms.lock-hash`；本地已存在同哈希镜像则跳过（构建与消费共用宿主 docker daemon，**本流水线即用新镜像**）。
...
- **前端镜像**（`ci-frontend:node22`）：`node:22-slim` → 复制 `apps/desktop/` 与 `frontend-mobile/` 的 `package.json`/`package-lock.json` → `npm ci`（`npm_config_registry` 指向 npmmirror）。
