...
- **报告归档**：`coverage/` 作 artifacts（`expire_in: 30 days`，`when: always`），便于失败回溯；`text` reporter 供作业日志直读。
...
**先登记 Kiwi TCMS 用例取得编号，再实施并在任务测试记录中回填编号**。按「一任务一条用例」约定，本任务登记 **1 条**（编号以平台为准，登记后回填）：
