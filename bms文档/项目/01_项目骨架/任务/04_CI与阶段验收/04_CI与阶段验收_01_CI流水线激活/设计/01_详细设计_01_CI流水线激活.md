...
| `desktop-check` | test | `ci-frontend:node22` | `apps/desktop/package.json` | `cp -a /opt/ci/apps/desktop/node_modules apps/desktop/node_modules` → `cd apps/desktop && npm run lint && npm run test:cov` | ESLint 有告警（`--max-warnings 0`）或用例失败或覆盖率 < 70% 即红 |
| **`mobile-check`** | test | `ci-frontend:node22` | `frontend-mobile/package.json` | 同构（复制 `/opt/ci/frontend/apps/mobile/node_modules`） | 同上（移动端） |
| `desktop-build` | build | `ci-frontend:node22` | `apps/desktop/package.json` | `npm run build`（`vue-tsc -b && vite build`） | 构建失败即红 |
| `mobile-build` | build | `ci-frontend:node22` | `frontend-mobile/package.json` | `npm run build` | 同上（移动端） |
...
3. **CI job**：`.gitlab-ci.yml` 新增 `backend-modules`（lint）与 `mobile-check`（test）；`desktop-check` / `mobile-check` 改跑 `npm run test:cov` + artifacts；`backend-test` 注释口径对齐（核心 80% 前置）。
