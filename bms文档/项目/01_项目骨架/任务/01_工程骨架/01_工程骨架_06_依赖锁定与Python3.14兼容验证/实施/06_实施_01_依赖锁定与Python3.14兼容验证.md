...
- 未触发回退，故《项目规划说明》2.1/17 与《开发部署规划》第 10 节不改动。
...
| 3 | Celery / SpiffWorkflow 不在锁定依赖 | 规划为阶段二占位 | 用 `uv run --with` 临时环境冒烟，不动 `uv.lock` | 设计 §3 |
...
- Celery / SpiffWorkflow 仅安装冒烟，**不代表阶段二功能验收**。
