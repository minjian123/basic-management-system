...
- **依据**：需求 [01-6](../../../../需求/01_需求_工程骨架.md#r01-6)、《总体项目规划》第 10 节风险表、《开发部署规划》第 10 节、《项目规划说明》2.1/3.1/17。
...
| Celery / SpiffWorkflow 属阶段二占位未安装 | 需按需求做**安装冒烟**（不写入锁定依赖） |
...
- 回退未发生时，《项目规划说明》2.1/17 与《开发部署规划》第 10 节不改动（口径未变）。
...
- Celery / SpiffWorkflow 仅安装冒烟，**不代表阶段二功能验收**（接入时再按模块验证）。
