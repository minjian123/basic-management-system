...
**范围**：Python 3.14.4 下全量运行/开发依赖的安装与 import 冒烟、Celery/SpiffWorkflow 临时环境安装冒烟、dmPython 连接达梦 `SELECT 1`、锁文件复现与全量回归。不含：SQLAlchemy 三方言接入（02-5）、Celery/SpiffWorkflow 功能验收（后续）、free-threading 构建。
