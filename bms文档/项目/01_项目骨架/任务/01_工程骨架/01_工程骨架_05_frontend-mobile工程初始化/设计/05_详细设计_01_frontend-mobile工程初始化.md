...
- **范围**：仅 frontend-mobile 工程与其冒烟验证；业务页面与移动端 UI 细节不做（默认页占位，随后续细化）；认证与免登（企业微信/钉钉）随对应阶段。
...
| `router/routes.ts` | 路由实例与壳路由；动态路由骨架（阶段六按权限注入） | 不硬编码业务路由表 |
