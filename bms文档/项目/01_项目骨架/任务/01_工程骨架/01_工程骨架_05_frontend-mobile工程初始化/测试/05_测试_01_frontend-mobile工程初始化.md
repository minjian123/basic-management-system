...
**范围**：默认页标题与 backend 连通状态（成功/降级两态）、工程门禁（lint/vue-tsc/build）、开发代理连通（`/info` 重写）。不含：375×667 视口与安全区像素级断言（随 04-1 E2E）、移动端业务交互（后续阶段）、免登（阶段六）。
...
- **Kiwi 标记对位（2026-09-13）**：本任务四个测试文件原复用桌面端编号（`home.spec.ts` 19 / `base.spec.ts` 21 / `utils.spec.ts` 23 / `http.spec.ts` 25），与平台移动端题面（**20 / 22 / 24 / 26**）不符；已按「编号以平台为准」口径改号（本记录第 1/2 节所列编号本就是平台口径，改后代码与记录一致，桌面端保持 19 / 21 / 23 / 25 不变）。
- 375×667 视口与安全区断言随 04-1 Playwright E2E（04-1 e2e 范围已补该断言项）。
- 覆盖率口径已补入 04-1 门禁清单（全量 `src/**` + 阈值定案）；当前双端 18 用例、覆盖率 100%。
