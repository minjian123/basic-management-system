...
[文档首页](../../../../../../文档首页.md) › [01 任务文档](../01_工程骨架_01_monorepo仓库骨架.md) › 01 实施记录　|　[← 01 工程骨架](../../01_工程骨架.md)　[详细设计 →](../设计/01_详细设计_01_monorepo仓库根骨架.md)
...
| 详细设计 | [01 详细设计_01](../设计/01_详细设计_01_monorepo仓库根骨架.md) |
| 实施日期 | 2026-09-10 |
| 实施人 | minjian |
| 实施环境 | 开发机（Ubuntu）；Python 3.14.4（uv 0.12.7）；Node 22.23.2（nvm 管理）；pnpm 11.x；依赖镜像：阿里云 PyPI、淘宝 npmmirror |
...
| 3 | apps/desktop 最小占位 | `pnpm run build` 与 dev 5173 通过 |
| 4 | frontend-mobile 最小占位 | `pnpm run build` 与 dev 5174 通过 |
...
```bash
pnpm create vite@latest apps/desktop -- --template vue-ts
```
...
```bash
pnpm install   # npmmirror 源，added 48 packages in 5s
pnpm run build # ✓ 11 modules transformed；dist/index.html 0.39 kB + js 59.88 kB
pnpm run dev   # 5173 返回占位页 HTML（title=BMS PC 管理端）
```

### 3.4 frontend-mobile 占位

以调整后的 `apps/desktop/` 为基复制，差异：`name`=`bms-frontend-mobile`、端口 `5174`、标题与 `App.vue` 文案改移动端。`pnpm install`（48 包）、`pnpm run build` 通过、dev 5174 可访问占位页。
...
按 [§5 验证结果](#verify) 逐项验证后，分两条提交：`e470985 docs(设计)`（定案修正）、`2390527 feat(项目)`（占位与根文件，39 文件）。另按设计 §10 步骤 8 补做 fresh clone 模拟：本地克隆 → `uv sync` / `pnpm install --frozen-lockfile` → 三工程启动，全部通过（见 §5）；随后连同实施记录提交一并推送远端（至 `1dfeaf8`）。
...
| 4 | 停止 dev 后台任务后 5173/5174 仍被占用 | `pnpm run dev` 经 npm 包装，停止任务只杀了主进程，vite 子进程残留 | 按端口定位 PID 精确 kill，确认端口释放 | 本记录 |
...
| PC 前端可启动 | `pnpm run build` + dev 5173 | 通过（build 11 modules；dev 返回占位页） |
| 移动端可启动 | `pnpm run build` + dev 5174 | 通过（同上） |
| README 快速启动照做可通 | 按「快速启动」三工程命令逐条执行 | 通过 |
| fresh clone 照做可通 | 本地克隆 → `uv sync` / `pnpm install --frozen-lockfile` → 三工程启动 | 通过（`/healthz` 200；5173/5174 返回占位页；构建产物哈希与工作区一致） |
...
- 首次安装用 `pnpm install` 生成锁文件；README 快速启动按设计写 `pnpm install --frozen-lockfile`（锁文件就位后可用）。
