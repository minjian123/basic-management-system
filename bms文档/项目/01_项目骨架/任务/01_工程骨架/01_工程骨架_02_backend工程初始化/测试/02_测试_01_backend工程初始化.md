...
**范围**：本任务「工程初始化 + 最小可运行」——应用工厂基线、根路由、`/healthz` 存活与基线端点可用性。不含：`/readyz` 依赖检查（03-3）、租户拓扑与模块注册（02-14 ~ 02-16）、前端工程（01-04/05）。
...
口径：本任务为任务级覆盖率快照；覆盖率门禁（核心 ≥ 80% / 整体 ≥ 70%）随 04-1 接入 CI，`pytest-cov` 机制已就位。
