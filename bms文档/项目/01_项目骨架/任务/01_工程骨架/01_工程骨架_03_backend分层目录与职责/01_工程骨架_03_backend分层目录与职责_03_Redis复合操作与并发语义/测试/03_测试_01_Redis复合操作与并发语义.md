...
**范围**：`replace_if_equal`（命中/未命中/缺失）、`get_and_remove`（命中/缺失）、`update_atomic`（成功/缺失/重试/超限）、`get_locked`（提交/缺失）；真实 Redis 上的 Lua 原子与 WATCH 行为。不含：Redis Cluster、阶段十四压测、02-3 异常体系替换后的响应口径。
...
- Redis Cluster 与压测口径随阶段十四；02-3 接管 `ConcurrentConflictError` 后如需调整断言在设计修订同步。
