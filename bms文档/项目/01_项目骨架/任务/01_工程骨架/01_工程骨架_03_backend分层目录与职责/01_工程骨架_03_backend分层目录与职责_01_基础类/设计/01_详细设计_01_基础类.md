...
- **依据**：需求 [01-3](../../../../../需求/01_需求_工程骨架.md#r01-3) 第 4/5 条、《[后端开发规范](../../../../../../../规范/后端开发规范.md)》「目录与分层职责」节「模块组织」节「后端基座体系」节、《[命名规范](../../../../../../../规范/命名规范.md)》第 3/6 节、2026-09-10 设计调整确认（根基类 + 集合基类）。
...
├── core/collections.py                    # 新增：BaseSorted + SortedList/SortedDict/SortedSet
...
1. 取字段：dataclass 优先 `dataclasses.fields`（声明字段序，失败回退）；其他对象取 `__dict__` 中不以 `_` 开头的公开项；
...
| 类 | 继承 | 有序口径 |
| --- | --- | --- |
| `BaseSorted[T]` | `BaseObject, ABC` | 公共方法落点（不直接实例化） |
| `SortedList[T]` | `BaseSorted` + `sortedcontainers.SortedList` | 按元素（或构造 `key=`）升序插入 |
| `SortedDict[K, V]` | `BaseSorted` + `sortedcontainers.SortedDict` | 按键升序 |
| `SortedSet[T]` | `BaseSorted` + `sortedcontainers.SortedSet` | 按元素升序去重 |

**公共方法**（`BaseSorted` 统一提供，三类继承）：
...
| 9 | 集合基类 | 新增 `core/collections.py`：`BaseSorted` + `SortedList/SortedDict/SortedSet`（sortedcontainers 插入即有序） |
