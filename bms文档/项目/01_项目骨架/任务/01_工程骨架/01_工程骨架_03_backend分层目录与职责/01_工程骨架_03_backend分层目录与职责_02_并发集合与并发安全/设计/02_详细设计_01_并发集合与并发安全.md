...
- **依据**：需求 [01-3](../../../../../需求/01_需求_工程骨架.md#r01-3) 第 6 条、《[后端开发规范](../../../../../../../规范/后端开发规范.md)》「异步与并发」节、《[命名规范](../../../../../../../规范/命名规范.md)》Redis key 约定、《[架构设计 · 总体架构](../../../../../../../设计/架构设计/03_架构设计_总体架构.md)》《[架构设计 · 多租户路由](../../../../../../../设计/架构设计/11_架构设计_子系统_多租户路由.md)》《[架构设计 · 性能与安全](../../../../../../../设计/架构设计/30_架构设计_性能与安全.md)》、2026-09-10 设计确认。
...
- 写一致性核心三操作（`set` / `set_if_absent` / `delete`）以 **Lua** 原子保证 Hash 与 ZSET 一致（`EVALSHA` + 脚本摘要缓存，脚本随包管理）；
- 读-改-写类复合操作（`replace_if_equal` / `get_and_remove` / `update_atomic` / `get_locked`）由 **01-3-3 补全**（Lua CAS + WATCH 乐观重试）；纯批量操作用 **pipeline** 降 RTT；
- 乐观并发备选 `WATCH/MULTI`（冲突重试上限见 01-3-3 设计）。
...
- **验收口径**：微基准作为调参依据（分片数、读写比、锁策略），硬指标随阶段十一/十二 **500 并发 / P99 ≤ 1s** 压测验收；
...
7. 验证：`pytest` / `ruff` / `pyright` / 覆盖率 100% / 微基准报告；integration 用例随 04_02。
...
| 遍历快照一致 | 快照一致性用例 |
| Redis 有序与原子可用 | Kiwi 17（fakeredis 全绿；真实 Redis integration 随 04_02） |
...
- 业务缓存策略（TTL/穿透/雪崩）归阶段十二（缓存三防）；本任务只提供集合封装。
- 已实施（2026-09-10）：见 [实施记录 01](../实施/02_实施_01_并发集合与并发安全.md)、[测试记录 01](../测试/02_测试_01_并发集合与并发安全.md)；真实 Redis integration 随 04_02。
