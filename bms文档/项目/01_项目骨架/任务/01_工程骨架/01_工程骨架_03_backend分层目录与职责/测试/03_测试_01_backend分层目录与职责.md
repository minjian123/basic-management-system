...
**范围**：本任务「分层骨架 + demo 走通」——分层目录与职责 docstring 的可用性证明、`/healthz` 迁移回归、根路由回归、demo 四件套（api→service→repository）全量 CRUD 与失败分支。不含：数据库落库（03 域）、统一响应/异常体细化（02-3）、认证与权限（阶段三）。
...
- Allure 报告与 Kiwi 结果导入留 05-4；认证/权限/越权类用例待阶段三。
