...
**范围**：默认页标题与 backend 应用信息渲染（成功/降级两态）、工程门禁（lint/vue-tsc/build）、开发代理链路（`/info` 重写、`/api`、`/healthz`）。不含：真实浏览器 E2E 与视觉断言（随 05-1 重验证层）、业务接口与权限（阶段二）、OpenAPI 生成（随 05 域）。
...
- 覆盖率当前口径为「已导入文件」，全量 `src/**` 采集与阈值随流水线门禁任务。
- Element Plus 全量引入构建告警（>500 kB）作为优化项登记，不影响验收。
