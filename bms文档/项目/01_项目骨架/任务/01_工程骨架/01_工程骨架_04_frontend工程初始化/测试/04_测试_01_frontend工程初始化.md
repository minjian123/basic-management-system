...
**范围**：默认页标题与 backend 应用信息渲染（成功/降级两态）、工程门禁（lint/vue-tsc/build）、开发代理链路（`/info` 重写、`/api`、`/healthz`）。不含：真实浏览器 E2E 与视觉断言（随 04-1 重验证层）、业务接口与权限（阶段六）、OpenAPI 生成（随 04 域）。
...
- 真实浏览器链路与 375×667 之外的视觉断言随 04-1 Playwright E2E。
- 覆盖率当前口径为「已导入文件」，全量 `src/**` 采集与阈值已补入 04-1 门禁清单。
