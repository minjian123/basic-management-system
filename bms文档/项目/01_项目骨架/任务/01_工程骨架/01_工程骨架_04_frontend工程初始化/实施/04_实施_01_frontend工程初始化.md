# apps/desktop 工程初始化实施记录

> 项目骨架 · 01 工程骨架 · 子任务 04 · 实施记录 01

[文档首页](../../../../../../文档首页.md) › [04 apps/desktop 工程初始化](../01_工程骨架_04_frontend工程初始化.md) › 01 实施记录　|　[详细设计 →](../设计/04_详细设计_01_frontend工程初始化.md)
...
- 开发依赖：eslint 10 + eslint-plugin-vue 10 + typescript-eslint 8 + @eslint/js + @vue/eslint-config-prettier + prettier 3、vitest 5 + @vue/test-utils + jsdom + @vitest/coverage-v8、sass、openapi-typescript 7.13、vue-eslint-parser；版本范围写入 package.json，实际版本由 pnpm-lock.yaml 锁定并提交。
...
`pnpm install --frozen-lockfile` → `lint` → `test`（Vitest 2 条）→ `build`（vue-tsc -b + vite build）；起 backend + dev 后用 curl 实测代理链路；随后形成本批提交（等用户指令）。

## 4. 问题与处置 <a id="issues"></a>

| # | 问题 / 现象 | 原因 | 处置 | 落点 |
| --- | --- | --- | --- | --- |
| 1 | `pnpm install` ERESOLVE：openapi-typescript@7 声明 peer `typescript ^5` | 项目为 TypeScript 6（create-vue 新基线） | `.npmrc` 设 `legacy-peer-deps=true`（附注释，待其支持 TS6 后移除）；`pnpm install --frozen-lockfile` 可复现 | `.npmrc` |
| 2 | ESLint 报找不到 `vue-eslint-parser` | legacy-peer-deps 模式不自动安装 peer | 两工程显式 `pnpm add -D vue-eslint-parser` | `package.json` |
...
| 验证点（完成标准） | 方法 | 结果 |
| --- | --- | --- |
| `pnpm install --frozen-lockfile` 可复现 | 命令 | 通过（npmmirror，锁文件生效） |
| 代理与响应解析链路 | 起 backend + `pnpm run dev` 后 curl | 通过（见下） |
| `pnpm run lint` | 命令 | 通过（--max-warnings 0） |
| `pnpm run test` | 命令 | 通过（2 passed，Kiwi 19） |
| `pnpm run build` / `vue-tsc` | 命令 | 通过 |
...
- `openapi-typescript` peer 声明滞后（^5 vs 项目 TS 6）：**已评估**——该包运行时不依赖 TypeScript，`pnpm run gen:api` 在 TS6 下实测可用（对本地 OpenAPI 生成 396 行，42ms）；维持 `legacy-peer-deps` 并注明，待上游支持后移除。
- 页面渲染断言当前由 Vitest mock 覆盖；真实浏览器链路（含视觉/视口）随 04-1 Playwright E2E。
- 401 刷新、token 持久化、动态路由注入留阶段六（代码内 TODO 标注）。
