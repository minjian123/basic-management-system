...
- **范围**：仅 apps/desktop 工程与其冒烟验证；业务页面/UI 布局不做（默认页占位）；OpenAPI 类型生成为占位（随 04 域契约接入）；认证与 401 处理留 TODO（后续）。
...
| `vue-i18n` | `^11` | 国际化 |
| `socket.io-client` | `^4` | 实时通道预留（阶段六接入） |
...
| `stores/useUserStore.ts` | 用户/token 占位（token 仅内存） | 不持久化 token 到 localStorage（阶段六定案） |
