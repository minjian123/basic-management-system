...
level = "DEBUG"
format = "console"
# 慢请求阈值（dev 严：便于发现性能问题；2026-09-14 03-2 日志体系回写）
slow_request_ms = 500
...
level = "INFO"
format = "json"
# 慢请求阈值（test 基线；2026-09-14 03-2 日志体系回写）
slow_request_ms = 1000
...
level = "WARNING"
format = "json"
# 慢请求阈值（prod 松：避免偶发抖动刷 WARNING；2026-09-14 03-2 日志体系回写）
...
| health | check_timeout_ms / total_timeout_ms | 否 | `2000 / 5000` | `BMS_HEALTH__CHECK_TIMEOUT_MS` / `BMS_HEALTH__TOTAL_TIMEOUT_MS` |
...
| 15 | 慢请求阈值三环境（2026-09-14，03-2 日志体系实施期回写） | `config.{env}.toml` 补 `[log].slow_request_ms` = 500 / 1000 / 2000，落实「dev 严、prod 松」 | §4.4 |
| 16 | `[health]` 分区（2026-09-14，03-3 健康检查实施期回写） | `config.toml` 新增 `[health]`（`check_timeout_ms` / `total_timeout_ms`，毫秒；默认 2000 / 5000），三环境共用基线；`HealthSettings` 并入 `Settings`，两键列入 `.env.example` | §4.3 |
