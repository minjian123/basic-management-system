...
| 雪花 ID | `app/core/id.py` | 不变（`initial_worker_id()` 保留 `BMS_WORKER_ID` 导入期兜底，启动后由 `configure_id_generator` 以配置为准覆盖） |
...
| 风险 / 开放项 | 说明 | 处置 |
| --- | --- | --- |
| `BMS_WORKER_ID` 与 `BMS_APP__WORKER_ID` 并存 | `id.py` 导入期读 `BMS_WORKER_ID`，配置为 `BMS_APP__WORKER_ID` | 部署统一改用 `BMS_APP__WORKER_ID`；启动后 `configure_id_generator` 以配置为准；后续阶段可移除旧名 |
