...
| --- | --- | --- | --- |
| `RedisHealthCheck` | `redis` | `redis.asyncio` 客户端 `PING` | 本任务（后续） |
...
- 鉴权豁免：探针不在 Bearer 校验范围（与登录 / 刷新同列豁免清单，认证阶段落地时生效）；限流排除清单预留（后续）；请求日志排除已在 03-2 落地（不记任何级别）。
...
**先登记 Kiwi TCMS 用例取得编号，再写代码并标注 `@pytest.mark.kiwi_id(N)`**。本任务新增 **2 条**用例（已登记 **Kiwi 64 / Kiwi 65**，2026-09-14）；Kiwi 45（基座契约）随字段更名更新断言后**保留回归**；Kiwi 2（`/healthz`）保留。
...
| 鉴权豁免清单、限流排除清单 | 认证（后续）/ 限流阶段落地时按探针名单排除 |
...
| # | 事项 | 定稿口径 | 落点 |
| --- | --- | --- | --- |
| 1 | 检查项启用范围 | redis + database 同启（platform 库 `SELECT 1`）；minio 随后续 | §6.1 |
...
| 10 | Kiwi 方案 | 新增 2 条（Kiwi 64 探针契约 / Kiwi 65 聚合与启动态，2026-09-14 平台登记）；Kiwi 45 断言随改名更新并保留；Kiwi 2 保留 | §9 |
...
| 15 | 实施期偏差（2026-09-14） | 无设计偏差：按设计落地；整体超时捕获以 `contextlib.suppress(TimeoutError)` 实现（等价设计伪码 try/except），`redis-py` 部分未知类型（`from_url` / `ping`）配 pyright 定向忽略 | §5.2 / §6.2 |
