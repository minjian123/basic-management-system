...
2. **4 端口 Null**：新增 `NullCacheRegion` / `NullAuditCapturer` / `NullTask` / `NullEventPublisher`（组合轨自动登记；事件域仅发布轨，消费轨随阶段八）。
...
- **遗留**：事件域消费轨 Null（`EventConsumer`）随阶段八；四能力真实实现随对应阶段；MinIO 端到端上传下载 / 桶策略随阶段八文件管理。
