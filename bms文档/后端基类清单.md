...
| `BaseScopedRepository[T]` | `repositories/base_scoped_repository.py` | **作用域过滤中间层**：软删除 + 数据范围条件（`_scope_conditions` / `_matches_scope`） |
...
| 数据范围注入 | `scope/base.py` | `DataScope(BaseCapability)`（`read_predicate` / `allow_write`）+ `ScopeCondition` + `NullDataScope`；`BaseRepository._apply_data_scope` 钩子、`BaseScopedRepository` 落过滤 |
