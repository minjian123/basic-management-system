...
| `BaseFactory` | `core/factory.py` | 工厂基类（继承 `BasePluggable`）：一切工厂类之根——创建入口标识（`key` / `plugin_name` / `contract_version`）+ 参数校验（`validate` 默认空实现）+ 抽象创建（`create`）+ 产出物释放登记（经 `ResourceManager`）；各域工厂基类承公共创建逻辑，工厂只创建与装配、不承载业务判定 | **待实施**（需求 02-54，阶段一补做窗口**首个**） |
...
| `BaseAsyncResource` | `core/capability.py` | 异步资源生命周期（`aclose()` + `async with`） | `EngineRegistry` / 工厂链（迁移后）等继承 |
...
| 基类 / 能力 | 代码位置 | 职责 | 状态 |
| --- | --- | --- | --- |
| `EngineFactory` | `db/engine.py` | 按 `db_key` 创建 / 缓存 `AsyncEngine`（连接池、`pool_pre_ping`、`aclose`） | 已交付（SQLite 可跑）；**改挂工厂链待实施（需求 02-54）** |
...
| 验证码渠道（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 滑块 / 短信渠道契约（挑战类型 / 轨迹凭证校验 / 短信发送与一次性校验 / 场景枚举与策略）：**扩展 `02-31` 验证码域**，`CaptchaChallenge` 容纳挑战类型、`NullCaptcha` 扩展为恒定通过——**接口占位待实施（需求 02-42）** |
| 组织主数据查询（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 统一主数据查询出口（用户 / 岗位 / 部门树 / 按标识批量回显）：`BaseOrgDataSource` + `NullOrgDataSource`；数据范围过滤与脱敏由实现侧接入——**接口占位待实施（需求 02-43）** |
| 字典取数（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 按类型取数（版本比对、级联、按值子集、批量合并、翻译）：`BaseDictSource` + `NullDictSource`——**接口占位待实施（需求 02-44）** |
| 对象存储扩展（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 分片上传与秒传判定：`BaseMultipartUpload`（`initiate` / `upload_part` / `complete` / `abort` / `check`）+ `NullMultipartUpload`；**扩展 `02-17` 对象存储域**，分片状态机归文件管理模块上层——**接口占位待实施（需求 02-45）** |
| 通知中心（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 列表 / 详情 / 未读计数 / 已读（单条 / 批量 / 全部）/ 删除：`BaseNotificationCenter` + `NullNotificationCenter`；未读数以服务端为唯一口径——**接口占位待实施（需求 02-46）** |
| 列表查询与偏好（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 列表查询契约（多列排序 / 分页 / 筛选序列化）+ 查询方案三级：`BaseQuerySchemeStore` + `NullQuerySchemeStore`；列偏好键 `list.{form_key}` 经 02-50——**接口占位待实施（需求 02-47）** |
| 全局检索（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 多域检索 / 审计日志检索 / 文件内容检索 + 降级字段：`BaseGlobalSearch` + `NullGlobalSearch`；**扩展 `02-19` 检索域**，可检索域按权限并集——**接口占位待实施（需求 02-48）** |
| AI 对话流式（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 流式对话（增量事件序列 / 中断）与会话模型：`BaseChatStream` + `NullChatStream`；**扩展 `02-18` LLM 适配域**，会话以审计数据为派生视图——**接口占位待实施（需求 02-49）** |
| 用户偏好（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 通用偏好读写（`get` / `set` / `reset`，键规范 `{域}.{键}`）：`BasePreferenceStore` + `NullPreferenceStore`；表 `sys_user_preference`——**接口占位待实施（需求 02-50）** |
| 租户自助（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 我的租户 / 切换 / 品牌：`BaseTenantSelfService` + `NullTenantSelfService`；与 `02-15` 解析链协作——**接口占位待实施（需求 02-51）** |
| 打印模板（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 导出 PDF / 批量打印：`BasePrintExporter` + `NullPrintExporter`；产物经 02-17 对象存储——**接口占位待实施（需求 02-52）** |
| 图标与代码校验（横切扩展，2026-09-20 补登） | 待定（详细设计定） | 图标注册表（`BaseIconRegistry` + `sys_icon`）与代码校验（`BaseCodeValidator`：SQL 测试 / 表达式校验）+ 空实现——**接口占位待实施（需求 02-53）** |
...
| `BaseFactory`（工厂基类） | `BaseFactory` | 同名同职责：一切工厂类之根（创建入口标识 / 参数校验 / 产出物释放登记）；层级 `BaseFactory → 各域工厂基类 → 具体工厂类`，**公共创建逻辑只在工厂基类维护、工厂只创建与装配**（后端待实施，需求 02-54） |
...
- **继承链**（按层分组）：
  ...
  - 异常 `BizError → BaseObject`、段位基 → `BizError`、子类 → 段位基。
   ...
   - 工厂链 `EngineFactory` / `SessionFactory` / `ApplicationFactory` / `IdGeneratorFactory` / 插件实现工厂（`LocalObjectStorageFactory` / `MinioObjectStorageFactory` / `HealthCheckRegistryFactory` / `NullMaskerFactory`）→ 各域工厂基类 → `BaseFactory` → `BasePluggable` → `BaseCapability` + `BaseAsyncResource` → `BaseObject`（工厂基类与各域工厂基类、既有工厂迁移**待实施**，需求 02-54；上行列「`EngineFactory → BaseAsyncResource`」为迁移前现状）。
