...
| 服务运行时基座 | `core/service.py`、`core/run.py` | 服务身份 `ServiceIdentity`（名称 / 版本 / 中文名，继承 `BaseObject`）+ 运行时 `ServiceRuntime`（继承 `BaseObject`；登记身份、暴露 `draining`、`start_drain()` 幂等摘流）+ `attach_service` 助手（包声明 + `[app].service` 覆盖解析、日志上下文绑定、回落就绪）；`bind_service_identity` 经 structlog 全局上下文注入 `service` / `service_version`；`core/run.py` 提供 `ServiceServer`（uvicorn 子类，信号到达先摘流再优雅收尾）/ `build_server`（读 `[server]` 配置）/ `run_service`（`python -m bms_<服务名>` 启动入口） | 已交付（02_02，2026-09-22） |
| 服务应用装配基座 | `application.py` | `service_lifespan`（共享应用生命周期：启动校验 → 插件装配 → 开发库自动建表 → 就绪标记；关闭取消就绪并释放资源）+ `BaseServiceApplicationFactory(BaseApplicationFactory)`（服务应用工厂基座：服务身份 / 中间件 / 异常处理 / 状态与前缀工厂 / 引擎注册表 / 租户源 / 插件装配 / 探针统一挂载 / 根路由；`service_routers()` / `prepare_settings()` / `configure_service()` 钩子）。**置包根（组合根）**：装配触达 `api` / `services`，置包根以满足 `core → api/services` 反向依赖约束；各服务只声明身份与业务路由 | 已交付（02_03，2026-09-22） |
...
| 模块注册制 | `services/module_registry.py`、`repositories/module_repository.py` | `ModuleRegistry` / `ModuleRecord`（继承 `BaseObject`；服务目录与契约登记单一来源 `SERVICE_CATALOG`：注册要素唯一性与格式校验、归属分组 / 建设批次 / 服务与契约版本登记，`ops/check_modules.py` 复用其规则）；`ModuleRepository`（`BaseDbRepository[SysModule]` 只读查询：按分组 / 状态 / 服务维度） | 已交付（阶段三；服务目录升格 03_01，2026-09-22） |
