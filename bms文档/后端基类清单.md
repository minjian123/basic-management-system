...
| 基类 / 能力 | 代码位置 | 职责 | 状态 |
| --- | --- | --- | --- |
| `EngineFactory` | `db/engine.py` | 按 `db_key` 创建 / 缓存 `AsyncEngine`（连接池、`pool_pre_ping`、`aclose`） | 已交付（SQLite 可跑）；**已挂工厂链 `EngineFactory → BaseDbFactory → BaseFactory`（02-54，2026-09-20）** |
...
| 列表查询与偏好 | `listing/base.py`、`schemas/filters.py` | `BaseQuerySchemeStore(BasePluggable)`（`key = plugin_key = "query_scheme_store"`；异步 `list` / `get` / `save` / `delete` / `resolve_default`——三级优先级个人 > 租户 > 平台）+ `NullQuerySchemeStore`（空 / None / 原样）+ 数据契约 `QueryScheme` / `QuerySchemeScope` / `QuerySchemeTarget` / `ListPreference` / `ListColumnPreference` + 键构建 `build_list_pref_key`（`list.{form_key}`，读写经 02-50）+ 筛选契约 `FilterSpec` / `BaseFilterQuery`（操作符复用 `SCOPE_OPERATORS`）+ 提供者 `get_query_scheme_store`；占位路由 `/api/v1/query-schemes` | 已交付（02-4-20，2026-09-20；真实筛选执行 / 方案落库随列表接口 / 通用能力阶段） |
