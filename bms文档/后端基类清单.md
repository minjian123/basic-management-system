...
| 中间层 | 提供者注册表 `BaseProviderRegistry[ItemT]` | 抽象类 | `BasePluggable` | `app/core/registry.py` | 注册表公共实现：`register` 唯一性拒重 / `get` 未命中 `None` / `keys` / `values` 保序（聚合模板留各域） | 已交付（阶段三 02-2，2026-09-15；其余 3 域注册表随 03-3 收敛） |
...
| 能力域横切 | 健康检查项注册表 `BaseHealthCheck` / `BaseHealthCheckRegistry` | 抽象类 / 类 | `BaseObject` / `BaseProviderRegistry` | `app/health/`（`base.py` / `registry.py` / `checks.py` / `null.py`） | 依赖就绪：检查项契约（`name` + 异步 `check`）+ 注册表（`register` 唯一性 / `get` / `keys` / `checks` + 聚合模板 `aggregate`——并发 + 单项 / 整体两级超时）+ 结果 / 报告契约（`HealthCheckResult` / `HealthCheckReport`，`{name, ok, error}` / `{ok, checks}`）；真实注册表 `local`（`HealthCheckRegistry`，装配工厂注入超时与 `redis` / `database` 检查项）与 `null` 缺省均经插件注册表解析（`/readyz` 经聚合） | 已交付（02-2 插件化） |
...
| `BaseProviderRegistry` | `core/registry.py` | 提供者注册表中间层（继承 `BasePluggable`）：`register` 唯一性拒重 / `get` 未命中 `None` / `keys` / `values` 保序；`_provider_key` 钩子供各域 | 健康检查注册表继承（02-2）；其余 3 域注册表随 03-3 收敛 |
...
| `BaseProviderRegistry` | `core/registry.py` | 注册表公共实现（`register` 唯一性 / `get` 未命中 / `keys` / `values`），收敛 4 个域注册表与 Null 注册表 | **已交付**（02-2，2026-09-15；健康已接入，其余 3 域随 03-3） |
...
| 数据查询提供者（横切扩展） | `query/base.py`、`query/null.py` | `BaseQueryProvider(BaseObject)`（抽象 `key` + 异步 `query(params) -> QueryResult`）+ `BaseQueryProviderRegistry(BasePluggable)`（`key = "query_provider_registry"`；`register` / `get` / `keys` + `query` 聚合模板）+ `NullQueryProvider` / `NullQueryProviderRegistry`（空结果 / 固定返回）+ `QueryResult`（`rows` / `total`）/ 提供者 `get_query_provider_registry` |
...
- **阶段三基座候选**：`BasePluggable` + `core/plugin.py`（P0 必补，**已交付 2026-09-15**；40 个能力域基类改继承随 **02-1 已完成**）、`BaseProviderRegistry`（P1，**已提前交付 02-2，2026-09-15**；其余 3 域注册表随 03-3 收敛）、`BaseProvider`（P1 收敛提取，随 03-2）、契约测试基座（P1，随 03-1）、配置源与日志落点中间层（P2 暂缓）——逐项见《[阶段三需求提出清单](项目/03_后端插件化/README.md)》，落地后回写本清单与平台《架构设计 · 后端基础类体系》。
