...
- **代码路径口径（2026-09-22 工作区化）**：本清单「代码位置」为**包根相对路径**——共享基座库路径根为 `backend/libs/bms_core/src/`（`bms_core/...`），平台服务路径根为 `backend/services/platform/src/`（`bms_platform/...`）；工程结构与依赖方向见《[后端开发规范](规范/后端开发规范.md)》「目录与分层职责」节。
