...
| 基类 | 代码位置 | 职责与关键成员 |
| --- | --- | --- |
| `BaseRepository[T]` | `repositories/base_repository.py` | 仓储**契约**（异步）：CRUD 抽象 + `exists` / `list_page` / `list_cursor` / 软删硬删出口派生 + 数据源（只读标记）/ 分片路由钩子 + `_guard_version` 乐观锁转译 |
| `BaseMemoryRepository[T]` | `repositories/base_memory_repository.py` | 内存基线（异步：字典存储 + 自增 ID，继承 `BaseScopedRepository` 按作用域过滤），子类只实现同步 `_build` / `_apply`；测试替身，硬删语义（无软删字段） |
...
| `BaseDbRepository[T]` | `repositories/base_db_repository.py` | **数据库实现**（异步真实落库，继承 `BaseScopedRepository`）：会话构造注入 + `model` 类属性；CRUD / `SELECT EXISTS` 存在性 / 页码与偏移游标分页 / 基础 ORDER BY（默认 `id` 升序）；作用域条件 → SQL WHERE；写入严格白名单（拒 `id` / 审计 / `version` / `deleted_at`）；租户 `create` 注入与 `update` 禁改；`delete` 默认软删 + `soft_delete` / `hard_delete` 出口（硬删穿透软删过滤）；乐观锁冲突转 409 |
