...
### 9.1 前端镜像（命名对照，2026-09-16 登记） <a id="cross-phase-frontend"></a>

前端基础类体系与后端同构（同层同职责），命名与落点见《[前端基类清单](前端基类清单.md)》；下表登记**跨端命名对照**，供双端评审与联调对齐：

| 后端基类 | 前端对应 | 说明 |
| --- | --- | --- |
| `BasePluggable`（插件轨） | —（前端为**组合轨**，无对应） | 前端以「继承承载身份 + 片段组合承载横切」两轨分工替代 |
| `BaseCapability` | `BaseCapability` + 能力登记（`frontend/packages/core/src/mechanisms/capability.ts` + `capabilities/manifest.ts`） | 同名同职责：能力机制（声明 / 依赖登记 / 单向校验 / 开发态告警），已于 2026-09-16 交付（`02_07` S2 类化） |
| `BaseProviderRegistry` / `BaseProvider` | `BaseRegistry` / `BaseRegistryItem` + `useRegistryBase`（`src/base/provider.ts`） | **同职责不同名**：前端命名按项目口径（不加 `Provider`），已于 2026-09-16 交付 |
| `BaseError`（错误码段位） | `BaseError`（`extends Error`，`src/base/error.ts`） | 提示映射走 i18n `error.{code}`；**段位数值已对齐（2026-09-16）**：镜像平台 `10001` PARAM / `10003` CONFLICT / `10005` RATE_LIMIT / `30001` PERMISSION；前端内建保留子段 `19xxx`（`19001` 未实现） |
| —（后端无对应） | 预置 31 能力片段（`src/components/base/`） | 前端特有的正交横切能力集合（开放集合，新增即登记） |

> 双端**只对齐职责与命名口径，不共享代码**；任一侧新增机制基类时同步本表与《前端基类清单》。
