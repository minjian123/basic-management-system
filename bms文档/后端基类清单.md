...
| `build_session_factory` / `get_db` / `get_uow` | `db/session.py` | `async_sessionmaker`、请求级会话依赖与工作单元依赖（每请求独立、退出释放） | 已交付 |
...
| 事件发布 / 消费 | `events/base.py` | `EventEnvelope`（`tenant_id` / `trace_id`）+ `EventPublisher` / `EventConsumer`（→ `BaseEventWorker`：`event_type`） |
