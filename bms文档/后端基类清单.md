...
| 数据访问横切 | 异步引擎 `EngineFactory` / 会话 `get_db`·`get_read_db`·`get_write_db`·`get_uow` / 读写绑定 / 主库可用性 `PrimaryHealth` | 类 / 函数 | `BaseAsyncResource` | `app/db/engine.py`、`session.py`、`routing.py`、`health.py` | 引擎主 / 副本创建与缓存、请求级会话读写选引擎、写与事务强制主库、主库故障只读降级 | 已交付（02-01，2026-09-22；真库实测归 02-05） |
| 多租户 | 租户上下文 `TenantContext` / 引擎注册表 `EngineRegistry` | 类 | `BaseObject` / `BaseAsyncResource` | `app/db/tenant.py`、`registry.py` | 租户解析链、引擎注册与生命周期（平台常驻 / 懒加载 / LRU / 读写角色） | 已交付（引擎读写 02-01，2026-09-22；租户库键解析与 LRU 实测归 02-02） |
...
| HTTP 中间件 | `api/middleware.py` | `RequestLoggingMiddleware` / `TraceIdMiddleware` / `ReadOnlyMiddleware`（继承 `BaseObject`；请求日志 / 链路 ID 注入 / 只读标记——按 HTTP 方法设置 `read_only` 上下文，供读写分离选引擎） | 已交付（2026-09-16 审计补登记；`ReadOnlyMiddleware` 02-01，2026-09-22） |

## 8. 数据访问与多租户基座 <a id="data-access"></a>

| 基类 / 能力 | 代码位置 | 职责 | 状态 |
| --- | --- | --- | --- |
| `EngineFactory` | `db/engine.py` | 按 `(db_key, 读写角色)` 创建 / 缓存引擎：写主库、只读轮询副本（无副本回落主）、四库方言（MySQL / PostgreSQL / 达梦同步 / SQLite）、连接池按服务（`[app].service`）、`connect_args` 建连超时、`create_sync`（达梦与连通性）、`drop` / `aclose` | 已交付（02-01，2026-09-22）；**已挂工厂链 `EngineFactory → BaseDbFactory → BaseFactory`（02-54，2026-09-20）** |
| `get_db` / `get_read_db` / `get_write_db` / `get_uow` / `SessionFactory` | `db/session.py` | `async_sessionmaker`；`get_db` 按请求只读标记选引擎、`get_read_db` / `get_write_db` 显式强制、`get_uow` 绑定主库会话（写与事务强制主库）；每请求独立、退出释放；公共依赖汇总 `api/deps.py` | 已交付（02-01，2026-09-22） |
...
| `TenantContext` / 解析链 | `db/tenant.py` | 租户解析链 + `get_tenant` 依赖 + 豁免路径；未知租户 404 / 80001 | 已交付（占位；真实取数归 02-02） |
| `EngineRegistry` | `db/registry.py` | `{db_key → engine}` 注册表：平台常驻、租户懒加载、LRU、`get(read_only=...)` 读写角色、连接预算（`pool_budget_warnings` 启动告警）、并发保护 | 已交付（02-01，2026-09-22；租户库键解析 / 跨实例锁 / 预算按活跃租户数归 02-02） |
