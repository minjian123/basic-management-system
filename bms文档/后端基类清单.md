...
| 能力域横切 | 事件发布 `EventPublisher` / `NullEventPublisher` | 抽象类 / 类 | `EventPublisher → BaseEventWorker → BasePluggable` | `app/events/`（`base.py` / `null.py`） | 事件：事件类型 + 发布 / 事务发布（`event_type` / `publish` / `publish_transactional`）；占位空操作（消费轨随阶段八） | 已交付（05，2026-09-15） |
...
| 事件发布（横切扩展） | `events/base.py`、`events/null.py` | `EventPublisher → BaseEventWorker(BasePluggable)`（`key = "event"`；`event_type` / `publish` / `publish_transactional`）+ `NullEventPublisher`（空操作）+ `EventEnvelope` / 消费轨 `EventConsumer`（随阶段八）/ 提供者 `get_event_publisher` |
...
- **继承链**（按层分组）：
  ...
  - 事件发布 `NullEventPublisher` → `EventPublisher` → `BaseEventWorker` → `BasePluggable` → `BaseCapability` → `BaseObject`（消费轨随阶段八）。
