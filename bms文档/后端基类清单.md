...
| 数据访问横切 | 异步引擎 `EngineFactory` / 会话 `get_db`·`get_read_db`·`get_write_db`·`get_uow` / 读写绑定 / 主库可用性 `PrimaryHealth` | 类 / 函数 | `BaseAsyncResource` | `bms_core/db/engine.py`、`session.py`、`routing.py`、`health.py` | 引擎主 / 副本创建与缓存、请求级会话读写选引擎（**按请求租户库键路由**）、写与事务强制主库、主库故障只读降级 | 已交付（02-01，2026-09-22；租户路由 02-02，2026-09-22；真库实测归 02-05） |
