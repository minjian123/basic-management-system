...
| 模块基类 | 数据访问契约 `BaseRepository[T]` / 作用域过滤 `BaseScopedRepository[T]` / 内存基线 `BaseMemoryRepository[T]` / DB 骨架 `BaseDbRepository[T]` | 抽象类 | 根系 | `app/repositories/` | CRUD 契约 + 派生方法 + 数据源 / 分片路由；软删除 + 数据范围 + 租户条件过滤 | 已交付（DB 占位；租户条件 02-02） |
...
| `BaseScopedRepository[T]` | `repositories/base_scoped_repository.py` | **作用域过滤中间层**：软删除 + 数据范围 + **租户条件**（`_scope_conditions` / `_matches_scope`，覆盖读写；`tenant_scoped` 且上下文租户有主键时强制注入 `tenant_id` 条件；操作符 eq/ne/in/like/gt/gte/lt/lte/between/is_null/is_not_null） |
...
| `get_db` / `get_read_db` / `get_write_db` / `get_uow` / `SessionFactory` | `db/session.py` | `async_sessionmaker`；`get_db` 按请求只读标记选引擎（**请求租户上下文存在时取租户库键引擎，否则回落平台库**）、`get_read_db` / `get_write_db` 显式强制、`get_uow` 绑定主库会话（写与事务强制主库）；每请求独立、退出释放；公共依赖汇总 `api/deps.py` | 已交付（02-01，2026-09-22；租户路由 02-02，2026-09-22） |
