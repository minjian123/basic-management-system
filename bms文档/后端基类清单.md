...
| 中间层 | 插件化 `BasePluggable` | 抽象类 | `BaseCapability` | `app/core/plugin.py` | 实现可替换装配：插件键 / 实现名 / 注册与解析 / 契约版本 / 生命周期；继承自动登记与显式注册双入口 | 待交付（**阶段三首务**，见平台《架构设计 · 扩展点与插件化》） |
...
| `BasePluggable` | `core/plugin.py` | 插件化中间层（继承 `BaseCapability`）：插件键 / 实现名 / 注册与解析 / 契约版本 / 生命周期 | 能力域基类继承（推荐演进，见平台《架构设计 · 扩展点与插件化》） |
...
**示例四（插件化 → 抽中间层，本体系新增）**：在 `BaseCapability` 与能力域基类之间插 `BasePluggable`，承载「实现可替换」的装配语义（插件键 / 实现名 / 注册与解析 / 契约版本 / 生命周期），能力域基类改继承之、业务方法不变；见平台《架构设计 · 扩展点与插件化》。
...
- **阶段三基座候选**：`BasePluggable` + `core/plugin.py`（P0 必补）、`BaseProvider` / `BaseProviderRegistry` / 契约测试基座（P1 收敛提取）、配置源与日志落点中间层（P2 暂缓）——逐项见《[阶段三需求提出清单](项目/03_后端插件化/README.md)》，落地后回写本清单与平台《架构设计 · 后端基础类体系》。
