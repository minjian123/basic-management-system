...
[概要设计](01_概要设计_总览.md) › 20-开放接口管理　|　[← 19-导入导出](19_概要设计_导入导出.md)　|　[21-Webhook 管理 →](21_概要设计_Webhook管理.md)

## 1. 模块概述与职责 <a id="overview"></a>

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**开放接口管理**模块，以及「系统集成」（对外 API）、「开放接口管理模块」「核心数据表清单」「租户配额」章节；
架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)08-接口与集成 节点（对外 API、OAuth2、调用审计）。

- **模块定位**：管理第三方应用对 BMS 开放接口（/api/open）的接入：应用注册、OAuth2 Client Credentials 鉴权、scope 授权、IP 白名单、调用审计与配额控制。
- **职责边界**：本模块只管开放接口**接入侧**（鉴权/授权/审计/限流），具体开放业务接口的语义归各业务模块；出站方向的 Webhook 推送归 21-[Webhook 管理](21_概要设计_Webhook管理.md)。
...
接口前缀与统一响应约定见[《API 接口规范》](../../规范/API接口规范.md)（第 2~3 节）；本模块对外开放 `/api/open` 前缀，与内部接口隔离。
