...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**菜单管理**模块，与「权限模型」（菜单→表单→业务、按钮→动作、字段可见可编辑）、「前端页面清单与菜单机制」「初始化数据」「核心数据表清单」章节。架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)15-权限计算引擎、08-前端架构节点。
...
职责边界：菜单/表单/按钮/字段实体与业务/动作码由**平台超管**统一维护（menu / business / action 业务）；租户侧可见、可分配、不可增删；字段权限授予（sys_role_field）落[角色管理](06_概要设计_角色管理.md)（role:grant），本模块提供配置入口；菜单变更发布 `sys.form.updated` 事件，触发 AI 帮助文档自动更新草稿。
...
| 自定义图标维护 | 租户自定义图标清单（图标键 / 名称 / 分组 / 标签 / SVG 内容）的新增 / 编辑 / 启停 / 删除（删除前置引用检查） | icon:manage |
...
- 菜单/表单/字段/按钮变更（增删改、挂接调整）发布 `sys.form.updated` 事件（AI 帮助更新触发，不订阅 Webhook）
- 变更即实时：菜单树与表单元数据缓存（Redis 短 TTL）随变更失效，用户下次请求取新元数据，无需发版
- 图标键（icon key）口径：菜单 / 卡片等业务字段一律存**完整 icon key**——`el:` 平台官方 / `biz:` 业务自绘 / `custom:` 租户自定义 / `van:` 移动端；租户自定义图标的 icon key 由后端基座按 `custom:{图标键}`（图标键 kebab-case）统一派生，业务与前端不得自拼前缀
- 自定义图标为**租户库**数据（租户各自维护）：清单浏览登录即可，新增 / 编辑 / 启停 / 删除需 `icon:manage`；被菜单 / 卡片引用时先解除引用或用「停用」代替，不可直接删除；上传仅 SVG（类型白名单 + 大小上限），内容清洗（移除脚本 / 事件属性 / 外链）由后端兜底；图标不存在按通用「资源不存在」处理，模块不新增码位
- 图标来源边界：平台官方图标随代码发布（构建期清单）、业务自绘图标随代码发布与业务图标清单按需加载，二者**不落租户库表**；仅租户自定义图标落 `sys_icon`

### 2.3 流程步骤 <a id="func-flow"></a>

1. **新增菜单**：平台超管建菜单（含 path/component）→ 挂表单 → 表单挂业务码 → 配置按钮（挂动作）与字段 → 补 i18n 文案 → 发布 sys.form.updated
...
sequenceDiagram
    ...
    菜单服务->>AI 帮助: 发布事件 sys.form.updated
    Note over 平台超管: 用户访问
...
协作要点：菜单树与表单元数据是前端动态路由与侧边栏的唯一依据（前端不硬编码路由表，新增菜单/表单/按钮/字段无需发版）；sys.form.updated 事件驱动 AI 帮助文档智能维护（自动生成更新草稿并通知维护人，人工审核后发布）；菜单缓存键 `bms:{tenant}:menu:{locale}:{version}`，含 locale 维度。
...
| sys_icon | code、name、category、tags、svg、status | 【租户库】租户自定义图标（`code` kebab-case 唯一，前端 icon key = `custom:{code}`）；平台 / 业务图标随代码发布不落表 |
| sys_icon_i18n | 主表 ID、locale、翻译字段 | 自定义图标名称多语言附表，联合主键（主表 ID + locale） |
...
- sys_icon：图标键 code 唯一（复合软删除位），索引 category；sys_icon_i18n 联合主键（主表 ID + locale）
...
- 变更 → 菜单缓存失效（短 TTL）→ 用户下次请求取新元数据；sys.form.updated 事件 → AI 帮助更新草稿
...
| GET | /api/v1/icons | 自定义图标清单（按分组 / 状态 / 关键字过滤），登录即可 | —（登录用户） |
| GET / POST / PUT / DELETE | /api/v1/icons、/api/v1/icons/{code} | 自定义图标维护（详情 / 新增 / 编辑 / 启停 / 删除，删除前置引用检查） | icon:manage |
...
| icon:manage | 租户自定义图标维护（清单浏览登录即可；默认归属系统管理员） |
...
- 全部平台 MVP 页面（用户管理、角色管理、报表等）挂接完整：菜单→表单→业务、按钮→动作链路齐全（产品业务页面在产品仓库侧定义与挂接）
...
| 自定义图标管理 | PC | 租户自定义图标清单（浏览 / 搜索 / 复制 icon key / 引用计数）、新增（上传 SVG）/ 编辑 / 启停 / 删除（引用检查） |
...
- sys.form.updated 事件触发 AI 帮助更新草稿链路验证通过；元数据缓存失效即时生效
