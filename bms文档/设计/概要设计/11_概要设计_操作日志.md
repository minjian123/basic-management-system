...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**操作日志**模块，与「核心数据表清单」「[审计合规](28_概要设计_审计合规.md)」
「性能与高并发设计」「多数据源策略（分库分表）」「初始化数据」等章节；架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)
**17-审计哈希链**、**11-数据访问与分片**、**14-事件总线**、
**12-[任务调度](23_概要设计_任务调度.md)**等节点。
...
| 架构依据 | 17-审计哈希链（prev_hash/record_hash 构造、链序、校验巡检）、11-数据访问与分片（按月分片路由）、14-事件总线（RocketMQ 分区有序消费）、15-任务调度（分片表预创建/归档） |
| 相邻协作 | 认证与会话（登录审计联动）、权限引擎（log 业务三权分立审计管理员）、[归档管理](30_概要设计_归档管理.md)（90 天归档）、[全文检索](33_概要设计_全文检索.md)（阶段十六按月索引） |
...
- **哈希链与链序**：哈希链构造（`record_hash = SHA256(prev_hash + 规范化记录内容)`、分片表内成链与跨月首尾衔接）、链序保证（RocketMQ 分区有序串行消费）、物理保留与 90 天归档前链校验等机制见[《架构设计 · 审计哈希链》](../架构设计/17_架构设计_子系统_审计哈希链.md#chain)
...
4. **消费落库**：event-worker 分区有序消费 → 按架构 15 哈希链机制计算 record_hash → 写入 `sys_operation_log_yyyyMM`；异常：消费失败重试（ack 确认），持续失败告警
5. **查询**：审计管理员按条件分页查询（游标分页兜底大结果集）；日志不可修改、不可删除
6. **归档**：超 90 天按策略归档，归档前链完整性校验（机制见架构 15）
...
归档库只读查询（审计管理员）→ 合规销毁记录销毁日志。阶段十六全文检索经事件同步至 ES 按月索引。
...
（事件总线事件模型中无 operation 域独立事件）；阶段十六全文检索消费同源事件同步 ES 索引。
