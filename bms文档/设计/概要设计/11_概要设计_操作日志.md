...
| 相邻协作 | 认证与会话（登录审计联动）、权限引擎（log 业务三权分立审计管理员）、[归档管理](30_概要设计_归档管理.md)（90 天归档）、[全文检索](33_概要设计_全文检索.md)（阶段十五按月索引） |
...
- **哈希链与链序**：哈希链构造（`record_hash = SHA256(prev_hash + 规范化记录内容)`、分片表内成链与跨月首尾衔接）、链序保证（RocketMQ 分区有序串行消费）、物理保留与 90 天归档前链校验等机制见[《架构设计 · 审计哈希链》](../架构设计/15_架构设计_子系统_审计哈希链.md#chain)
...
归档库只读查询（审计管理员）→ 合规销毁记录销毁日志。阶段十五全文检索经事件同步至 ES 按月索引。
...
（事件总线事件模型中无 operation 域独立事件）；阶段十五全文检索消费同源事件同步 ES 索引。
