...
| 架构依据 | 18-审计哈希链（prev_hash/record_hash 构造、链序、校验巡检）、12-数据访问与分片（按月分片路由）、15-事件总线（RocketMQ 分区有序消费）、16-任务调度（分片表预创建/归档） |
...
- **哈希链与链序**：哈希链构造（`record_hash = SHA256(prev_hash + 规范化记录内容)`、分片表内成链与跨月首尾衔接）、链序保证（RocketMQ 分区有序串行消费）、物理保留与 90 天归档前链校验等机制见[《架构设计 · 审计哈希链》](../架构设计/18_架构设计_子系统_审计哈希链.md#chain)
