...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**字典管理**模块，与「核心数据表清单」「数据规范（多语言）」「性能与高并发设计（缓存策略）」「权限模型」「初始化数据」等章节；架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)**07-数据架构**节点（4.1 字典与[系统参数](10_概要设计_系统参数.md)管理、5 缓存架构）与 **11-数据访问与分片**节点。
...
| 架构依据 | 07-数据架构（字典与系统参数管理、缓存架构三防）、11-数据访问与分片（租户库路由）、08-接口与集成（API 规范） |
...
| 字典条目维护 | 条目增删改查：code、label、value、**color（语义色，可空）**、排序、启用/停用 | dict:manage |
| 多语言 label 维护 | 类型与条目各自关联 `_i18n` 附表，按语言维护翻译文案 | dict:manage |
| 运行时字典查询 | 按类型 + locale 返回启用条目列表，供各业务模块下拉/展示 | 登录即可（业务通用依赖） |
| 字典高级查询 | 按字典类型的属性字段/查询提供者（注册实现类）构造组合条件，取项回填或作业务筛选；查询方案个人/共享；扩展属性 schema 维护；规则见[《组件设计·字典字段》](../组件设计/06_字段类/01_组件设计_字典字段/01_组件设计_字典字段.md)第 8 节 | 登录即可（查询）；dict:query-scheme（方案管理） |
...
- **高级查询与属性隔离**：字典类型可定义扩展属性（sys_dict_attr，平台预置 / 租户自建），条目属性值存 `sys_dict_item.attr_json`；该列默认**不进入**普通运行时缓存与下拉链路，仅由高级查询接口按命中子集分页返回，属性值与数量设上限（组件设计·字典字段 8.3/8.4）
- **查询方案泛化**：`sys_query_scheme` 由「字典高级查询方案」泛化为通用**查询方案**——`target=items` 供字典高级查询取项/筛选，`target=business` 供列表页筛选方案（`field_key=form_key`，见《组件设计 · 查询筛选区》「查询方案」节）；通用接口 `/api/v1/query-schemes`，列表筛选方案的租户/共享方案维护需动作权限码 `query:scheme`（个人方案免权限码）
...
> Redis 故障时缓存失效直连 DB 并日志告警（降级协议见[架构 05](../架构设计/07_架构设计_数据架构.md) 与规划「降级」节）；Redis key 命名遵循 `bms:{租户|global}:{域}:{业务键}` 规范。
...
| sys_dict_item | id（雪花）、type_id、code、label、value、parent_id、attr_json、color、sort、status、deleted_at、审计字段、version | 字典条目，type_id 逻辑引用字典类型，parent_id 逻辑引用条目自身 id（级联联动：上级字典值过滤下级选项）；attr_json 为扩展属性值（JSON，可空，普通运行时接口不返回）；**color** 为条目的语义色（`success`/`warning`/`danger`/`info`/`primary`，可空），供状态标签取色（见[《组件设计 · 状态标签与描述列表》](../组件设计/07_展示类/03_组件设计_状态标签与描述列表/03_组件设计_状态标签与描述列表.md)「取色优先级」节） |
| sys_dict_attr | id（雪花）、type_id、attr_key、data_type、operators、widget、options、sort、status、scope、deleted_at、审计字段、version | 字典类型的扩展属性定义（高级查询可查字段）；scope=platform/tenant；平台预置、租户自建 |
...
| sys_dict_attr_i18n | dict_attr_id、locale、name | 属性名多语言，联合主键（dict_attr_id + locale） |
| sys_query_scheme | id（雪花）、scope、owner_id、target、dict_type、field_key、provider_key、conditions、params、layout、is_default、shared、status、审计字段、version | 查询方案（scope=platform/tenant/user 三级；conditions/params/layout 为 JSON）。**已泛化**：`target=items` 为字典高级查询方案（`dict_type` 填字典类型）；`target=business` 为**列表筛选方案**（`field_key=form_key`、`dict_type`/`provider_key` 为空），契约见[《组件设计 · 查询筛选区》](../组件设计/07_展示类/02_组件设计_查询筛选区/02_组件设计_查询筛选区.md)「查询方案」节 |

### 4.2 索引与存储策略 <a id="data-index"></a>

- 索引：sys_dict_type (type) 唯一索引；sys_dict_item (type_id, code) 复合唯一索引（与 deleted_at 组复合唯一索引）、sort 排序字段索引、`(type_id, parent_id, sort)` 复合索引（级联过滤）、`(type_id, label)` 复合索引（远程搜索前缀匹配）、`(type_id, value)` 复合索引（单值翻译）；sys_dict_attr `(type_id, attr_key)` 复合唯一索引、`(type_id, status, sort)`；sys_query_scheme `(scope, owner_id, dict_type)`、`(dict_type, status)`、`(target, field_key, scope, owner_id)`（泛化后列表筛选方案按表单维度查询）；`_i18n` 附表按 (主表 ID + locale) 联合主键查询
- 分片：字典为小型热配置数据，不做按月分片
- 归档：字典为在用配置数据，不进入归档流转；审计留痕走操作日志与字段级审计（sys_data_audit_log，按月分片）
- 缓存：`bms:{tenant}:dict:{locale}:{type}`，短 TTL + 随机偏移；缓存键含 locale 与租户前缀；全局版本键 `bms:{tenant}:dict:version`（字典变更 INCR）；进程内内存字典缓存（dogpile.cache 双 Region：小字典整类型 + 超大字典已查条目局部子集，按版本惰性比对）供查询接口与业务翻译共用，零网络往返；超大字典批量翻译走「局部缓存 + 一次性 IN 查询」防 N+1。**扩展属性 attr_json 不进入上述普通缓存**（避免放大 L1/Redis/localStorage），仅高级查询接口按命中子集返回
...
| GET | /dicts/{type} | 运行时字典查询（按 locale 返回启用条目；登录即可，业务通用依赖）。可选参数：`version`（客户端本地版本，一致时 items 返回 null）、`keyword`（远程搜索，模糊匹配 label/value/code）、`parent_id`（级联过滤）、`values`（逗号分隔指定 value 集合，仅返回这些条目的子集——超大字典列批量翻译用）、`limit`（条数上限，探针传 2001 判别超大字典）；响应 data 为 `{version, items, has_more, total}`；**条目字段含 `color`（语义色，可空）**，供状态标签取色（`POST /dicts/batch` 同结构） | 登录即可 |
...
| POST | /dicts/{type}/advanced-query | 高级查询统一入口：body `{target:'items'\|'business', conditions?, provider?, params?, page, size}`；条件组走通用引擎，指定 provider 走注册实现类；返回条目/业务记录分页（不返回 attr_json 之外的普通缓存字段） | 登录即可 |
| GET | /dicts/{type}/attrs | 获取字典类型的扩展属性 schema（供高级查询条件构建器渲染） | 登录即可 |
...
- 限流：接口级限流（slowapi，Redis 后端，IP/用户维度）；运行时查询接口为高频路径，单独配置较宽限流；批量接口（/dicts/batch）、远程搜索（keyword 参数）与高级查询（/dicts/{type}/advanced-query，可触发 provider 复杂查询）分别限流，防止超大字典场景被刷
...
| 参数 | 默认 | 说明 |
| --- | --- | --- |
| dict_attr_enabled | true | 扩展属性与高级查询总开关；关闭后高级查询退化为固定字段模式 |
| dict_attr_max_bytes | 1024 | 单条 attr_json 长度上限（字节），超出入库拒绝 |
| dict_attr_max_per_type | 50 | 每字典类型属性数量上限 |
...
| 端 | 页面 | 说明 |
| --- | --- | --- |
| PC 管理端 | 字典管理 | 类型列表 + 条目列表双栏布局；类型/条目 CRUD 弹窗；多语言 label 维护；启用/停用；排序；扩展属性（sys_dict_attr）维护 |
