...
| 字典高级查询 | 按字典类型的属性字段/查询提供者（注册实现类）构造组合条件，取项回填或作业务筛选；查询方案个人/共享；扩展属性 schema 维护；规则见[《组件设计·字典字段》](../组件设计/01_字段类/01_组件设计_字典字段/01_组件设计_字典字段.md)第 8 节 | 登录即可（查询）；dict:query-scheme（方案管理） |
