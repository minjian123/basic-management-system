...
| sys_dict_item | id（雪花）、type_id、code、label、value、parent_id、attr_json、sort、status、deleted_at、审计字段、version | 字典条目，type_id 逻辑引用字典类型，parent_id 逻辑引用条目自身 id（级联联动：上级字典值过滤下级选项）；attr_json 为扩展属性值（JSON，可空，普通运行时接口不返回） |
