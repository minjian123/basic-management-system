...
[《架构设计》](../架构设计/01_架构设计_总览.md)19-工作流节点、16-实时推送节点与 20-通知公告节点。
...
- **协作关系**：
  ...
  - 通知与实时推送（[架构 17](../架构设计/20_架构设计_子系统_通知公告.md)/[架构 13](../架构设计/16_架构设计_子系统_实时推送.md)）：待办生成 → 站内信 + Socket.IO（approval.todo）+ 待办角标
  - 事件总线：wf.* 事件发布，Webhook 可订阅；审批关键操作落[操作日志](11_概要设计_操作日志.md)与字段级审计
  - AI 服务（阶段十七）：智能审批辅助（RAG 参考意见）、AI 自动审批（开关默认关闭）
...
| ai.auto_execute / ai.auto_approve | false | AI 自动执行写操作/自动审批开关（阶段十七，默认关闭，后者依赖前者） |
