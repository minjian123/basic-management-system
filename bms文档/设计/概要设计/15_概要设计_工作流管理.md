...
[概要设计](01_概要设计_总览.md) › 15-工作流管理　|　[← 14-会话管理](14_概要设计_会话管理.md) · [16-通知中心 →](16_概要设计_通知中心.md)

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**工作流管理**模块与「工作流引擎」「初始化数据」
章节（BPMN 流程定义、流程实例、待办/已办、审批记录、会签/或签、流程版本管理），架构设计依据
[《架构设计》](../架构设计/01_架构设计_总览.md)21-工作流节点、18-实时推送节点与 22-通知公告节点。

## 1. 模块概述与职责 <a id="overview"></a>

工作流管理模块提供通用审批能力：BPMN 流程定义（bpmn-js 拖拽建模、条件分支可视化、版本管理）、
流程实例执行（SpiffWorkflow 引擎）、待办/已办/我发起、审批记录与会签/或签。业务模块（由产品仓库接入，biz 采购/付款审批等示例）
通过「启动流程实例 + 消费审批结果」接入，流程引擎与业务单据解耦。

- **模块定位**：BMS 通用工作流引擎的管理端与执行端，业务接入按《平台扩展接入流程》由产品仓库注册（biz 采购/付款审批等示例）
- **职责边界**：流程定义（wf_process）、实例（wf_instance）、待办（wf_task）、审批记录（wf_record）全生命周期；引擎本身不持久化，状态全部由 BMS 自建表维护
- **协作关系**：
  - 业务单据（biz 采购/付款审批，产品侧设计）：提交单据启动实例；流程结束事件回调业务侧
  - 通知与实时推送（[架构 17](../架构设计/22_架构设计_子系统_通知公告.md)/[架构 13](../架构设计/18_架构设计_子系统_实时推送.md)）：待办生成 → 站内信 + Socket.IO（approval.todo）+ 待办角标
  - 事件总线：wf.* 事件发布，Webhook 可订阅；审批关键操作落[操作日志](11_概要设计_操作日志.md)与字段级审计
  - AI 服务（后续）：智能审批辅助（RAG 参考意见）、AI 自动审批（开关默认关闭）
...
6. 全部节点完成：实例置已完成（wf.instance.finished），回调业务模块（如生成下游单据）
...
- **幂等**：审批提交必须携带 `Idempotency-Key`（Redis SETNX 前置去重 + 任务状态唯一约束兜底），防止重复点击重复推进；**前端幂等键按内容派生**（任务标识 + 动作 + 意见 + 目标，同内容同键、内容变更换键），重试与重复点击复用同键。**重复提交被幂等键拦截时按成功处理**（返回首次结果，前端提示「已提交」而不进入失败态）
- **限流**：审批提交按用户限流；流程定义保存/发布按管理员限流
- **接口补充口径**（随前端审批展示与流程建模落地明确，仅登记变更、不改本节接口小节）：审批展示需实例详情下发**结构化节点序列与当前节点**（不要求前端解析 BPMN XML 生成进度）与**定义版本快照 XML**（只读图用，**须含图形布局信息 DI**）；动作除同意 / 驳回外需明确**转办 / 委托**（`POST /wf/tasks/{task_id}/transfer`）与**撤回**（`POST /wf/instances/{id}/withdraw`，仅发起人）及「仅评论」（不推进流程）；建模侧需明确建模器支持的 **BPMN 子集**（开始 / 结束事件、用户任务、排他 / 并行网关、顺序流）与用户任务属性结构（审批人来源枚举、多人规则会签 / 或签、超时、通知）、网关出线条件与默认流载体，以及保存草稿 / 发布版本 / 历史版本接口口径与错误码（60006 / 60007）。
...
| ai.auto_execute / ai.auto_approve | false | AI 自动执行写操作/自动审批开关（阶段十九，默认关闭，后者依赖前者） |

- **种子数据**：初始业务流程模板由产品仓库提供并登记（biz 采购/付款审批等）；wf 业务码与动作码入平台库种子
...
- 业务流程模板（biz 示例）可完整流转并结束：提交 → 审批节点推进 → 实例完成
