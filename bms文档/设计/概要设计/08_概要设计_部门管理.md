...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**部门管理**模块，与「核心数据表清单」（sys_dept）、「权限模型」（主体链、动作级数据权限）「事件总线事件模型」章节。架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)13-权限计算引擎节点。
...
- dept.changed 事件：驱动全文检索主数据索引同步（阶段十六）、数据范围缓存失效
