...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**部门管理**模块，与「核心数据表清单」（sys_dept）、「权限模型」（主体链、动作级数据权限）「事件总线事件模型」章节。架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)15-权限计算引擎节点。
...
职责边界：部门不直接授权；「用户→部门→角色」收敛由权限计算引擎处理；数据范围规则配置在[角色管理](06_概要设计_角色管理.md)（role:grant），本模块提供部门数据支撑；部门变更发布 `sys.dept.changed` 事件（不订阅 Webhook）。
...
- 部门移动：校验新父部门不能是自身或其后代（防环）；移动后级联更新子树全部 ancestors 并发布 sys.dept.changed
...
2. **移动部门**：校验防环 → 更新节点 parent_id/ancestors → 级联更新子树 ancestors（单事务）→ 发布 `sys.dept.changed` → 权限版本 +1
3. **删除部门**：引用检查（子部门/用户/岗位/角色绑定）→ 无引用则软删除 → 发布 sys.dept.changed
...
sequenceDiagram
    ...
    Note over DB: 权限版本 +1
    权限计算引擎->>事件总线: 发布事件 sys.dept.changed
    Note over 管理员: 业务列表
    Note over DB: 数据范围规则 dept_id=@dept_subtree
    业务查询->>部门服务: 注入子树过滤
```

协作要点：数据范围规则按部门维度下钻（@dept_subtree 展开为子树 id 集合注入查询）；部门角色绑定收敛与权限版本失效由权限计算引擎统一处理；sys.dept.changed 事件供全文检索索引同步等消费（Webhook 不可订阅）。
...
- sys.dept.changed 事件：驱动全文检索主数据索引同步（后续）、数据范围缓存失效
...
异常处理要求：移动操作级联更新与校验同事务，失败整体回滚（防半棵树）；并发移动/删除用 version 乐观锁；停用部门不影响历史数据引用（仅退出权限收敛与范围匹配）；sys.dept.changed 发布失败不影响主事务（事件总线重试）。
...
- sys.dept.changed 事件发布与消费验证通过；字段级审计与哈希链校验通过
