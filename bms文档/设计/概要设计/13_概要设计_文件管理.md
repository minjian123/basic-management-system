...
**18-文件管理**节点（分片状态机、安全控制、在线预览与下载）与 **12-[任务调度](23_概要设计_任务调度.md)**、
**14-事件总线**节点。
...
| 架构依据 | 18-文件管理（分片状态机、安全控制、预览下载）、15-任务调度（孤儿分片清理）、14-事件总线（file.uploaded）、10-多租户路由（存储配额） |
| 相邻协作 | [租户管理](25_概要设计_租户管理.md)（storage_bytes 存储配额超限拦截）、任务调度（孤儿分片回收）、审计哈希链（上传/下载关键操作落[操作日志](11_概要设计_操作日志.md)）、[全文检索](33_概要设计_全文检索.md)（阶段十六文件内容索引） |
...
阶段十六全文检索经 file.uploaded 事件驱动文件内容解析（PDF/Word/Excel/TXT）入 ES 索引。
...
file.uploaded 供全文检索文件内容解析（阶段十六）等内部消费，不开放 Webhook 订阅。
