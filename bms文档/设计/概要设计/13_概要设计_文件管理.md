...
**20-文件管理**节点（分片状态机、安全控制、在线预览与下载）与 **12-[任务调度](23_概要设计_任务调度.md)**、
**16-事件总线**节点。
...
| 架构依据 | 20-文件管理（分片状态机、安全控制、预览下载）、17-任务调度（孤儿分片清理）、16-事件总线（file.uploaded）、12-多租户路由（存储配额） |
| 相邻协作 | [租户管理](25_概要设计_租户管理.md)（storage_bytes 存储配额超限拦截）、任务调度（孤儿分片回收）、审计哈希链（上传/下载关键操作落[操作日志](11_概要设计_操作日志.md)）、[全文检索](33_概要设计_全文检索.md)（阶段十八文件内容索引） |
...
| 打印 / 导出 PDF 产物 | 单据打印与列表导出 PDF 的**产物落对象存储、限时取址下载 / 分享**（来源 = 打印 / 导出）；产物文件名 / 保留期 / 清理与单据关联归本模块 | file:download |
...
打印 / 导出 PDF 产物经**统一导出出口**生成后写入同一私有桶（租户前缀），元数据落 sys_file（来源=打印/导出）
→ 限时取址下载 / 分享 → 按保留期清理；导出留痕落操作日志。
阶段十八全文检索经 file.uploaded 事件驱动文件内容解析（PDF/Word/Excel/TXT）入 ES 索引。
...
file.uploaded 供全文检索文件内容解析（后续）等内部消费，不开放 Webhook 订阅。
