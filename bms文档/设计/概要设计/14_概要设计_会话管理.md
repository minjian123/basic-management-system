...
[概要设计](01_概要设计_总览.md) › 14-会话管理　|　[← 13-文件管理](13_概要设计_文件管理.md) · [15-工作流管理 →](15_概要设计_工作流管理.md)

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**会话管理**模块与「认证与安全」章节
（多端会话、会话有效标记、强制踢出），架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)14-认证与会话节点与 18-实时推送节点。
...
- **协作关系**：
  - 认证与会话（[架构 09](../架构设计/14_架构设计_子系统_认证与会话.md)）：读写 `sys_session` 与会话 Redis 标记 `bms:{tenant}:sess:{session_id}`
  - 实时推送（[架构 13](../架构设计/18_架构设计_子系统_实时推送.md)）：踢出时广播 `session.revoked` 事件关闭该会话全部实时连接
...
- **幂等（2026-09-26，01_04）**：踢出以 `revoked_at` 状态幂等——重复踢出已撤销会话返回 `20013`（`Idempotency-Key` 请求头去重归幂等基座真实化后接入，本期未启用）
- **限流**：踢出为治理操作，按用户维度限流（如 20 次/分钟）
- **发布事件**：本模块不发布 RocketMQ 事件；踢出广播为 Socket.IO 实时事件 `session.revoked`（经 `BaseRealtimePublisher` 占位，真实推送归阶段八；Webhook 不订阅，与事件总线无关）
...
| session.device_check | false | 是否开启设备/IP 一致性校验（可选强度，见[架构 09](../架构设计/14_架构设计_子系统_认证与会话.md)） |

> **配置落点（2026-09-26，01_04）**：`session.max_active` 本期取平台默认配置 `[session].max_active`（环境变量 `BMS_SESSION__MAX_ACTIVE`）；租户级 `sys_config` 覆盖归后续阶段（仅改配置读取源，不动判定逻辑）。`session.device_check` 随 01_05 每请求校验链接入。

- **种子数据**：业务码 session、动作码 query/kick 入平台库种子（sys_business/sys_action）；三类内置角色中系统管理员预授本模块菜单/表单权限

## 7. 错误码与异常处理 <a id="error"></a>

本模块错误码位于 **2xxxx 认证段**（20001-29999 认证：登录/刷新/登出/验证码/找回密码/会话）。**码位避让（2026-09-26，01_03）**：登录类错误码占用 `20002`（账号或密码错误）/ `20003`（账号已锁定）/ `20004`（账号已停用），本模块会话错误码整体后移至 `20011~20016`（原表误标 `20001~20006`，与登录类冲突，已修正）：

| 错误码 | 说明 | 场景 |
| --- | --- | --- |
| 20011 | 会话不存在 | 查询/踢出不存在的 session_id |
| 20012 | 会话已失效 | 目标会话已登出/已过期/已被踢出 |
| 20013 | 会话已撤销 | 重复踢出已撤销会话（配合幂等键返回首次结果） |
| 20014 | 拒绝操作当前会话 | 不可在会话管理页踢出正在使用的系统管理员会话（除本人） |
| 20015 | 活跃会话数达上限 | 登录链路提示（本模块列表页同步展示） |
| 20016 | 无权操作该会话 | 越权踢出非本租户会话（租户边界强制过滤） |
