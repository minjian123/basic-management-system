...
（多端会话、会话有效标记、强制踢出），架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)12-认证与会话节点与 16-实时推送节点。
...
- **协作关系**：
  - 认证与会话（[架构 09](../架构设计/12_架构设计_子系统_认证与会话.md)）：读写 `sys_session` 与会话 Redis 标记 `bms:{tenant}:sess:{session_id}`
  - 实时推送（[架构 13](../架构设计/16_架构设计_子系统_实时推送.md)）：踢出时广播 `session.revoked` 事件关闭该会话全部实时连接
...
| session.device_check | false | 是否开启设备/IP 一致性校验（可选强度，见[架构 09](../架构设计/12_架构设计_子系统_认证与会话.md)） |
