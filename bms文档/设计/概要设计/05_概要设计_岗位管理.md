...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**岗位管理**模块，与「核心数据表清单」（sys_post、sys_user_post、sys_role_assign）、「权限模型」（主体链）章节。架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)13-权限计算引擎节点。
...
- 岗位数据纳入全文检索主数据索引（阶段十六，用户/部门/角色/字典/单据搜索），经 RocketMQ 事件驱动同步
