...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**租户管理**模块，与「多数据源策略（租户独立库）」「权限模型（平台超管）」「初始化数据（租户开通种子）」「认证与安全（租户识别）」「部署与运维（子域名路由/泛域名 TLS）」「[审计合规](28_概要设计_审计合规.md)（合规留存）」等章节；架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)**12-多租户路由**节点。
...
- 租户开通全流程可用：自动建库 + 迁移 + 种子（对应规划说明阶段七「平台超管与租户管理（开通/停用/域名/配额）」）
