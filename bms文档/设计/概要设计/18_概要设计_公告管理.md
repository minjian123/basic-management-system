...
- **协作关系**：公告发布/更新/下线时经事件总线发布 `notice.published` 事件，供 Webhook 订阅推送；公告数据（sys_notice）属关键表，字段级变更进入数据变更审计（sys_data_audit_log）；过期自动下线依赖[任务调度](23_概要设计_任务调度.md)能力做定时扫描兜底。
