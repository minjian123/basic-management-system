...
- 异常处理要求：发布写操作在单事务内完成，失败整体回滚不产生半发布状态；过期扫描任务失败落 sys_task_log 并可重试，读取侧实时校验兜底；公告正文按富文本处理，统一以 **HTML** 入库（轻 Markdown 只作为编辑方式），XSS 防护为**双向**白名单清洗——提交时前端净化 + 后端白名单清洗为权威口径，渲染前前端二次净化（不采用「Element Plus 默认转义」口径，富文本渲染必须净化后注入），白名单与帮助中心同源，见[《组件设计 · 富文本字段》](../组件设计/08_组件设计_富文本字段/08_组件设计_富文本字段.md)。
