...
- **职责边界**：分类树与文章内容全生命周期管理、前台展示与阅读反馈；AI 智能维护（阶段十九）仅在本节概述，详细设计见 AI 服务文档
- **协作关系**：
  - 全文检索（阶段十八）：文章发布/更新/删除事件驱动 ES 同步（bms-main 主数据索引）
  - AI 服务（阶段十九）：帮助文章为知识库问答机器人知识源（RAG，按租户隔离）；帮助文档智能维护生成草稿
...
| AI 智能维护（阶段十九） | AI 生成/更新草稿、form.updated 事件自动触发、质量评估报告；结果一律为草稿，人工审核后发布 | help:manage |
...
- **sys_config 参数**：无独立业务参数；AI 帮助维护相关开关随 AI 能力（阶段十九）管理
...
| 帮助管理（文章维护） | PC | 文章在线编辑（草稿/发布/下架/置顶）、target 关联、多语言维护、统计视图、AI 生成草稿入口（阶段十九） |
