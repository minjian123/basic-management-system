...
> **正文内容口径**：文章正文统一以 **HTML** 入库（轻 Markdown 档保存时解析为 HTML，不保留原文列）；清洗为前后端同源白名单**双向**清洗（提交时前端净化 + 后端权威清洗，渲染前前端二次净化），编辑器与净化契约见[《组件设计 · 富文本字段》](../组件设计/04_字段类/07_组件设计_富文本字段/07_组件设计_富文本字段.md)。
