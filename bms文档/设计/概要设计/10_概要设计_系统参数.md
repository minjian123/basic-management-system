...
| 相邻协作 | 认证域（密码策略/会话上限/找回密码限流）、文件管理（上传大小上限）、[会话管理](14_概要设计_会话管理.md)（活跃会话数）、[AI 能力](34_概要设计_AI能力.md)（自动执行/审批开关，阶段十六） |
...
| ai.auto_execute | false | AI 自动执行写操作开关（阶段十六，默认关闭） | AI 能力 |
| ai.auto_approve | false | AI 自动审批开关（阶段十六，默认关闭；依赖 ai.auto_execute 开启，未开启时配置拒绝保存） | AI 能力 + 工作流 |
