...
- 列表内联切换：用户列表「状态」列以开关**内联切换**（列表页无操作列），复用 `PUT /users/{id}/status`，**不新增 toggle 接口**；需记录级可编辑（`sys_role_field.editable`）与 `user:update` 动作权限；**停用为危险开关**，切换前二次确认，失败回滚原值并提示错误码文案（契约见[《组件设计 · 开关与枚举字段》](../组件设计/01_字段类/08_组件设计_开关与枚举字段/08_组件设计_开关与枚举字段.md)）
