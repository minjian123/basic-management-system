...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**用户管理**模块，与「核心数据表清单」「认证与安全」「权限模型」「事件总线事件模型」章节。架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)12-认证与会话、13-权限计算引擎节点。
...
- 用户变更经 RocketMQ 事件驱动：操作日志异步落库、Webhook 推送、全文检索主数据索引同步（阶段十六）
