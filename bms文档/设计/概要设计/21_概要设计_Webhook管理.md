...
[概要设计](01_概要设计_总览.md) › 21-Webhook 管理　|　[← 20-开放接口管理](20_概要设计_开放接口管理.md)　|　[22-系统监控 →](22_概要设计_系统监控.md)

## 1. 模块概述与职责 <a id="overview"></a>

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**Webhook 管理**模块，以及「系统集成」（出站集成）、「事件总线事件模型」「核心数据表清单」章节；
架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)08-接口与集成（事件模型清单）与 14-事件总线（Webhook 推送组）节点。

- **模块定位**：将 BMS 内部事件经 RocketMQ 事件总线异步推送至外部注册 URL，实现事件驱动的对外集成（出站方向），与开放接口（入站方向，20-[开放接口管理](20_概要设计_开放接口管理.md)）互为补充。
...
2. 事件发布：业务模块产生事件（如产品模块 pur.apply.submitted），经 RocketMQ 事务消息发布。
...
接口前缀、统一响应与鉴权约定见[《API 接口规范》](../../规范/API接口规范.md)（第 2~3、6 节）。
...
- 事件推送成功：业务事件（如审批通过、单据生成、公告发布）发生后，订阅 URL 收到 HMAC 签名校验通过的推送；
