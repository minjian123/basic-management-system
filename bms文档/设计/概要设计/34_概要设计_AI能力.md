...
- **协作关系**：
  - 权限计算引擎（[架构 10](../架构设计/13_架构设计_子系统_权限计算引擎.md)）：对话权限码 `ai:chat`、模型配置 `ai:manage`；RAG 检索上下文遵守动作级数据权限范围
  - 事件总线（[架构 11](../架构设计/14_架构设计_子系统_事件总线.md)）：embedding 管线经 RocketMQ 事件驱动（复用 event-worker）；消费 `form.updated` 驱动帮助草稿更新
  - [报表中心](27_概要设计_报表中心.md)：智能问数（NL2SQL）基于数据集白名单 + 只读从库执行
  - 工作流：审批辅助与自动审批（审批节点白名单）；AI 内容生成（审批意见、申请理由草稿）
  - [文件管理](13_概要设计_文件管理.md)/[全文检索](33_概要设计_全文检索.md)（[架构 15](../架构设计/18_架构设计_子系统_文件管理.md)/[架构 20](../架构设计/23_架构设计_子系统_全文检索.md)）：OCR 衔接文本解析（扫描件 ocr_pending → OCR 后回写）；语义搜索与关键词检索互补
  - 审计哈希链（[架构 14](../架构设计/17_架构设计_子系统_审计哈希链.md)）：ai_chat_log 按月分片；审计异常检测输入源
  - [帮助中心](17_概要设计_帮助中心.md)：帮助文档智能维护（草稿生成、质量评估，人工审核后发布）
  - 可观测性（[架构 22](../架构设计/25_架构设计_子系统_可观测性.md)）：AI 成本/延迟指标；异常检测告警接 Alertmanager
...
按《[架构设计 · 接口与集成](../架构设计/08_架构设计_接口与集成.md#api-error)》错误码分段表未为本模块单列段位，本模块错误码位于 **1xxxx 通用段**
