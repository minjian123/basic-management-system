...
> BMS · LLM 适配、八项能力与安全闸门（阶段十六）

[概要设计](01_概要设计_总览.md) › 34-AI 能力　|　[← 33-全文检索](33_概要设计_全文检索.md) · [35-首页工作台 →](35_概要设计_首页工作台.md)

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**AI 能力**模块与「选型说明（AI（阶段十六））」
「核心数据表清单」「认证与安全（AI 安全）」「事件模型（form.updated）」「部署与运维（Milvus/PaddleOCR）」
「开发计划与验收标准（阶段十六）」章节，架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)21-子系统-AI 服务节点
与 20-子系统-全文检索节点（OCR 衔接文本解析）。

## 1. 模块概述与职责 <a id="overview"></a>

AI 能力模块（阶段十六）提供统一 LLM 适配层与八项 AI 能力（智能报表助手、智能审批辅助、AI 内容生成、
...
- **协作关系**：
  ...
  - [文件管理](13_概要设计_文件管理.md)/[全文检索](33_概要设计_全文检索.md)（[架构 15](../架构设计/16_架构设计_子系统_文件管理.md)/[架构 20](../架构设计/21_架构设计_子系统_全文检索.md)）：OCR 衔接文本解析（扫描件 ocr_pending → OCR 后回写）；语义搜索与关键词检索互补
...
按《[架构设计 · 接口与集成](../架构设计/06_架构设计_接口与集成.md#api-error)》错误码分段表未为本模块单列段位，本模块错误码位于 **1xxxx 通用段**
...
- OCR 解析扫描件回写全文检索索引可用（衔接阶段十五 ocr_pending）；AI 成本/延迟指标纳入监控
