...
> BMS · LLM 适配、八项能力与安全闸门（阶段十九）

[概要设计](01_概要设计_总览.md) › 34-AI 能力　|　[← 33-全文检索](33_概要设计_全文检索.md) · [35-首页工作台 →](35_概要设计_首页工作台.md)

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**AI 能力**模块与「选型说明（AI（阶段十九））」
「核心数据表清单」「认证与安全（AI 安全）」「事件模型（form.updated）」「部署与运维（Milvus/PaddleOCR）」
《[总体项目规划](../../规划/总体项目规划.md)》「阶段内容与完成标准（阶段十九）」章节，架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)21-子系统-AI 服务节点
与 20-子系统-全文检索节点（OCR 衔接文本解析）。

## 1. 模块概述与职责 <a id="overview"></a>

AI 能力模块（阶段十九）提供统一 LLM 适配层与八项 AI 能力（智能报表助手、智能审批辅助、AI 内容生成、
...
- **协作关系**：
  ...
  - 事件总线（[架构 11](../架构设计/16_架构设计_子系统_事件总线.md)）：embedding 管线经 RocketMQ 事件驱动（复用 event-worker）；消费 `form.updated` 驱动帮助草稿更新
  - [报表中心](27_概要设计_报表中心.md)：智能问数（N片段SQL）基于数据集白名单 + 只读从库执行
...
| 能力 | 说明 | 结合模块 |
| --- | --- | --- |
| 智能报表助手 | N片段SQL 问数：数据集白名单 + 只读从库 + 安全校验防注入；结果图表化 | 报表中心 |
...
| 帮助文档智能维护 | 自动编写/更新帮助草稿 + 质量评估；输入源设计文档 + 表单元数据；手动/form.updated/定时三种触发；草稿人工审核发布 | 帮助中心 |
...
| POST | /api/v1/ai/report/query | 智能问数（N片段SQL：数据集白名单 + 只读从库 + 安全校验，结果图表化） | ai:chat（+rpt:query 数据集权限） |
...
| 10206 | 数据集不在白名单 | N片段SQL 目标数据集无权限或非白名单（拒绝生成 SQL） |
...
- N片段SQL 安全执行验证通过：数据集白名单、只读从库执行、防注入（12006/12007 拒绝场景验证通过）
