...
> BMS · LLM 适配、八项能力与安全闸门（后续）

[概要设计](01_概要设计_总览.md) › 34-AI 能力　|　[← 33-全文检索](33_概要设计_全文检索.md) · [35-首页工作台 →](35_概要设计_首页工作台.md)

本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**AI 能力**模块与「选型说明（AI 能力）」
「核心数据表清单」「认证与安全（AI 安全）」「事件模型（sys.form.updated）」「部署与运维（Milvus/PaddleOCR）」
《[总体项目规划](../../规划/总体项目规划.md)》「阶段内容与完成标准」章节，架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)21-子系统-AI 服务节点
...
AI 能力模块（后续）提供统一 LLM 适配层与八项 AI 能力（智能报表助手、智能审批辅助、AI 内容生成、
...
- **协作关系**：
  - 权限计算引擎（[架构 10](../架构设计/15_架构设计_子系统_权限计算引擎.md)）：对话权限码 `ai:chat`、模型配置 `ai:manage`；RAG 检索上下文遵守动作级数据权限范围
  - 事件总线（[架构 11](../架构设计/16_架构设计_子系统_事件总线.md)）：embedding 管线经 RocketMQ 事件驱动（复用 event-worker）；消费 `sys.form.updated` 驱动帮助草稿更新
  - [报表中心](27_概要设计_报表中心.md)：智能问数（NL2SQL）基于数据集白名单 + 只读从库执行
  - 工作流：审批辅助与自动审批（审批节点白名单）；AI 内容生成（审批意见、申请理由草稿）
  - [文件管理](13_概要设计_文件管理.md)/[全文检索](33_概要设计_全文检索.md)（[架构 15](../架构设计/20_架构设计_子系统_文件管理.md)/[架构 20](../架构设计/25_架构设计_子系统_全文检索.md)）：OCR 衔接文本解析（扫描件 ocr_pending → OCR 后回写）；语义搜索与关键词检索互补
  - 审计哈希链（[架构 14](../架构设计/19_架构设计_子系统_审计哈希链.md)）：ai_chat_log 按月分片；审计异常检测输入源
  - [帮助中心](17_概要设计_帮助中心.md)：帮助文档智能维护（草稿生成、质量评估，人工审核后发布）
  - 可观测性（[架构 22](../架构设计/27_架构设计_子系统_可观测性.md)）：AI 成本/延迟指标；异常检测告警接 Alertmanager
...
| 帮助文档智能维护 | 自动编写/更新帮助草稿 + 质量评估；输入源设计文档 + 表单元数据；手动/sys.form.updated/定时三种触发；草稿人工审核发布 | 帮助中心 |
...
4. **帮助文档智能维护**：
   1. 三种触发：维护人手动点击生成/更新草稿；菜单/表单/字段/按钮变更经 `sys.form.updated` 事件自动生成更新草稿并通知维护人；定时质量评估扫描
   2. 输入源：设计文档、表单元数据（sys_form/sys_field/业务权限码清单）
...
sequenceDiagram
    ...
    participant 总线 as 事件总线(sys.form.updated)
    participant 维护 as AI 维护任务
    participant 输入 as 输入源(设计文档/表单元数据)
...
| POST | /api/v1/ai/help/{article_id}/generate-draft | 生成/更新帮助文章草稿（手动触发；sys.form.updated 事件自动触发走事件通道） | help:manage |
...
- **发布事件**：帮助草稿生成完成可发布 AI 生成内容事件（可选 Webhook 订阅）；消费 `sys.form.updated`（AI 帮助更新触发）
...
按《[架构设计 · 接口与集成](../架构设计/09_架构设计_接口与集成.md#api-error)》错误码分段表未为本模块单列段位，本模块错误码位于 **1xxxx 通用段**
（10001-19999 通用：参数校验、限流、系统内部；段内 102xx 子段为本模块规划，避开 10xxxx/11xxxx/12xxxx 业务模块段，见《[项目规划说明](../../规划/项目规划说明.md)》「错误码段规则与注册」节），一经发布稳定不变：
...
| 10215 | 帮助草稿生成失败 | 输入源不足（设计文档/表单元数据缺失） |
...
- 帮助文档 AI 生成草稿与人工审核发布链路验证通过；sys.form.updated 事件触发更新验证通过；质量评估报告可用
- OCR 解析扫描件回写全文检索索引可用（衔接阶段十八 ocr_pending）；AI 成本/延迟指标纳入监控
