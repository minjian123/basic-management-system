...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**任务调度**模块，与「选型说明（Celery / Redis）」「多数据源策略（分片表预创建）」「初始化数据（初始定时任务种子）」「认证与安全（180 天未登录账号锁定）」「部署与运维（beat 单副本）」等章节；架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)**13-任务调度**节点。
...
按《[架构设计 · 接口与集成](../架构设计/06_架构设计_接口与集成.md#api-error)》5 位错误码分段表，本模块归属 **7xxxx（70001-79999）通知/邮件/任务调度** 段，一经发布稳定不变；文案由前端按 i18n 映射，后端不承担文案拼装。示例：
...
- 任务页面可管理：增删改、启停、手动触发全部生效（对应规划说明阶段十一「任务页面可管理」）
- 执行记录可查：成功/失败/耗时/错误信息完整（「执行记录可查」）
- 内置种子任务就绪：孤儿分片清理、日志归档、分片表预创建、每日全量备份（对应「初始化数据」与阶段十一验收）
