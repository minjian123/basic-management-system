...
[概要设计](01_概要设计_总览.md) › 23-任务调度　|　[← 22-系统监控](22_概要设计_系统监控.md)　|　[24-邮件通知 →](24_概要设计_邮件通知.md)
...
本节对应[《项目规划说明》](../../规划/项目规划说明.md)「功能模块」之**任务调度**模块，与「选型说明（Celery / Redis）」「多数据源策略（分片表预创建）」「初始化数据（初始定时任务种子）」「认证与安全（180 天未登录账号锁定）」「部署与运维（beat 单副本）」等章节；架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)**17-任务调度**节点。
...
- **与相邻模块协作**：[文件管理](13_概要设计_文件管理.md)（孤儿分片回收）、[审计合规](28_概要设计_审计合规.md)（日志归档前链完整性校验）、[归档管理](30_概要设计_归档管理.md)（两段式归档调度）、[备份管理](31_概要设计_备份管理.md)（每日全量备份、sys_backup_task 驱动）、认证与会话（180 天未登录锁定扫描）、[回收站](32_概要设计_回收站.md)（软删除残留物理清理）、[公告管理](18_概要设计_公告管理.md)（过期自动下线扫描）
...
2. 执行周期：worker 消费任务 → 同步执行 → 成功/失败分别落 `sys_task_log` → 失败按策略有限次重试（ack 确认）→ 执行完成落 `sys.task.completed` 事件
...
sequenceDiagram
    ...
    worker->>RocketMQ: 发布 sys.task.completed 事件
```

- **与缓存协作**：分布式锁、pub/sub 变更通知走 Redis（key 前缀 `bms:global:`，锁 TTL 与 beat 心跳对齐）
- **与消息队列协作**：任务队列走 Redis broker；执行完成事件经 RocketMQ 事件总线发布（sys.task.completed，Webhook 默认不可订阅）——两套通道互不干扰
...
按《[架构设计 · 接口与集成](../架构设计/09_架构设计_接口与集成.md#api-error)》5 位错误码分段表，本模块归属 **7xxxx（70001-79999）通知/邮件/任务调度** 段，一经发布稳定不变；文案由前端按 i18n 映射，后端不承担文案拼装。示例：
...
- 任务页面可管理：增删改、启停、手动触发全部生效（对应规划说明阶段十三「任务页面可管理」）
- 执行记录可查：成功/失败/耗时/错误信息完整（「执行记录可查」）
- 内置种子任务就绪：孤儿分片清理、日志归档、分片表预创建、每日全量备份（对应「初始化数据」与阶段十三验收）
