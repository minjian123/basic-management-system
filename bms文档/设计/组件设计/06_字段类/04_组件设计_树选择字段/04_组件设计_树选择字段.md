...
> TreeSelectField · DeptTreeSelect · MenuTreeSelect · useTreeData（PC 目标落点 `frontend/packages/ui-ep/src/components/field/`；能力组合 `field` + `input-control` + `tree-data`；实现随域 05+ 回补波交付）
...
定义 BMS 前端（PC 渲染插件 `frontend/packages/ui-ep`，目标落点 `frontend/packages/ui-ep/src/components/field/`；移动端按需 `frontend/packages/ui-vant/src/components/field/`；宿主 `frontend/apps/*` 消费）**树形结构选择字段**的通用组件设计：通用树选择（TreeSelectField）、部门树选择（DeptTreeSelect）、菜单树选择（MenuTreeSelect），以及树数据取数与缓存工具 `useTreeData`。适用于需要从层级数据中选择节点的场景：所属部门、上级部门、数据权限范围、菜单挂载点、分类/分组节点。
...
> 本篇为组件设计体系字段类「树选择字段」；组织成员（人员）选择见 [组织选择字段](../05_组件设计_组织选择字段/05_组件设计_组织选择字段.md)节点。

## 2. 组件清单与职责 <a id="components"></a>

| 组件 / 工具 | 位置 | 职责 |
| --- | --- | --- |
| `TreeSelectField.vue` | `frontend/packages/ui-ep/src/components/field/`（目标落点，随域 05+ 回补波交付） | 通用树选择（`el-tree-select`）：单选/多选、联想过滤、可选叶子约束、懒加载 |
| `DeptTreeSelect.vue` | `frontend/packages/ui-ep/src/components/field/`（目标落点，随域 05+ 回补波交付） | 部门树选择：绑定部门接口，排除自身/子树（选上级部门时防环） |
| `MenuTreeSelect.vue` | `frontend/packages/ui-ep/src/components/field/`（目标落点，随域 05+ 回补波交付） | 菜单树选择：绑定菜单接口，排除自身/子树（选上级菜单） |
| `useTreeData()` | `frontend/packages/core/src/capabilities/tree-data.ts`（树域能力 `tree-data`，无投影按需回补） | 树数据取数：接口拉取、展平建索引、路径查找、缓存与失效 |

- 字段类组件统一受控：`modelValue` / `update:modelValue` + `change`（见[《组件设计 · 总览》](../../01_组件设计_总览.md)）。
- 命名遵循[《命名规范》](../../../../规范/命名规范.md)。

> **实现口径（2026-09-17，新体系；实现随域 05+ 回补波交付）**：组件目标落点 PC `frontend/packages/ui-ep/src/components/field/`（移动端按需 `frontend/packages/ui-vant/src/components/field/`），宿主 `frontend/apps/*` 消费；按字段类口径组合核心能力 `field` + `input-control` + 树域能力 `tree-data`（旧「片段」已类化为 `frontend/packages/core/src/capabilities/<key>.ts` 核心能力类，登记 `capabilities/manifest.ts`）；原域包装（`BaseInput` / `BaseTree` 等）与旧 `useXxx` 组合式随旧层删除（历史经 git 追溯），按「核心能力 + 投影 + 插件包装」随域 05+ 回补波重建；旧 `field` 组件目录与宿主工具目录落点已删除。
...
```mermaid
flowchart LR
    A["页面/表单挂载"] --> B{"树数据能力 tree-data 缓存命中?"}
    B -- 是 --> C["直接返回缓存树"]
...
| 场景 | 展示 |
| --- | --- |
| 只读回显 | 显示节点**完整路径**（如「总部 / 研发部 / 前端组」），由 树数据能力 `tree-data`（原 `useTreeData`）`pathOf(id)` 得出 |
...
- **移动端**：移动端渲染插件 `frontend/packages/ui-vant` 用 Vant 级联/弹层树选择，数据源与路径回显口径同源，不直接复用 PC 组件（同接口多实现，契约套件同一套断言）。
