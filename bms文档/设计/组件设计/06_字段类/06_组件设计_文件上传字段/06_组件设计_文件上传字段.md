...
定义 BMS 前端 `apps/desktop/` **文件与图片上传字段**的通用组件设计：通用文件上传（FileUploadField）、图片上传（ImageUploadField）、只读文件列表（FileListField），以及上传引擎组合式函数 `useFileUpload`（分片、秒传、断点续传、进度、重试）。适用于附件、证明材料、头像、图片凭证等上传场景。
