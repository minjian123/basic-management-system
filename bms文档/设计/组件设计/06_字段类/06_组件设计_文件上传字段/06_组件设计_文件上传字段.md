...
> **实现口径（2026-09-17，新体系；实现随域 05+ 回补波交付）**：组件目标落点 PC `frontend/packages/ui-ep/src/components/field/`（移动端按需 `frontend/packages/ui-vant/src/components/field/`），宿主 `frontend/apps/*` 消费；按字段类口径组合核心能力 `field` + `input-control` + 上传引擎能力 `upload-engine`（依赖 预签名能力 `presigned-url`）（已类化为 `frontend/packages/core/src/capabilities/<key>.ts` 核心能力类，登记 `capabilities/manifest.ts`；`upload-engine` 无投影按需回补）；原域包装（`BaseInput` 等）与旧 `useXxx` 组合式随旧层删除（历史经 git 追溯），按「核心能力 + 投影 + 插件包装」随域 05+ 回补波重建；旧 `field` 组件目录与宿主工具目录落点已删除。
