...
> FileUploadField · ImageUploadField · useFileUpload · FileListField（PC 目标落点 `frontend/packages/ui-ep/src/components/field/`；能力组合 `field` + `input-control` + `upload-engine` / `presigned-url`；实现随域 05+ 回补波交付）
...
定义 BMS 前端（PC 渲染插件 `frontend/packages/ui-ep`，目标落点 `frontend/packages/ui-ep/src/components/field/`；移动端按需 `frontend/packages/ui-vant/src/components/field/`；宿主 `frontend/apps/*` 消费）**文件与图片上传字段**的通用组件设计：通用文件上传（FileUploadField）、图片上传（ImageUploadField）、只读文件列表（FileListField），以及上传引擎组合式函数 `useFileUpload`（分片、秒传、断点续传、进度、重试）。适用于附件、证明材料、头像、图片凭证等上传场景。

本节对应[《项目规划说明》](../../../../规划/项目规划说明.md)前端与文件管理章节；上游设计依据[《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)（文件表 `sys_file`、整包/分片上传/下载预览接口、类型与限额）与[《架构设计 · 文件管理子系统》](../../../架构设计/20_架构设计_子系统_文件管理.md)（对象存储私有桶、分片状态机、断点续传、孤儿分片回收）；页面形态依据[《布局设计 · 表单页》](../../../布局设计/08_布局设计_表单页.html)（附件字段、只读/编辑分态）。

> 本篇为组件设计体系字段类「文件上传字段」；富文本内嵌图片上传见 [富文本字段](../07_组件设计_富文本字段/07_组件设计_富文本字段.md)节点（复用本节点上传引擎）。

## 2. 组件清单与职责 <a id="components"></a>

| 组件 / 工具 | 位置 | 职责 |
| --- | --- | --- |
| `FileUploadField.vue` | `frontend/packages/ui-ep/src/components/field/`（目标落点，随域 05+ 回补波交付） | 通用文件上传：单/多文件、类型白名单、大小限制、进度、失败重试、拖拽 |
| `ImageUploadField.vue` | `frontend/packages/ui-ep/src/components/field/`（目标落点，随域 05+ 回补波交付） | 图片上传：缩略图预览、多图、可选压缩与尺寸校验、头像裁剪形态 |
| `FileListField.vue` | `frontend/packages/ui-ep/src/components/field/`（目标落点，随域 05+ 回补波交付） | 只读文件列表：文件名、大小、下载、预览（图片/PDF） |
| `useFileUpload()` | `frontend/packages/core/src/capabilities/upload-engine.ts`（能力 `upload-engine`，depends `presigned-url`；无投影按需回补） | 上传引擎：分片、秒传（哈希）、断点续传、并发控制、进度聚合 |
...
> **实现口径（2026-09-17，新体系；实现随域 05+ 回补波交付）**：组件目标落点 PC `frontend/packages/ui-ep/src/components/field/`（移动端按需 `frontend/packages/ui-vant/src/components/field/`），宿主 `frontend/apps/*` 消费；按字段类口径组合核心能力 `field` + `input-control` + 上传引擎能力 `upload-engine`（依赖 预签名能力 `presigned-url`）（旧「片段」已类化为 `frontend/packages/core/src/capabilities/<key>.ts` 核心能力类，登记 `capabilities/manifest.ts`；`upload-engine` 无投影按需回补）；原域包装（`BaseInput` 等）与旧 `useXxx` 组合式随旧层删除（历史经 git 追溯），按「核心能力 + 投影 + 插件包装」随域 05+ 回补波重建；旧 `field` 组件目录与宿主工具目录落点已删除。
...
sequenceDiagram
    ...
    participant 引擎 as 上传引擎能力（upload-engine）
...
| 后端兜底 | 前端限制仅为体验，后端按类型/大小/病毒扫描兜底（[《架构设计 · 文件管理子系统》](../../../架构设计/20_架构设计_子系统_文件管理.md)） |
| 违规提示 | 类型不允许（错误码 50101）、超出大小（50102）等，组件内联提示（见 [请求封装](../../09_基础类/01_组件设计_请求封装/01_组件设计_请求封装.md)错误码分段 5xxxx） |
...
| 秒传判定（**上游待补**） | `POST /files/check`（入参 `sha256` + `size`，命中返回既有 `file_id`）：上游文件管理未定义该接口，本节点登记为上游变更，须先补契约与实现 | [《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)、[《架构设计 · 文件管理子系统》](../../../架构设计/20_架构设计_子系统_文件管理.md) |
...
| 弱网 | 失败可续传；进度可视；超时按请求层配置（上传实例超时更长，见 [请求封装](../../09_基础类/01_组件设计_请求封装/01_组件设计_请求封装.md)） |
...
- **字段权限**：`visible=false` 不渲染；`editable=false` 传 `disabled`（见 [权限指令](../../09_基础类/02_组件设计_权限指令/02_组件设计_权限指令.md)）；下载/预览受 `file:download` 等动作权限约束，后端校验。
- **i18n**：上传提示（类型/大小/数量）、进度文案、错误提示走 vue-i18n；文件名按原样展示（不翻译）。
- **移动端**：移动端渲染插件 `frontend/packages/ui-vant` 用 Vant `Uploader`（支持拍照/相册），分片与秒传能力同源（大文件可退化为直传），不直接复用 PC 组件（同接口多实现，契约套件同一套断言）。
...
> 组件设计节点 · 与[《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)[《架构设计 · 文件管理子系统》](../../../架构设计/20_架构设计_子系统_文件管理.md)[《布局设计 · 表单页》](../../../布局设计/08_布局设计_表单页.html)[《命名规范》](../../../../规范/命名规范.md)配套
