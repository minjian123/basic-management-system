...
定义 BMS 前端 `frontend/apps/desktop/`（Vue 3 + TypeScript + Axios）**桌面端全部 HTTP 交互**的通用底座：Axios 统一实例与拦截器、统一响应解包 `request<T>()`、模块 API 基类 `BaseApi`、异步状态组合式函数 `useRequest`、列表页公共逻辑 `useListPage`、错误码到提示文案的映射，以及 401 静默刷新机制。适用于管理端所有模块 API（`src/api/*.ts`）与全部页面组件。
...
- **实现口径（2026-09-16，`02_06_01` 交付）**：`src/api/{http,request,error,token,adapter}.ts` + `src/utils/{useRequest,useListPage}.ts`（**双端同款**；核心不依赖 UI 库与路由，提示 / 跳转 / 刷新经 `configureHttpAdapter` 入口注入）；业务错误码在 `request` 层判定并抛 `ApiError`（继承 `BaseError`）；**后端 `/auth` 未就绪，401 未注入 `refreshHandler` 时按会话失效占位**（清会话 + 跳登录、不请求；并发收敛与单次重放已实现）；`useListPage.query` 为 **reactive**（原 Ref 口径更新）；`BaseApi` 写方法自动生成 `Idempotency-Key`。
