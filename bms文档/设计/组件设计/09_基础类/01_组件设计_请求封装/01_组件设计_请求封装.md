...
> 可交互原型：[19_原型设计_请求封装.html](01_原型设计_请求封装.html)（演示统一响应解包、错误码 → i18n 文案、401 静默刷新与并发排队、幂等键、useRequest 竞态处理与列表分页）

## 1. 目的与适用范围 <a id="purpose"></a>

定义 BMS 前端 `frontend/apps/*/`（宿主层；Vue 3 + TypeScript + Axios）**PC 管理端全部 HTTP 交互**的通用底座：Axios 统一实例与拦截器、统一响应解包 `request<T>()`、模块 API 基类 `BaseApi`、异步状态组合式函数 `useRequest`、列表页公共逻辑 `useListPage`、错误码到提示文案的映射，以及 401 静默刷新机制。适用于管理端所有模块 API（`frontend/apps/*/src/api/*.ts`）与全部页面组件。

本节对应[《项目规划说明》](../../../../规划/项目规划说明.md)前端章节与「性能与高并发设计」；架构依据[《架构设计 · 接口与集成》](../../../架构设计/09_架构设计_接口与集成.md)（统一响应、错误码分段、分页、幂等）与[《架构设计 · 前端架构》](../../../架构设计/08_架构设计_前端架构.md)（Axios 注入 access token、401 刷新、响应拦截统一错误提示）；编码约定依据[《前端开发规范》](../../../../规范/前端开发规范.md)「请求封装」「基础类与继承约定」节。

> 本篇为基础类「请求封装」节点。基础类不带界面或为横切能力，其契约被字段类、展示类、交互类组件共同复用；编码风格与通用约定以[《前端开发规范》](../../../../规范/前端开发规范.md)为准，本文档只描述组件化/工具契约层面的设计。

## 2. 组件清单与职责 <a id="components"></a>

| 组件 / 工具 | 位置 | 职责 |
| --- | --- | --- |
| `http`（Axios 实例） | `frontend/apps/*/src/api/http.ts` | 统一 baseURL、超时、请求/响应拦截器；注入 token、解包响应、统一错误 |
| `request<T>()` | `frontend/apps/*/src/api/request.ts` | 泛型请求函数：解开 `{code, message, data}`，成功返回 `data`，失败抛 `ApiError` |
| `BaseApi` | `frontend/apps/*/src/api/base.ts` | 模块 API 基类：封装 `get/post/put/delete`，禁止绕过基类裸用 axios |
| `ApiError` | `frontend/apps/*/src/api/error.ts` | 统一错误对象（含 code / httpStatus / message / details / traceId） |
| `useRequest<T>()` | `frontend/apps/*/src/utils/useRequest.ts` | 异步状态与竞态：loading / data / error / run / refresh / cancel |
| `useListPage<T>()` | `frontend/apps/*/src/utils/useListPage.ts` | 列表页公共逻辑：查询参数、分页、排序、加载、重置、选择行 |
| `tokenManager` | `frontend/apps/*/src/api/token.ts` | access token 仅存内存、refresh 轮换、并发刷新排队、登出清理 |
| `errorMessage()` | `frontend/apps/*/src/api/error.ts` | 错误码 → i18n 文案映射（`error.{code}`），含兜底 |

- 命名遵循[《命名规范》](../../../../规范/命名规范.md)前端命名（工具函数 camelCase、`use` + 名词的组合式函数、API 模块 `frontend/apps/*/src/api/模块.ts`）。
- 移动端（frontend/apps/mobile）工程独立，不复用 PC 的 `BaseApi` / `useListPage`；统一响应解包与错误码映射机制同源（见第 12 节）。
- **实现口径（2026-09-17，新体系）**：宿主层已交付——`frontend/apps/*/src/api/{http,request,error,token,adapter}.ts` + `frontend/apps/*/src/utils/{useRequest,useListPage}.ts`（**同接口多实现**（+ 契约套件同一套断言，`@bms/core/testing`）；提示 / 跳转 / 刷新经 `configureHttpAdapter` 入口注入）；业务错误码在 `request` 层判定并抛 `ApiError`（继承核心 `BaseError`，`frontend/packages/core/src/mechanisms/error.ts`）；`BaseApi` 继承核心根系 `BaseFrontend`（`frontend/packages/core/src/base/BaseFrontend.ts`），写方法自动生成 `Idempotency-Key`；**后端 `/auth` 未就绪，401 未注入 `refreshHandler` 时按会话失效占位**（清会话 + 跳登录、不请求；并发收敛与单次重放已实现）；`useListPage.query` 为 **reactive**（原 Ref 口径更新）。

## 3. 统一响应与错误码契约 <a id="contract"></a>

### 3.1 响应与分页结构 <a id="contract-response"></a>

沿用[《架构设计 · 接口与集成》](../../../架构设计/09_架构设计_接口与集成.md)「接口规范」节，前端类型由 openapi-typescript 从后端 OpenAPI 自动生成，与 Pydantic schema 一致：
...
- 语言包运行时拉取（`GET /api/v1/i18n/messages`，Redis + localStorage 二次缓存）见[《架构设计 · 国际化》](../../../架构设计/24_架构设计_子系统_国际化.md)。
...
| 超时 | 默认 15s；上传/导出/导入走独立实例或按请求覆盖（30s 起，见 [文件上传字段](../../06_字段类/06_组件设计_文件上传字段/06_组件设计_文件上传字段.md) 与 [导入导出](../../08_交互类/01_组件设计_导入导出/01_组件设计_导入导出.md)） |
...
```typescript
// frontend/apps/*/src/api/request.ts
...
- 模块 API 文件 `frontend/apps/*/src/api/模块.ts`，函数以 get/query/create/update/delete 开头（[《命名规范》](../../../../规范/命名规范.md)）；查询参数类型继承 `BasePageQuery`，实体继承 `BaseEntity`（`id` 为 string，雪花 BIGINT 以字符串传输）。
...
| 项 | 设计 |
| --- | --- |
| 幂等键 | 写接口（POST/PUT/DELETE）由 `BaseApi` 自动生成 `Idempotency-Key`（UUID v4）；同一业务操作重试复用同一键；后端 Redis SETNX 前置拦截 + 唯一约束兜底（见[《架构设计 · 接口与集成》](../../../架构设计/09_架构设计_接口与集成.md)） |
...
- `useListPage` 由展示类「查询筛选区」与「通用表格」复用；服务端数据不重复入 store（[《前端开发规范》](../../../../规范/前端开发规范.md)状态管理约定），列表/详情由页面组件持有。
...
- frontend/apps/mobile 工程独立，使用自身轻量请求封装（同一 `{code,message,data}` 契约与错误码映射），不复用 `BaseApi` / `useListPage`。
...
- □ 统一实例 `frontend/apps/*/src/api/http.ts`，模块 API 继承 `BaseApi`，无裸用 axios
