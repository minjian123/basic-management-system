...
[文档首页](../../../../文档首页.md) › [项目规划说明](../../../../规划/项目规划说明.md) › [组件设计](../../01_组件设计_总览.md) › 打印与导出 PDF　|　[← 租户切换](../21_组件设计_租户切换/21_组件设计_租户切换.md)

> 可交互原型：[22_原型设计_打印与导出PDF.html](22_原型设计_打印与导出PDF.html)（演示打印模板（页眉/标题/字段/表格/页脚）、打印预览、浏览器打印、服务端导出 PDF、批量打印与黑白打印样式）
...
- **批量打印**：多选后打印 → 异步任务 → 完成通知下载（[06 交互类 02 导入导出](../02_组件设计_导入导出/02_组件设计_导入导出.md)、[06 交互类 19 批量操作栏](../19_组件设计_批量操作栏/19_组件设计_批量操作栏.md)）。
- **产物存储**：生成的 PDF 存[《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)，可下载/分享/归档。

上游/协作：[《布局设计 · 表单页》](../../../布局设计/08_布局设计_表单页.html)（打印入口/预览）、[《布局设计 · 设计令牌》](../../../布局设计/02_布局设计_设计令牌.html)（打印样式令牌：字号/边距/线宽）、[《概要设计 · 导入导出》](../../../概要设计/19_概要设计_导入导出.md)（导出任务）、[《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)（PDF 存储）、[05 展示类 01 通用表格](../../05_展示类/01_组件设计_通用表格/01_组件设计_通用表格.md)（列表导出/打印列）、[06 交互类 13 表单渲染器](../13_组件设计_表单渲染器/13_组件设计_表单渲染器.md)（单据字段渲染口径）、[05 展示类 03 状态标签与描述列表](../../05_展示类/03_组件设计_状态标签与描述列表/03_组件设计_状态标签与描述列表.md)（明细展示）。

> 本篇为组件设计体系交互类第 22 篇（本批补充节点收官）。**范围边界**：**导出任务/异步机制**归[06 交互类 02 导入导出](../02_组件设计_导入导出/02_组件设计_导入导出.md)与[《概要设计 · 导入导出》](../../../概要设计/19_概要设计_导入导出.md)；**PDF 文件存储**归[《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)；**单据字段与布局元数据**归[06 交互类 13 表单渲染器](../13_组件设计_表单渲染器/13_组件设计_表单渲染器.md)/[《概要设计 · 表单定制》](../../../概要设计/36_概要设计_表单定制.md)（本节点只做打印/导出的渲染与编排）。
...
| `brand` | BrandBrief \| null | null | 租户品牌（Logo/名称，见 [06 交互类 20](../20_组件设计_主题与品牌化/20_组件设计_主题与品牌化.md)） |
...
| 批量打印 | 多选 → 逐份/合并 → 异步任务 → 完成通知下载（[06 交互类 19 批量操作栏](../19_组件设计_批量操作栏/19_组件设计_批量操作栏.md)） |
| 品牌 | 页眉使用租户 Logo/名称（[06 交互类 20 主题与品牌化](../20_组件设计_主题与品牌化/20_组件设计_主题与品牌化.md)） |
| 水印 | 支持水印（样张/作废/机密）与页码、打印时间、打印人（审计友好） |
| 黑白 | 黑白模式去除背景色/彩色，仅线条与文字，保证可读 |
| 长明细 | 明细跨页时表头重复、页码连续（分页策略） |
| 数据为空 | 空单据打印提示/占位 |
| 多语言 | 模板文案与字段名按 locale；金额/日期按格式化（[07 基础类 03 格式化工具](../../07_基础类/03_组件设计_格式化工具/03_组件设计_格式化工具.md)） |
...
| 品牌 | 页眉 Logo/名称 | [06 交互类 20 主题与品牌化](../20_组件设计_主题与品牌化/20_组件设计_主题与品牌化.md) |
...
- **权限**：打印/导出按业务操作权限（[07 基础类 02 权限指令](../../07_基础类/02_组件设计_权限指令/02_组件设计_权限指令.md)）；数据范围随查询；导出留审计（含敏感字段脱敏策略）。
- **i18n**：模板文案/字段名按 locale；金额/日期/数字格式化（[07 基础类 03 格式化工具](../../07_基础类/03_组件设计_格式化工具/03_组件设计_格式化工具.md)）；多语言模板字段。
...
> 组件设计节点（补充批次收官）· 与[《布局设计 · 表单页》](../../../布局设计/08_布局设计_表单页.html)[《布局设计 · 设计令牌》](../../../布局设计/02_布局设计_设计令牌.html)[《概要设计 · 导入导出》](../../../概要设计/19_概要设计_导入导出.md)[《概要设计 · 文件管理》](../../../概要设计/13_概要设计_文件管理.md)[《概要设计 · 表单定制》](../../../概要设计/36_概要设计_表单定制.md)[05 展示类 01 通用表格](../../05_展示类/01_组件设计_通用表格/01_组件设计_通用表格.md)[06 交互类 02 导入导出](../02_组件设计_导入导出/02_组件设计_导入导出.md)[06 交互类 13 表单渲染器](../13_组件设计_表单渲染器/13_组件设计_表单渲染器.md)[06 交互类 19 批量操作栏](../19_组件设计_批量操作栏/19_组件设计_批量操作栏.md)[06 交互类 20 主题与品牌化](../20_组件设计_主题与品牌化/20_组件设计_主题与品牌化.md)[07 基础类 03 格式化工具](../../07_基础类/03_组件设计_格式化工具/03_组件设计_格式化工具.md)《命名规范》配套
