...
定义 BMS 前端**字典字段**的通用组件设计：下拉选择（DictSelect）、树形级联（DictCascader）、标签翻译工具（getDictLabel）与字典缓存（useDictStore）。适用于 `frontend/` 管理端全部业务表单页（用户、部门、角色、业务表单等）与列表页中的字典列渲染。
...
> 组件设计体系为新增设计文档体系；本文档为该体系第 02 篇（首个组件节点，总纲见[《组件设计 · 总览》](../01_组件设计_总览.md)）。文档涉及多处上游变更（批量接口新增、运行时查询接口扩展、sys_dict_item 增加 parent_id、L1 缓存与压测计划），见第 8 节「需同步的既有设计」，待用户确认后同步修订[《概要设计·09 字典管理》](../../概要设计/09_概要设计_字典管理.md)、《项目规划说明》核心数据表清单与相关架构节点。
