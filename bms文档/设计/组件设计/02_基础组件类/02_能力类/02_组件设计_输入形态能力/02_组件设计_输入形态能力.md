...
> 可交互原型：[02_原型设计_输入形态能力.html](02_原型设计_输入形态能力.html)（演示值绑定、前后缀/前置后置插槽、清空、只读、输入法组合事件与聚焦/失焦；并与 能力 `value`/`field` 字段语义区分）

## 1. 目的与适用范围 <a id="purpose"></a>

定义 BMS 前端 `frontend/packages/*`（核心 `frontend/packages/core` + 按需的 Vue 投影 `frontend/packages/vue`；渲染插件 `ui-ep` / `ui-vant` 按需）**输入形态能力（原「输入形态能力」）**的组件设计：核心能力类 `BaseInputControl`（`frontend/packages/core/src/capabilities/input-control.ts`，登记 `capabilities/manifest.ts`）（+ 按需的 Vue 投影），为**所有输入型组件**（文本框、文本域、密码、数字、下拉、日期等）提供**通用输入形态**——值绑定、插槽（前缀/后缀/前置/后置）、清空、只读、输入法组合事件。它是组件设计体系**能力**，继承 [组件根基类](../../01_机制基类/02_组件设计_组件根基类/02_组件设计_组件根基类.md)。
...
> **实现口径（2026-09-17，新体系）**：原「输入形态能力」（旧 `useInputControl` 组合式）已类化为核心能力类 `BaseInputControl`（`frontend/packages/core/src/capabilities/input-control.ts`，继承 `BaseCapability`，登记 `capabilities/manifest.ts`）；本能力暂无 Vue 投影（按需回补，`useXxx` 只落 `frontend/packages/vue/src/bindings/`），`BaseInputControl.vue` 组件包装为按需形态（历史未落码）；旧实现（原 `useXxx` 组合式）已随旧层删除（历史经 git 追溯）。`depends` 由能力机制开发态校验（单向、无循环）；契约语义（Props / 事件 / 插槽 / 行为规则）保持不变。
