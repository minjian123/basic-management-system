...
| `get_db` / `get_uow` / `build_session_factory` | `db/session.py` | `async_sessionmaker` 会话工厂、请求级会话依赖与工作单元依赖（每请求独立、退出释放） | 已交付 |
...
| 事件发布 / 消费 | `events/base.py` | `EventEnvelope`（`event_type` / `payload` / `tenant_id` / `trace_id`）+ `EventPublisher` / `EventConsumer`（→ `BaseEventWorker`：`event_type`；发布 / `consume`） | 八 系统集成与消息 |
