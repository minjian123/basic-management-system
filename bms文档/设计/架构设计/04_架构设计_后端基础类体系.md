...
| 层 / 支线 | 基类 | 代码落地位置 | 实施阶段 |
| --- | --- | --- | --- |
| 根系 根基类 | `BaseObject` | `app/core/base.py` | **阶段一已交付** |
...
| `BaseModel` | `models/base.py` | ORM 公共字段与约定（雪花 ID、软删除、审计字段、乐观锁、四库类型映射）+ 软删除辅助 `soft_delete()` / `restore()`；审计由 ORM 事件自动填充，`version_id_col` 乐观锁；根系，横切按插层扩展 | 已交付 |
...
| `BaseProviderRegistry[ItemT]` | `core/provider.py`（03-2 自 `core/registry.py` 归位；后者过渡导出） | 提供者注册表中间层（继承 `BasePluggable`）：`register` 唯一性拒重 / `get` 未命中 `None` / `keys` / `values` 保序；`_provider_key` 钩子供各域（统一取 `provider.key`） | 已交付（阶段三 02-2，2026-09-15；**4 域注册表全部接入，03-3 收敛完成**） |
...
| `CacheRegion` / `NullCacheRegion` | `cache/base.py`、`cache/null.py` | 缓存 Region 契约（`key = "cache"`；`domain` / `get` / `set` / `delete` / `get_global_version` + `build_key` 规范 / `is_stale` 版本判定）+ 占位实现（恒未命中 / 空操作 / 版本 0；真实实现随通用能力阶段） | 已交付（05，2026-09-15） |
| `AuditCapturer` / `NullAuditCapturer` | `audit/base.py`、`audit/null.py` | 审计捕获契约（`key = "audit"`；`is_audited` / `capture` → 审计事件）+ 占位实现（不判定、返回占位事件；真实落库随审计阶段） | 已交付（05，2026-09-15） |
| `BaseTask` / `NullTask` | `tasks/base.py`、`tasks/null.py` | 任务调度契约（`key = "task"`；`name` / `queue` / `run`）+ 占位实现（不执行；调度入库随阶段六） | 已交付（05，2026-09-15） |
| `EventPublisher` / `NullEventPublisher` | `events/base.py`、`events/null.py` | 事件发布契约（`key = "event"`；`event_type` / `publish` / `publish_transactional`）+ 占位实现（空操作；消费轨随阶段八） | 已交付（05，2026-09-15） |
...
| `BaseObjectStorage` / `LocalObjectStorage` / `MinioObjectStorage` / `NullObjectStorage` / `StoredObject` / `PresignedUrl` | `storage/base.py`、`storage/local.py`、`storage/minio.py`、`storage/null.py` | 对象存储能力域契约（`key = "object_storage"`；异步 `put` / `get` / `delete` / `exists` / `presign`）+ `local` 真实读写（键安全 / 原子写 / `file://` 预签名，缺省）/ `minio` 延迟导入（密钥环境注入）/ `null` 占位（固定返回）+ 对象元数据 / 预签名结果数据契约；常量 `STORAGE_BUCKET`（`bms-files`）/ `DEFAULT_PRESIGN_TTL`（3600）/ 提供者 `get_object_storage` | 已交付（04-1，2026-09-15；真实连通随文件管理阶段） |
...
| `BaseQueryProvider` / `BaseQueryProviderRegistry` / `NullQueryProvider` / `NullQueryProviderRegistry` / `QueryResult` | `query/base.py` | 数据查询提供者契约（继承 `BaseProvider`：抽象 `key` + 抽象 `describe` + 异步 `query`，只读约束）+ 共享注册表契约（继承 `BaseProviderRegistry`；`key = "query_provider_registry"`；登记 / 解析 / 清单继承公共实现 + `query` 聚合模板，未命中抛 `NotFoundError`）+ 占位实现（登记真实、聚合空结果 / 固定返回、不执行 SQL）+ 结果数据契约 | 已交付（占位） |
...
| `BaseFieldType` / `BaseFieldTypeRegistry` / `NullFieldTypeRegistry` | `fieldtype/base.py` | 字段类型提供者契约（继承 `BaseProvider`：抽象 `key` + 抽象 `describe` + 同步 `validate` / `render_metadata` / `column_type`）+ 注册表契约（继承 `BaseProviderRegistry`；`key = "field_type_registry"`；登记 / 解析 / 清单继承公共实现 + `validate` / `column_type` 聚合模板）+ 占位实现（登记真实、恒定通过 / 固定映射）；常量 `COLUMN_TYPE_DIALECTS`（四库）/ `NULL_COLUMN_TYPE` / 提供者 `get_field_type_registry` | 已交付（占位） |
...
| `BaseDashboardCardProvider` / `BaseDashboardCardRegistry` / `NullDashboardCardRegistry` | `dashboard/base.py` | 工作台卡片提供者契约（继承 `BaseProvider`：抽象 `key` + 抽象 `describe` + 同步 `metadata` / 异步 `fetch`）+ 注册表契约（继承 `BaseProviderRegistry`；`key = "dashboard_card_registry"`；登记 / 解析 / 清单继承公共实现 + `metadata` / `fetch` 聚合模板）+ 占位实现（登记真实、空卡片集）；常量 `CARD_TYPES`（builtin / dataset）/ 提供者 `get_dashboard_card_registry` | 已交付（占位） |
...
- **继承链**（按层分组）：
  ...
  - 缓存 `NullCacheRegion` → `CacheRegion` → `BaseCapability` → `BaseObject`（真实实现随通用能力阶段）。
  - 审计捕获 `NullAuditCapturer` → `AuditCapturer` → `BaseCapability` → `BaseObject`（真实落库随审计阶段）。
  - 任务 `NullTask` → `BaseTask` → `BaseCapability` → `BaseObject`（调度入库随阶段六）。
  - 事件发布 `NullEventPublisher` → `EventPublisher` → `BaseEventWorker` → `BaseCapability` → `BaseObject`（消费轨随阶段八）。
  - 开放接口服务端 / scope `NullOAuthServer` / `NullScopeChecker` → `BaseOAuthServer` / `BaseScopeChecker` → `BaseCapability` → `BaseObject`。
  - 对象存储 `LocalObjectStorage` / `MinioObjectStorage` / `NullObjectStorage` → `BaseObjectStorage` → `BaseCapability` → `BaseObject`。
  ...
  - 数据查询 `BaseQueryProvider` → `BaseProvider` → `BaseCapability` → `BaseObject`（组合轨，不入插件注册表），`NullQueryProviderRegistry` → `BaseQueryProviderRegistry` → `BaseProviderRegistry` → `BaseCapability` → `BaseObject`。
  ...
  - 字段类型 `BaseFieldType` → `BaseProvider` → `BaseCapability` → `BaseObject`（组合轨），注册表 `NullFieldTypeRegistry` → `BaseFieldTypeRegistry` → `BaseProviderRegistry` → `BaseCapability` → `BaseObject`。
  - 国际化翻译 `NullTranslator` → `BaseTranslator` → `BaseCapability` → `BaseObject`。
  - 工作台卡片 `BaseDashboardCardProvider` → `BaseProvider` → `BaseCapability` → `BaseObject`（组合轨），注册表 `NullDashboardCardRegistry` → `BaseDashboardCardRegistry` → `BaseProviderRegistry` → `BaseCapability` → `BaseObject`。
...
| `BaseProviderRegistry` | 中间层（注册表契约） | `register` 唯一性 + `get` 未命中 + `keys` / `values` 公共实现，收敛 4 个域注册表与其 Null 注册表；插件注册表复用唯一性语义 | **已交付**（02-2，2026-09-15；03-2 归位 `core/provider.py`；4 域全部收敛随 03-3） |
...
| 文件存储（对象存储，横切扩展） | `storage/base.py`、`storage/local.py`、`storage/minio.py` | `BaseObjectStorage(BaseCapability)`（`key = "object_storage"`；异步 `put` / `get` / `delete` / `exists` / `presign`）+ `LocalObjectStorage`（真实读写，缺省）/ `MinioObjectStorage`（延迟导入）/ `NullObjectStorage`（固定返回、不连 MinIO / 不落盘）+ `StoredObject` / `PresignedUrl` / `STORAGE_BUCKET`（`bms-files`）/ `DEFAULT_PRESIGN_TTL`（3600）/ 提供者 `get_object_storage`；下载 / 预览走预签名 URL | 契约与 `local` / `minio` 已交付（04-1，2026-09-15）；真实连通 / 桶策略随文件管理阶段 |
