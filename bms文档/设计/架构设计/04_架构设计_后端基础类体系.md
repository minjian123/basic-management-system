...
| `BasePluggable` | `core/plugin.py` | 插件化中间层（继承 `BaseCapability`）：插件键 `plugin_key` / 实现名 `plugin_name` / 契约版本 `contract_version` / `setup`·`aclose` 生命周期 / `describe`；子类登记进插件注册表，按配置解析（`resolve_plugin`）；继承自动登记与显式注册双入口 | 待交付（**阶段三首务**，见 [10-扩展点与插件化](10_架构设计_子系统_扩展点与插件化.md)） |
...
| `BaseHealthCheck` | `health/base.py` | 健康检查项契约（抽象属性 `name` + 异步 `check()` 返回 `HealthCheckResult`），继承 `BaseObject`；真实检查项 `RedisHealthCheck`（兼 `BaseAsyncResource`）/ `DatabaseHealthCheck` 见 `health/checks.py` | 已交付 |
...
| 候选 | 形态 | 说明 | 优先级 / 状态 |
| --- | --- | --- | --- |
| `BasePluggable`（`core/plugin.py`） | 中间层（继承 `BaseCapability`） | 插件键 / 实现名 / 契约版本 / 生命周期 / 双轨注册 + 插件注册表与 `resolve_plugin` | **P0 必补**（已登记待交付，阶段三首务） |
| 能力域基类改继承 `BasePluggable` | 改造面 | 40 个 `BaseCapability` 直接子类，业务方法不变 | P0（随插件化设计） |
| 提供者改经 `resolve_plugin` | 改造面 | 35 个 `get_xxx(request)` 退化为薄封装；`config.toml` provider/options + 启动校验 | P0（随插件化设计） |
| Null 实现拆分与目录对齐 | 改造面 | 38 个 `NullXxx` 由 `base.py` 迁至同域 `null.py`、导入路径调整（见《架构设计 · 扩展点与插件化》「代码结构」节） | P0 配套（阶段三前置） |
| `BaseProvider` | 中间层（注册项契约） | 统一注册项契约（抽象 `key`），收敛 `BaseFieldType` / `BaseQueryProvider` / `BaseDashboardCardProvider` / `BaseHealthCheck` | P1（提取收敛） |
| `BaseProviderRegistry` | 中间层（注册表契约） | `register` 唯一性 + `get` 未命中 + `keys` 公共实现，收敛 4 个域注册表与其 Null 注册表；插件注册表复用唯一性语义 | P1（提取收敛） |
| 契约测试基座（测试侧） | 测试侧基座 | 统一「注册表 + 全部注册实现」契约套件（唯一性 / 未命中语义 / Null 无副作用 / 聚合模板） | P1（阶段三验收配套） |
