...
| 基类 | 文件 | 职责 | 状态 |
| --- | --- | --- | --- |
| `BaseObject` | `core/base.py` | 所有可继承类的公共方法唯一落点（`to_dict` 声明字段优先——dataclass / Pydantic `model_dump` / `__dict__`；稳定序 `to_json`；统一字符串输出；主键相等与哈希） | 已交付 |
