...
| `BizError` 及子类 | `core/exceptions.py` | 业务异常基类（`BizError(BaseObject, Exception)`，携带错误码 / `http_status` / `data`）+ 骨架子类；全局处理器 → 统一响应 | 已交付 |
...
- **配置基座**：`app/core/config.py` 分区模型（继承 `BaseSchema`）+ `get_settings()`（结构占位，返回默认值、不读文件 / env）；真实 `config.toml` 读取 + `BMS_` 环境变量覆盖 + `BMS_ENV` 分层与启动校验由 03-1 落地后回填（12-factor，敏感项只走环境变量，不入库、不入镜像）。
