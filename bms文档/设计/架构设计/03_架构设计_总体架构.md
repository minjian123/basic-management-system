...
并给出开发计划（十六阶段）与架构落地章节的映射关系。对应[《项目规划说明》](../../规划/项目规划说明.md)的「技术栈」
「项目目录结构」「开发计划与验收标准」章节。
...
graph TD
    ...
    C --> G["AI（阶段十六）<br/>Milvus<br/>PaddleOCR<br/>LLM 适配层"]
...
| event-worker | RocketMQ 事件消费者（多消费组）：操作日志异步化、Webhook 推送、审计链分区有序消费、ES 索引同步（阶段十五）、embedding 管线（阶段十六） | 多副本 |
...
| AI（阶段十六） | LLM 适配层（外部 API + 私有化端点可配）、Milvus 向量库、PaddleOCR（本地 + 多模态 API 可切换） |
...
| AI（阶段十六） | Milvus（milvus + etcd，MinIO 复用）、PaddleOCR（可选） |
| 健康检查 | /healthz、/readyz |
...
《项目规划说明》开发计划十六阶段与架构落地章节对应关系（概要设计与详细设计按阶段展开）。十六阶段按**里程碑分层**分组（定义见规划第 20 节）：**Alpha 内部可用 = 阶段一~七**（平台底座与前端组件库完整）、**Beta 功能完整 = 阶段八~十二**（实际业务闭环 + 非功能加固 + 全部前端，此后冻结功能范围）、**GA 正式发布 = 阶段十三~十六**（增强能力齐备，通过 MVP 验收后定版）：

| 阶段 | 内容 | 对应架构节点 |
| --- | --- | --- |
| 一 项目骨架 | monorepo、配置/日志/健康检查、SQLAlchemy 接入、平台/租户库拓扑、批量迁移、CI、模块注册表（sys_module：模块简称/表前缀/错误码段/事件域平台统一维护，启动与 CI 校验唯一）；L1a 前端数据/契约基类 | 总体架构、数据访问与分片、多租户路由、测试与质量、部署与运维、模块注册 |
| 二 前端组件库 | 前端基础类体系 L0-L4、布局/容器/基础控件/基础类组件（地基波约 45 节点）、组件单测与原型对齐；字段/展示/交互类按对应阶段回补 | 前端组件体系、前端架构 |
| 三 认证与安全 | 双 token、限流、验证码、会话、锁定、SSO、密码策略、脱敏 | 认证与会话、性能与安全 |
| 四 RBAC 基础模块 | 用户/岗位/角色/菜单/权限码/字段权限/数据范围/三权分立/平台超管与租户管理（含模块开通勾选与租户模块开关） | 权限计算引擎、多租户路由、数据架构、模块注册 |
| 五 通用能力 | 事件总线 + Celery 接入、字典/参数、文件、导入导出、通知/公告、帮助中心、回收站、邮件短信、日志异步化、字段审计、国际化、归档、首页工作台后端（卡片注册表/平台与角色模板/个人布局与一键重置接口）、表单定制后端（布局配置表与三级解析/字段属性/租户自建字段动态 DDL 与跨方言类型映射/服务端动态校验/查询区与明细区配置）；组件回补（字段类/展示类） | 事件总线、任务调度、文件管理、通知公告、帮助中心、国际化、审计哈希链、首页工作台、表单定制、前端组件体系 |
| 六 工作流引擎 | SpiffWorkflow、bpmn-js、会签/或签、版本管理；组件回补（审批流展示/流程建模器） | 工作流、前端组件体系 |
| 七 系统集成与消息 | 开放接口（OAuth2）、Webhook、事件模型 | 接口与集成、事件总线 |
| 八 业务外置（采购/收付款） | 采购、收付款业务剥离至 **biz 仓库**设计与实施（申请/三级审批/订单/供应商档案、收付/退款单据与审批、流水台账、支付渠道），平台提供工作流、通知、事件总线等能力 | 工作流、通知公告、事件总线（biz 侧设计） |
| 九 业务外置（供应商/销售/仓储） | 供应商/销售/仓储等业务剥离至 **biz 仓库**设计与实施，平台提供模块注册、接口与集成等能力 | 模块注册、接口与集成（biz 侧设计） |
| 十 分布式与监控 | 分片落地、租户路由演练、监控告警、缓存管理、备份、链路追踪、任务调度 | 数据访问与分片、可观测性、任务调度、部署与运维 |
| 十一 性能加固与压测 | 缓存三防、限流降级熔断、索引、locust | 性能与安全 |
| 十二 前端全页面 | 主布局、首页工作台（卡片拖拽定制/自建图表卡片/一键重置，登录后默认页）、表单布局管理（vuedraggable 设计器：分区/分组/列数/跨列/顺序/标签宽度、查询表单区、明细区列配置、字段属性、租户自建字段）、全部管理页（含用户喜好：主题/列表页个性化/通知偏好/工作台自定义）、i18n、个人中心、E2E（组件库阶段二已交付地基波） | 前端架构、前端组件体系、首页工作台、表单定制、测试与质量 |
| 十三 报表 BI | 数据集、报表设计器、大屏、工作台数据集图表卡挂接（gridstack 统一布局）；组件回补（图表卡/报表设计器/大屏设计器） | 报表 BI、首页工作台、前端组件体系 |
| 十四 移动端 H5 | 审批中心、通知、公告、免登 | 前端架构、认证与会话、工作流 |
| 十五 全文检索 | ES 接入、事件同步、索引重建、文件解析；组件回补（全局搜索） | 全文检索、事件总线、前端组件体系 |
| 十六 AI 能力 | LLM 适配、智能报表助手（NL2SQL）、智能审批辅助、AI 内容生成、语义搜索与文件问答（RAG/Milvus）、OCR 与自动分类、审计异常检测、问答机器人、帮助文档智能维护；AI 自动执行写操作与 AI 自动审批（独立开关，默认关闭）；组件回补（AI 助手） | AI 服务、帮助中心、性能与安全、前端组件体系 |
...
- 前端组件库（阶段二地基波）可用：基础类体系 L0-L4 与布局 / 容器 / 基础控件 / 基础类组件、组件单测通过、与可交互原型一致
