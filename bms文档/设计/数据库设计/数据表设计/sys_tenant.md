...
| 状态 | 已落库（平台链迁移 `0001_sys_tenant_module`，2026-09-22；种子见 `ops/seed_tenant.py`） |
...
- **分片**：不分片（平台库常驻）。
- **归档**：不归档（租户注册生命周期数据）。
- **迁移**：随**平台链** Alembic 迁移落地（`alembic/versions/platform/0001_sys_tenant_module.py`，2026-09-22 已落库；命令 `alembic -n alembic:platform upgrade head`）；种子走 `ops/seed_tenant.py`（demo / acme 两行，幂等），建表分支随迁移就位退化为纯种子路径。
