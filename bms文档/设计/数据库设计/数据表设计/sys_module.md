...
| 状态 | 已落库（`ops/seed_module.py` 幂等建表 + 平台域四行种子，2026-09-22；迁移归 01_04） |
...
- **迁移**：新增模块 = 一个 Alembic revision（建表 + 注册种子）原子落地，平台库单库执行；平台注册表运行时不提供增删改。当前过渡：平台域种子先经 `ops/seed_module.py` 幂等落库（复用 `PLATFORM_MODULES` 单一来源），Alembic 迁移与 SQLite 全量自动建表归 01_04（就位后种子脚本退化为纯种子）。
