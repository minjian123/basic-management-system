# sys_dict_type_i18n（字典类型多语言附表）

> BMS · 数据库设计 · 数据表设计

[文档首页](../../../文档首页.md) › [数据库设计总览](../01_数据库设计_总览.md) › [已设计数据表登记](../01_数据库设计_总览.md#tables-registry) › sys_dict_type_i18n
...
| ORM 模型 | `app/dict/models.py::SysDictTypeI18n`（继承 `BaseModel`） |
...
| 字段 | 类型 | 可空 | 约束 / 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `dict_type_id` | BIGINT | 否 | 逻辑引用 `sys_dict_type.id` | 字典类型 ID |
| `locale` | VARCHAR(16) | 否 | 与 `dict_type_id` 复合唯一 | 语言（zh-CN / en-US） |
| `label` | VARCHAR(64) | 否 | — | 类型名翻译（缺省回退主表 `name`） |
