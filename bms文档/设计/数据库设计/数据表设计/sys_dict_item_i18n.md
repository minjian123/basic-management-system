# sys_dict_item_i18n（字典条目多语言附表）

> BMS · 数据库设计 · 数据表设计

[文档首页](../../../文档首页.md) › [数据库设计总览](../01_数据库设计_总览.md) › [已设计数据表登记](../01_数据库设计_总览.md#tables-registry) › sys_dict_item_i18n
...
| ORM 模型 | `app/dict/models.py::SysDictItemI18n`（继承 `BaseModel`） |
...
| 字段 | 类型 | 可空 | 约束 / 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `dict_item_id` | BIGINT | 否 | 逻辑引用 `sys_dict_item.id` | 字典条目 ID |
| `locale` | VARCHAR(16) | 否 | 与 `dict_item_id` 复合唯一 | 语言（zh-CN / en-US） |
| `label` | VARCHAR(128) | 否 | — | 条目标签翻译（缺省回退主表 `label`） |
...
| 名称 | 类型 | 列 | 说明 |
| --- | --- | --- | --- |
| `uq_dict_item_i18n` | 唯一 | `(dict_item_id, locale)` | 条目 + 语言唯一 |
| `ix_sys_dict_item_i18n_dict_item_id` | 普通 | `(dict_item_id)` | 主表关联 |
| `ix_sys_dict_item_i18n_deleted_at` | 普通 | `(deleted_at)` | 软删除过滤 |

- 无物理外键；`type_id` / `dict_item_id` / `dict_type_id` / `dict_attr_id` 为逻辑引用（同租户库），`parent_id` 逻辑引用条目 `value`（级联父值）。 运行时取数按 locale 关联本表，缺省回退主表默认 label。
