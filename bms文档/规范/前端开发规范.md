...
本节强制口径由**机器护栏**执行（不依赖自觉）：CI 继承护栏（扫描基类链 / 组件包装 / 片段声明）、依赖边界测试（import 方向断言，禁止反向 / 重复实现）、契约测试套件（片段与基类契约，所有实现跑同一套）、清单对账脚本（《[前端基类清单](../前端基类清单.md)》↔ 代码双向校验）、注册表「不注册不可用」。护栏清单、落点与阶段随《[架构设计 · 前端组件体系](../设计/架构设计/05_架构设计_前端组件体系.md)》（「执行护栏」节）与阶段四任务完成标准登记，本规范不重复展开。
