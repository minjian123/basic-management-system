...
- **基础类与继承约定（强制）**：公共字段/方法只在基础类维护，模块不得重复实现——
  - 契约类型：响应/分页继承 `ApiResponse<T>` / `PageResponse<T>`；实体继承 `BaseEntity`（`id` 为 string——雪花 BIGINT 超 JS 安全整数，JSON 边界以字符串传输）；列表查询参数继承 `BasePageQuery`
  - API 模块：继承 `BaseApi`（`src/api/base.ts`），统一路径前缀与请求方法，禁止绕过基类裸用 axios
  - Store：列表/详情场景用 `createCrudStore`（`src/stores/base.ts`）派生
  - 序列化：对拍/存档/日志输出用 `stableStringify`（`src/utils/serialize.ts`），禁止直接 `JSON.stringify` 做稳定比对
  - 双工程同款基线：frontend 与 frontend-mobile 同步扩展，不得只在单端加基类能力
