...
> **主体开发（平台内建主线）用继承保统一，项目二次开发 / 定制用组合保灵活**——两轨不互斥，汇入同一登记与契约测试体系。本条为 **§3「前端基类体系」的前置口径**，细则见 §3.4「继承与组合双轨」；AI 侧同步见《[AI开发规范](AI开发规范.md)》「架构铁律」节与工作区 `AGENTS.md`。
...
3. **两轨汇入同一体系**：同一登记清单 + 同一套契约测试（`@bms/core/testing` 契约用例工厂对全部实现跑同一套断言），**均须过契约测试**。
...
- **必须从根派生（强制）**：一切基类从根系 `BaseFrontend` / `useFrontendBase` 派生（契约/数据根、组件根、机制基类、能力层、域基类同）；UI 组件必须挂到组件根或域基类的组件包装上；任何层不得绕过链层另起炉灶。
- **必须经能力取得横切行为（强制）**：正交横切行为必须经链上能力层取得（核心 `BaseXxx extends BaseCapability` 类化 + 登记 + 按需投影），禁止在组件内自实现（同后端「禁止绕过 `BaseService` 直连 `BaseRepository`」）。
- **继承与组合双轨（强制，2026-09-17 修正）**：**主体内建（平台主线）一律走继承轨**——能力即继承链上的层、统一受控；**项目二次开发 / 定制走组合轨**——不继承平台基类、经注册通道接入、不回改平台基类；两轨汇入同一登记与契约测试体系，详见 3.4 节。
...
flowchart TD
    ...
    COMP --> CAP["能力层 BaseXxx extends BaseCapability（核心）"]
    CAP --> DOM["域基类（输入 / 展示 / 树 / 图表 / 编辑器 / 字段 …）"]
    DOM --> C["具体组件 / 件（插件：ui-ep / ui-vant）"]
    CAP --> PROJ["投影 useXxx（frontend/packages/vue/src/bindings）"]
    PROJ --> C
```
...
- 投影 `useXxx` 是继承轨在 Vue 侧的**薄适配**（observable → `ref` / `computed`，不含判定逻辑），只存在于 `frontend/packages/vue/src/bindings/`；换渲染框架即换一份适配 + 渲染插件，链本身不动。

**组合轨（二次开发 / 定制，非主体）**：

- 项目二次开发、租户定制、第三方实现**不继承平台基类**，经统一注册通道登记接入（登记 + 契约测试），**不回改平台基类**；
- 组合轨实现与继承轨**汇入同一登记与同一套契约测试**（`@bms/core/testing` 契约用例工厂对全部实现跑同一套断言）；
...
| 新层 / 新能力形态 | 落点 |
| --- | --- |
| 多个组件可自由选配的横切能力（形态 / 值 / 权限 / 数据源 / 上传…） | 新增**能力层**（`capabilities/<域>.ts` + 登记；投影按需） |
...
- **契约测试套件**（`@bms/core/testing`）：确认 / 权限 / 容器件 / 权限按钮 / 反馈件 / 输入控件（文本三件 `05_01`、数字框与下拉框 `05_02`、选择三件「单选 / 复选 / 开关」`05_03`）等契约由各插件跑**同一套断言**——「同接口多实现」的机器保障；
