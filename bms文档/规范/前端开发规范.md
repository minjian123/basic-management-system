...
- **实现口径（现行）**：格式化纯函数落 `frontend/apps/*/src/utils/format.ts`（一律 `Intl`、按用户时区 / locale、空值与非法值占位；默认基准经 `configureFormatDefaults` 注入，时区偏好经 `useTimezone` 占位管理）；名义格式化器注册表随展示域回补波按新体系重建（原片段层 `formatters.ts` 已随旧层删除）；校验规则工厂落 `frontend/apps/*/src/utils/validators.ts`（空值跳过、必填归 `required`）。
