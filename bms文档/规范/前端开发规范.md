...
> 本项目 · frontend/apps/desktop（Vue 3 + Vite + TS）开发约定

[文档首页](../文档首页.md) › 规范 › 前端开发规范　|　[同级：后端开发规范 →](后端开发规范.md)　|　[命名规范 →](命名规范.md)

## 1. 概述 <a id="overview"></a>

本文档定义本项目前端（frontend/apps/desktop PC 管理端；frontend/apps/mobile 移动端按同约定执行）的开发规范：
...
**新体系分层（S0 ~ S5 落地，强制）**：前端基类体系 = **纯 TS/JS 框架无关核心**（`frontend/packages/core`）+ 可替换渲染插件。单仓多包 workspace：
...
- **新增能力 → 新增能力片段**（`useXxx`，正交可组合）：七形态等预置片段只是首批，非封闭枚举；机制横切归机制基类（`src/base/`：片段机制 `BaseCapability`、占位 `BasePlaceholder`、异步资源 `BaseAsyncResource`、注册表 `BaseRegistry` + 注册项 `BaseRegistryItem`、错误 `BaseError`、订阅 `BaseSubscription`）；**新增域语义 → 新增域基类**（按需，如 `BaseTree` / `BaseChart` / `BaseEditor`）——两者均不改动根系 / 组件根。
  ...
  - **能力层落点（强制）**：能力类（原「片段」）统一落 `frontend/packages/core/src/capabilities/<能力域>.ts`（`BaseXxx extends BaseCapability`，纯 TS；登记表 `frontend/packages/core/src/capabilities/manifest.ts`）+ 领域共享逻辑 `frontend/packages/core/src/domain/` + 契约类型 `frontend/packages/core/src/contracts/`；Vue 组合式投影落 `frontend/packages/vue/src/bindings/useXxx.ts`（`useXxx` 只存在于此）；具体组件落各渲染插件 `frontend/packages/ui-ep|ui-vant/src/components/<域>/`。**旧 `src/components/base/` 片段层已随 S4c / S6 删除**（历史经 git 追溯），新代码一律走上表新体系；能力**不含 UI 视觉与业务语义**，横切能力经根系 / 组件根取得，不得重复实现。
