...
| — | 具体组件 | — | L4 | 03 基础控件 / 04 字段类 / 05 展示类 / 06 交互类 |
...
- `BaseInput`（输入域）：继承 `BaseField`，做输入域的最终细化，供 04 字段类与 03 基础控件继承。
- `DisplayBase`（展示域）：组合 `BaseDisplayControl` + `BaseValue`，做展示域细化，供 05 展示类与字段只读回显继承。
