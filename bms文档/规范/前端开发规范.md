...
> 本项目 · apps/desktop（Vue 3 + Vite + TS）开发约定

[文档首页](../文档首页.md) › 规范 › 前端开发规范　|　[同级：后端开发规范 →](后端开发规范.md)　|　[命名规范 →](命名规范.md)

## 1. 概述 <a id="overview"></a>

本文档定义本项目前端（apps/desktop PC 管理端；frontend-mobile 移动端按同约定执行）的开发规范：
...
- 双工程独立：apps/desktop 与 apps/mobile 各自 package.json / lock 与 ESLint/Prettier/TS 配置，互不共享；npm 管理；**前端单仓多包 workspace**（`packages/*` 见 §3「新体系分层」）
- **包级门禁（S7 工程化批次）**：`packages/*` 各自 `eslint.config.js`（根 `devDependencies` 统一装 eslint / typescript-eslint / eslint-plugin-vue / vue-eslint-parser / @vue/eslint-config-prettier），各包 `npm run lint`（`--max-warnings 0`）并汇入根 `npm run lint` / `npm run check`（CI `core-check` 自动覆盖）；格式化规则交 Prettier（`skip-formatting`），不在 ESLint 重复
...
**新体系分层（S0 ~ S5 落地，强制）**：前端基类体系 = **纯 TS/JS 框架无关核心**（`packages/core`）+ 可替换渲染插件。单仓多包 workspace：

| 层 | 落点 | 约束 |
| --- | --- | --- |
| 核心 | `packages/core`（根系 / 机制 / 能力类 / 领域 / 契约；测试工具 `@bms/core/testing`） | **不得依赖 Vue / UI 库 / 插件 / 宿主**（护栏 `guard-core-framework-agnostic.spec.ts`）；能力类必须继承机制基类并登记 |
| Vue 绑定插件 | `packages/vue`（组合式投影 `useXxx` / 指令 / 令牌样式） | 依赖 core，不依赖 UI 库与宿主；`useXxx` 只存在于此 |
| 渲染插件 | `packages/ui-ep`（PC，Element Plus）/ `packages/ui-vant`（移动端，Vant） | 依赖 core + vue；插件之间不互相依赖 |
| 宿主装配 | `apps/desktop` / `apps/mobile`（路由 / 菜单运行时 / 页面 / 注入接线） | 依赖插件与 core；宿主不含基座实现 |

**依赖方向（强制）**：宿主 → 插件 → core；**「同接口多实现 + 契约测试同一套断言」**——同一契约由各渲染插件实现，契约用例工厂（`@bms/core/testing`）对全部实现跑同一套断言（案例：容器件 8 件 + `PermButton` 在 `ui-ep` / `ui-vant` 双实现全绿，S4b）。
...
- **同一契约多实现（S4 起，替代原「双端同款基线」）**：`packages/ui-ep`（PC）与 `packages/ui-vant`（移动端）是**同一契约的两个实现**，不要求字节级同款；一致性由 `@bms/core/testing` 契约用例工厂（同一套断言）与 `@bms/core` 共享逻辑（单实现）共同保证。新增基础能力时：**先落 core / vue（框架无关）**，再按需在各渲染插件提供实现与契约用例——不得只在单端新增导致契约漂移。
...
- **新增能力 → 新增能力片段**（`useXxx`，正交可组合）：七形态等预置片段只是首批，非封闭枚举；机制横切归机制基类（`src/base/`：片段机制 `BaseCapability`、占位 `BasePlaceholder`、异步资源 `BaseAsyncResource`、注册表 `BaseRegistry` + 注册项 `BaseRegistryItem`、错误 `BaseError`、订阅 `BaseSubscription`）；**新增域语义 → 新增域基类**（按需，如 `BaseTree` / `BaseChart` / `BaseEditor`）——两者均不改动根系 / 组件根。
  ...
  - **片段机制契约要点（强制）**：片段以 `useCapabilityBase({ key, depends })` 声明（`key` kebab-case，与《前端基类清单》『能力片段』节 一致）；`depends` **只登记片段→片段的单向依赖**（节点正文的「协作 / 消费」关系不入表）；开发态校验「依赖已登记 / 无循环 / `key` 合法」，违规默认告警（`strict` 抛错），生产态零开销；「不得依赖上层」由 `02_05` 的 import 方向静态护栏把关。
  ...
  - **能力层落点（强制）**：能力类（原「片段」）统一落 `packages/core/src/capabilities/<能力域>.ts`（`BaseXxx extends BaseCapability`，纯 TS；登记表 `packages/core/src/capabilities/manifest.ts`）+ 领域共享逻辑 `packages/core/src/domain/` + 契约类型 `packages/core/src/contracts/`；Vue 组合式投影落 `packages/vue/src/bindings/useXxx.ts`（`useXxx` 只存在于此）；具体组件落各渲染插件 `packages/ui-ep|ui-vant/src/components/<域>/`。**旧 `src/components/base/` 片段层已随 S4c / S6 删除**（历史经 git 追溯），新代码一律走上表新体系；能力**不含 UI 视觉与业务语义**，横切能力经根系 / 组件根取得，不得重复实现。
...
> 合规审计（基类链 / 片段声明 / 组件根 / 令牌 / 清单对账 / 双端一致性）按《[前端审计规范](前端审计规范.md)》执行：检查维度与现有资产见其 §5 与附录。
...
- 空态与加载分级：空状态优先**文案引导**（空态组件按场景矩阵；插画统一 `src/assets/images/empty-*.svg`，kebab-case，不内联大段 SVG）；首屏用骨架屏、局部刷新用内容遮罩（`delay` 防闪）、按钮提交用按钮内 loading（EP 自带）；错误页**不展示技术堆栈**，独立路由 `/403` `/500` 与未知路径兜底 `/404`（`03_02`）
...
- 阈值口径：初次落地取「当前基线 + 20%」（2026-09-15：apps/desktop 85 / 63 KB，frontend-mobile 83 / 83 KB）；
  因需求增长需放宽时，在同一提交中调整 `budget.json` 并说明原因（评审可见）。**调整记录**：2026-09-16 `apps/desktop` 85 / 63 → **100 / 75 KB**（原因：请求层统一错误提示引入 `ElMessage` 与消息样式，`02_06_01`）；2026-09-16 `apps/desktop` 100 / 75 → **130 / 90 KB**（原因：错误页独立路由 chunk 与 EP 按钮样式、i18n 文案入产物，域 03 组件将持续增长，`03_02`）；2026-09-16 `apps/desktop` 130 / 90 → **200 / 100 KB**（原因：宿主完整装配（守卫 / 基座 / axios 链）与组件库代码进入产物，此前未被引用被 tree-shake，`03_05`）。
