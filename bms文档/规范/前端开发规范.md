...
- 双工程独立：apps/desktop 与 frontend-mobile 各自 package.json / lock 与 ESLint/Prettier/TS 配置，互不共享；npm 管理
...
- **新增能力 → 新增能力片段**（`useXxx`，正交可组合）：七形态等预置片段只是首批，非封闭枚举；机制横切归机制基类（`src/base/`：片段机制 `BaseCapability`、占位 `BasePlaceholder`、异步资源 `BaseAsyncResource`、注册表 `BaseRegistry` + 注册项 `BaseRegistryItem`、错误 `BaseError`、订阅 `BaseSubscription`）；**新增域语义 → 新增域基类**（按需，如 `BaseTree` / `BaseChart` / `BaseEditor`）——两者均不改动根系 / 组件根。
  ...
  - **能力层落点（强制，S4 起）**：能力类（原「片段」）统一落 `packages/core/src/capabilities/<能力域>.ts`（`BaseXxx extends BaseCapability`，纯 TS；登记表 `packages/core/src/capabilities/manifest.ts`）+ 领域共享逻辑 `packages/core/src/domain/` + 契约类型 `packages/core/src/contracts/`；Vue 组合式投影落 `packages/vue/src/bindings/useXxx.ts`（`useXxx` 只存在于此）；具体组件落各渲染插件 `packages/ui-ep|ui-vant/src/components/<域>/`。**旧 `src/components/base/` 片段层仅作为 `apps/desktop` 宿主内遗留（待收口），新代码一律走上表新体系**；能力**不含 UI 视觉与业务语义**，横切能力经根系 / 组件根取得，不得重复实现。
