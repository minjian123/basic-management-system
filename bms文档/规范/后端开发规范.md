...
与前端「基础类体系」对称：后端所有**基类**收敛为一套分层体系，**公共字段/方法只在对应层基类维护，任何模块不得重复实现**；调整某一层基类即全链生效（无需逐模块改造，适配上百模块与海量接口场景）。基座以 **L0 根基类 `BaseObject`** 为根，向上分出三条支线——**集合体系**（有序 / 进程内并发 / 跨副本）、**模块基类**（`BaseRepository` / `BaseService` / `BaseSchema` / `BaseModel` 等）、**core 横切基座**（配置 / 异常 / 安全 / 日志）；有新的类类型就加一种基类，有新的横切能力就加一层基类。本规范为**后端基座体系的权威**（强制用法口径），分层与基类清单见《[架构设计 · 后端基础类体系](../设计/架构设计/33_架构设计_后端基础类体系.md)》，详细契约见项目骨架 01-3-1《基础类详细设计》。
...
| 集合 | 进程内并发集合 `ConcurrentSorted*` | 类 | `BaseCollection` | 多线程共享：原子 / 快照语义 |
| 集合 | 跨副本有序封装（Redis） | 封装 | — | 多 worker / 跨副本共享：Redis 为事实源 + 只读快照 |
| 模块基类 | 数据访问基类 `BaseRepository[T]` | 抽象类（模板方法） | L0 | 通用 CRUD 流程 + 派生方法 + 数据源 / 分片路由 |
| 模块基类 | 服务基类 `BaseService[T]` | 类 | L0 | 通用 CRUD 委派仓储 + 统一不存在语义 |
...
- 提供：公共方法唯一落点——序列化、稳定序序列化、统一字符串输出、主键相等与哈希；不含任何业务或框架语义。
...
- **进程内并发（强制）**：多线程共享走并发集合，读写与遍历须满足原子 / 快照语义；锁内不得做 IO 与外部回调。
- **跨副本共享（强制）**：跨副本 / 多 worker 共享走跨副本有序封装（Redis 为事实源）；**禁止进程内集合跨副本共享**。
...
- `BaseRepository[T]`：通用 CRUD 流程（模板方法）+ 派生方法；数据源与分片路由由基类注入，模块仓储只实现差异部分，不得自行切换数据源或拼装路由。
- `BaseService[T]`：通用 CRUD 委派仓储；不存在语义由基类统一抛出；模块服务只写业务规则，不得拼 SQL、不得跨模块直连仓储。
- `BaseSchema`：Pydantic 公共配置，并继承 `BaseObject` 获得统一序列化；请求/响应模型**必须**继承，禁止直接继承 `pydantic.BaseModel`。
- `BaseModel`：ORM 公共字段与约定（雪花 ID、软删除、审计字段、乐观锁、四库类型映射）；业务模型**必须**继承，禁止重复声明公共字段。

### 3.5 core 横切基座 <a id="base-class-core"></a>

- **配置基座**：配置分区 + 环境变量覆盖 + 启动校验；敏感项只走环境变量，不入库、不入镜像。
- **异常与错误码**：统一异常基类 `BizError` + 常用子类，业务异常在 services 层抛出，全局处理器统一转统一响应；错误码分段见第 6 节「异常与错误码」与《架构设计 · 接口与集成》，模块不得自定义响应结构或错误码段。
...
- 新增/调整基类须同步：本规范第 3 节 + 《[架构设计 · 后端基础类体系](../设计/架构设计/33_架构设计_后端基础类体系.md)》「基座分层」「基类清单与继承链」登记 + 对应阶段任务基座清单登记 + 相关详细设计。
...
**示例一（共享范围扩大 → 逐层加层，本体系已发生的真实演进）**：集合从"单线程有序"起步（`BaseCollection` → `Sorted*`）；出现多线程共享后，处理方式不是在每个使用点各自加锁，而是**插入进程内并发层** `ConcurrentSorted*`（原子操作 + 快照遍历）；再出现跨副本 / 多 worker 共享，又**插入跨副本层** `redis_collections`（Redis 为事实源 + 只读快照）。三层各自只解决一类共享问题，下层完全不动。
