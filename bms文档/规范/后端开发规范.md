...
| 模块基类 | 数据访问基类 `BaseRepository[T]` / 数据库实现骨架 `BaseDbRepository[T]` | 抽象类（模板方法） | L0 | 通用 CRUD 流程 + 派生方法 + 数据源 / 分片路由 |
...
- **继承链**：模块类 → 对应基类 → `BaseObject`；`BaseSchema → (pydantic.BaseModel, BaseObject)`；集合类 `Sorted*` → `BaseSorted` → `BaseObject`、`ConcurrentSorted*` → `BaseConcurrentSorted` → `BaseSorted` → `BaseObject`；事务服务 `BaseTransactionalService` → `BaseService` → `BaseObject`；异常统一继承 `BizError`。
