...
后端所有**基类**收敛为一套分层体系，**公共字段/方法只在对应层基类维护，任何模块不得重复实现**；调整某一层基类即全链生效（适配上百模块与海量接口场景）。**基类清单**（分层、职责、代码位置、继承链、状态）由《[后端基类清单](../后端基类清单.md)》提供，本规范只定**强制性规则口径**（各能力域的强制用法细则见平台《[架构设计 · 后端基础类体系](../设计/架构设计/04_架构设计_后端基础类体系.md)》「强制用法（细则）」节）；体系机制同见该节点，详细契约见对应详细设计。
...
- **模块独立性（数据所有权，为可拆分性保留）**：模块只拥有自己的数据——禁止跨模块直写他模块的表、共享 ORM 关系或跨模块 JOIN；跨模块读经服务层接口或事件投影；边界声明（表前缀 / 业务码 / 错误码段 / 事件域）随模块注册并受 CI 校验。
...
### 3.4 执行护栏（指引） <a id="base-class-guard"></a>

本节强制口径由**机器护栏**执行（不依赖自觉）：CI 继承扫描（扫描 `app/` 下无基类 / 仅继承 `object` / 内建异常等禁止基类的类，命中即失败）、分层依赖硬校验（`core/` / `repositories/` / `services/` 等 import 方向断言）、契约测试套件（注册表 + 全部注册实现跑同一套用例）、插件启动校验（provider 存在性 / `(plugin_key, plugin_name)` 唯一 / 契约版本 / 依赖可用性，失败即拒启）、清单对账（《[后端基类清单](../后端基类清单.md)》↔ 代码双向校验）。护栏明细与落点随《[架构设计 · 后端基础类体系](../设计/架构设计/04_架构设计_后端基础类体系.md)》与《[架构设计 · 测试与质量](../设计/架构设计/32_架构设计_测试与质量.md)》登记，本规范不重复展开。
