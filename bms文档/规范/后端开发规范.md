...
| 数据访问横切 | 工作单元 `UnitOfWork`（`NullUnitOfWork` 空实现 / `DbUnitOfWork` 会话实现） | 类 | — | 统一异步事务公共方法 `begin` / `commit` / `rollback` |
| 中间层基类 | 占位 `BasePlaceholder` / `BaseNullObject` / `BaseStub`；能力域 `BaseCapability` / `BaseEventWorker`；资源 `BaseAsyncResource` | 类 | `BaseObject` | 占位标记与统一抛错、能力域标识 / 事件类型、异步资源释放 |
...
- `BaseRepository[T]`：仓储**契约**（异步）——CRUD 抽象 + `exists` / `list_page` / `list_cursor` 派生方法 + 数据源 / 分片路由钩子 + `_guard_version` 乐观锁转译；只定契约，不含存储实现。
- `BaseMemoryRepository[T]`：内存基线（异步：字典存储 + 自增 ID），继承契约层，子类只实现同步 `_build` / `_apply`；作为测试替身 / 骨架期实现。
- `BaseDbRepository[T]`：数据库实现骨架（**异步**占位、不连库），继承契约层；真实 CRUD / 软删除 SQL / 多源读写路由随落库阶段回补。
...
- `UnitOfWork`（工作单元，`db/unit_of_work.py`）：统一异步事务公共方法（`begin` / `commit` / `rollback`）；`NullUnitOfWork` 无副作用、`DbUnitOfWork` 基于 `AsyncSession`（02-5-1 已交付）；事务相关横切只改此处。
- `BasePlaceholder` / `BaseNullObject` / `BaseStub`（`core/capability.py`）：占位实现中间层——空实现（Null Object）与未实现占位（`_not_implemented()` 统一抛错）；`BaseDbRepository`、`BaseSecurity` 原语、`Null*` 继承之。
- `BaseCapability` / `BaseEventWorker`（`core/capability.py`）：能力域契约中间层（`key`）与事件工作单元（`event_type`）；跨阶段基座、事件发布 / 消费继承之。
- `BaseAsyncResource`（`core/capability.py`）：异步资源生命周期（`aclose()` + `async with`）；`EngineFactory` 继承，`create_app` lifespan 关闭时统一释放。
...
- **配置基座**：`core/config.py` 分区模型继承配置分区公共基 `BaseSettings`（`BaseSettings → BaseSchema`，拒绝未知键 / 字段名填充）+ `get_settings()` 入口（结构占位，默认值、不读文件 / env）；真实 `config.toml` + `BMS_` 环境变量覆盖 + `BMS_ENV` 分层 + 启动校验随 03-1 落地；敏感项只走环境变量，不入库、不入镜像。
- **异常与错误码**：`core/exceptions.py` 统一异常基类 `BizError`（携带错误码 / `http_status` / `data`）+ **段位基类**（`GeneralError` 1xxxx / `AuthError` 2xxxx / `UserOrgError` 3xxxx）+ 骨架子类；业务异常在 services 层抛出，`api/errors.py` 全局处理器统一转统一响应 `ApiResponse`；错误码分段见第 6 节「异常与错误码」与《架构设计 · 接口与集成》，模块不得自定义响应结构或错误码段。
- **安全基座**：`core/security.py` 安全原语中间层基类 `BaseSecurity(BaseObject)` + `PasswordHasher` / `TokenCodec` / `SessionSecurity`（接口签名占位），供认证与会话复用；**真实实现随认证阶段**，模块不得自行实现哈希 / 令牌逻辑，密钥只走环境变量。
- **日志基座**：`core/logging.py` 日志门面中间层 `BaseLogger` + 占位实现 `StdoutLogger`（stdout、不落文件），入口 `configure_logging` / `get_logger`；真实 structlog JSON / 上下文 / 脱敏随 03-2；基类与模块不得绕过统一日志门面直接 print 或另起 logger。
...
- **继承链**：模块类 → 对应基类 → `BaseObject`；`BaseSchema → (pydantic.BaseModel, BaseObject)`；`ApiResponse → BaseSchema`；`BaseSettings → BaseSchema`；集合类 `Sorted*` → `BaseSorted` → `BaseObject`、`ConcurrentSorted*` → `BaseConcurrentSorted` → `BaseSorted` → `BaseObject`；仓储 `BaseMemoryRepository` / `BaseDbRepository` → `BaseRepository` → `BaseObject`；事务服务 `BaseTransactionalService` → `BaseService` → `BaseObject`；异常 `BizError` → `BaseObject`、段位基 `GeneralError` / `AuthError` / `UserOrgError` → `BizError`、子类 → 段位基。
...
**示例二（实现口径切换 → 停在基类改，不动模块类）**：`BaseRepository` / `BaseService` 原为同步实现；02-5-1 数据访问底座已**在基类统一切换异步**（调用面改动集中一次），模块类不改（只实现同步 `_build` / `_apply`）；达梦同步驱动例外口径按《架构设计 · 数据访问与分片》与「异步与并发」节执行。
