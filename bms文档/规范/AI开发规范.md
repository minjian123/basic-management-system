...
1. **主体内建必须继承**：平台自有实现一律沿基类体系继承链派生（后端与前端**固定前三段：总基类 → 能力域基类 → 插件基类**，同名同序；其下每条链长短由功能与抽象需要决定，组件侧为 **组件根 → 能力基类 → 组件基类 → 具体件**）；**横切功能即继承链上的基类**；公共字段与方法只在链上对应基类维护；不得绕过链层另起炉灶，**不得在链外自由挂接功能**。**前端继承链全系统只有《[前端基类清单](../前端基类清单.md)》一份**，其他文档一律引用。
2. **二次开发 / 定制用组合**：不继承平台基类，经统一注册通道登记接入，不回改平台基类。
3. **两轨汇入同一体系**：同一注册表 / 登记清单 + 同一套契约测试（后端契约版本校验、前端契约用例工厂），**均须过契约测试**。
4. **依赖方向**：只准上层依赖下层，禁止反向依赖；层 / 能力之间的依赖须在登记表显式声明，禁止隐式自由挂接。
5. **返工教训（2026-09-17 · 本轮为第三次口径修正）**：前端曾把该口径写成「组合轨默认、能力不在继承链上、域基类只是可选语义归类」（一次返工），后修正为「能力入继承链」（二次返工）；**本轮进一步推翻「固定分层 + 能力片段」框架**——层级不固定、横切功能一律落为继承链上的基类、「能力类」实体不再存在（能力声明机制由能力域基类承载）。**AI 动手前必须核本铁律；发现文档口径与铁律冲突时，先报冲突、按铁律执行并回写冲突文档，不得沿旧口径继续实施——不允许出现第四次返工。**
...
| 4    | 设计         | `bms文档/设计/`（架构 / 概要 / 布局 / **数据库设计** / 组件 / 原型）           | 知道该朝什么方向做、大概做成什么样子；**涉及数据库或表设计时再读 `设计/数据库设计/`**：总览 → 数据规范（字段级通用约定）→ 方言特性（各库差异）→ 相关表文件 |
...
### 3.2 数据库与表设计的处理方式 <a id="database"></a>

凡任务内容涉及**建表 / 改字段 / 加索引 / 方言差异 / 迁移**，除本规范第 3 节流程外，按下列顺序处理（口径见《[数据库开发规范](数据库开发规范.md)》「与数据库设计的关联与事实源」节）：

1. **先读设计**：`bms文档/设计/数据库设计/` 下按「总览 → 数据规范 → 方言特性（MySQL / PostgreSQL / 达梦）→ 相关表文件」读取；表结构只认表文件，**不复制字段清单**到需求 / 任务 / 详细设计等文档（一律相对链接引用）。
2. **表结构变更顺序（强制）**：**先改表文件**（含「变更记录」追加）→ 再改 ORM 模型 → 再生成 Alembic 迁移（分链，见《[数据库开发规范](数据库开发规范.md)》「迁移与建表口径」节）→ 最后回写总览登记状态与表文件状态（两处一致）。
3. **新建表**：先创建表文件并在《数据库设计总览》「已设计数据表登记」登记，再写模型与迁移；未登记视为未定稿。
4. **通用约定**（公共字段、类型映射、时间、多语言附表、索引与唯一约束、分片与归档、库划分与迁移归属）只在《数据规范》维护，表文件不重复；通用约定变更先改《数据规范》再传导。
5. **方言差异**（类型落点 / DDL / 空值语义 / 标识符与模式 / 索引与约束 / 事务与连接）：先在真库实测，再回写对应方言特性文档并标「**实测**」或「**待验**」；**未实测结论不得写入表文件或规范**。
6. **登记与校验**：表文件 ↔ 总览登记 ↔ 目录一致性、迁移与模型零漂移由基座校验脚本断言（见《[数据库开发规范](数据库开发规范.md)》「与数据库设计的关联与事实源」节）；实施记录须登记核对结论与偏差归口。
...
| 5 测试            | **先登记 Kiwi TCMS 用例取得编号**（分类 / 优先级 / 状态 / 标签 / `text` 字段与批量脚本模板见《[KiwiTCMS部署使用说明](../资料/开发服务器/linux/KiwiTCMS部署使用说明.md)》「用例约定」节与「用例登记（脚本化批量）」节），再写用例并在代码中标注用例编号；占位契约用护栏用例（依赖注入可解析、应用可启动、固定返回可断言） | 用例代码 + 平台编号 |
| 6 验证            | 后端测试、lint、格式化检查、类型检查、基座校验**全绿**（命令与门禁口径见《[测试规范](测试规范.md)》与工程说明）；新增模块覆盖率 100%                                                                                                                                                                                                     | 门禁结果            |
| 7 登记回写        | 架构（基类清单 / 横切基座 / 跨阶段基座契约表）、《后端基类清单》、《后端开发规范》「后端基座体系」节、计划（后续阶段待办 + 父任务偏差跟踪）、任务与父任务状态（需求文档不承载进度）；**涉及数据库时另加**：《数据库设计》表文件（含变更记录）、总览「已设计数据表登记」状态、方言特性文档（实测结论）                                                                                                                                                         | 登记落点表          |
...
- **编号对齐**：Kiwi 用例号以平台自增主键为准——**先在平台登记取得编号**，再回填代码内的用例编号标注与任务记录；平台既有编号不得复用或改派。
...
- **测试产物复用不开口子**：一切针对构建产物的测试**必须消费已构建的固定版本产物、只跑受变更影响的部分**（《[测试规范](测试规范.md)》「被测物与执行范围」节）；**不得以「这次情况不一样」为由自行绕过或另起源码 / 临时构建物**——确需例外的，先向用户逐项说明并确认，未确认不得实施。
...
- □ 构建产物测试（镜像 / 产物）已消费**固定版本产物**且**只跑受变更影响的部分**，未以「情况不同」绕过（例外已上报确认）
...
- □ 涉及数据库 / 表设计：已按第 3.2 节读取数据库设计体系，表结构变更先改表文件（含变更记录）→ 模型 → 迁移 → 状态两处回写
- □ 方言差异结论已真库实测并回写对应方言特性文档（标「实测」或「待验」）；未实测结论未写入表文件或规范
- □ 新增 / 变更表已在《数据库设计总览》登记，且表文件 ↔ 登记 ↔ 目录一致性校验通过
