...
功能:
  ...
  2. 按 用例ID+失败摘要 生成缺陷指纹，查 GitLab 未关闭的同指纹 Issue：
     - 不存在 -> 自动创建 Issue（标题/描述含 commit、用例 ID、dump 路径、环境标记）
     - 已存在 -> 追加评论（附新 dump 路径与 commit）
  3. 输出 Issue URL 与归档路径（供 CI 日志与后续人工处理）
...
def build_description(args, fp: str, dump_path: Path) -> str:
    return (
        f"**自动缺陷上报**（指纹 `{fp}`）\n\n"
        f"- 缺陷指纹：`{fp}`\n"
        ...
        f"处理方式：按《测试规范》9 节，修复验证按「代码 commit + 用例 + dump 导入」重建现场。"
    )
...
def main() -> int:
    ...
    if not (os.environ.get("GITLAB_API_URL") and os.environ.get("GITLAB_API_TOKEN")):
        print("[跳过] 未配置 GITLAB_API_URL / GITLAB_API_TOKEN，仅归档 dump；请人工建 Issue 并引用上述路径。")
        return 0

    fp = fingerprint(args.case_id or "n/a", args.summary)
    project_id = os.environ.get("CI_PROJECT_ID", "")
    if not project_id:
        print("[跳过] 未配置 CI_PROJECT_ID，仅归档 dump。")
        return 0

    print(f"[2/3] 查询同指纹未关闭 Issue（指纹 {fp}）")
    issue = find_open_issue(project_id, fp)
    if issue:
        note = f"再次失败（commit `{commit}`）：新现场 dump `{dump}`，见流水线 {args.pipeline_url}"
        ...
        issue = api(f"projects/{project_id}/issues", "POST", {
            ...
            "description": build_description(args, fp, dump),
            "labels": LABEL_AUTO,
        })
        url = issue["web_url"]
