...
import { sortVisible, useMenuStore } from '@/stores/menu'

import MenuNode from './MenuNode.vue'
import { filterMenuTree, findAncestorKeys, menuKey, type MenuItem } from './types'
...
const source = computed<MenuItem[]>(() =>
  sortVisible(controlled ? (props.menus ?? []) : store.visibleMenus),
)
...
function onOpen(index: string): void {
  if (!controlled) {
    store.setExpanded(index, true)
  }
}

function onClose(index: string): void {
  if (!controlled) {
    store.setExpanded(index, false)
  }
}
...
<template>
  <div v-bind="elAttrs" :class="base.nsClass('side-menu')">
    ...
    <el-menu
      ...
      :default-openeds="controlled ? undefined : store.expandedKeys"
