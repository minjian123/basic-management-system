...
| 项目管理平台 | 禅道（ZenTao 开源版，mjbk Docker Compose 常驻，宿主端口 8070） | 全中文一体化需求/任务/迭代/甘特图/看板；官方 Docker 镜像一键部署；开源版 ZPL 自用免费、不随产品交付；与 GitLab、Kiwi TCMS 职责分工——禅道管需求/任务/迭代/进度，GitLab Issue 只承载代码缺陷与 MR 关联，测试用例仍归 Kiwi TCMS（禅道测试模块不使用） |
...
| zentao | easysoft/zentao（禅道官方镜像） | 8070（容器内 80） | 命名卷（NVMe SSD） | 1.5 GB | 阶段一 |
...
| 禅道 | 随 Docker 命名卷整机级镜像（同 MinIO/ES 行）；另禅道页面备份功能每日导出至 `/mnt/data/backup/zentao/` | 7 天 |
