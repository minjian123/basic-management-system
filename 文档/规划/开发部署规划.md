...
本文档规划 BMS 项目开发环境的整体部署方案：在《[本地资源](../用户文档/本地资源.md)》记载的两台机器上（开发服务器 **mjbk** 已装 Ubuntu 24.04.4 LTS 桌面版，开发机 mjpc 为 Windows 11），搭建可支撑《[项目规划说明](项目规划说明.md)》全部 14 个开发阶段、直至 MVP 验收（含 500 并发压测）的完整开发环境，包括代码托管与 CI/CD、开发依赖服务、常驻数据库、监控与备份。
...
1. **分支模型**：`main` 保护分支；开发在 `feature/xxx` 分支进行，push 至 mjbk GitLab 后提 Merge Request。
...
> 本文档依《文档生成规范》编写 · 关键决策已逐项确认 · 更新日期：2026-08-15（mjbk 新增 2T NVMe SSD：Docker data-root 与 GitLab 数据迁至 NVMe，HDD 仅备份与快照）