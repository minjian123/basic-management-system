...
- 《[架构设计-部署与运维](../设计/架构设计/28_架构设计_部署与运维.md)》的部署拓扑与集群设计要点
...
| 项目管理 | 禅道（ZenTao 开源版，Docker Compose 常驻，宿主端口 8070） | 浏览器访问项目管理平台（需求/任务/迭代/甘特图/看板/文档） |
...
| 项目管理平台 | 禅道（ZenTao 开源版，mjbk Docker Compose 常驻，宿主端口 8070） | 全中文一体化需求/任务/迭代/甘特图/看板；官方 Docker 镜像一键部署；**数据库复用 bms-mysql（`zentao` 库与专用账号，.env 的 `ZENTAO_*`）**，随 MySQL 每日 dump 备份；开源版 ZPL 自用免费、不随产品交付；与 GitLab、Kiwi TCMS 职责分工——禅道管需求/任务/迭代/进度，GitLab Issue 只承载代码缺陷与 MR 关联，测试用例仍归 Kiwi TCMS（禅道测试模块不使用） |
...
graph TD
    A["deploy/"] --> B["compose/"]
    B --> B1["base.yml redis、mysql、postgres、minio、rocketmq、elasticsearch、zentao"]
...
| zentao | easysoft/zentao（禅道官方镜像） | 8070（容器内 80） | 数据库复用 bms-mysql（`zentao` 库）；程序数据命名卷（NVMe SSD） | 1.5 GB | 阶段一 |
...
内存合计：阶段一约 9.5 GB，阶段九后约 16.5 GB，阶段十五后约 20.5 GB；叠加 GitLab（4 GB）与 Ubuntu 桌面 + Docker 开销（约 5 GB），峰值约 29.5 GB，32 GB 预算内留有余量，部署后监控实际占用并按第 11 节调优。
...
1. **分支模型**：`main` 保护分支；开发在 `feature/xxx` 分支进行，push 至 mjbk GitLab 后提 Merge Request；单人开发口径——核心模块改动提交前 AI 交叉评审、MR 附评审结论，不设强制人工批准（见《[项目规划说明](项目规划说明.md)》「环境与配置」节 git 分支条目）。
2. **MR 流水线**（gitlab-runner 在 mjbk 执行）：
   - 后端：ruff 检查 + pytest（SQLite，pytest-cov 覆盖率门禁：核心 ≥ 80%、整体 ≥ 70%）
   - 前端：ESLint + Vitest + frontend / frontend-mobile 双端构建
3. **main 流水线**（MR 合入后）：
   - Playwright E2E（服务容器起 backend（SQLite）与前端产物）
   - MySQL/PostgreSQL/达梦 DM8 三库方言集成测试：job 连 mjbk 常驻三库，建 `bms_test` 前缀测试库 → Alembic 迁移 → 集成测试 → 删库清理
   - 构建 `bms-backend`、`bms-frontend` 镜像推送 GitLab Registry（5050）
   - 容器镜像安全扫描：GitLab Container Scanning（Trivy 引擎）扫描 Registry 镜像，CVSS ≥ 9 高危漏洞阻断发版
   - 导出 swagger.json 契约快照存档
   - 测试报告：Allure 报告生成归档（CI 产物），测试结果经官方插件导入 Kiwi TCMS 用例库归档
   - 测试失败自动上报：任一自动化测试 job 失败时先导出测试库现场 dump 再清理，调 `deploy/tools/defect/defect_capture.py` 生成 REPRO 复现包（dump + 日志 + repro.json + REPRO.md）并自动建/复用 GitLab Issue；指派修复由 `ai_fix.py`（本地模型优先、云端兜底，bot 提 MR 人工合入）处理，`reproduce.py` 一键复现验证（详见《[测试规范](../规范/测试规范.md)》9 节）
4. **依赖升级**：Renovate 自动提升级 MR（原生支持自托管 GitLab）。
5. **GitHub 归档**：GitLab push mirror 单向同步 main 至 GitHub 只读归档仓库。
...
| 禅道 | 业务数据在 bms-mysql 的 `zentao` 库，随 MySQL 每日 dump（`/mnt/data/backup/mysql/`）；程序配置与附件在命名卷 `zentao-data` 随整机级镜像；另禅道页面备份功能每日导出至 `/mnt/data/backup/zentao/` | 7 天 |
...
| 8070 | 禅道（ZenTao） | 项目管理平台，第一批起（阶段一），容器内 80；仅内网 |
...
| 第一批 | mjbk：redis、mysql、postgres、dameng、minio 容器 + GitLab CE + gitlab-runner + 仓库初始化 + .gitlab-ci.yml 落地（达梦 dmPython 驱动兼容性同步验证）+ kiwi-tcms（复用 mysql，宿主 8060）+ zentao（禅道官方镜像，宿主 8070） | 阶段一 |
...
| 内存预算约 29.5 GB（三库常驻 + 桌面 + Docker） | ES 堆限 1–2 GB；GitLab 4 GB 封顶；禅道 1.5 GB 封顶；达梦仅联调与 CI 时段可停（docker compose stop dameng）；按需关闭 Grafana/Loki（阶段九前不启动）；部署后监控 `docker stats` 与实际占用 |
| 禅道资源与许可（ZPL 1.2 非 OSI 认证） | 内存占用与商业合规 | 内存预算 1.5 GB 封顶（部署后 docker stats 监控实际占用）；仅作内部项目管理工具、不随产品交付，商业再分发或对外提供 SaaS 服务需采购商业授权 |
...
| mjbk 外网不可达（GitHub mirror、Renovate、镜像拉取） | 镜像提前缓存到 mjbk；GitHub 归档镜像与 Renovate 暂缓，待网络恢复后启用，不影响内网开发主链路 |
...
- 本文档确定方案与节奏；**《部署手册》**（阶段九交付，规划「文档计划」节）细化 Compose 文件、环境变量清单、备份恢复演练步骤、安全响应头说明；禅道（开发期工具）的容器编排、端口 8070 与备份细节一并纳入。
...
> 本文档依《文档生成规范》编写 · 关键决策已逐项确认 · 更新日期：2026-08-15（mjbk 新增 2T NVMe SSD：Docker data-root 与 GitLab 数据迁至 NVMe，HDD 仅备份与快照） · 修订：2026-08-22（同步《[项目规划说明](项目规划说明.md)》：阶段数笔误 14→15、main 流水线补镜像扫描（Trivy，CVSS≥9 阻断）、分支模型补 MR 单人 + AI 交叉评审口径）