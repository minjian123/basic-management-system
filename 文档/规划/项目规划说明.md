...
> 本平台**不自营业务模块**：采购/收付款/供应商/销售/仓储等重业务作为产品承载于独立仓库 biz（《企业运营管理规划》），创作业务承载于 cw（《项目规划说明》），业务功能清单见各产品仓库；本平台只提供通用能力（上表）。产品接入方式见[《平台可扩展性规划》](平台可扩展性规划.md)。
...
| 八 业务外置（供应商/销售/仓储） | 供应商/销售/仓储等业务随 biz 仓库排期接入（平台扩展方式，见「平台扩展接入流程」节），不在本平台阶段计划 | 见 biz 《企业运营管理规划》 |
...
> MVP 验收标准：登录（含图形验证码）→ 主体链 RBAC → 各模块 CRUD → 日志 → 文件管理全链路可用；工作流引擎经流程定义/实例/待办流转验证通过、审批事件经 Webhook/消息总线推送成功；外部系统经 OAuth2 Client Credentials 调用开放接口鉴权通过；真实分片规则查询正确；系统监控与链路追踪可见；缓存管理可用；任务调度可管理；会话强制踢出即时生效；账号锁定与手动解锁可用；公告发布与已读统计可用；通知批量已读可用；帮助中心可用；首页工作台可用（内置卡片展示、个人布局定制、角色模板与平台默认模板、一键重置）；表单定制可用（三级布局解析与权限叠加、字段属性动态校验、租户自建字段动态 DDL 加列、查询区与明细区配置、导入导出联动）；用户喜好可用（跨端同步）；回收站可用且审计留痕；多租户独立库隔离验证通过；租户配额超限拦截生效；SSO 登录与 JIT 建号可用、BMS 作 IdP 通过；报表与大屏可用；移动端审批与免登可用；审计合规可用（字段级审计、哈希链校验、三权分立、密码策略）；国际化扩展可用；归档管理可用（含审计链跨归档校验）；备份任务与记录可查、恢复演练登记可用；Excel 导入导出可用；分片上传与断点续传可用；短信发送记录可用；安全专项用例集通过；浏览器与移动端真机兼容基线通过；压测达到 500 并发 / P99 ≤ 1s；MySQL/PostgreSQL/达梦 DM8 三库 CI 通过；Docker Compose 一键部署成功。
...
BMS 平台**不自营业务**：业务以产品仓库承载（biz 企业运营管理、CW 创作系统等），按本章流程便捷接入平台。接入方式分两轨：**平台扩展**（主推，产品代码随独立仓库演进、运行时并入平台）与**独立服务**（独立部署，经认证/开放接口/事件总线集成）。机制方向见[《平台可扩展性规划》](平台可扩展性规划.md)，本章为接入操作流程。
...
平台域固定前缀与产品业务模块前缀统一登记于 sys_module（平台库，种子维护，启动与 CI 校验唯一），新增模块先查注册表防冲突：
...
> 产品前缀在产品仓库规划中申请（如 biz 的 pur_/pay_/sup_/sale_/wh_、cw 的 cw_），平台登记查重防冲突，避免跨产品误用。
...
| 段 | 区间 | 覆盖产品/模块 |
| --- | --- | --- |
| 10xxxx | 100001-109999 | biz·采购 |
| 11xxxx | 110001-119999 | biz·收付款 |
| 12xxxx | 120001-129999 | biz·销售（规划中） |
| 13xxxx | 130001-139999 | biz·仓储（规划中） |
| 14xxxx | 140001-149999 | biz·供应商 |
| 15xxxx+ | 150001 起顺延 | 后续产品（cw 等）预留 |

> 错误码段随产品模块在 sys_module 注册分配，注册即定、稳定不变；1xxxx-9xxxx 平台段不因产品扩展调整。

### 23.6 示例产品模块设计 <a id="addon-example"></a>

业务模块简要设计（销售 sale_、仓储 wh_ 等核心表/菜单/主流程）按本章注册要素在 **biz** 仓库《企业运营管理规划》中展开，此处不复述；CW 创作系统模块见 cw 仓库《项目规划说明》。
...
| Alpha / Beta / GA | 三层里程碑分组：内部可用（阶段一~六）/ 功能完整并冻结范围（阶段九~十一，七/八为业务外置注记）/ 正式发布（十二~十五） | 开发计划与验收标准 |
