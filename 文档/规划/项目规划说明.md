...
核心数据表统一规范：雪花 ID 主键、软删除（deleted_at）、审计字段（created_at/created_by/updated_at/updated_by）、乐观锁（version）、时间 UTC 存储、关联表唯一联合主键；平台域固定前缀 sys_/wf_/rpt_/ai_，业务模块表用注册制独立前缀。库划分：平台库（bms_platform，标注【平台库】）、租户库（bms_tenant_{code}，一租户一库）、归档库（bms_archive，只读）；跨库引用用逻辑外键（code）。多语言文案不入主表、存 `{主表}_i18n` 附表。
...
| sys_dict_type / sys_dict_item | 字典类型与条目（平台库存默认，租户库可覆盖/自建）；parent_id 逻辑引用条目自身 id，支撑级联联动；多语言 label 存各自 _i18n 附表 |
...
| wf_record | 审批记录 |
| pur_purchase_apply | 采购申请单 |
| pur_purchase_order | 采购订单，审批通过后生成 |
| sup_supplier | 供应商档案（独立模块 sup_），采购申请/订单引用 |
| pay_receipt | 收款单（登记→确认），可关联任意业务（如采购订单退款来源） |
| pay_payment | 付款单（申请→审批→执行），审批复用工作流引擎 |
| pay_refund | 退款单（原收款退回/原付款追回），退款金额 ≤ 原单余额校验 |
| pay_payment_channel | 支付渠道配置（渠道抽象预留；本期仅 manual 线下转账） |
| pay_payment_flow | 统一收付款流水台账，每笔收/付/退款落流水 |
...
| *_i18n 附表 | sys_module_i18n、sys_menu_i18n、sys_business_i18n、sys_action_i18n、sys_field_i18n、sys_dict_type_i18n、sys_dict_item_i18n、sys_mail_template_i18n、sys_dashboard_card_i18n、sys_help_article_i18n、sys_field_ext_i18n；联合主键（主表 ID + locale） |
...
> MVP 全量清单（业务码逐码说明、动作码含义与典型权限码）见《[架构设计 · 子系统 · 权限计算引擎](../设计/架构设计/11_架构设计_子系统_权限计算引擎.md)》7 节与《[英文简称规范](../规范/英文简称规范.md)》5 节，此处不再重复维护。
...
- 错误码：平台段 1xxxx~9xxxx 为 5 位数字（万位为模块段、后 4 位段内序号），附加模块扩展段 10xxxx 起升为 6 位（分配见"附加业务模块规划"节）；一经发布稳定不变，文案由前端按 i18n 映射。平台分段表：
...
主键（雪花 ID）、软删除（复合唯一索引 + 回收站）、审计字段、乐观锁、UTC 时间、多语言 i18n 附表、字符集、索引与 SQL 规范等约定见《[架构设计 · 数据架构](../设计/架构设计/05_架构设计_数据架构.md)》4 节。
...
> 机制细节（双 token 存储/轮换、限流验证码、密码与账号策略、会话黑名单与踢出、密钥轮换、上传与基础防护、AI 开关约束）见《[架构设计 · 子系统 · 认证与会话](../设计/架构设计/10_架构设计_子系统_认证与会话.md)》与《[架构设计 · 性能与安全](../设计/架构设计/27_架构设计_性能与安全.md)》3 节。
...
- **数据安全（数安法）**：数据分类分级——客户与供应商信息、收付款流水列为重要数据，从严管控（访问审计 + 导出审批 + 归档加密存储）；销毁动作留销毁日志备查。
- **涉外场景预留**：i18n 支持 RTL 语言暗示潜在海外交付；若涉及境外用户或数据出境，另行评估 GDPR 合规与数据出境安全评估要求，不在 MVP 范围。

> 法律合规为架构约束，同口径详见《[架构设计 · 目标与约束](../设计/架构设计/02_架构设计_目标与约束.md)》3.4 节。
...
> 工具链流程与口径详见《[测试规范](../规范/测试规范.md)》9 节。
...
**准出标准**（阶段交付与 MVP 验收，同时满足）：P0/P1 缺陷清零；功能用例通过率 ≥ 98%；安全专项用例集全部通过；压测达到 500 并发 / P99 ≤ 1s；MySQL/PostgreSQL/达梦 DM8 三库 CI 通过；代码覆盖率达标——核心模块（认证/RBAC/工作流/审计/收付款）行覆盖 ≥ 80%、整体 ≥ 70%（后端 pytest-cov、前端 Vitest coverage，CI 门禁）。
...
覆盖范围：初始菜单/表单/按钮/字段与业务/动作种子（含 i18n 附表英文文案）、基础字典、初始工作流定义（采购申请三级审批、付款申请审批模板）、初始支付渠道（线下转账 manual）、基础邮件模板、帮助分类与基础文章、工作台卡片与平台默认模板、初始定时任务（孤儿分片清理/日志归档/分片表预创建/每日备份）；平台库种子（超管账号首次启动强制改密、平台统一挂接与默认字典参数模板）；租户开通种子（三类内置管理员角色权限互斥、默认 IdP 占位配置、初始归档策略 90 天）；语言包（中英文 sys_i18n_locale/sys_i18n_message）；表单定制无独立布局种子（空布局按字段 sort 回退默认栅格）。

> 种子结构、顺序与幂等要求见《[架构设计 · 部署与运维](../设计/架构设计/30_架构设计_部署与运维.md)》6 节。
...
数据库每日全量备份 + Binlog/WAL（达梦 dmrman），保留 30 天异机存储、归档库纳入备份；MinIO 对象定期增量备份 + 生命周期策略；恢复定期演练，目标 RTO ≤ 4 小时、RPO ≤ 1 小时；数据归档全表可配策略（sys_archive_policy），热表迁归档库可在线查询、按保留期冷化对象存储，过期文件与软删除残留由 Celery 物理清理。
...
> 完整检查单与上线步骤见《[架构设计 · 部署与运维](../设计/架构设计/30_架构设计_部署与运维.md)》12 节。

### 19.8 运营支持模型 <a id="deploy-support"></a>

平台运营以一人兼职为主，值班窗口内对负载、告警、备份与缺陷渠道保持响应；问题分级（内部底线 SLO）P0 平台不可用/数据泄露/审计链校验失败、P1 核心链路故障、P2 单功能降级、P3 轻微问题，响应与处置时限逐级收紧；按月复核容量/成本水位与外部 API 用量；密钥轮换与凭据抽查；版本支持期 ≥ 12 个月、过期仍收安全修复、主版本升级提前公告。

> 值班口径、问题分级时限表、容量与成本监控、License 健康、机密与权限复核见《[架构设计 · 部署与运维](../设计/架构设计/30_架构设计_部署与运维.md)》13 节。
...
> 本文档为《项目规划说明》的 HTML 版 · 依《文档生成规范》编写 · 与《开发部署规划》配套 · 转换日期：2026-08-11 · 更新日期：2026-09-09（瘦身迁移：第 6、8~19 节深度机制章节改「摘要 + 引用架构/概要节点」，章节标题、编号与锚点全部保留，机制单一事实源下沉至架构 05/06/08/09/10/11/15/27/28/29/30 与概要数据库设计等节点，执行依据见《规划说明瘦身迁移实施》；本次同批修订：补齐 05 sys_form code、i18n 附表与软删除口径、11 业务/动作码明细、27 AI 自动执行/审批开关依赖、30 种子与发布/运营、02 法律合规 3.4） · 此前修订：2026-09-09（审计修订：补齐《命名规范》《文档生成规范》《graphify 部署使用说明》等引用链接；sys_form 补主键 code、ai_file_segment 补 tenant_id；19.5 补版本号与 Alpha/Beta/GA 分层映射；2.2 补 Node 运行时基线；新增 2.6 项目预算、19.7 GA 正式上线计划、19.8 运营支持模型；3 节与 20 节增补对架构 31 ADR 与《总体项目规划》排期的引用，避免与下游文档重复维护） · 此前修订：2026-08-22（新增：里程碑分层 Alpha/Beta/GA、法律合规 13.1、可用性目标与 SLO、版本与发布管理 19.5、容量规划 19.6、开放接口防重放 9.1、MR 评审规则 17 节、镜像扫描安全专项、灰度发布 19.5、前端性能预算 15 节、用户侧文档交付项 21 节、风险管理 25 章；补全：许可表遗漏组件、错误码双轨口径统一 8 节、无障碍排除声明 24 节、术语表 26 章；个人开发适配：MR 单人口径 + AI 交叉评审 17 节、等保按需启动 13.1、测试 AI 辅助总则与 UAT 预走查 16.1、兼容矩阵缩减 16 节、用户文档 AI 初稿口径 21 节、发布检查单自动采集 19.5、SBOM 自动生成 2.5、种子数据 AI 生成 18 章、测试报告自动汇总 16.5）