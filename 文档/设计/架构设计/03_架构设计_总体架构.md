...
| 缺陷管理 | `scripts/defect/`：defect_capture.py（测试失败自动上报 + REPRO 复现包：dump/堆栈/repro.json/REPRO.md）、reproduce.py（一键复现：检出 commit + 导入 dump + 跑用例）、ai_fix.py（双轨 AI 修复代理：本地模型优先云端兜底，bot 提 MR 人工合入） |
