...
本节点定义部署与运维架构：部署拓扑、集群设计要点、基础设施编排、配置分层、初始化数据、故障应对与备份恢复。对应[《项目规划说明》](../../规划/项目规划说明.html)的「部署与运维」「集群设计要点」「基础设施编排」「环境与配置」「初始化数据」「备份与恢复」章节。
...
- 开发环境依赖（Redis/RocketMQ/ES/MinIO）由 Docker Compose 统一提供；Milvus 随阶段十五加入；MySQL/PostgreSQL/达梦 DM8 三库常驻开发服务器（见《[开发部署规划](../../规划/开发部署规划.html)》）
