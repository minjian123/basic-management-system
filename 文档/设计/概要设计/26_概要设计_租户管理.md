...
本节对应[《项目规划说明》](../../规划/项目规划说明.html)「功能模块」之**租户管理**模块，与「多数据源策略（租户独立库）」「权限模型（平台超管）」「初始化数据（租户开通种子）」「认证与安全（租户识别）」「部署与运维（子域名路由/泛域名 TLS）」「[审计合规](29_概要设计_审计合规.md)（合规留存）」等章节；架构设计依据[《架构设计》](../架构设计/01_架构设计_总览.md)**08-多租户路由**节点。
