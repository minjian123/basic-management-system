...
| sleep_mjbk.py | 远程进入系统睡眠（S3，含二次确认） | 睡眠mjbk.bat |
...
> 睡眠命令为 `echo '{密码}' | sudo -S systemctl suspend`，让机器进入 **S3 睡眠**（内存保电、网卡仍供电）。睡眠只是暂停而非关机，唤醒后服务自动继续；期间服务（GitLab / 数据库等）短暂不可用，属正常。与唤醒的对应关系：睡眠用 `sleep_mjbk.py`，唤醒用 `wake_mjbk.py`（WOL）。
...
> 睡眠是「关机」的轻量替代：若只是短时间不用，用 `sleep_mjbk.py` 睡下、`wake_mjbk.py` 唤醒即可，比关机/开机更快；要彻底断电再走关机流程。
...
- **夜间服务停用**：00:00 – 08:00 期间 mjbk 处于睡眠，GitLab / 数据库 / MinIO 等服务**不可用**；如需夜间访问，先 `wake_mjbk.py` 唤醒。
- **仅睡眠不关机**：每日循环是 S3 睡眠，内存保电；断电（如停电、意外关机）后需 `wake_mjbk.py` 唤醒开机，timer 会随开机自动恢复。
...
> 关联：《开发服务器部署使用说明总览》《GitLab迁移使用说明》 · 生成日期：2026-08-18 · 修订：2026-09-04（新增远程睡眠脚本与每日自动睡眠/唤醒）