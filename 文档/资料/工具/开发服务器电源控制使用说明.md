...
[文档首页](../../文档首页.html) › 资料 › 工具 › 开发服务器电源控制使用说明　|　[同级：GitLab 迁移使用说明 →](GitLab迁移使用说明.md)
...
全部位于 `deploy/tools/wol/`，纯 Python 标准库实现，无第三方依赖；`.bat` 为双击入口（自动跳到仓库根目录再调用对应脚本）。
...
1. 远程关机：`python deploy\tools\wol\shutdown_mjbk.py`（确认后执行），或 SSH 后执行 `sudo shutdown -h now`；
2. 开发机 ping 不通即确认关机完成；
3. 远程唤醒：`python deploy\tools\wol\wake_mjbk.py`，等待就绪提示后即可 SSH 使用。
...
> 本文档为 AI 生成 · 关联：《开发服务器部署使用说明》《GitLab迁移使用说明》 · 生成日期：2026-08-18