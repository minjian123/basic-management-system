...
| bg.js | opencode 插件：向 AI 助手注册上述三个工具 | opencode 自动加载 |

全部位于 `deploy/tools/bg/`。
...
| 参数 | 适用 | 说明 |
| --- | --- | --- |
| --name | 三个脚本 | 任务名（唯一标识，查询/停止时使用；同名会覆盖） |
| --command | bg-run | 要执行的命令，经 pwsh 执行 |
| --workdir | bg-run | 工作目录（默认当前目录） |
| --timeout | bg-run | 超时秒数（0=不限，默认 0）；超时自动终止任务 |
| --base | 三个脚本 | 状态目录（默认 `%USERPROFILE%\.bg`，三个脚本须一致） |
...
flowchart TD
    ...
    B -- 否 --> D["bg_run 后台启动"(
    D --> E["立即返回任务 ID"(
    E --> F["bg_status 秒级轮询<br/>（间隔 10-30 秒）"(
    F --> G{"FINISHED？"}
    G -- 否 --> F
    G -- 是 --> H["读取输出与 stderr"(
```
...
| 现象 | 可能原因与处理 |
| --- | --- |
| bg_status 返回 NOT_FOUND | 任务名拼写不一致，或状态目录（--base）不一致；确认三个脚本使用同一任务名与同一 base。 |
...
> 本文档为 AI 生成 · 关联：《开发服务器电源控制使用说明》《本地多模态接入方案》 · 生成日期：2026-08-18