...
全部位于 `deploy/tools/bg/`（watch_pipeline.py 位于 `deploy/tools/gitlab/`）。
...
```bash
python deploy\tools\bg\bg-run.py --name 下载依赖 --command "pnpm install" [--workdir D:\AI\xxx( [--timeout 60(
```

查询、等待与停止：

```bash
python deploy\tools\bg\bg-status.py --name 下载依赖
python deploy\tools\bg\bg-wait.py --name 下载依赖 [--timeout 600] [--interval 5] [--tail 8]
python deploy\tools\bg\bg-stop.py --name 下载依赖
```
...
状态查询: python D:\Develop\bms\deploy\tools\bg\bg-status.py --name '下载依赖'
...
| 插件工具不可用 | 检查 `.opencode/opencode.json` 的 plugin 数组是否登记 `./deploy/tools/bg/bg.js`，登记后重启 opencode。 |
