...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [部署与运维](../技术栈知识档案总览.md#ops) › GitLab 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **定位**：BMS 研发协作中枢——代码仓库、MR、CI 流水线、镜像 Registry 一体（《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节）；仅作开发与 CI 基础设施，不随产品交付。
- **版本**：GitLab CE 1x.x 系列（每年 3 个大版本）；gitlab-runner 1x.x 系列。
- **许可**：MIT，免费开源（《[项目规划说明](../../../规划/项目规划说明.html#stack-license)》2.5 节）。
- **落地形态**：mjbk 自托管（HTTP 8080 + Registry 5050 + SSH 2222），runner 以容器方式运行（《[开发部署规划](../../../规划/开发部署规划.html#server-gitlab)》4.5 节）。
...
- **代码托管 + 协作**：仓库、MR、Issue（缺陷载体）一体；Kiwi TCMS 缺陷链接指向 GitLab Issue（《[项目规划说明](../../../规划/项目规划说明.html#test-defect)》16.2 节）。
- **CI 流水线**：`.gitlab-ci.yml` 定义——MR 流水线：后端 ruff + pytest（含覆盖率门禁）、前端 ESLint + Vitest（含 coverage 门禁）、双端构建；main 流水线：另含 Playwright E2E 独立 job、MySQL/PostgreSQL 双库方言集成测试、构建镜像推送 Registry、导出 swagger.json 契约快照（《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节）。
- **runner 配置**：gitlab-runner 以容器方式运行，挂载 docker.sock、executor=docker，开发环境并发上限 2，避免 CI 抢占 GitLab 与开发服务资源（《[开发部署规划](../../../规划/开发部署规划.html#server-gitlab)》4.5 节）。
- **测试归档**：Allure 报告作 CI 产物归档，执行结果经官方插件导入 Kiwi TCMS（《[项目规划说明](../../../规划/项目规划说明.html#test-report)》16.5 节）。
- **缺陷自动上报**：任一自动化测试失败即由 `deploy/tools/defect/defect_capture.py` 归档 REPRO 复现包并自动创建 / 复用 GitLab Issue（fingerprint 去重）（《[项目规划说明](../../../规划/项目规划说明.html#test-defect)》16.2 节）。
- **GitHub 只读归档**：GitLab 配置 push mirror 单向同步 main 至 GitHub 归档仓库，GitHub 侧改动不回传、不承载协作（《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节）。
...
- **external_url**：首次部署必须配对外访问地址，否则 GitLab 生成的仓库 / 克隆 URL 不正确（《[开发部署规划](../../../规划/开发部署规划.html#server-gitlab)》4.5 节）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节 | 部署与运维技术栈（CI/CD 条目） |
| 《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节 | 选型说明：一体托管 + 流水线定义 + GitHub 归档 |
| 《[项目规划说明](../../../规划/项目规划说明.html#test)》16 节 | 测试策略：CI 执行、缺陷上报、结果归档 |
| 《[开发部署规划](../../../规划/开发部署规划.html#server-gitlab)》4.5 节 | mjbk GitLab + runner + Renovate 部署实录 |
| 《[开发部署规划](../../../规划/开发部署规划.html#workflow)》7 节 | 开发工作流与 CI：分支模型、流水线、依赖升级 |
| 《[GitLab 部署使用说明](../../开发服务器/GitLab部署使用说明.html)》 | mjbk GitLab 安装配置内部文档 |
| 《[GitLab 迁移使用说明](../../工具/GitLab迁移使用说明.html)》 | 仓库迁移到 GitLab 的操作说明 |
