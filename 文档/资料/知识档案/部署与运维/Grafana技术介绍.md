...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [部署与运维](../技术栈知识档案总览.md#ops) › Grafana 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **定位**：BMS 可观测性三件套的统一展示层（《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节）。
- **版本**：1x.x 系列（每年 2-3 个大版本，持续迭代）。
- **许可**：AGPL-3.0。BMS 仅作独立展示服务部署、不修改源码，无传染影响；Grafana Enterprise 为商业版，本项目不用（《[项目规划说明](../../../规划/项目规划说明.html#stack-license)》2.5 节）。
- **落地形态**：mjbk 以 Docker Compose 容器常驻，浏览器访问；BMS 系统监控页跳转 / 嵌入（《[项目规划说明](../../../规划/项目规划说明.html#deploy-compose)》19.3 节）。
...
- **统一展示**：Prometheus 指标、Loki 日志、Jaeger 链路三类数据源挂在一个 Grafana 上，指标异常 → 跳日志 → 跳链路，一处排障（《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节）。
- **BMS 内跳转 / 嵌入**：系统监控页提供 Grafana 入口（页面跳转或同源嵌入），依赖状态（DB / Redis / MinIO）与缓存管理同页呈现（《[项目规划说明](../../../规划/项目规划说明.html#modules)》5 节系统监控模块）。
- **嵌入前提**：nginx 配 X-Frame-Options SAMEORIGIN，兼容同源 Grafana 嵌入（《[项目规划说明](../../../规划/项目规划说明.html#deploy-cluster)》19.2 节）。
- **监控面示例**：健康检查、Socket.IO 连接数、Redis key 分布 / 内存占用、DB 依赖状态等看板（《[项目规划说明](../../../规划/项目规划说明.html#modules)》5 节）。
...
- **AGPL 边界**：独立部署、不改源码、不深度定制，遵守《[项目规划说明](../../../规划/项目规划说明.html#stack-license)》2.5 节约束；若未来计划深度定制，提前评估许可。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节 | 部署与运维技术栈（可视化条目） |
| 《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节 | 选型说明：统一展示 + BMS 内跳转 / 嵌入 |
| 《[项目规划说明](../../../规划/项目规划说明.html#modules)》5 节 | 系统监控模块：Grafana 跳转 + 依赖状态 + 缓存管理 |
