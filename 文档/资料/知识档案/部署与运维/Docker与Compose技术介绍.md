...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [部署与运维](../技术栈知识档案总览.md#ops) › Docker 与 Compose 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **定位**：BMS 的容器化底座，保证开发与生产环境一致（《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节）。
- **版本**：Engine 为 2x.x 系列（持续更新，装最新稳定版即可）；Compose v2 用 Go 重写，已内置到 `docker compose` 命令行。
- **许可**：Apache-2.0，免费开源、无商用订阅（《[项目规划说明](../../../规划/项目规划说明.html#stack-license)》2.5 节）。
...
- **一键编排**：backend（多副本）+ celery-worker + celery-beat + event-worker + frontend（nginx）+ redis + rocketmq（nameserver + broker）+ elasticsearch + minio + 数据库 + 监控全家桶，全部由 Compose 拉起（《[项目规划说明](../../../规划/项目规划说明.html#deploy-compose)》19.3 节）。
- **开发生产一致**：本地与 mjbk 共用同一套 Compose 编排文件；开发期依赖（Redis / RocketMQ / ES / MinIO）本地不装原生服务，统一容器提供（《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节）。
...
- **安装方式**：mjbk 为 Ubuntu 24.04.4，apt 清华 docker-ce 源直装 Docker Engine，systemd 开机自启（见《[DockerEngine 部署使用说明](../../开发服务器/DockerEngine部署使用说明.html)》）。
- **数据落盘**：IO 敏感数据（数据库 / ES / MinIO / 镜像）走 NVMe SSD 命名卷，备份与读多写少数据走 HDD（《[开发部署规划](../../../规划/开发部署规划.html#server-disk)》4.2 节）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节 | 部署与运维技术栈（Docker 条目） |
| 《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节 | 选型说明：开发生产一致、一键编排 |
| 《[项目规划说明](../../../规划/项目规划说明.html#deploy-compose)》19.3 节 | 基础设施编排：初版 Compose 服务清单 |
| 《[开发部署规划](../../../规划/开发部署规划.html#server-base)》4.1 节 | mjbk 基础环境：清华 docker-ce 源安装实录 |
| 《[DockerEngine 部署使用说明](../../开发服务器/DockerEngine部署使用说明.html)》 | mjbk Docker 安装与配置内部文档 |
