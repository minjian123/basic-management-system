...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [部署与运维](../技术栈知识档案总览.md#ops) › Prometheus 与 Alertmanager 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **定位**：BMS 可观测性三件套（指标 / 日志 / 链路）中的指标与告警层（《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节）。
- **版本**：Prometheus 3.x 系列（持续迭代）；Alertmanager 随主仓库版本走。
- **许可**：Apache-2.0，免费开源（《[项目规划说明](../../../规划/项目规划说明.html#stack-license)》2.5 节）。
- **落地形态**：mjbk 以 Docker Compose 容器常驻，与 Loki / Grafana / Jaeger 一并纳入监控编排（《[项目规划说明](../../../规划/项目规划说明.html#deploy-compose)》19.3 节）。
...
- **指标采集**：Prometheus 拉取后端 `/metrics` 及各组件 exporter（Redis、数据库、node 等）的指标（《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节）。
- **告警推送**：Alertmanager 统一出口——邮件走原生 SMTP receiver；企业微信 / 钉钉群机器人经 webhook receiver 转 BMS 内部接口转换格式后推送（《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节）。
- **看板展示**：指标经 Grafana 统一可视化，BMS 系统监控页跳转 / 嵌入（见《[Grafana 技术介绍](Grafana技术介绍.md)》）。
- **监控面**：覆盖健康检查、Socket.IO 连接数、缓存（Redis key 分布 / 内存）、DB/Redis/MinIO 依赖状态（《[项目规划说明](../../../规划/项目规划说明.html#modules)》5 节系统监控模块）。
- **链路协同**：trace_id 与 structlog 的 request_id 关联，指标、日志、链路在 Grafana 内互跳（《[项目规划说明](../../../规划/项目规划说明.html#sel-backend)》3.1 节 OpenTelemetry 条目）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明](../../../规划/项目规划说明.html#stack-ops)》2.4 节 | 部署与运维技术栈（监控条目） |
| 《[项目规划说明](../../../规划/项目规划说明.html#sel-ops)》3.4 节 | 选型说明：指标采集 + 三类告警通道 |
| 《[项目规划说明](../../../规划/项目规划说明.html#deploy-topo)》19.1 节 | 部署拓扑：监控组件清单 |
