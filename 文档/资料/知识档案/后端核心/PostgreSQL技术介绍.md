...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [后端核心](../技术栈知识档案总览.md#backend) › PostgreSQL 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- 生产数据库之一，与 MySQL、达梦 DM8 三库并行支持；开发/测试默认 SQLite（见《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend)）。
- **psycopg 3 驱动**：PostgreSQL 官方维护、原生异步（dialect `postgresql+psycopg`），替代已冻结的 psycopg2（见《[项目规划说明》3.1 节](../../../规划/项目规划说明.html#sel-backend)）。
- 数据层主从：PostgreSQL 主从（读写分离 + 故障切换），1 平台库 + N 租户库 + 1 归档库（见《[项目规划说明》19.1 节](../../../规划/项目规划说明.html#deploy-topo)）。
- 软删除优化：PostgreSQL 可另加部分唯一索引优化（基础方案四库统一的 (唯一字段, deleted_at) 复合唯一索引）（见《[项目规划说明》11.1 节](../../../规划/项目规划说明.html#dbrule)）。
- 备份与恢复：每日全量备份 + WAL 时间点恢复（PITR），保留 30 天异机存储（见《[项目规划说明》19.4 节](../../../规划/项目规划说明.html#deploy-backup)）。
- CI 三库方言集成测试：复用 mjbk 常驻 PostgreSQL，job 内建 bms_test 前缀测试库，迁移 → 测试 → 清理（见《[开发部署规划](../../../规划/开发部署规划.html)》）。
...
- **方言差异**：部分索引、JSONB、ILIKE 等是 PG 特性，ORM 层统一用跨方言写法，方言优化只在三库验证通过的迁移/查询中使用（见《[项目规划说明》11 节](../../../规划/项目规划说明.html#dbcompat)）。
- **连接池规划**：与 MySQL 同口径——worker × (pool_size + max_overflow) ≤ max_connections 的 70%（见《[项目规划说明》14 节](../../../规划/项目规划说明.html#perf)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend) | 技术栈：PostgreSQL 16+ 与 psycopg 3 条目 |
| 《[项目规划说明》3.1 节](../../../规划/项目规划说明.html#sel-backend) | 选型说明：psycopg 3 替代 psycopg2 |
| 《[项目规划说明》11.1 节](../../../规划/项目规划说明.html#dbrule) | 数据规范：软删除与部分唯一索引优化 |
| 《[PostgreSQL 部署使用说明](../../开发服务器/PostgreSQL部署使用说明.html)》 | 开发服务器 mjbk 上的部署与访问细节 |
