...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [后端核心](../技术栈知识档案总览.md#backend) › MySQL 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- 生产数据库之一，与 PostgreSQL、达梦 DM8 三库并行支持；开发/测试默认 SQLite（见《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend)）。
- **aiomysql 异步驱动**（dialect `mysql+aiomysql`）；同步 fallback 可换 PyMySQL；备选 asyncmy 性能更好但维护活跃度一般，故不采用（见《[项目规划说明》3.1 节](../../../规划/项目规划说明.html#sel-backend)）。
- 数据层主从：MySQL 主从读写分离 + 故障切换，1 平台库 + N 租户库 + 1 归档库（见《[项目规划说明》19.1 节](../../../规划/项目规划说明.html#deploy-topo)）。
- 字符集 utf8mb4；连接池按 worker 数规划（≤ 70% max_connections）；慢查询日志开发联调用（见《[项目规划说明》11.1 节](../../../规划/项目规划说明.html#dbrule)与《[开发部署规划](../../开发服务器/开发服务器部署使用说明.html)》）。
- 备份与恢复：每日全量备份 + Binlog 时间点恢复（PITR），保留 30 天异机存储（见《[项目规划说明》19.4 节](../../../规划/项目规划说明.html#deploy-backup)）。
- CI 三库方言集成测试：复用 mjbk 常驻 MySQL，job 内建 bms_test 前缀测试库，迁移 → 测试 → 清理（见《[开发部署规划](../../../规划/开发部署规划.html)》）。
...
- **GPL 传染误读**：社区版 GPL-2.0 只约束 MySQL 自身及衍生修改；BMS 作为独立应用通过协议连接、不修改不分发其源码即不传染。介意或需官方支持时采购商业版（见《[项目规划说明》2.5 节](../../../规划/项目规划说明.html#stack-license)）。
...
- **保留字**：如 sys_config 的 config_key 字段即因 key 为保留字改名，新表设计时避开保留字（见《[项目规划说明》6 节](../../../规划/项目规划说明.html#tables)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend) | 技术栈：MySQL 8.x 与 aiomysql 条目 |
| 《[项目规划说明》2.5 节](../../../规划/项目规划说明.html#stack-license) | GPL-2.0 使用边界与商用注意事项 |
| 《[项目规划说明》11 节](../../../规划/项目规划说明.html#dbcompat) | 数据库兼容：跨方言类型与字符集约定 |
| 《[MySQL 部署使用说明](../../开发服务器/MySQL部署使用说明.html)》 | 开发服务器 mjbk 上的部署与访问细节 |
...
| 《[数据库开发规范](../../../规范/数据库开发规范.html)》 | 字符集、命名、索引等落地约束 |
