...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [后端核心](../技术栈知识档案总览.md#backend) › Alembic 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
| 批量迁移 | 遍历全部租户库逐一执行迁移：新租户开通自动建库 + 迁移 + 种子（见《[项目规划说明》10 节](../../../规划/项目规划说明.html#datasource)） |

## 3. 在 BMS 项目中的用途 <a id="usage"></a>

- 作为唯一 schema 版本管理工具：目录 `alembic/` + `alembic.ini`，URL 从 config.toml 读取，不写死连接串（见《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend)）。
- 按数据源分别维护迁移脚本：只管理各库 schema；平台库、租户库、归档库各走一套迁移链（见《[项目规划说明》10 节](../../../规划/项目规划说明.html#datasource)）。
- **方言差异分环境验证**：维护 MySQL / PostgreSQL / 达梦 DM8 三套方言迁移；SQLite 开发库自动建表不跑迁移（见《[项目规划说明》11 节](../../../规划/项目规划说明.html#dbcompat)）。
- CI 三库方言集成测试：连 mjbk 常驻三库，建 bms_test 前缀测试库 → Alembic 迁移 → 集成测试 → 删库清理，保证迁移脚本在每类库上真实可执行（见《[开发部署规划](../../../规划/开发部署规划.html)》与《[项目规划说明》16 节](../../../规划/项目规划说明.html#test)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend) | 技术栈：数据库迁移条目（URL 来自 config.toml） |
| 《[项目规划说明》10 节](../../../规划/项目规划说明.html#datasource) | 多数据源策略：按数据源分迁移、租户批量迁移 |
| 《[项目规划说明》11 节](../../../规划/项目规划说明.html#dbcompat) | 数据库兼容：三套方言迁移维护口径 |
| 《[开发部署规划](../../../规划/开发部署规划.html)》 | CI 三库方言测试与 bms_test 测试库流程 |
| 《[SQLAlchemy 技术介绍](SQLAlchemy技术介绍.md)》 | 迁移的对象：模型声明与元数据 |
| 《[MySQL 技术介绍](MySQL技术介绍.md)》等三库文档 | 各库方言特性与差异来源 |
| 《[数据库开发规范](../../../规范/数据库开发规范.html)》 | 表结构变更与迁移脚本的流程要求 |
