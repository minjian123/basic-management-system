...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [后端核心](../技术栈知识档案总览.md#backend) › 达梦 DM8 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- 生产数据库之一（信创场景选配），与 MySQL、PostgreSQL 三库并行支持；开发/测试默认 SQLite（见《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend)）。
- **dmPython 驱动**：达梦官方同步驱动，dialect `dm+dmPython`；对 Python 3.14 兼容性未知，纳入阶段一逐依赖验证，不兼容则按规划口径整体回退 Python 3.13（见《[项目规划说明》3.1 节](../../../规划/项目规划说明.html#sel-backend)）。
- 三库方言测试：mjbk 常驻 MySQL/PostgreSQL/达梦，CI job 内建 `bms_test` 前缀测试库执行迁移与集成测试后清理（见《[开发部署规划](../../../规划/开发部署规划.html)》）。
- 数据库兼容约束：ORM 层禁用方言特有功能，JSON 等类型用跨方言通用类型兜底，分页统一 limit/offset；Alembic 维护三套方言迁移脚本，达梦迁移单独验证（见《[项目规划说明》11 节](../../../规划/项目规划说明.html#dbcompat)）。
- 备份：达梦 DM8 用 dmrman 备份/归档，每日全量 + 归档，保留 30 天异机存储（见《[项目规划说明》19.4 节](../../../规划/项目规划说明.html#deploy-backup)）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明》2.1 节](../../../规划/项目规划说明.html#stack-backend) | 技术栈：达梦 DM8 与 dmPython 条目 |
| 《[项目规划说明》3.1 节](../../../规划/项目规划说明.html#sel-backend) | 选型说明：dmPython 与 Python 3.14 兼容性验证口径 |
| 《[项目规划说明》11 节](../../../规划/项目规划说明.html#dbcompat) | 数据库兼容：跨方言类型、分页、软删除唯一索引约定 |
| 《[达梦 DM8 部署使用说明](../../开发服务器/达梦DM8部署使用说明.html)》 | 开发服务器 mjbk 上的部署、库与账号细节 |
