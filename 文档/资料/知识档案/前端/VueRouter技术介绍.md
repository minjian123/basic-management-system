...
[文档首页](../../../文档首页.html) › [知识档案](../技术栈知识档案总览.md) › [前端](../技术栈知识档案总览.md#frontend) › Vue Router 技术介绍　|　[← 返回总览](../技术栈知识档案总览.md)
...
- **前端路由（2.2 节）**：frontend 与 frontend-mobile 双工程各自使用 Vue Router 4 管理页面导航（见《[项目规划说明](../../../规划/项目规划说明.html#stack-frontend)》2.2 节）。
- **动态路由（15 节菜单机制）**：菜单树由后端按用户权限动态返回，前端根据菜单树动态注册路由与生成侧边栏；前端不硬编码路由表，新增菜单/表单/按钮/字段无需发版（见《[项目规划说明](../../../规划/项目规划说明.html#pages)》15 节）。
- **路由守卫配合业务权限做菜单级控制（3.2 节）**：全局守卫在每次导航前校验登录态与路由 meta 中的权限码，无权页面重定向到 403/首页（见《[项目规划说明](../../../规划/项目规划说明.html#sel-frontend)》3.2 节与 7 节权限模型的双重控制）。
- **静默续期（12 节认证与安全）**：access token 仅存前端内存、刷新页面后静默续期——路由守卫恰是"刷新后先续期再放行"的落点（见《[项目规划说明](../../../规划/项目规划说明.html#security)》12 节）。
- **路由懒加载**：按 15 节前端性能要求，页面组件全部懒加载，控制首屏体积。
- **目录约定**：动态路由生成逻辑集中在 `src/router/`（见《[项目规划说明](../../../规划/项目规划说明.html#structure)》4 节目录结构）。
...
| 文档 | 说明 |
| --- | --- |
| 《[项目规划说明](../../../规划/项目规划说明.html#stack-frontend)》2.2 节 | 前端技术栈（Vue Router 4 条目） |
| 《[项目规划说明](../../../规划/项目规划说明.html#pages)》15 节 | 菜单机制：动态路由生成与侧边栏、前端不硬编码路由表 |
| 《[项目规划说明](../../../规划/项目规划说明.html#sel-frontend)》3.2 节 | 选型理由：路由守卫配合业务权限做菜单级控制 |
| 《[前端开发规范](../../../规范/前端开发规范.html)》 | 路由与目录约定、提交规范 |
