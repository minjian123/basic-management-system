...
[文档首页](../../文档首页.html) › [资料](../工具/Ubuntu安装部署使用说明.md) › [开发服务器部署使用说明](开发服务器部署使用说明.md) › Kiwi TCMS 部署使用说明　|　[← 上一个：GitLab](GitLab部署使用说明.md)

## 1. 目的与适用范围 <a id="purpose"></a>

mjbk 上的 Kiwi TCMS（容器 `bms-kiwi`）是 BMS 项目的**测试用例唯一管理平台**（《[项目规划说明](../../规划/项目规划说明.html)》16.3）：
...
BMS 项目中的用法（《[测试规范](../../规范/测试规范.html)》）：用例按模块建 **Category**、用 **Tag** 组织；用例名称/描述使用中文；自动化用例在代码中以用例 ID 标注关联；pytest/Playwright 执行结果经官方插件导入平台归档；平台缺陷链接指向 GitLab Issue。
...
- 《[开发部署规划](../../规划/开发部署规划.html)》：Kiwi TCMS 部署位（宿主 8060、复用 MySQL）
- 《[项目规划说明](../../规划/项目规划说明.html)》：16.3 测试用例管理（Kiwi TCMS）
- 《[测试规范](../../规范/测试规范.html)》：用例库组织、结果导入、缺陷管理流程
- 《[命名规范](../../规范/命名规范.html)》：容器名 `bms-组件`、数据库命名

> 本文档为 AI 生成 · 依《文档生成规范》编写 · 记录 2026-08-19 实际部署过程（kiwitcms/kiwi:latest，MySQL 8.4 复用）