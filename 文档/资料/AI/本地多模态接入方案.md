...
[文档首页](../../文档首页.html) › [项目规划说明](../../规划/项目规划说明.html) › 资料 › 本地多模态接入方案

## 1. 目的与适用范围 <a id="purpose"></a>

为 BMS 项目（[《项目规划说明》](../../规划/项目规划说明.html)）建设**本地多模态能力接入**：
把本机 LM Studio 上的多模态模型（qwen3.8-27b）的**全部能力**（识图、多模态对话、文本理解）开放给 AI 助手使用。
...
```mermaid
 flowchart LR
     A["opencode 主会话"] -->|"multimodal_chat / screenshot"| M["MCP server<br/>deploy/tools/multimodal/mcp_server.py"]
     ...
     F["场景提示词模板<br/>deploy/tools/multimodal/prompts/"] -->|"注入评审规则"| M
```
...
- **语言统一**：server 用 Python（官方 mcp SDK），与 `deploy/tools` 全 Python 工具链一致。
...
- 本地 LM Studio 无鉴权时 apiKey 填占位值（`local`）；云端密钥仅走环境变量（见第 8 节）。
- 修改 opencode.json 后需**重启 opencode** 生效。

### 3.3 推理模型注意 <a id="model-reasoning"></a>

> qwen3.8-27b 是推理模型，LM Studio 默认先输出大量思考 token（`reasoning_content`），带图评审可能几万 token 都未结束。客户端默认携带 `reasoning_effort: "none"` 关闭思考（实测评审数秒完成）；不识别该参数的 API 会自动去掉后重试（见 [MCP server](#mcp-server) 实现）。
...
- 多图对比建议不超过 2 张（本地模型上下文有限）。

## 4. MCP server <a id="mcp-server"></a>

入口 `deploy/tools/multimodal/mcp_server.py`（mcp 官方 SDK 2.x，uv 项目 `deploy/tools/multimodal/pyproject.toml`），stdio 协议，暴露两个工具：

### 4.1 注册的工具 <a id="mcp-tool"></a>
...
### 4.2 配置（opencode.json） <a id="mcp-config"></a>
...
"mcp": {
  "multimodal-local": {
    ...
    "command": [
      ...
      "run",
      "--project",
      "D:/Develop/bms/deploy/tools/multimodal",
      "python",
      "D:/Develop/bms/deploy/tools/multimodal/mcp_server.py"
    ],
    "enabled": true
  }
}
...
- 依赖安装：`uv sync --project deploy/tools/multimodal --index https://mirrors.aliyun.com/pypi/simple/`（虚拟环境 `.venv` 已 gitignore）。
- 修改配置后需**重启 opencode** 生效；MCP 工具与插件工具同样在会话中直接可调用。

### 4.3 运行与验证 <a id="mcp-run"></a>
...
uv run --project deploy/tools/multimodal python -c "import sys; sys.path.insert(0, 'deploy/tools/multimodal'); import mcp_server; print(mcp_server.multimodal_chat(text='你好'))"
```

## 5. Python CLI 备用 <a id="cli"></a>

与 MCP server 共用同一套实现，供 bash 直接调用（不占主会话工具）：

```bash
# 通用识图客户端（需环境变量 VISION_API_ENDPOINT / VISION_API_KEY / VISION_API_MODEL）
python deploy/tools/multimodal/vision_analyze.py --image page.png --prompt prototype-review
python deploy/tools/multimodal/vision_analyze.py --dir screenshots/ --out report.json
python deploy/tools/multimodal/vision_analyze.py --images a.png,b.png --text "对比两张图"

# 截图（Edge 无头）
python deploy/tools/multimodal/screenshot.py --url "file:///D:/.../03_主框架.html" --out temp/vision/out.png
```
...
## 6. 截图服务（Edge 无头） <a id="screenshot"></a>

本机 Windows 环境使用系统自带 Edge 无头模式渲染 HTML/URL 为 PNG，无需安装额外浏览器：

```bash
python deploy/tools/multimodal/screenshot.py --url "file:///D:/Develop/bms/文档/设计/原型设计/03_通用骨架/03_主框架.html" --out temp/vision/out.png --width 1440 --height 900 --budget 3000
```
...
### 7.1 场景机制 <a id="scene-mechanism"></a>

- **一张图 + 一段提示词**：`multimodal_chat` 接受任意本地图片路径与提示词（模板名或直传文本），模板名解析顺序：`prompts/` 下文件 → 文件路径 → 直接当文本用。
- **模板即场景**：每个场景对应一个 `deploy/tools/multimodal/prompts/` 下的 md 模板，已有模板：
...
### 7.2 场景示例：原型视觉评审 <a id="scene-prototype"></a>
...
## 9. 实施步骤与检查清单 <a id="deploy"></a>

### 实施步骤

1. 确认本机 Python/uv 环境与 LM Studio 服务可用（`http://127.0.0.1:1234/v1/models` 返回模型列表）。
2. 建 uv 项目 `deploy/tools/multimodal/`（依赖 `mcp` 官方 SDK），迁移 `vision_analyze.py` / `screenshot.py` / `prompts/` / 报告模板。
3. 编写 `mcp_server.py`（`multimodal_chat` + `screenshot`），官方 Client 冒烟测试通过。
4. 配置 `.opencode/opencode.json`（mcp 段 + provider 段）。
5. 删除旧插件 `vision.js` 及 `deploy/tools/vision/` 目录，重启 opencode 验证 MCP 工具可用。
...
> 本文档为 AI 生成 · 依《文档生成规范》编写 · 本地多模态接入方案（原《视觉识图辅助方案》演进） · 生成日期：2026-08-18