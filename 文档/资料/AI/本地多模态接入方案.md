...
```mermaid
 flowchart LR
     A["opencode 主会话"] -->|"multimodal_chat / screenshot"| M["MCP server<br/>scripts/tools/multimodal/mcp_server.py"]
     ...
     F["场景提示词模板<br/>scripts/tools/multimodal/prompts/"] -->|"注入评审规则"| M
```
...
入口 `scripts/tools/multimodal/mcp_server.py`（mcp 官方 SDK 2.x，uv 项目 `scripts/tools/multimodal/pyproject.toml`），stdio 协议，暴露两个工具：
...
"mcp": {
  "multimodal-local": {
    ...
    "command": [
      ...
      "run",
      "--project",
      "D:/Develop/bms/scripts/tools/multimodal",
      "python",
      "D:/Develop/bms/scripts/tools/multimodal/mcp_server.py"
    ],
    "enabled": true
  }
}
...
- 依赖安装：`uv sync --project scripts/tools/multimodal --index https://mirrors.aliyun.com/pypi/simple/`（虚拟环境 `.venv` 已 gitignore）。
...
uv run --project scripts/tools/multimodal python -c "import sys; sys.path.insert(0, 'scripts/tools/multimodal'); import mcp_server; print(mcp_server.multimodal_chat(text='你好'))"
```
...
python scripts/tools/multimodal/vision_analyze.py --image page.png --prompt prototype-review
python scripts/tools/multimodal/vision_analyze.py --dir screenshots/ --out report.json
python scripts/tools/multimodal/vision_analyze.py --images a.png,b.png --text "对比两张图"

# 截图（Edge 无头）
python scripts/tools/multimodal/screenshot.py --url "file:///D:/.../03_主框架.html" --out temp/vision/out.png
```
...
```bash
python scripts/tools/multimodal/screenshot.py --url "file:///D:/Develop/bms/文档/设计/原型设计/03_通用骨架/03_主框架.html" --out temp/vision/out.png --width 1440 --height 900 --budget 3000
```
...
- **模板即场景**：每个场景对应一个 `scripts/tools/multimodal/prompts/` 下的 md 模板，已有模板：
...
2. 建 uv 项目 `scripts/tools/multimodal/`（依赖 `mcp` 官方 SDK），迁移 `vision_analyze.py` / `screenshot.py` / `prompts/` / 报告模板。
