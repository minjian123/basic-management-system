...
> BMS · backend（FastAPI）开发约定

[文档首页](../文档首页.md) › 规范 › 后端开发规范　|　[同级：前端开发规范 →](前端开发规范.md)　|　[命名规范 →](命名规范.md)

## 1. 概述 <a id="overview"></a>

本文档定义 BMS 后端（backend，FastAPI）的开发规范：分层职责、编码与命名、异常与错误码、
...
graph TD
    A["backend/"] --> B["app/"]
    B --> B1["core/ 配置、安全、异常处理（config/security/exceptions）"]
    B --> B2["api/ 路由层（按模块），依赖注入；只做参数校验与路由分发"]
    B --> B3["models/ SQLAlchemy 模型（按模块文件，如 user.py）"]
    B --> B4["schemas/ Pydantic 请求/响应模型（XxxCreateRequest/XxxResponse）"]
    B --> B5["services/ 业务逻辑与事务边界（XxxService）"]
    B --> B6["repositories/ 数据访问，多数据源/分片路由（XxxRepository）"]
    B --> B7["db/ 异步引擎、AsyncSession 工厂、读写分离路由"]
    B --> B8["tasks/ Celery 任务定义（snake_case 动词短语）"]
    B --> B9["ws/ Socket.IO 连接与会话事件"]
    B --> BA["i18n/ 国际化管理（语言包接口、运行时翻译）"]
    A --> C["alembic/ 迁移脚本（MySQL / PostgreSQL 双套方言）"]
    A --> D["tests/ pytest（fixtures/ seeds/）"]
    A --> E["config.toml"]
    A --> F["pyproject.toml"]
```
...
5 位数字，万位为模块段、后 4 位为段内序号；**一经发布稳定不变**；新增错误码按段内序号递增，禁止复用已发布码。
...
- 集成测试：双库方言迁移、读写分离、分片路由、哈希链校验（CI 服务容器）
...
> 依《文档生成规范》编写 · 与《前端开发规范》《命名规范》《架构设计》配套 · 生成日期：2026-08-06
