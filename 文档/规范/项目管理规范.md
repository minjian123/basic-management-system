...
```bash
python scripts/zentao/zentao.py stories create --product 1 \
...
```bash
python scripts/zentao/zentao.py stories list --product 1
python scripts/zentao/zentao.py stories search --product 1 --name 用户
python scripts/zentao/zentao.py stories delete --id 5
```
...
```bash
# 单个任务（创建 + 指派）
python scripts/zentao/zentao.py tasks create --execution 3 --name "mjbk Ubuntu 基础（静态IP/清华源/SSH免密）" \
  --estimate 4 --begin 2026-08-24 --end 2026-09-07 --to minjian

# 批量拆子任务（--parent 走 URL 参数）
python scripts/zentao/zentao.py tasks batch-create --execution 3 --parent 1 --file subtasks.json
```
...
```bash
python scripts/zentao/zentao.py tasks update --id 85 --desc-file desc-85.txt
python scripts/zentao/zentao.py tasks get --id 85   # 读回校验
```
...
```bash
python scripts/zentao/zentao.py tasks list --execution 3                 # 迭代 3 全部任务
python scripts/zentao/zentao.py tasks search --server --assigned-to minjian --status doing   # 22.5 服务端过滤
python scripts/zentao/zentao.py tasks search --parent 1                  # 客户端过滤：某父任务子任务（服务端不支持 parent）
python scripts/zentao/zentao.py executions list --project 1              # 全部迭代（M0–M15）
```
...
```bash
python scripts/zentao/zentao.py tasks start --id 85
python scripts/zentao/zentao.py tasks finish --id 85 --consumed 4
python scripts/zentao/zentao.py tasks close --id 85
```
