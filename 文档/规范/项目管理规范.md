...
- 需求**创建走 Web 界面**：产品 → 需求 → 添加需求（21.x 实测 API 创建静默失败，见《[禅道 API 使用说明](../资料/AI/禅道API使用说明.md)》踩坑 #7）。
- 列表/查看/更新/删除可用 CLI 或 Web：
...
- □ 需求：主题「对象 + 动作（要点）」、描述三段式、优先级/分类/所属产品/关联里程碑齐全，经 Web 界面录入
