...
- **阶段四（前端组件库）进行中（2026-09-16 起）**：需求基线 39 条（地基波 18 / 依赖回补 21）；需求域 02 基础组件类（6 项 / 88h）与域 03 布局组件类（需求 03-1 ~ 03-5 + 令牌遗留 / 6 子任务 / 62h）**均已全部完成（2026-09-16，合计 150h）**——域 02 双端 `src/base/`（根系 / 组件根 / 6 机制基类）与 `src/components/base/`（31 能力 + 4 域基类）**同款交付**（`diff -r` 无差异）；域 03 组件（弹窗抽屉表单 / 异常与空状态 / 结构布局 / 侧边菜单 / 三壳）经宿主实际装配（路由守卫 / 菜单 → 路由注册 / keep-alive / 单测 47 文件 301 用例），用例 Kiwi 698 ~ 741 入 Kiwi TCMS；其余域（04 ~ 10）随任务基线补排。
...
│   │   └── …                         # 端口固定 5174；消费 `@bms/core` / `@bms/vue` / `@bms/ui-vant`（S4c 起，旧 `src/base` 与能力层已删除）
