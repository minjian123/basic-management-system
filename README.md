...
- 开发环境部署方案已定：两台 Windows 11 机器（开发服务器 mjbk / 开发机 mjpc），详见[《开发部署规划》](文档/规划/开发部署规划.html)。
...
| 部署 | Docker Compose / nginx / Rancher Desktop（开发环境容器引擎） |
...
- **开发服务器 mjbk**（192.168.0.107，常开）：GitLab CE（HTTP 8080 + Registry 5050）、gitlab-runner、开发依赖服务（Redis/RocketMQ/MinIO/ES，Docker Compose）、常驻 MySQL 8 / PostgreSQL 16 / 达梦 DM8 三库、监控与 Milvus（按阶段引入）。
- **开发机 mjpc**（192.168.0.124）：本地编码与运行开发服务器（uv + uvicorn、npm + Vite），日常开发默认 SQLite，内网访问 mjbk 共享服务。
- 远程操作 mjbk：WinRM（5985）与 SSH（22，密钥认证已配置），详见[《开发服务器部署使用说明》](文档/资料/开发服务器部署使用说明.html)。
...
├── 资料/          # 本地资源、开发服务器部署使用说明、graphify 部署使用说明
└── 设计/          # 架构设计、概要设计、布局设计
```
