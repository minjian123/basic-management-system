...
- **URL**：`--url` 优先；否则按库键 `tenant_{code}` 经 `url_template` 解析（配置 `database.tenants.*`）；
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code", required=True, help="租户编码（全小写；库键 tenant_{code}）")
...
def _resolve_url(code: str, override: str) -> str:
    """取租户库连接串（显式覆盖优先，否则按库键经模板解析）。
...
async def run(args: argparse.Namespace) -> InitResult:
    ...
    url = _resolve_url(args.code, args.url)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        print(f"[init_tenant] 租户 {args.code} | 库键 {build_tenant_db_key(args.code)} | 目标 {_masked(url)}")
