"""服务目录升格（平台链 0002）：sys_module 增服务维度与契约版本字段，段位改可空并去唯一。

Revision ID: 0002_sys_module_catalog
Revises: 0001_sys_tenant_module
Create Date: 2026-09-22

- 归属链：`platform`（平台库）；四库通用，SQLite 经 `batch_alter_table` 兼容（重建表）；
- 新列口径与 ORM 模型（`bms_core/models/platform.py::SysModule`）逐项一致；
...
down_revision: str | None = "0001_sys_tenant_module"
