"""事务性发件箱三表（`platform:platform` 链 0003，05_03）：sys_outbox + sys_event_consumed + sys_event_dead_letter。

Revision ID: 0003_sys_outbox
Revises: 0002_sys_module_catalog
Create Date: 2026-09-23

- 归属链：`platform:platform`（`platform` 服务的平台服务库 `bms_platform`）；脚本四库通用；
- 字段 / 索引口径与 ORM 模型（`bms_core/models/outbox.py`）逐项一致；
- 三表为基础设施账本：唯一约束**不并入 `deleted_at`**（发件箱 / 幂等要求跨方言严格唯一）；
- 基础设施表**每服务自有**，各服务每条链各含三表；本服务租户链同构脚本见
  `alembic/versions/platform/tenant/0002_sys_outbox.py`；表集由表归属登记派生（`db/migration.py`）。
"""
