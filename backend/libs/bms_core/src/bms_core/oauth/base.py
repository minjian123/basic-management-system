...
口径：本域覆盖**开放接口 client 侧**（Client Credentials）、**服务身份自签**（双类 JWT 之一的服务 JWT，
见同域 `token.py` / `jwt.py` / `verify.py` / `keys.py`）与**用户双 token 自签**（`user_token.py` / `user_jwt.py`，
access / refresh `aud=api`）；外部 IdP 仅承担 SSO 授权码流程（`bms_core/idp/`，登录链路归阶段六）；
