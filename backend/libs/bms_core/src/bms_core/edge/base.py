...
- `EdgeIdentity`：信任判定通过后解析出的可信身份（用户 / 租户 / scope / 服务身份）。
...
占位口径：缺省 `provider` 空串解析到 `NullEdgeTrust`（恒定不信任，不引入假身份）；网关专属标记头
实现 `MarkerEdgeTrust` 为可替换的过渡实现，真实校验随 07_03（服务 JWT / mTLS）回补。
"""
...
class EdgeIdentity(BaseObject):
    ...
    def from_headers(cls, headers: Mapping[str, str]) -> EdgeIdentity:
        ...
        return cls(user_id=user_id, tenant_code=tenant_code, scopes=scopes, service_identity=service_identity)
