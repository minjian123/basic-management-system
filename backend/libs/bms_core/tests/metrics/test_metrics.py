...
from bms_core.api.deps import get_metrics
...
from bms_core.metrics.base import DEPENDENCIES, METRIC_KINDS, METRIC_NAMES, BaseMetrics
...
def test_metric_kinds_and_names() -> None:
    """指标类型清单（计数器 / 瞬时值 / 直方图）与候选指标名（六类 + 边界 / 发件箱 / 库数量 / 阶段度量）无重复。"""
