...
- **三级指令独立确认（2026-08-22 定稿）**：说"提交"只做 commit；说"推送"只做 push；说"合并"= 自动创建 MR → 等 MR 流水线跑通 → **汇报 MR 链接后停住**。
- **合并按钮红线（2026-08-22 定稿）**：merge 只能由用户本人在 GitLab 界面点击，AI 任何情况下不得调用 merge API / 自动合并。"合并"指令只要求做到"MR 就绪"，不包含执行 merge。
- **MR 描述三要素（2026-08-22 定稿）**：创建 MR 时必须把 diff 摘要、流水线结果、评审结论写入 MR 描述；核心模块（认证/RBAC/工作流/审计/收付款）改动须先经独立会话的 AI 交叉评审，评审结论一并写入描述。用户要在点击合并按钮前于描述中看到全部三项，缺项视为 MR 未就绪。
