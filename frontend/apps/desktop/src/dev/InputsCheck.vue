...
/**
 * 基础控件类收口核对页（开发专用，PC / Element Plus 侧）：八件渲染 + 跨件一致性自检上屏。
 *
 * - `?stage=1`（默认）：静态首屏（三态 / 只读两种呈现 / 壳装配 / 栅格 / 清空入口）；
 * - `?stage=2`：自动交互（写值 / 失焦 / 点选 / 危险确认取消）后断言受控写入 / 写门禁 / change / 校验。
 *
 * 断言只依赖跨端稳定面（`bms-*` 类名钩子与壳类名）；内核类名仅用于「是否渲染输入元素」判定。
 */
...
async function writeDisabled(key: FieldKey): Promise<void> {
  if (key === 'switch') {
    await clickIn(`dis-${key}`, '.el-switch, .van-switch')
    return
  }
  if (key === 'select' || key === 'radio' || key === 'checkbox') {
    await clickIn(
      `dis-${key}`,
      'label, .el-checkbox, .el-radio, .el-check-tag, .van-checkbox, .van-radio, .van-tag, .van-button',
    )
    return
  }
  const selector = key === 'textarea' ? 'textarea' : 'input'
...
async function runChecks(): Promise<void> {
  ...
  if (interactive) {
    ...
    record(
      ...
      `${hit.length}/8（${FIELDS.map((item) => item.name).join(' / ')}）`,
    )
  } else {
    record('① 受控写入', '八件写值 → 值更新', 'pending', 'stage=2 自动写值后断言')
  }

  // ② 三态（空值占位 / 0 与 false 视为已填 / 多值文本按选项顺序）
  ...
  if (interactive) {
    ...
    record('③ 写门禁', '禁用实例写值被拒', held.length === FIELDS.length ? 'pass' : 'fail', `${held.length}/8 值未变`)
  } else {
  ...
  const disabledMode =
    ...
    hasIn('rod-checkbox', 'input[disabled]') &&
    hasIn('rod-switch', '.is-disabled, [disabled]')
  record(
    '⑦ 只读呈现',
    '默认纯文本不渲染输入元素',
  ...
  await nextTick()
  const clearOk = hasIn('clear-text', '.el-input__clear, .van-field__clear')
  record(
    '⑨ 遗留项',
    '05_01 清空按钮渲染（浏览器层）',
    clearOk ? 'pass' : 'fail',
  ...
  if (interactive) {
    ...
    await clickIn('danger-switch', '.el-switch, .van-switch')
    await new Promise((resolve) => setTimeout(resolve, 300))
    const dialogVisible = document.querySelector('.el-message-box, .van-dialog') !== null
    await clickIn(
      'danger-switch',
      '.el-message-box__btns .el-button, .van-dialog__cancel, .van-dialog__footer .van-button',
    )
    // 等弹窗关闭动画结束（避免遮挡截图）
...
onMounted(async () => {
  ...
  try {
    ...
    results.value = [{ group: '自检', name: '执行异常', status: 'fail', detail: String(error) }]
  }
...
<template>
  <div class="check-page">
    <h1>基础控件类 · 收口核对页（开发专用 / Element Plus）</h1>
    ...
    <section class="card">
      ...
      <p class="muted">
        弹层交互（下拉面板 / 移动端底部面板）与移动端分段观感、tag 关闭交互在「交互后」截图与移动端核对页中人工核对。
      </p>
    </section>
  </div>
</template>
