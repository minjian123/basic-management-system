/** 基础令牌落地与消费（Kiwi 723）：SCSS 编译产物断言——基础令牌五组 / 引用映射 / Vant 映射 / 属性协议保留。 */
...
describe('基础令牌落地（Kiwi 723）', () => {
  ...
  it('Vant 映射：四语义色（primary / success / warning / danger）', () => {
    expect(css).toContain('--van-primary-color: var(--bms-color-primary)')
    expect(css).toContain('--van-success-color: var(--bms-color-success)')
    expect(css).toContain('--van-warning-color: var(--bms-color-warning)')
    expect(css).toContain('--van-danger-color: var(--bms-color-danger)')
  })
})
