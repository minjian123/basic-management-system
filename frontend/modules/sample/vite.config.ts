...
/** 固定预览端口（本地独立预览；与 demo 5002 区分，发布存储仍为 5002）。 */
const PORT = 5003
...
// 样例模块工程（MF remote）：独立构建、产物独立（dist/ 不并入主应用产物）；
    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: {
          ...
          // 误判为「模块自有块全局污染」；独立为 vendor 块后页面块只含模块自有代码（与宿主同一分包口径）。
          // Rolldown 分包口径为 `output.codeSplitting.groups`（MF 插件接管分包，`manualChunks` 被忽略）。
          codeSplitting: {
