// kiwi_id: 981
/**
 * 演示模块契约用例：经 `@bms/core/testing` 契约工厂跑**同一套断言**（注入上下文 / 注册声明 /
...
).entries.find((entry) => entry.name === demoModule.manifest.name)

describeModuleContract('演示模块契约（Kiwi 981）', {
  definition: demoModule,
  manifestEntry,
  facts: {
