#!/usr/bin/env node
/**
 * 共享面白名单与体积护栏（零依赖；挂 CI `shared-deps-check`，并被发布前强校验与模块契约用例复用）。
 *
 * 目的：共享面是**受控集合**——新增依赖必须先登记（进 `shared` 或 `notShared`），
 * 不得由某个模块工程私自扩面；同时防「不共享」演化为无限膨胀。
 *
 * 断言（遍历宿主与全部模块工程 `frontend/modules/*`）：
 *   1. **构建配置取自单一来源**：各工程 `vite.config.ts` 都必须经 `loadSharedDependencies` 取共享声明
 ...
 *   3. **实装版本满足版本要求**：各工程 `package.json` 中受控条的实装版本须满足单一来源的 `requiredVersion`；
 *   4. **非共享项体积阈值**：模块产物（`dist/**​/*.{js,css}`）gzip 合计不得超过单一来源为该非共享项登记的阈值。
 *
 * 用法（CLI；发布前强校验与模块契约用例走导出的纯函数，`requireArtifacts: false` 时不要求产物已构建）：
...
export function checkSharedWhitelist(options = {}) {
  ...
  for (const project of projects) {
    const config = join(project.dir, 'vite.config.ts')
  ...
  for (const project of projects) {
    ...
    for (const name of dependencies) {
      ...
      if (project.role === '模块' && !whitelist.has(name)) {
  ...
  for (const moduleDir of listModuleDirs(frontendDir)) {
    ...
    for (const [name, entry] of Object.entries(notSharedEntries)) {
      ...
      if (!existsSync(dist)) {
        if (requireArtifacts) problems.push(`模块产物目录不存在：${dist}（请先构建后再跑本护栏）`)
        break
      }
      const actualKb = measureGzip(dist) / 1024
      if (actualKb > limit) {
...
function main(argv) {
  ...
  try {
    const problems = checkSharedWhitelist({ frontendDir })
    if (problems.length > 0) {
      console.error(`[shared-whitelist] 不通过：${problems.join('；')}`)
      exit(1)
    }
    const source = readSharedSource({ root: frontendDir ?? FRONTEND_DIR })
    const whitelist = [...Object.keys(source.shared ?? {}), ...Object.keys(source.notShared ?? {})]
    console.log(
      `[shared-whitelist] 通过：白名单 ${whitelist.join(' / ')}；依赖与实装版本合规；` + `模块产物体积在阈值内`,
    )
  } catch (error) {
    console.error(`[shared-whitelist] 不通过：${error instanceof Error ? error.message : String(error)}`)
    exit(1)
  }
}
