...
/**
 * 密码框（PC / Element Plus 侧）：密码输入（《组件设计 · 密码框》）。
 *
 * 安全底线：**不回填**（编辑态留空表示不修改）、明文切换仅组件内存态、值不写日志 / 不落存储、
 * 可禁浏览器自动填充；强度提示经核心纯函数 `evaluatePasswordStrength`（双端同规则）。
 */
...
<template>
  ...
  >
    <template #default="slot">
      ...
      >
        <template v-if="showToggle" #suffix>
          <button
            type="button"
