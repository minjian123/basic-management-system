...
const canManage = computed(
  () => props.managePermission.length === 0 || checkPerm(props.managePermission),
)
...
function onLoadCompat(node: unknown, resolve: (data: never[]) => void, reject: (error?: Error) => void): void {
  onLoad(
    node as { data?: TreeNode },
    resolve as unknown as (data: TreeNode[]) => void,
    reject,
  )
}

/** EP `el-tree` 回调类型兼容包装（`filter-node-method`） */
function filterNodeCompat(value: unknown, data: unknown): boolean {
  return filterNode(String(value ?? ''), data as TreeNode)
}

/** EP `el-tree` 回调类型兼容包装（`node-click`；事件参数未用） */
function onNodeClickCompat(_event: Event, data: unknown): void {
  onNodeClick(data as TreeNode)
}

/** EP `el-tree` 回调类型兼容包装（`node-contextmenu`；仅取坐标） */
function onNodeContextMenuCompat(event: Event, data: unknown): void {
  onNodeContextMenu(event as MouseEvent, data as TreeNode)
}
  ...
  <div v-bind="elAttrs" :class="base.nsClass('master-tree')">
    ...
    <div :class="base.nsClass('master-tree-body')">
      ...
      <el-tree
        ...
        :filter-node-method="filterNodeCompat"
        @node-click="onNodeClickCompat"
        @check="onCheck"
        @node-contextmenu="onNodeContextMenuCompat"
      >
