...
/** EP 类型面收口：`line` 对应 EP 空值（默认线型） */
const epType = computed<'card' | 'border-card' | ''>(() => (props.type === 'card' || props.type === 'border-card' ? props.type : ''))
...
<template>
  <el-tabs
    ...
    @tab-change="emit('tab-change', String($event))"
    @tab-remove="emit('tab-remove', String($event))"
