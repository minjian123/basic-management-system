...
/**
 * 内容遮罩 loading（移动端）：局部刷新时覆盖指定区域（不整页闪烁），支持延时显示防闪。
 *
 * 契约与 `ui-ep` 同源（props / 类名钩子一致）：局部覆盖（缺省）/ `delay` 防闪烁 /
 * `text` / `fullscreen`；加载指示经 `van-loading` 映射。
 */

import { useI18n } from 'vue-i18n'
import { computed, onBeforeUnmount, ref, useAttrs, watch } from 'vue'

import { Loading as VanLoading } from 'vant/es/loading'
