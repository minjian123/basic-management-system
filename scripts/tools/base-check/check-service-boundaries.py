...
5. **表 / 表前缀跨服务唯一**：各服务 `models` 声明的表名不得跨服务重复；表前缀不得被两个服务声明
   （**`sys_` 平台域共享前缀例外**）；
6. **跨服务库访问（本任务新增）**：按服务目录表前缀归属判定——① 声明越权（服务声明他服务前缀的表）；
   ② 引用越界（原始 SQL / `ForeignKey` 目标 / 字符串表名 token 指向他服务表前缀）；
7. **读侧出口例外白名单核对（本任务新增）**：`deploy/boundaries/data_ownership_exceptions.json` 结构 /
...
def check() -> int:
    ...
    # 规则 5：表 / 表前缀跨服务唯一（`sys_` 平台域共享前缀例外）
    tables: dict[str, str] = {}
    ...
    # 规则 6①：声明越权（`__tablename__` 前缀归属他服务）
    for package, (root, shared) in packages.items():
    ...
    # 规则 6②：引用越界（原始 SQL / ForeignKey / 已知表名字符串 token）
...
def _owner_of(table: str) -> str:
    ...
    Returns:
        str: 归属标签。
    """
    return prefix_owner(table_prefix_of(table)) or "未登记"
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        run(False, "sys_ 共享前缀声明放行")

        declare(svc / "api", "pur_order")
        declare(org / "models", "pur_item")
        run(True, "表前缀跨服务重复拦截")

        declare(svc / "api", "sys_icon")
        declare(org / "models", "org_item")
        run(False, "表 / 表前缀各归其主放行")

        declare(svc / "api", "ord_item")
        declare(org / "models", "org_item")
        ...
        declare(org / "models", "org_item")
        declare(svc / "api", "sys_icon")
        (org / "repository.py").write_text('SQL = "select * from pur_order"\n', encoding="utf-8")
        run(True, "跨服务库访问（原始 SQL）拦截")

        (org / "repository.py").write_text("import bms_core\n", encoding="utf-8")
        declare(org / "models", "pur_order")
        run(True, "跨服务声明越权拦截")
        ...
        (org / "models/m.py").write_text('__tablename__ = "org_item"\n', encoding="utf-8")
        (org / "repository.py").write_text('SQL = "select * from pur_order"\n', encoding="utf-8")
        run(False, "命中例外白名单放行")
