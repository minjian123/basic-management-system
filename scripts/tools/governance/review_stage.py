...
# 架构升级触发条件（微服务《10 项目评估》§7 E1~E5；微前端《06 项目评估》§7 F1~F8；人工判断，脚本只提醒不判定）
MICROSERVICE_TRIGGERS = (
    "E1 单机资源触顶（单机多容器持续逼近资源上限）",
    "E2 编排复杂度超 Compose（自愈/滚动升级/多节点调度成为刚需）",
    "E3 服务与团队规模上升（服务数持续增长、需独立交付节奏的团队 ≥ 2）",
    "E4 一致性问题频发（手写状态机+补偿失控，或跨服务流程 ≥ 4~5 步且含人工节点）",
    "E5 合规/隔离（客户或合规要求物理隔离）",
)
...
def check_review_triggers() -> Check:
    """架构升级触发条件提醒：打印微服务 E1~E5 与微前端 F1~F8 供人工逐条判断（脚本不作自动判定）。"""
    return Check(
        "架构升级触发条件复核（人工）",
        True,
        "E1~E5（微服务）与 F1~F8（微前端）见知识档案对应评估篇；本项为提醒，不自动判定",
    )
